CREATE OR REPLACE
PACKAGE ATG_GS_EndecaExport_PartialPkg IS
...
CREATE OR REPLACE
PACKAGE BODY ATG_GS_EndecaExport_PartialPkg 
AS
...
PROCEDURE Export_GS_Partial_Controller (p_Id varchar2, p_dataCenter varchar2)
  IS
    ...
    v_prod                  NUMBER :=0; 
    v_prod_trans            NUMBER :=0; 
    v_prod_relation         NUMBER :=0; 
    v_sku                   NUMBER :=0; 
    v_brand                 NUMBER :=0; 

    rowcount_price            NUMBER;
    rowcount_brand            NUMBER;
    rowcount_cat              NUMBER;
    rowcount_prod             NUMBER;
    rowcount_sku              NUMBER;
    rowcount_prod_trans       NUMBER;
    rowcount_prod_tab         NUMBER;
    rowcount_prod_reln        NUMBER;
    rowcount_media            NUMBER;
    rowcount_sku_eco          NUMBER;
    rowcount_sku_site         NUMBER;
    rowcount_sku_trans        NUMBER;
    rowcount_sku_attr         NUMBER;
    rowcount_sku_attr_trans   NUMBER;
    rowcount_attr_trans       NUMBER;
  ...
  BEGIN
		...
		IF p_Id = 'production' 
		THEN
			SELECT COUNT(*) INTO current_deployments FROM BBB_PIM_STG_STOFU.ECP_FEED_MONITORING 
		...
		--Check for any current Web Endeca Feed Generation
		 ...
		 SELECT count(*) into current_web_feed_generation FROM BBB_PIM_STG_STOFU.BBB_DEPLOYMENT_POLLING WHERE LOWER(ID) = p_Id and data_center = p_dataCenter and endeca_in_progress='Y';
		 ...
		 SELECT count(*) into current_gs_feed_generation FROM BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG WHERE data_center = p_dataCenter and status ='RUNNING';
		 dbms_output.put_line('Endeca Feed Generation for GS '||current_gs_feed_generation);
	   
		IF current_deployments > 0 and current_web_feed_generation > 0 and current_gs_feed_generation > 0
		THEN
			
				dbms_output.put_line('Execution of Endeca for PARTIAL Feed terminated ');
		else 
				select execution_date into v_last_mod_date from (select execution_date from BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG  where feed_type='PARTIAL' and Data_center=p_dataCenter and status='COMPLETE' order by execution_date desc) WHERE ROWNUM=1;
				dbms_output.put_line('going to execute '||v_last_mod_date);
				
				 insert into BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG (FEED_NAME, FEED_TYPE, STATUS, EXECUTION_DATE, DATA_CENTER) values ('PARTIAL-'||to_char(systimestamp, 'YYYY-MM-DD HH24:MI:SS'), 'PARTIAL', running_status , sysdate, p_dataCenter);
				commit;
			...
			update BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG set STATUS = complete_status where FEED_TYPE='PARTIAL' and STATUS=running_status and data_center = p_dataCenter;
			commit;
		END IF;
		...
		WHEN OTHERS THEN  -- handles all other errors
          rollback;
          update BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG set STATUS = failure_status where FEED_TYPE='PARTIAL' and STATUS=running_status and data_center = p_dataCenter;
          commit;
        -- UTL_MAIL.send( sender=>'kkumar1@SAPIENT.COM', recipients=>'kkumar1@SAPIENT.COM', subject=>'testing subject', message=>'testing body');
        dbms_output.put_line('FATAL ERROR. Please check logs for SPs getting executed in sequence.');
 ...
 PROCEDURE Export_GS_Full_Controller (p_Id varchar2, p_dataCenter varchar2) IS
  ...
  BEGIN
    ...
    THEN
        ...
        ELSE
            insert into BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG (FEED_NAME, FEED_TYPE, STATUS, EXECUTION_DATE, DATA_CENTER) values ('FULL-'||to_char(systimestamp, 'YYYY-MM-DD HH24:MI:SS'), 'FULL', running_status , sysdate, p_dataCenter);
			commit;
			...
			update BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG set STATUS = complete_status where FEED_TYPE='FULL' and STATUS=running_status and data_center = p_dataCenter;
          commit;
    END IF;
    ...
    WHEN OTHERS THEN  -- handles all other errors
          rollback;
          update BBB_PIM_STG_STOFU.BBB_GS_ENDECA_DB_LOG set STATUS = failure_status where FEED_TYPE='FULL' and STATUS=running_status and data_center = p_dataCenter;
          commit;
        -- UTL_MAIL.send( sender=>'kkumar1@SAPIENT.COM', recipients=>'kkumar1@SAPIENT.COM', subject=>'testing subject', message=>'testing body');
        dbms_output.put_line('FATAL ERROR. Please check logs for SPs getting executed in sequence.');
