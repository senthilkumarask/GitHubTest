...
public class SiteFlagTranslation extends DerivationMethodImpl {
	...
	protected static final String WEB_OFFERED="webOffered";
	protected static final String GUIDED_BBB_SITE_ID="GS_BedBathUS";
	protected static final String GUIDED_CA_SITE_ID="GS_BedBathCanada";
	protected static final String GUIDED_BAB_SITE_ID="GS_BuyBuyBaby";
	protected static final String PRODUCT_DISABLE="prodDisable";
	protected static final String DISABLE="disable";
	protected static final String GUIDED_BBB_WEB_OFFERED="gsBBBWebOffered";
	protected static final String GUIDED_CA_WEB_OFFERED="gsBABWebOffered";
	protected static final String GUIDED_BAB_WEB_OFFERED="gsCAWebOffered";
	protected static final String GUIDED_BBB_WEB_DISABLED="gsBBBDisabled";
	protected static final String GUIDED_CA_WEB_DISABLED="gsBABDisabled";
	protected static final String GUIDED_BAB_WEB_DISABLED="gsCADisabled";
	...
	//static initializer
	static {
		Derivation.registerDerivationMethod(DERIVATION_NAME,SiteFlagTranslation.class);
	}

	/**
	 * Set the name, display name and short description properties.
	 */
	public SiteFlagTranslation() {
	...
	private static ApplicationLogging mLogger =
			ClassLoggingFactory.getFactory().getLoggerForClass(SiteFlagTranslation.class);
	...
	public Object derivePropertyValue(RepositoryItemImpl pItem)
		...
		//TBSNext change - ignore "TBS_" portion of siteID
		if( !StringUtils.isEmpty(siteId) && siteId.startsWith("TBS_")) {
			siteId = siteId.substring(4);
			if(getLogger().isLoggingDebug()){
				getLogger().logDebug("Changing TBS siteID to: " + siteId);
			}
		}
...
//		If code flow is coming from scheduled job
		...
		try{
			...
			//STOFU CHANGE: WebOffered and disable flag set according to the guided application site id check.
			if(siteId.contains(getGSSitePrefix()))
			{
				if (property.equalsIgnoreCase(WEB_OFFERED))

				{
					if (siteId.equals(GUIDED_BBB_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_BBB_WEB_OFFERED);
					}
					if (siteId.equals(GUIDED_BAB_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_BAB_WEB_OFFERED);
					}
					if (siteId.equals(GUIDED_CA_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_CA_WEB_OFFERED);
					}
				} else if (property.equalsIgnoreCase(PRODUCT_DISABLE)
						|| property.equalsIgnoreCase(DISABLE)) {

					if (siteId.equals(GUIDED_BBB_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_BBB_WEB_DISABLED);
					}
					if (siteId.equals(GUIDED_BAB_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_BAB_WEB_DISABLED);
					}
					if (siteId.equals(GUIDED_CA_SITE_ID)) {
						return getProperyValue(pItem, GUIDED_CA_WEB_DISABLED);
					}
				}
				...
				// If translations are empty AND Property is not defined,
				// returning the Default Value
				if ((null == translations || translations.isEmpty())
						|| (null == property || property.isEmpty())) {
					if (getLogger().isLoggingDebug()) {
						getLogger()
								.logDebug(
					...
					for (RepositoryItem translation : translations) {
						...
						tempAttributeName = (String) translation
								.getPropertyValue(ATTRIBUTE_PROPERTY);

						// Condition to check for siteId, reqLocale and
						// attribute
						if ((null != tempSiteItem && (tempSiteItem
								.getRepositoryId().equalsIgnoreCase(siteId)))
								&& (null != tempAttributeName && (tempAttributeName
										.equalsIgnoreCase(property)))) {
							if (getLogger().isLoggingDebug()) {
								getLogger()
										.logDebug(
												"Found Translation for site: "
														+ siteId
														+ " and attribute: "
														+ property);
							}
							...
							} else if (null != tempLocale
									&& (tempLocale.equalsIgnoreCase(reqLocale))) {
								if (getLogger().isLoggingDebug()) {
									getLogger().logDebug(
											"Found Translation for Locale:"
													+ reqLocale);
								}
								value = this.setValues(translation, value);
								if (getLogger().isLoggingDebug()) {
									getLogger().logDebug(
											"Final Returned Value" + value);
								}
								return value;
							}
						}
