...
public class GenericListTranslation extends DerivationMethodImpl {

	protected static final String DERIVATION_NAME = "GenericListTranslation";
	...
	//static initializer
	static {
		Derivation.registerDerivationMethod(DERIVATION_NAME, GenericListTranslation.class);
	}

	/**
	 * Set the name, display name and short description properties.
	 */
	public GenericListTranslation() {
	...
	private static ApplicationLogging mLogger =
			ClassLoggingFactory.getFactory().getLoggerForClass(GenericListTranslation.class);
		...
		try {
			if (null != pItem) {
				if (null == propName) {
					...
					final List<RepositoryItem> itemList = (List<RepositoryItem>) 
							getProperyValue(pItem, propName);

					if (BBBUtility.isListEmpty(itemList) || BBBUtility.isEmpty(this.getItemDescProperty())) {
						if (this.getLogger().isLoggingDebug()) {
