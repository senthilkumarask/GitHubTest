...
public class SiteTranslation extends DerivationMethodImpl {
	...
	//static initializer
	static {
		Derivation.registerDerivationMethod(DERIVATION_NAME,SiteTranslation.class);
	}

	/**
	 * Set the name, display name and short description properties.
	 */
	public SiteTranslation() {
	...
	private static ApplicationLogging mLogger =
			ClassLoggingFactory.getFactory().getLoggerForClass(SiteTranslation.class);
	...
	public Object derivePropertyValue(RepositoryItemImpl pItem)
		...
		try {
			if (BBBUtility.isListEmpty(exps)) {//if (null == exps || exps.size() == 0) {
				return null;
			}
			else {
		...
		if(BBBUtility.isEmpty(siteId)){//if(null == siteId || siteId.isEmpty()){
		...
		try{
			...
			if( (BBBUtility.isCollectionEmpty(translations)) || (BBBUtility.isEmpty(property))){//if( (null == translations || translations.isEmpty() ) || (null == property || property.isEmpty())){
				if(getLogger().isLoggingDebug()){
