...
echo  ./../../bin/startSQLRepository -m DCS.Versioned -m BBBStore.EStore.versioned.catalog -m BBBStore.StoreRest.repositories -m BBBStore.StoreRest.versioned -s bbb_impexp_bcc_test5 -import $2 -repository $3 -project $projectname -user publishing -comment $projectname -activity merchandising.manageCommerceAssets

./../../bin/startSQLRepository -m DCS.Versioned -m BBBStore.EStore.versioned.catalog -m BBBStore.StoreRest.repositories -m BBBStore.StoreRest.versioned -s bbb_impexp_bcc_test5 -import $2 -repository $3 -project $projectname -user publishing -comment $projectname -activity merchandising.manageCommerceAssets
exit 0
else
	echo "Wrong input File. Please provide the correct xml file with absolute path."
	exit 0
