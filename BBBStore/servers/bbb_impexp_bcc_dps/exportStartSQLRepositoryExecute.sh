...
echo ./../../bin/startSQLRepository -m DCS.Versioned -m BBBStore.EStore.versioned.catalog -s bbb_impexp_bcc_dps -export all $1 -repository $2

./../../bin/startSQLRepository -m DCS.Versioned -m BBBStore.EStore.versioned.catalog -s bbb_impexp_bcc_dps -export all $1 -repository $2
exit 0
else
	echo "Wrong input File. Please provide the correct xml file with absolute path."
	exit 0
