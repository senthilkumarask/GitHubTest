...
echo ..\..\bin\startSQLRepository -m BBBStore.core -m BBBStore.servertype.bbb_stg -s bbb_impexp_stg_prod_dc1 -import %1 -repository %2

..\..\bin\startSQLRepository -m BBBStore.core -m BBBStore.servertype.bbb_stg -s bbb_impexp_stg_prod_dc1 -import %1 -repository %2
	goto :afterBlank
)else (
...
echo Please pass the following two parameters in mentioned order :    1. Import File(Provide xml file with absolute path)    2. Repository Name(with config path)
