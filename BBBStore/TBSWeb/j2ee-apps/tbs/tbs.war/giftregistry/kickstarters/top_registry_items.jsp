...
<dsp:importbean
    ...
    <dsp:input bean="EmailAFriendFormHandler.fromPage" type="hidden" value="topRegistryItems" />
	 				    ...
	 				    <ul class="prodInfo">
							<li class="prodName"><dsp:a page="${finalUrl}?skuId=${skuId}" title="${productName}" onclick="javascript:s_crossSell('registry update, top registry items');">${productName}</dsp:a></li>
							 ...
							 <li class="prodPrice prodPriceSALE">
								...
								<input type="hidden" name="certonaPrice" value="${omniPrice}" />
								<dsp:input bean="GiftRegistryFormHandler.addedItemPrice" type="hidden" name="itemPrice" value="${omniPrice}" />
					        </li>
...
<script type="text/javascript">
   ...
   (function() {
        var cAddItemFlag = $('input[name="cAddItemFlag"]').val() || "false",
            cfProdId = $('input[name="cProd"]').val() || 0,
            cfSkuId = $('input[name="cSkuId"]').val() || 0,
            cfQty = $('input[name="cQty"]').val() || 0,
            cfPrice = $('input[name="cPrice"]').val() || 0;
        ...
        if (cAddItemFlag && cAddItemFlag.trim().toLowerCase() === "true") {
            ...
            if (typeof s !== 'undefined' && typeof addItemRegistryOmniture === 'function') {
                addItemRegistryOmniture({
                    'product': ';' + cfProdId + ';;;;eVar30=' + cfSkuId,
                    'var1': 'Top registry items',
                    'var23': '${eventType}',
                    'var23': '${registryId}',
