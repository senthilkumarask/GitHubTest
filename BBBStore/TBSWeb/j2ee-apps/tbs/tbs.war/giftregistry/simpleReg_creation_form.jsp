...
<dsp:importbean bean="/atg/userprofiling/Profile" />
  ...
  <dsp:importbean bean="/com/bbb/simplifyRegistry/droplet/SimpleRegFieldsDroplet" />
	...
	<bbb:pageContainerNoHeaderFooter section="${section}" titleString="${titleString}" pageVariation="${pageVariation}">
      <jsp:attribute name="bodyClass"> ${topClass} createRegistry createSimpleRegistryBody useCertonaAjax useFB useAdobeActiveContent</jsp:attribute>
      ...
      <jsp:body>
	  	...
	  	<c:choose>
            <c:when test="${minifiedCSSFlag == 'true'}">
                <link rel="stylesheet" type="text/css" href="${cssPath}/_assets/tbs_assets/css/legacy/registry_combined.css?v=${buildRevisionNumber}" />
            </c:when>
            <c:otherwise>
      ...
      <dsp:getvalueof id="currentSiteId" bean="Site.id" />
			<div class="large-12 row">
				<div class="large-12 columns siteHeader"> 
					<div id="siteLogo"> 
						<c:choose>
			...
			<%-- <dsp:form id="registryFormPost" action="?_DARGS=/store/giftregistry/simpleReg_creation_form.jsp.registryFormPost#" class="cb form post" method="post"> --%>
				...
				<div class="row padBottom_10">
					<div class="columns large-6 cb">                
						<label class="padBottom_5" for="fullName"><bbbl:label key="lbl_reg_your_name" language ="${pageContext.request.locale.language}"/></label>
					...
					<div class="columns large-6 cb">                
						<label class="padBottom_5" for="txtPrimaryRegistrantbabyMaidenName" id="lbltxtPrimaryRegistrantbabyMaidenName"><bbbl:label key="lbl_reg_maiden_name" language ="${pageContext.request.locale.language}"/></label>
						...
						<c:set var="regmaidenname">	<bbbl:label key="lbl_reg_ph_maidenname" language ="${pageContext.request.locale.language}"/></c:set>
		                        ...
		                        <dsp:tagAttribute name="aria-labelledby" value="lbltxtPrimaryRegistrantbabyMaidenName errortxtPrimaryRegistrantbabyMaidenName"/>
				...
				<div class="row padBottom_10">
					<label class="columns large-12 padBottom_5" for="emailField"><bbbl:label key="lbl_account_info" language ="${pageContext.request.locale.language}"/></label>
					...
					<div class="inputField cb large-6 columns passwordWrapper">
								...
								<div class="row splitName hidden">
									<div class="inputField cb large-5 columns">
										...
										<dsp:input id="regCoRegisFirstName"
											...
											<dsp:tagAttribute name="aria-required" value="true" />
											<dsp:tagAttribute name="aria-labelledby" value="lblregCoRegisFirstName errortxtCoRegistrantFirstName" />
					               	...
					               	<c:if test="${inputListMap['CoRegistrantLastName'].isDisplayonForm}">	
										...
										<div class="inputField cb large-7 columns">
					                    	...
					                    	<dsp:input id="regCoRegisLastName"
												...
												<dsp:tagAttribute name="minlength" value="2"/>	
	                                            <dsp:tagAttribute name="aria-required" value="true" />
	                                            <dsp:tagAttribute name="aria-labelledby" value="lblregCoRegisLastName errortxtCoRegistrantLastName" />
	                                            <dsp:tagAttribute name="placeholder" value="${lastName}" />
											</dsp:input>
					               		</div>
				               		</c:if>
	...
	<c:set var="phdate"><bbbl:label key='lbl_reg_ph_date' language ='${pageContext.request.locale.language}'/></c:set>
					<div class="columns large-6 cb">                
						<div class="row">
                         ...
                         <div class="inputField cb large-6 columns eventsDate">
								<label class="padBottom_5" for="eventField">${eventLabelHolder}</label>
								<div class="inputWithCalender">
									<dsp:input id="eventField"
		                                       bean="GiftRegistryFormHandler.registryVO.event.eventDate"
		                                            type="text" name="arrivalDate" readonly='true'  iclass="${isRequired}">
		                                            <dsp:tagAttribute name="aria-required" value="true"/>
		                                            <%-- <dsp:tagAttribute name="aria-labelledby" value="lbltxtRegistryExpectedDate${CADate} errortxtRegistryExpectedDate${CADate}"/> --%>
		                                       		 <dsp:tagAttribute name="placeholder" value="${phdate}"/>
					...
					<c:set var="required" value=""></c:set> 
						...
						<div class="columns large-6 cb" >
							...
							<div class="customRadios customRadiosHolder row boyGirlWrap">
			 	 ...
			 	 <c:if test="${regVO.primaryRegistrant.contactAddress.addressLine1 ne '' && regVO.primaryRegistrant.contactAddress.addressLine1 ne null}">
					...
					<c:if test="${inputListMap['showShippingAddress'].isDisplayonForm}">
						...
						<div class="row ${splitShipAddress} ${showshipbox}" id="shippingNewAddrLine1">
							<div class="inputField cb large-6 columns ${sAddr} combineInput">
							<c:set var="shipAdd"><bbbl:label key="lbl_reg_ph_shippingAddress" language ="${pageContext.request.locale.language}"/></c:set>
								...
								<input type="text" name="txtRegistryCurrentShippingAddressName"
									...
									checked="true" maxlength="120" placeholder="${shipAdd}"
									aria-labelledby="lbltxtRegistryCurrentShippingAddress errortxtRegistryCurrentShippingAddress" 
									<c:choose>
							...
							<div class="inputField cb large-6 columns ${sApt}">	
						...
						<div class="row futureAddField <c:if test='${movingFutureCheckboxFromBean ne "futureShippingDateSelected"}'>hidden</c:if>">
							<div class="large-12 columns addressFields" id="shippingFutureAdd">
								<c:if test="${regVO.shipping.futureshippingAddress.addressLine1 ne '' && regVO.shipping.futureshippingAddress.addressLine1 ne null}">
									...
									<label for="txtRegistryFutureShippingAddress" class="txtOffScreen" id="lbltxtRegistryFutureShippingAddress"><bbbl:label key='lbl_reg_ph_futureaddress' language ='${pageContext.request.locale.language}'/></label>
							 ...
							 <c:set var="required" value=""></c:set> 
							 	<c:if test="${inputListMap['MobileNumber'].isMandatoryOnCreate}"> <c:set var="required" value="required"/>  </c:if> 
									<div class="inputField cb large-6 columns">
										...
										<label class="padBottom_5" for="mobilePhone"><bbbl:label key="lbl_reg_cell_no" language ="${pageContext.request.locale.language}"/></label>
											<c:choose>
											<c:when test="${isTransient}"> 
				...
				<div class="row padBottom_10">
					...
					<dsp:input id="networkAffiliation" bean="GiftRegistryFormHandler.registryVO.networkAffiliation" type="hidden" value="N"/>
						...
						<dsp:input bean="GiftRegistryFormHandler.registryCreationErrorURL" type="hidden" value="${contextPath}/giftregistry/simpleReg_creation_form.jsp"/> --%>
						        <dsp:input bean="GiftRegistryFormHandler.fromPage"  type="hidden" value="simpleRegistryCreationForm" />
