...
<dsp:page>
   ...
   <c:when test="${babySite eq 'baby' }">
   		<c:set var="babyPage" value="/giftregistry/simpleReg_creation_form.jsp"/>
   </c:when>
   <c:otherwise>
  	 	<c:set var="babyPage" value="/giftregistry/simpleReg_creation_form_bbus.jsp"/>
   </c:otherwise>
   </c:choose> --%>

	<dsp:form
		action="${contextPath}/giftregistry/simpleReg_creation_form_bbus.jsp}">
		...
		<div class="simpleRegTypeWrap">
			...
			<%-- <dsp:input bean="GiftRegistryFormHandler.ErrorURL" type="hidden"
				...
				value="${contextPath}/giftregistry/simpleReg_creation_form_bbus.jsp" /> --%>
				        <dsp:input bean="GiftRegistryFormHandler.fromPage"  type="hidden" value="simpleRegistryTypeSelect" />
