    		...
    		<c:if test="${chatglobalFlag == 'true'}">
    			<c:choose>
    				<c:when test="${divApplied}">
	    				<div class="${divClass}">
    						<a title="<bbbl:label key='lbl_click_to_chat_image_title' language='${pageContext.request.locale.language}'/>" class="chatNow liveChat" data-chatdata="${chatData}" href="${contextPath}/selfservice/click_to_chat.jsp" onclick="javascript:externalLinks('Click to Chat')";></a>
    					</div>
    				</c:when>
    				<c:otherwise>
    					<a title="<bbbl:label key='lbl_click_to_chat_image_title' language='${pageContext.request.locale.language}'/>" class="chatNow liveChat" data-chatdata="${chatData}" href="${contextPath}/selfservice/click_to_chat.jsp" onclick="javascript:externalLinks('Click to Chat')";></a>
