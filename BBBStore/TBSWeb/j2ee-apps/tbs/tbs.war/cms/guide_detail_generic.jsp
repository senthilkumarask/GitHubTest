<dsp:page>
	...
	<dsp:getvalueof var="url" vartype="java.lang.String" value="${contextPath}/s/${searchTerm}" />
	...
	<dsp:getvalueof var="title" param="title"/>
		...
		<c:set var="lnkGuidesBk" value="/tbs/static/GuidesPage" scope="request" />
		<c:if test="${fn:contains(referer, 'BabyGuidesAndAdviceLandingPage')}">
		<c:set var="brGuides" value="Baby Guides & Advice" scope="request" />
		<c:set var="lnkGuidesBk" value="/tbs/registry/BabyGuidesAndAdviceLandingPage" scope="request" />
		</c:if>
					...
					<c:choose>
						<c:when test="${currentSiteId == BuyBuyBabySite}">
							<div id="cmsBreadCrumbs" class="breadcrumbs grid_12 marTop_10 marBottom_5">
								<a href=${pageContext.request.contextPath}><bbbl:label key="lbl_reg_feature_home" language ="${pageContext.request.locale.language}"/></a>
							...
							<div id="cmsPageHead" class="grid_12 clearfix">
								<h1>${title}</h1>
								<%-- <a id="cmsLnkBack" href="${contextPath}/registry/GuidesAndAdviceLandingPage" title='<bbbl:label key="lbl_back_guide" language ="${pageContext.request.locale.language}"/>'><bbbl:label key="lbl_back_guide" language ="${pageContext.request.locale.language}"/></a> --%>
							</div>
						</c:when>
						<c:otherwise>
						<c:choose>
						<c:when test="${(currentSiteId  == 'BedBathCanada') && (babyCAMode == 'true')}">
						<c:set var="lnkRegistryBk" value="${contextPath}/page/BabyCanadaRegistry" scope="request" />
						</c:when>
						<c:otherwise>
						<c:set var="lnkRegistryBk" value="${contextPath}/page/Registry" scope="request" /></c:otherwise>
						</c:choose>
							<div id="cmsBreadCrumbs" class="breadcrumbs grid_12 marTop_10 marBottom_5">
								<a href=${pageContext.request.contextPath}><bbbl:label key="lbl_reg_feature_home" language ="${pageContext.request.locale.language}"/></a>
