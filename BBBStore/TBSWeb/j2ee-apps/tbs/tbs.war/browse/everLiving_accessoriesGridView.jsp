...
<dsp:getvalueof var="childProducts" param="collectionVO.childProducts"/>
		<c:if test="${childProducts ne null && not empty childProducts}">			
				<dsp:form name="testcollectionForm" id="testcollectionForm" method="post">
					<ul class="clearfix childProductItems small-block-grid-1 medium-block-grid-2 large-block-grid-3 collectionGridRow listGridToggle gridViewChildren">
								   ...
								   <%-- Product name --%>
													...
													<c:when test="${ratingAvailable == true}">
														...
														<c:choose>
															<c:when test="${totalReviewCount == 1}">
																<div class="prodReviewGrid prodReview clearfix prodReview<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/>" title="<dsp:valueof param="element.bvReviews.averageOverallRating"/>">
									...
									<div class="priceQuantityNotAvailable <c:if test="${inStock==true}">hidden</c:if>">
								      <div class="gridMessage">
                                             ...
                                             <c:otherwise>
								             <div class="info hidden"><a class="info lnkNotifyOOS" href="#"><bbbl:label key='lbl_pdp_product_notify_item_available' language="${pageContext.request.locale.language}" /> &raquo;</a></div>
								             </c:otherwise>
                                             </c:choose>
                                             </c:if>
								      </div>
									 ...
									 <%-- <li class="prodbuttonGrid grid_3 alpha clearfix">
											 <c:choose>
												...
												<div class="spinner fl">
													...
													<a title="<bbbl:label key='lbl_decrease_quantity' language="${pageContext.request.locale.language}" />" id="accGridDecQty${skuid}" class="scrollDown down" href="#"><span class="txtOffScreen"><bbbl:label key="lbl_decrease_quantity" language="${pageContext.request.locale.language}" /></span></a>
													...
													<a title="<bbbl:label key='lbl_increase_quantity' language="${pageContext.request.locale.language}" />" id="accGridIncQty${skuid}" class="scrollUp up" href="#"><span class="txtOffScreen"><bbbl:label key="lbl_increase_quantity" language="${pageContext.request.locale.language}" /></span></a> 
												</div>
												 </c:otherwise>
												 </c:choose>
											...
											<div class="width_3 gridStore">
												 ...
												 <c:choose>
													...
													<c:otherwise>
												 		...
												 		<div class="prodGridStore changeStore fr cb"><a href="javascript:void(0);">Find in Store</a></div>
													</c:otherwise>
												</c:choose>
												</c:if>
               ...
               <%--            <c:if test="${productCount % 4 == 0}">
                           ...
                           <ul class="clearfix collectionGridRow listGridToggle">
