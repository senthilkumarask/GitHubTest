							...
							<dsp:oparam name="output">
								...
								<c:choose>
									<c:when test="${menu eq 'SIZE'}">
									 <div class="small-12 columns size-selector sizePicker pdp-selector"> 
										<!-- <a class="small secondary radius button dropdown" data-dropdown="selectProductSize" href="#">SIZE<span></span></a> -->
										<ul class="swatches" data-dropdown-content="" id="selectProductSize" name="selectProductSize">
											<dsp:droplet name="ForEach">
												<dsp:param param="element" name="array" />	
												<%-- <dsp:oparam name="outputStart">
													<li class="selected"><a href="#">SIZE</a></li>
												</dsp:oparam>	 --%>									
												<dsp:oparam name="output">	
												<dsp:getvalueof var="attribute" param="element.rollupAttribute"/>
												<c:choose>
													<c:when test="${(sizeParam != null) && (sizeParam eq attribute)}">
														<li class="selected"><a href="#">${attribute}</a></li>
													</c:when>
													<c:otherwise>
											...
											<c:if test="${menu eq 'FINISH'}">
												...
												<dsp:param param="element" name="array" />	
													...
													<dsp:oparam name="output">
														...
														<c:choose>
															<c:when test="${(colorParam != null) && (colorParam eq attribute)}">
																<a href="#" class="${fn:toLowerCase(attribute)} fl selected" data="${attribute}" title="${attribute}" data-s7ImgsetID="${largeImagePath}" data-imgURLThumb="${scene7Path}/${thumbnailImagePath}">
																<span>
																	<c:choose>
															...
															<c:otherwise>
																<a href="#" class="${fn:toLowerCase(attribute)} fl" data="${attribute}" title="${attribute}" data-s7ImgsetID="${largeImagePath}" data-imgURLThumb="${scene7Path}/${thumbnailImagePath}">
																<span>
																	<c:choose>
																		<c:when test="${(finishImagePath != null) && (finishImagePath ne '')}">
																			<img src="${scene7Path}/${finishImagePath}" height="20" width="20" alt="${attribute}" class="noImageFound" />
