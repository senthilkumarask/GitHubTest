				...
				<dsp:form name="testcollectionForm" id="testcollectionForm" method="post">
					...
					<dsp:param param="collectionVO.childProducts" name="array" />
						...
						<dsp:oparam name="output">
						   ...
						   <fieldset class="registryDataItemsWrap listDataItemsWrap" itemscope itemtype="http://schema.org/Product">
						   ...
						   <%-- Image --%>
							...
							<div class="grid_3 alpha marBottom_10">
								<c:choose>
									...
									<dsp:a iclass="prodImg" page="${finalUrl}?poc=${parentProductId}&categoryId=${CategoryId}" title="${prodName}" onclick="${onClickEvent}">
										...
										<c:otherwise>
											<img class="noImageFound" src="${scene7Path}/${prodImage}" width="229" height="229" alt="${prodName}" />
										</c:otherwise>
										</c:choose>
									</dsp:a>
								</c:when>
							...
							<span onclick="${onClickEvent}" tabindex="0" class="quickView showOptionMultiSku block marTop_10"><bbbl:label key="lbl_product_quick_view" language="${pageContext.request.locale.language}" /></span>
							</div>
												...
												<c:choose>
													<c:when test="${ratingAvailable == true}">
														...
														<c:choose>
															...
															<c:otherwise>
																<div class="prodReviewGrid prodReviews ratingsReviews clearfix prodReview<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/>" title="<dsp:valueof param="element.bvReviews.averageOverallRating"/>">
																<dsp:a page="${finalUrl}?skuId=${catalogRefId}&amp;categoryId=${CategoryId}&amp;showRatings=true">
						...
						<%-- Attribute--%>
									...
									<%-- Price--%>	
									   ...
									   <c:choose>
										   ...
										   <c:otherwise>
											   <div class="small-12 columns quantity"> <button class="minus secondary"><span></span></button>
											   		<input type="tel" id="quantity" class="quantity-input" value="1" name="${skuid}" maxlength="2"/> <button class="plus secondary"><span></span></button> 
											   </div>
											</c:otherwise>
									  </c:choose>
														...
														<c:choose>
															...
															<div class="fl collectionCart">
																<div class=" button_active">
																	...
																	<input onclick="${onClickEvent}" type="button" class="button showOptionMultiSku" name="showOptionMultiSku" value="${chooseOptionBtn}" role="button" aria-pressed="false">
																</div>
															</div>
															...
															<c:otherwise>
																<%--LTL 498 start--%>
																<div class="<c:if test="${not empty skuid && not empty isLtlItem && isLtlItem}">hidden</c:if>">
																...
																<c:otherwise>
																	<div class="selectOption hidden">
																		...
																		<div class=" button_active">
																			<c:set var="chooseOptionBtn"><bbbl:label key='lbl_pdp_grid_choose_options' language="${pageContext.request.locale.language}" /></c:set>
																			<input type="button" class="button showOptionMultiSku" name="btnSelectOption" id="btnSelectOption${skuid}" value="${chooseOptionBtn}" role="button" aria-pressed="false" onclick=""/>
																		</div>
																	</div>
																<%--LTL 498 end--%>	
															</c:otherwise>
															...
															<c:when test="${transient == 'false'}">
																<c:choose>
																	<c:when test="${sizeValue>1}">
																		...
																		<div class="addToRegistry btnAddToRegistry marBottom_5 fr cb">
																			<div class="button addToRegistrybtn btnAddToRegistry">
																	...
																	<c:otherwise>
																			<div class="addToRegistry btnAddToRegNoRegistry marBottom_5 fr cb">
																				...
																				<div class="button addToRegistrybtn btnAddToRegNoRegistry">
															...
															<c:when test="${transient == 'true'}">
																<%-- <div class="addToRegistry addItemToRegNoLogin marBottom_5 fr cb">
   																...
   																<div class="addToRegistry clearfix">
																	<div>
																		<input  class="addItemToRegNoLogin tiny button expand secondary" name="addItemToRegNoLogin" type="button" role="button" value="${submitAddToRegistryBtn}" />
																	</div>
																</div>
											...
											<c:choose>
												...
												<c:otherwise>
													<div class="btnPDLinks padTop_10 clearfix cb">
														<div class="addToList fl padRight_5" id="btnAddToList${skuid}" onclick="rkg_micropixel('BedBathUS','wish')">
																...
																<c:choose>
																	...
																	<c:otherwise>
																		<div class="changeStore fr cb">
																			<a href="javascript:void(0);" role="link" onclick="pdpOmnitureProxy('${productID}', 'findinstore');rkg_micropixel('${appid}','findstore');"><bbbl:label key='lbl_compare_find_in_store' language="${pageContext.request.locale.language}" /></a>
																		</div>
																	</c:otherwise>
																</c:choose>
															</div>
