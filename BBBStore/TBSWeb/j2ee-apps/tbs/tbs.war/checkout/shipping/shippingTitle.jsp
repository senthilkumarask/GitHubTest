...
<dsp:page>
        ...
        <div class="clearfix">
            ...
            <c:choose>
                <c:when test="${link == 'single'}">
                    <h2 class="section fl"><bbbl:label key="lbl_bread_crumb_shipping" language="${pageContext.request.locale.language}" />:&nbsp;<span class="subSection"><bbbl:label key="lbl_shipping_single_page_title" language="${pageContext.request.locale.language}" /></span></h2>
                    ...
                    <div class="fr suffix_3 shippingOption marTop_10">
						...
						<strong>
						<bbbl:label key="lbl_shipping_multiple_page_link" language="${pageContext.request.locale.language}" />
						<c:choose>
						<c:when test="${MapQuestOn}"  >
							&nbsp;<bbbl:label key="lbl_or_store_pickup" language="${pageContext.request.locale.language}" />
						</c:when>
						</c:choose>
						</strong>
						</dsp:a>
                ...
                <c:when test="${link == 'multiple'}">
                    <h2 class="section fl"><bbbl:label key="lbl_bread_crumb_shipping" language="${pageContext.request.locale.language}" />:&nbsp;
                    	<span class="subSection">
                    	<c:choose>
                    		<c:when test="${orderHasLTLItem }">
                    	    	<bbbl:label key="lbl_shipping_advanced_page_title" language="${pageContext.request.locale.language}" />
                    		</c:when>
                    		<c:otherwise>
                    	    	<bbbl:label key="lbl_shipping_multiple_page_title" language="${pageContext.request.locale.language}" />
                    		</c:otherwise>
                    	</c:choose>
                    	</span>
                    </h2>
                    <dsp:droplet name="/com/bbb/commerce/shipping/droplet/DisplaySingleShippingDroplet">
                        ...
                        <dsp:oparam name="single">
                            <div class="fr suffix_3 shippingOption marTop_10">
                                <dsp:a page="/checkout/shipping/shipping.jsp"><strong><bbbl:label key="lbl_shipping_single_page_link" language="${pageContext.request.locale.language}" /></strong></dsp:a>
                            </div>
                        </dsp:oparam>                    
                    </dsp:droplet>
    ...
    <dsp:getvalueof var= "errorList" bean = "PayPalSessionBean.errorList"/>
	 ...
	 <c:if test="${paypalShipping eq true}">
		<span class="payPalAddressError error marLeft_10">
