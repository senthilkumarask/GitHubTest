		...
		<dsp:getvalueof var="shipMethodVOList" param="shipMethodVOList"/>
			<dsp:droplet name="ForEach">
				...
				<dsp:oparam name="output">
					...
					<c:if test="${shipMethodId ne 'LWA'}">
						...
						<label class="inline-rc radio gray-panel" id="lblshippingMethod${shipMethodDescription}" for="shippingMethod_${index}${countIndex}">
							<c:choose>
			...
			<c:if test="${isWhiteGlovePresent && skuAssemblyOffered }">
				<div class="checkboxItem input clearfix marLeft_25">
					<div class="checkbox columns small-1 <c:if test="${(selectedShippingMethod ne 'LW' && selectedShippingMethod ne 'LWA')}">checker</c:if>">
					...
					<div class="label">						
						<label for="assembly${index}">
							<span>Add Assembly - $<fmt:formatNumber pattern="0.00" value="${assemblyFees}"/></span>
						</label>
					</div>
				</div>
			</c:if>
