<dsp:page>
    ...
    <div class="row hidden" id="paymentForm">
        ...
        <div class="small-12 columns">
            ...
            <dsp:droplet name="BBBPaymentGroupDroplet">
                ...
                <dsp:oparam name="output">
                    ...
                    <dsp:getvalueof var="isKirsch" param="isKirsch"/>
                    <dsp:getvalueof var="isCMO" param="isCMO"/>
                    ...
                    <c:if test="${ValueLinkOn and not isCMO}">
                    	<div class="row">
                            <div class="small-12 columns">
	                            ...
	                            <div class="small-12 large-4 columns">
	                                <h3 class="checkout-title">Payment Method</h3>
	                                	<label class="inline-rc radio gray-panel payment-type-trigger register" for="registerPaymentOpt">
                                            <input id="registerPaymentOpt" type="radio" name="paymentOpt" value="register" checked="checked"/>
                                            <span></span>
                                            Pay at Register
                                            <%-- <div class="due-amount-payatregister hidden">
                                                 <bbbl:label key="lbl_payment_amtCharged_payatregister" language="${pageContext.request.locale.language}"/> <dsp:valueof value="${payAtRegisterTotal}" converter="currency"/>
                                            </div> --%>
                                        </label>
                                </div>
                               </div>
		                                            ...
		                                            <c:when test="${paypalOn and not ((orderType eq 'BOPUS_ONLY') or (orderType eq 'HYBRID'))}">
		                                                <%-- paypal is enabled --%>
		                                                    <label class="inline-rc radio gray-panel payment-type-trigger credit-card <c:if test="${isOrderAmtCoveredVar eq true}">disabled</c:if>" for="creditCartPaymentOpt">
		                                                        <c:choose>
		                                                            <c:when test="${payPalOrder ne true}">
		                                                                <input id="creditCartPaymentOpt" type="radio" name="paymentOpt" value="creditCard" <c:if test="${isOrderAmtCoveredVar eq false}">checked="checked"</c:if> />
		                                                            </c:when>
		                                                            <c:otherwise>
		                                                        ...
		                                                        <%-- <div class="due-amount-creditcard">
		                                                        	<bbbl:label key="lbl_payment_amtCharged" language="<c:out param='${language}'/>"/><dsp:valueof value="${priceValue}" converter="currency"/>
		                                                        </div> --%>
		                                                    </label>
		                                                    ...
		                                                    <%-- <div class="due-amount-paypal hidden">
		                                                   		 <bbbl:label key="lbl_payment_amtCharged_paypal" language="${pageContext.request.locale.language}"/> <dsp:valueof value="${priceValue}" converter="currency"/>
		                                                    </div> --%>
		                                                </label>
		                                            </c:when>
		                                            <c:otherwise>
		                                                                                    ...
		                                                                                    <div class="small-12 columns cvvPayUsingSavedCard <c:if test="${key != selectedId}">hidden</c:if>">
		                                                                                        ...
		                                                                                        <c:choose>
		                                                                                            <c:when test="${key != selectedId}">
		                                                                                                <dsp:input disabled="disabled" id="securityCodeCVV${key}" type="text" name="securityCodeCVV" iclass="textRight securityCodeCVV ${uiCreditCardType}" maxlength="4" bean="PaymentGroupFormHandler.verificationNumber" autocomplete="off" value="">
		...
		<%--                                            <dsp:property bean="CartModifierFormHandler.payPalErrorURL" value="/checkout/payment/billing_payment.jsp" priority="10"/> --%>
                    ...
                    <c:choose>
                       ...
                       <c:otherwise>
                           <dsp:form name="frmAddPayAtRegister" iclass="payment-type register" id="frmAddPayAtRegister" formid="frmAddPayAtRegister" action="" method="post">
                                <div class="row">
                                    <div class="small-12 columns">
                                        <dsp:input type="submit" value="Pay At Register" iclass="small button service right" bean="PaymentGroupFormHandler.pARPayment" id="paymentNextBtn">
                                            <dsp:tagAttribute name="aria-pressed" value="false"/>
