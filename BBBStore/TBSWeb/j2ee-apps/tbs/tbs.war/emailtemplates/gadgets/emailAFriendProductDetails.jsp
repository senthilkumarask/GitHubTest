<dsp:page>
  ...
  <dsp:getvalueof var="productThumbnailImageUrl" param="product.thumbnailImage"/>
          ...
          <c:choose>
            <c:when test="${not empty productThumbnailImageUrl}">
              ...
              <dsp:a page="${var_productUrl}">
                <img src='<c:out value="${httpServer}" /><dsp:valueof param="product.thumbnailImage"/>' title="${getDetailsTitleText}" alt="${imageAltText}"/>
              </dsp:a>
            </c:when>
            <c:otherwise>
      ...
      <c:otherwise>
          <td style="padding-right: 10px">
            <c:choose>
              <c:when test="${not empty productThumbnailImageUrl}">
                <%-- Thumbnail image exists --%> 
                  <img src='<c:out value="${httpServer}" /><dsp:valueof param="product.thumbnailImage"/>' title="${getDetailsTitleText}" border="0" alt="${imageAltText}"/>
              </c:when>
              <c:otherwise>
    ...
    <div style="margin-top:20px">
      <table border="0" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td align="center" style="background-color:#A3CAFF;padding-left:8px;padding-right:8px;text-align:center;font-family:Verdana,arial,sans-serif;font-size:12px;font-weight:bold"><a style="color:#FFFFFF;text-decoration:none" href="${var_productUrl}">View Details</a></td>
