<dsp:page>
	...
	<c:set var="pageTypeForCanonicalURL" value="TopConsultant" scope="request" />
	...
	<bbb:pageContainer>
		...
		<jsp:attribute name="pageWrapper">useFB useCertonaJs productDetails useBazaarVoice browseRegistryCollection </jsp:attribute>
	<jsp:body>
		...
		<div id="content" class="container_12 clearfix topConsultantsPicks" role="main">		
			<dsp:form method="post" id="frmRowAddToRegistry" action="${contextPath}/kickstarters/top_consultants_picks.jsp">
				<div class="registryDataItemsWrap listDataItemsWrap" style="display:none;">
					<input type="hidden" id="isLtlItem" class="frmAjaxSubmitData _isLtlItem addItemToRegis addItemToList" />
		            ...
		            <input type="hidden" name="isCustomizationRequired"  value="" class="frmAjaxSubmitData addItemToRegis _skuId addItemToList changeStoreSkuId" data-change-store-submit="skuId" data-change-store-errors="required" data-change-store-internaldata="true" />  
        	...
        	<div class="grid_12 topPicksHeader">
                ...
                <div class="grid_9 alpha">
                        <h1 class="txtOffScreen"><bbbl:label key="lbl_top_consultants_picks" language="${pageContext.request.locale.language}" /></h1>
                	<h2 class="mainHeader"><bbbl:label key="lbl_top_consultants_picks" language="${pageContext.request.locale.language}" /></h2>
                </div>
              ...
              <dsp:importbean bean="/com/bbb/kickstarters/droplet/KickStarterPagination" />
					...
					<dsp:param name="kickStarterType" value="Top Consultant" />
					<dsp:param name="siteId" value="${appid}" />
				...
				<div class="grid_3 omega topConsultantsPicksNav marTop_20" >
               		...
               		<c:if test="${not empty fn:trim(previousId)}">
						<c:choose>
							<c:when test="${not empty param.eventType && not empty param.registryId }">
							 <a href="${contextPath}/topconsultant/${previousId}/${param.registryId}/${param.eventType}"><bbbl:label key="lbl_top_consultants_picks_prev_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:when test="${not empty param.eventType && empty param.registryId }">
							 <a href="${contextPath}/topconsultant/${previousId}/${param.eventType}"><bbbl:label key="lbl_top_consultants_picks_prev_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:when test="${empty param.eventType && not empty param.registryId }">
							 <a href="${contextPath}/topconsultant/${previousId}/${param.registryId}"><bbbl:label key="lbl_top_consultants_picks_prev_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:otherwise>
							 <a href="${contextPath}/topconsultant/${previousId}"><bbbl:label key="lbl_top_consultants_picks_prev_list" language="${pageContext.request.locale.language}" /></a>
							</c:otherwise>
						</c:choose>
					</c:if>
					<c:if test="${not empty fn:trim(previousId) && not empty fn:trim(nextId)}">|</c:if>
               		<c:if test="${not empty fn:trim(nextId)}">
               			<c:choose>
							<c:when test="${not empty param.eventType && not empty param.registryId }">
								<a href="${contextPath}/topconsultant/${nextId}/${param.registryId}/${param.eventType}"><bbbl:label key="lbl_top_consultants_picks_next_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:when test="${not empty param.eventType && empty param.registryId }">
								<a href="${contextPath}/topconsultant/${nextId}/${param.eventType}"><bbbl:label key="lbl_top_consultants_picks_next_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:when test="${empty param.eventType && not empty param.registryId }">
								<a href="${contextPath}/topconsultant/${nextId}/${param.registryId}"><bbbl:label key="lbl_top_consultants_picks_next_list" language="${pageContext.request.locale.language}" /></a>
							</c:when>
							<c:otherwise>
								<a href="${contextPath}/topconsultant/${nextId}"><bbbl:label key="lbl_top_consultants_picks_next_list" language="${pageContext.request.locale.language}" /></a>
							</c:otherwise>
						</c:choose>
					</c:if>
                </div>
			...
			<dsp:oparam name="output">
				...
				<dsp:getvalueof var="imagePath" value="${kickStarterVO.imageUrl }"/>
           	...
           	<div class="grid_12 topConsultantDetails">
                <div class="grid_3 alpha">
                	<img class="topConsultantsPicksImage" src="${imagePath}" height="209" width="209" alt="top Consultants Picks Image"/>
                </div>	        	
	        	
	        	<div class="grid_6 topConsultantInfo">
	        		<h3 class="consultantName">${heading1}</h3>
	        		<h5>${heading2}</h5>
	        		<p>${description}</p>
					...
					<div class="grid_3 shopLookSocialLinks omega" >
	                    ...
	                    <c:set var="tcpTitle"><bbbl:label key="lbl_top_consultants_picks" language="${pageContext.request.locale.language}" /></c:set>
	                    
	                    <dsp:droplet name="/com/bbb/account/droplet/BBBURLEncodingDroplet">
	                        <dsp:param name="URL" value="${tcpTitle} : ${heading1} - ${heading2}"/>
	                        <dsp:oparam name="output">
	                	...
	                	<a href="//www.facebook.com/sharer.php?s=100&p[url]=${shareURL}&p[images][0]=${imagePath}&p[title]=${encodedHeading}" target="_blank" class="social_button"><img src="/_assets/global/images/kickstarters/button_facebook.png" alt="facebook"></a>
                	<a href="https://twitter.com/share?url=${shareURL}&text=${encodedHeading}" target="_blank" class="social_button"><img src="/_assets/global/images/kickstarters/button_twitter.png" alt="twitter"></a>
                	<a href="https://pinterest.com/pin/create/bookmarklet/?url=${shareURL}&media=${imagePath}&description=${encodedHeading}" target="_blank" class="social_button"><img src="/_assets/global/images/kickstarters/button_pinterest.png" alt="pinterest"></a>
	                	<a href="https://plus.google.com/share?url=${shareURL}" target="_blank" class="social_button"><img src="/_assets/global/images/kickstarters/button_google_plus.png" alt="google_plus"></a>
					
	                </div>
                </div>
	        	<dsp:getvalueof var="targeEventType" param="eventType"/>
	        	<c:if test="${empty param.eventType}"> 			
		             <dsp:getvalueof var="targeEventType" param="kickStarterVO.kickStarterPicklists[0].registryTypes[0].registeryType"/>
		        </c:if>
				<dsp:include page="/giftregistry/frags/registry_promo_boxes.jsp">	
					<dsp:param name="pageName" value="topConsultantDetails" />
           	...
           	<div id="shopCollection" class="collectionItemsBox grid_12 clearfix collectionGridView kickStarterCollection">
           	...
           	<div id="pickListDescription" class="grid_12  clearfix">
           		...
           		<div class="grid_12 alpha omega clearfix">
	           		<div class="grid_9 alpha">
	           			<h2 class="pickListDescriptionHeader"><bbbl:label key="lbl_top_consultant_collection_summary_header" language="${pageContext.request.locale.language}" /></h2>
					...
					<div class="grid_3 omega">
	        			<c:choose>				        
					        <c:when test="${isTransient == 'false' && sizeValue>=1}">
					        	<div class="pickListDescriptionAddButton">
	                	            <dsp:include page="/addgiftregistry/add_kickstarter_items_gift_registry.jsp">
								<dsp:param name="heading1" value="${heading1}" />
								</dsp:include>
	                	        </div>   
           	...
           	<c:set var="omniDesc" value="Popular items (top consultant)-${heading1}"/>            		
           	<dsp:include page="popular_items_grid.jsp?eventType=${targeEventType}" flush="true" >
           	<dsp:param name="omniDesc" value="${omniDesc}" /></dsp:include>		
