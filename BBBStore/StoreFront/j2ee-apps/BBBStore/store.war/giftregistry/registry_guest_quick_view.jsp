...
<dsp:importbean bean="/atg/userprofiling/Profile" />
	...
	<dsp:getvalueof var="userRegCount" param="userRegCount" />
	<dsp:getvalueof var="sessionRegId" param="sessionRegId"/>
	<dsp:getvalueof var="bopusAllowed" param="bopusAllowed"/>
...
<dsp:droplet name="ProductDetailDroplet">
	...
	<dsp:oparam name="output">
						...
						<c:otherwise>
							<c:choose>
								<c:when test="${!(not empty refNum && disableItemDetails eq 'true')}">
									<a class="regQVProdTitle" href="${contextPath}${pdpURL}"><dsp:valueof param="productVO.name" valueishtml="true" /></a>
								</c:when>
								<c:otherwise>
									<a class="regQVProdTitle" href="javascript:void(0);" oncontextmenu="return false;" class="cursorDefault"><dsp:valueof param="productVO.name" valueishtml="true" /></a></h2>
								</c:otherwise>
							</c:choose>
						</c:otherwise>
						</c:choose>
		...
		<div class="fr registryCTAContainer giftGiverCTA">
			<input type="hidden" name="registryId" value="${registryId}" class="frmAjaxSubmitData addItemToRegis addItemToList registryId" data-change-store-submit="registryId" data-change-store-errors="required"/>
            <input type="hidden" name="skuId" value="${skuId}" class="frmAjaxSubmitData addItemToRegis addItemToList skuId changeStoreSkuId" data-change-store-submit="skuId" data-change-store-errors="required" />
            <input type="hidden" name="prodId" value="${productId}" class="frmAjaxSubmitData addItemToRegis addItemToList productId" data-change-store-submit="prodId"/>
            <input type="hidden" name="rowId" value="${rowID}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="parentProdId" value="${productId}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <c:choose>
            <c:when test="${not empty sopt}">
            <input type="hidden" name="ltlDeliveryServices" value="${sopt}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            </c:when>
            <c:otherwise>
            <input type="hidden" name="ltlDeliveryServices" value="" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            </c:otherwise>
            </c:choose>
            <input type="hidden" name="ltlDeliveryPrices" value="${deliverySurcharge}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="alternateNum" value="" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="regItemOldQty" value="${regItemOldQty}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="itemTypes" value="${itemType}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="refNum" value="${refNum}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="updateDslFromModal" value="" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="itemPrice" value="" class="frmAjaxSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="bts" value="false" class="addToCartSubmitData addItemToRegis addItemToList" />
            <input type="hidden" name="isLtlItem" value="${ltlItemFlag}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
			<input type="hidden" name="ltlShipMethod" value="${sopt}" class="frmAjaxSubmitData addItemToRegis addItemToList" />
			...
			<div class="grid_2 fl productRequested">
				...
				<div class="input alpha marTop_5 clearfix spinner" tabindex="0" aria-label="Requested:${regItemOldQty}">
					${regItemOldQty}
				</div>
			</div>
			...
			<div class="grid_2 fl productPurchased">
				...
				<div class="input alpha marTop_5 clearfix spinner" tabindex="0" aria-label="Purchased:${qtyPurchased}">
					${qtyPurchased}
				</div>
			</div>

			<div class="grid_2 fl productQuantity">
				<%-- Requested Count --%>
				<div class="input alpha marTop_5 clearfix spinner">
                    <div class="text clearfix">
                        ...
                        <input id="quantityPurchased" aria-labelledby="${regItemOldQty} of ${productName}" title="${regItemOldQty} of ${productName}." name="qty" value="1" class="valQtyGiftReg input_tiny_giftView fl itemQuantity" data-change-store-errors="required digits nonZero" data-change-store-submit="qty" type="text" aria-required="false" maxlength="2">
          ...
          <%--  add to registry button starts --%>
            <c:if test="${not notloggedIn}">
			<div class="grid_2 fl addToRegistryContainer">
				<dsp:form>
					<c:choose>
						<c:when test="${personalizationType =='PB' || personalizationType =='CR'}">
					...
					<div class="fl marginButtonFix">
						<dsp:include page="/addgiftregistry/add_item_gift_registry.jsp">
							<dsp:param name="ltlFlag" value="${ltlItemFlag && currentSiteId ne BedBathCanadaSite}" />
			...
			<%--  add to registry button ends --%>

			<%--  add to cart button starts --%>

			<div class="grid_2 fl addToCartContainer">
				<c:choose>
					 <c:when test="${(regItemOldQty > qtyPurchased) || (regItemOldQty == 0 && qtyPurchased ==0)}">
					 <div class="addToCart clearfix marBottom_10 <c:if test="${emptyOutOfStockListFlag ne 'true'}">disabled</c:if> <c:if test="${(isInternationalCustomer && isIntlRestricted) || (isInternationalCustomer && not empty refNum) || (enableKatoriFlag eq false && not empty refNum)}">disabled</c:if>">
								<input type="button" class="button-Med addtocart" name="btnAddToCart" aria-label="Add ${productName} to your Cart " title="Add to cart" value="Add to Cart" role="button" <c:if test="${(isInternationalCustomer && isIntlRestricted) || (isInternationalCustomer && not empty refNum) || (enableKatoriFlag eq false && not empty refNum) || (emptyOutOfStockListFlag ne 'true')}">disabled="disabled"</c:if>/>
							</div>
			...
			<div class="btnPDLinks padTop_5 clearfix cb fr">
				<div class="addToList fl <c:if test="${isCustomizationRequired}">disabled opacity_1</c:if>" id="btnAddToList${skuid}">
 				...
 				<c:if test="${not bopusAllowed && MapQuestOn}">
 					<c:if test="${regItemOldQty > qtyPurchased}">
						<div class="rvChangeStore fl" tabindex="0" aria-labelledby="findVO productTitle inStoreVO"> 
	 						<a tabindex="-1" href="javascript:void(0);" role="link" onclick="pdpOmnitureProxy('${productId}', 'findinstore');rkg_micropixel('${appid}','findstore');" class="pickupStore changeStore <c:if test='${isInternationalCustomer or isPersonalized}'>disabled</c:if> "
			...
			<div class="clear"></div>
			<%--BBBSL-11683 | Adding refnum check to disable click of personalized items on guest view --%>
			<c:if test="${!(registryView eq 'guest' && not empty refNum)}">
		...
		<c:when test="${(skuVO.customizableRequired || skuVO.customizationOffered) && not empty skuVO.customizableCodes}">
			<script type="text/javascript">
                ...
                if (typeof s !== "undefined") {
                    ...
                    BBB.omnitureObj.backup_s("eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar47,productNum,prop7,prop8,prop12,prop25");
                    BBB.omnitureObj.clear_s("eVar3");
                    BBB.omnitureObj.delete_s("eVar1,eVar2,eVar4,eVar5,eVar6,eVar7,eVar8,eVar47,productNum,prop7,prop8,prop12,prop25");
                    ...
                    fixOmniSpacing();
                    s.t();
                    BBB.omnitureObj.restore_s("eVar1,eVar2,eVar3,eVar4,eVar5,eVar6,eVar7,eVar8,eVar47,productNum,prop7,prop8,prop12,prop25");
                }
			</script>
		</c:when>
		<c:otherwise>
