...
<c:set var="lblStoreNear"><bbbl:label key="lbl_accessibility_store_near_you" language="${pageContext.request.locale.language}" /></c:set>
	...
	<c:set var="showShipCustomMsg" value="true"/>
		...
		<dsp:droplet name="ProductDetailDroplet">
			...
			--%>
						...
						<div class="width_312px alpha fl grid_4 zoomProduct">
								...
								<div class="registryDataItemsWrap listDataItemsWrap clearfix <c:if test="${ (skuVO.customizableRequired || skuVO.customizationOffered) && not empty skuVO.customizableCodes}">personalizedItem</c:if>" data-refNum="">
									...
									<c:choose>
										<c:when test="${fromPage eq 'collectionGrid' || fromPage eq 'collectionList'}">
								            <dsp:a  onclick="javascript:(typeof quickViewCrossSell === 'function')?quickViewCrossSell():void(0);" page="${finalUrl}?poc=${parentProductId}&color=${color}&skuId=${skuId}" tabindex="0">
										 ...
										 <c:otherwise>
											<c:choose>
												<c:when test = "${not empty categoryId}">
													<dsp:a  onclick="javascript:(typeof quickViewCrossSell === 'function')?quickViewCrossSell():void(0);" page="${finalUrl}?categoryId=${categoryId}&color=${color}&skuId=${skuId}" tabindex="0">
														<dsp:valueof param="productVO.name" valueishtml="true" />
													</dsp:a>
											 	</c:when>
											 	<c:otherwise>
											 		<dsp:a  onclick="javascript:(typeof quickViewCrossSell === 'function')?quickViewCrossSell():void(0);" page="${finalUrl}?color=${color}&skuId=${skuId}" tabindex="0">
														<dsp:valueof param="productVO.name" valueishtml="true" />
													</dsp:a>
											 	</c:otherwise>
											</c:choose>
														...
														<ul class="swatches fl" id="colorSwatches" role="radiogroup">
															 <dsp:droplet name="/atg/dynamo/droplet/ForEach">
																	...
																	<dsp:oparam name="output">
																		...
																		<li class="${marginClass} colorSwatchLi" role="radio" aria-checked="${ariaChecked}" data-attr="${attribute}"
																...
																<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																	...
																	<dsp:oparam name="output">
																		...
																		<c:choose>
																			...
																			<c:set var="selectedClass" value="selected"/>
																			<div class="selectProdSizeBox ${selectedClass}">
																				<span>${attribute}</span>
																			</div>
																			</c:when>
																			<c:otherwise>
					...
					<c:choose>
						<c:when test="${(skuVO.customizableRequired || skuVO.customizationOffered) && not empty skuVO.customizableCodes && not isInternationalCustomer}">
							<div class="personalizationOffered clearfix cb fl">
								<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="${contextPath}${pdpUrl}?color=${color}&openKatori=true&skuId=${skuId}"</c:otherwise></c:choose> class="collectionPersonalize ${customizeAttr} <c:if test="${not enableKatoriFlag}">disabled</c:if>" role="button" data-sku="${pDefaultChildSku}" data-custom-vendor="${vendorName}" data-refnum="" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
									${personalizationTxt}
								</a>
								<c:choose>
							...
							<c:choose>
								<c:when test="${isInternationalCustomer}">
									...
									<c:when test="${(skuVO.customizableRequired || skuVO.customizationOffered) && not empty skuVO.customizableCodes}">
										<div class="personalizationOffered clearfix cb fl">
											<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="#"</c:otherwise></c:choose> class="personalize ${customizeAttr} disabled" role="button" data-sku="${pDefaultChildSku}" data-custom-vendor="${vendorName}" data-refnum="" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
												${personalizationTxt}
											</a>
									...
									<c:otherwise>
										<div class="personalizationOffered clearfix cb fl hidden ">
											<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="#"</c:otherwise></c:choose> class="personalize ${customizeAttr} disabled" role="button" data-sku="${pDefaultChildSku}" data-refnum="" data-quantity="" data-custom-vendor="${vendorName}" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
												${personalizationTxt}
											</a>
											<span class="personalizationIntlShippingMsg"><bbbt:textArea key="txt_pdp_per_unavailable_intl" language="${pageContext.request.locale.language}" /></span>
										</div>
								...
								<c:otherwise>
									<div class="personalizationOffered clearfix cb fl hidden ">
										<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="${contextPath}${pdpUrl}?color=${color}&openKatori=true&skuId=${skuId}"</c:otherwise></c:choose> class="collectionPersonalize ${customizeAttr} <c:if test="${not enableKatoriFlag}">disabled</c:if>" role="button" data-custom-vendor="${vendorName}" data-sku="${pDefaultChildSku}" data-refnum="" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
											${personalizationTxt}
										</a>
										<c:choose>
											<c:when test="${enableKatoriFlag}">
						...
						<%-- BPSI-2446 DSK VDC message & offset message changes --%>
									...
									<%-- LTL-PDP related changes Nikhil Koul start --%>
										...
										<div class="qvProdDescription marBottom_20">
											...
											<div id="qvProdDesc" class="bulletsReset rvProductDesc">
										  	...
										  	<div class="btnPD clearfix noPadBot">
											  	<div class="fr">
													  ...
													  <div class="quantityPDP">
														  <div class="spinner">
															  	<c:if test="${not frmComparisonPage}">
																	...
																	<a href="#" class="button-Med btnSecondary down" id="accListDecQty" title="<bbbl:label key='lbl_decrease_quantity' language="${pageContext.request.locale.language}" />"><span class="txtOffScreen"><bbbl:label key="lbl_decrease_quantity" language="${pageContext.request.locale.language}" /></span></a>
																	...
																	<a href="#" class="button-Med btnSecondary up" id="detCollIncQty" title="<bbbl:label key='lbl_increase_quantity' language="${pageContext.request.locale.language}" />"><span class="txtOffScreen"><bbbl:label key="lbl_increase_quantity" language="${pageContext.request.locale.language}" /></span></a>
																</c:if>
											       ...
											       <c:choose>
													...
													<div class="fl addToCart <c:if test="${isCustomizationRequired || inStock==false || (isInternationalCustomer && isIntlRestricted)}">disabled</c:if>">
															<input type="submit" class="button-Med btnSecondary addtocart" name="btnAddToCart" value="Add to Cart" onclick="rkg_micropixel('${appid}','cart')" role="button" <c:if test="${isCustomizationRequired || inStock==false || (isInternationalCustomer && isIntlRestricted)}">disabled="disabled"</c:if>/>
															<%--LTL 498 start--%>
															<c:choose>
				...
				<div class="lnkViewProductDetails clearfix">
				<dsp:droplet name="/atg/repository/seo/CanonicalItemLink">
  ...
  <dsp:param name="repositoryName"
     value="/atg/commerce/catalog/ProductCatalog"/>
  <dsp:oparam name="output">
    <dsp:getvalueof var="pdpURL" param="url" vartype="java.lang.String"/>
 </dsp:oparam>
</dsp:droplet>
