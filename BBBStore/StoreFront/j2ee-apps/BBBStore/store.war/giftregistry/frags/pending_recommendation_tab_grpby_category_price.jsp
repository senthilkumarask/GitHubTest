	...
	<c:set var="lblAboutThe"><bbbl:label key="lbl_accessibility_about_the" language="${pageContext.request.locale.language}" /></c:set>
			...
			<ul class="productDetailList giftViewProduct">
							...
							<a class="lnkQVPopup" href="${contextPath}${finalUrl}?skuId=${skuId}&registryId=${registryId}" data-pdp-qv-url="pdp_quick_view.jsp" data-skuId="${skuId}" data-registryId="${registryId}" data-productId="${productId}" data-refNum="${refNum}" title="${skuDisplayName}&nbsp;${skuListPriceCurrency}">
								<img class="fl productImage noImageFound" src="${scene7Path}/${skuImage}" alt="${skuDisplayName}&nbsp;${skuListPriceCurrency}"
							...
							<div class="productContainer grid_10 omega">
								<div class="productTab productContent prodInfo clearfix">
									...
									<c:choose>
										...
										<span class="prodReviews clearfix metaFeedback"><span class="ratingsReviews prodReviews<fmt:formatNumber value="${rating}" pattern="#0" /> ratingTxt"><span class="ariaLabel">${ratingsTitle}</span></span>
								...
								<span class="prodReviews metaFeedback">
                                    ...
                                    <span class="ratingsReviews ratingTxt"><span class="ariaLabel">${ratingsTitle}</span></span><span class="writeReview reviewTxt"><a href="${contextPath}${finalUrl}?categoryId=${CategoryId}&writeReview=true" title="${writeReviewLink}" role="link" aria-label="${writeReviewLink} ${lblAboutThe} ${productName}">${writeReviewLink}</a></span>
										...
										<c:if test="${skuColor != null}">
				                     	...
				                     	/store/giftregistry/pdp_quick_view.jsp?skuId=${skuId}&registryId=${registryId}&productId=${productId}&bbbModalDialog=true
											...
											data-pdp-qv-url="pdp_quick_view.jsp" data-skuId="${skuId}" data-registryId="${registryId}" data-productId="${productId}" data-refNum="${refNum}" title="${skuDisplayName}" role="link" aria-label="${lblProductQuickView} ${lblOfThe} ${skuDisplayName}"> <span
									...
									<input type="hidden" name="isLtlItem" value="${ltlItem}" />
										<c:if test="${not(ltlItem && fn:containsIgnoreCase(enableLTLRegForSite,'false'))}">
											<a href="#" onclick="s_objectID=&quot;//bedbathandbeyond.com:7003/store/giftregistry/view_registry_owner.jsp?registryId=${registryId}&amp;_39&quot;;return this.s_oc?this.s_oc(e):true"
											class="btnAddToRegistryRecommendation button-Med btnPrimary centerTxt <c:if test="${isInternationalCustomer}">disabled</c:if>" aria-label="<bbbl:label key="lbl_add_to_registry" language="${pageContext.request.locale.language}"/>" data-notify-reg="true" >
											<bbbl:label key="lbl_add_to_registry" language="${pageContext.request.locale.language}"/></a>
										</c:if>
										<a href="#" onclick="s_objectID=&quot;//bedbathandbeyond.com:7003/store/giftregistry/view_registry_owner.jsp?registryId=203518844&amp;_39&quot;;return this.s_oc?this.s_oc(e):true"
										class="btnAddToRegistryRecommendation declineRecommendation button-Med btnSecondary marTop_10">Decline</a>
									</div>
								</div>
