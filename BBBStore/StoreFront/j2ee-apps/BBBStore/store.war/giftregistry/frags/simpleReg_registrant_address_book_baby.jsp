...
<dsp:getvalueof var="shipZipFromBean" bean="GiftRegistryFormHandler.registryVO.shipping.shippingAddress.zip"/>
	...
	<c:if test="${!isTransient}">
	   ...
	   <dsp:getvalueof var="addressLine1" bean="Profile.shippingAddress.address1" />
		...
		<dsp:getvalueof  var="profileFirstName"
                     bean="Profile.firstName" />
        <dsp:getvalueof  var="profileLastName"
                     bean="Profile.lastName" />
		<dsp:getvalueof
			var="contactAddressLine1"
			bean="Profile.shippingAddress.address1" />
		<dsp:getvalueof
			var="contactAddressLine2"
			bean="Profile.shippingAddress.address2" />
		<dsp:getvalueof
			var="contactAddressCity"
			bean="Profile.shippingAddress.city" />
		<dsp:getvalueof
			var="contactAddressState"
			bean="Profile.shippingAddress.state" />
		<dsp:getvalueof
			var="contactAddressPostCode"
  ...
  <label for="txtRegistryRegistrantContactAddress" generated="true" class="contactValidAddMsg error errorLabel">
				...
				<c:if test="${inputListMap['useContactAddrAsShippingAddr'].isDisplayonForm && inputListMap['showShippingAddress'].isDisplayonForm && inputListMap['showContactAddress'].isDisplayonForm}">
               	...
               	<div class="checkboxItem inputField clearfix marTop_10 padBottom_5">
					<div class="checkbox">
						<c:choose>
					...
					<div class="label shippingAdd">
						<label id="lblradRegistryCurrentShippingAddress" for="lblradRegistryCurrentShippingAddress" class="addChkboxTxt txtStyle">
							...
							<span><bbbl:label key="lbl_reg_send_gifts" language ="${pageContext.request.locale.language}"/></span>
						</label>
				...
				<c:set var="lbl_reg_ph_apt_building"><bbbl:label key="lbl_reg_ph_apt_building" language ="${pageContext.request.locale.language}"/></c:set>
                  <div class="inputField clearfix grid_4 marTop_10 marBottom_10" id="shippingNewAddrLine1">
                 <c:choose>
					<c:when test="${shippingAddr ne '' && shippingAddr ne null}">
					<div class="grid_4 alpha omega">
					<label class="txtOffScreen" id="lblRegistryCurrentShippingAddress" for="txtRegistryCurrentShippingAddress">${lbl_reg_ph_shippingAddress}</label>
                    <input id="txtRegistryCurrentShippingAddress"
                        type="text"
                        aria-labelledby="lblRegistryCurrentShippingAddress "
                        name="txtRegistryCurrentShippingAddress1Name"
                         class="${isRequired} cannotStartWithWhiteSpace QASAddress1" maxlength="90" placeholder="${lbl_reg_ph_shippingAddress}" value="${shippingAddr}">
					</div>
				...
				<label class="txtOffScreen" id="lbltxtRegistryCurrentShippingAddress2" for="txtRegistryCurrentShippingAddressApt">${lbl_reg_ph_apt_building}</label>
                 ...
                 <dsp:input type="hidden" bean="GiftRegistryFormHandler.shippingAddress" id="shippingAddressSelect"/>
                    ...
                    <input id="txtRegistryCurrentShippingAddress"
                        type="text"
                         aria-labelledby="lblRegistryCurrentShippingAddress lblErrorRegistryCurrentShippingAddress"
                        name="txtRegistryCurrentShippingAddress1Name"
                         class="${isRequired} cannotStartWithWhiteSpace QASAddress1" maxlength="90" placeholder="${lbl_reg_ph_shippingAddress}" value="${profileShippingAddr}">
						 
						 <label for="txtRegistryCurrentShippingAddress" id="lblErrorRegistryCurrentShippingAddress" generated="true" class="contactValidAddMsg error errorLabel">
					</label>
