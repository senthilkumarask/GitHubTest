...
<div id="pending-registry">
				...
				<div class="sort_container">
					<ul class="inner_tab fl">
						...
						<li class="recommendInnerTab"><a href="javascript:void(0);" data-recomm="acceptedRecommendation" class="active"><bbbl:label
		 ...
		 <dsp:oparam name="output">
		   <dsp:getvalueof var="groupByFlag" param="groupByFlag" />
				...
				<c:if test="${groupByFlag eq 'date' }">
					...
					<c:set var = "countt" value= "0" scope="session"/>	
				<dsp:droplet name="ForEach">
					<dsp:param name="array" value="${recommendationProductList}" />
					
					<dsp:oparam name="output">
						<dsp:getvalueof var="skuId" param="element.skuId" />
						<dsp:getvalueof var="skuDisplayName" param="element.skuDisplayName" />
						...
						<dsp:getvalueof var="recommendedQuantity" param="element.recommendedQuantity" />
						<dsp:getvalueof var="acceptedQuantity" param="element.acceptedQuantity" />
						<dsp:getvalueof var="purchasedQuantity" param="element.purchasedQuantity" />
						<dsp:getvalueof var="ltlItem" param="element.ltl" />
						<dsp:getvalueof var="skuAttributes" param="element.skuAttributes" />
						<dsp:getvalueof var="ltlDeliveryServices" param="element.ltlShipMethod" />
	...
	<c:when test="${groupByFlag eq 'date' }">
		...
		<dsp:include page="accepted_recommendation_tab_grpby_recommender_date.jsp"
			flush="true">
			...
			<dsp:param name="acceptedQuantity" value="${acceptedQuantity}" />
			...
			<dsp:param name="ltlShipMethod" value="${ltlDeliveryServices}"/>
			<dsp:param name="skuAttributes" value="${skuAttributes}" />
			...
			<dsp:param name="skuIncartFlag" value="${skuIncartFlag}" />
				...
				<c:if test="${groupByFlag eq 'category' or groupByFlag eq 'price' or groupByFlag eq 'recommender'}">
									<dsp:include page="accepted_recommendation_tab_grpby_category_price.jsp" flush="true" >
										<dsp:param name="scene7Path" value="${scene7Path}"/>
									</dsp:include>
								</c:if>
		...
		<dsp:oparam name="error">
	          <c:if test="${pageNum eq '0'}">
	           <div class="emptyRecommenderMsg"><bbbt:textArea key="txt_empty_accepted_recommendations" language ="${pageContext.request.locale.language}"/></div>
	          </c:if>
        </dsp:oparam>
	</dsp:droplet>
   </div>
