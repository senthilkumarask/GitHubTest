		...
		<c:set var="topRegistryMax" scope="request">
		 ...
		 <dsp:droplet name="CertonaDroplet">
			...
			<c:if test="${empty view}">
				...
				<%--Commented this piece of code as part of BBBP-7676 Jsp precompilation story as this causes compilation errors 
				but this need more analysis on the errors--%>
				<%-- <dsp:form name="frmCertonaProduct" method="post" id="formID${idx} action="/store/giftregistry/kickstarters/">
	            ...
	            <dsp:input bean="GiftRegistryFormHandler.successURL" type="hidden" value="/store/giftregistry/view_registry_owner.jsp?eventType=${eventType}&registryId=${registryId}&sorting=${sorting}&view=${view}" />
	            <dsp:input bean="GiftRegistryFormHandler.errorURL" type="hidden" value="/store/giftregistry/view_registry_owner.jsp?eventType=${eventType}&registryId=${registryId}&sorting=${sorting}&view=${view}" />
											...
											<c:choose>
												...
												<c:set var="writeReviewLink"><bbbl:label key="lbl_grid_write_review_link" language="${pageContext.request.locale.language}" /></c:set>
													<li class="prodReview writeReview">
														<a  href="${finalUrl}?skuId=${skuId}&writeReview=true" role="link" aria-label="${writeReviewLink} ${lblAboutThe} ${productName}" onclick="javascript:s_crossSell('registry update, top registry items');"><bbbl:label key="lbl_grid_write_review_link" language="${pageContext.request.locale.language}" /></a>
													</li>
												</c:otherwise>
											</c:choose>
										</dsp:oparam>
