<dsp:page>
	...
	<dsp:importbean
		bean="/com/bbb/commerce/giftregistry/droplet/RecommendationInfoDisplayDroplet" />
	...
	<c:set var="pageSize">
		<bbbl:label key="lbl_recommendation_page_size"
	...
	<c:set var="body_class"
		value="visible invisible ui-accordion-content-active" />

	<c:choose>
		<c:when test="${emptyRegistrant eq 'true'}">
			<div id="recommendations">
				<div class="grid_12">
					<dsp:include page="/recommender/registrant_empty_landing.jsp"
						flush="true">
						<dsp:param name="registryType" value="${eventType}" />
						<dsp:param name="registryId" value="${registryId}" />
						<dsp:param name="channel" value="DesktopWeb" />
						<dsp:param name="siteId" value="${appid}" />
						<dsp:param name="regFirstName" value="${regFirstName}" />
						<dsp:param name="regEventDate" value="${regEventDate}" />
					</dsp:include>
				</div>
			</div>
			...
			<div id="pending-registry">
				...
				<div class="sort_container">
					<ul class="inner_tab fl">
						<li class="recommendInnerTab"><a href="javascript:void(0);" data-recomm="pendingRecommendation"
							class="active" ><bbbl:label key="lbl_registry_owner_pending"
					...
					<ul class="fr">
						<li class="padTop_5"><span class="padRight_5"> <bbbl:label
									key="lbl_mng_regitem_sortby"
									language="${pageContext.request.locale.language}" /></span></li>

						<li><select class="uniform" id="recommendationSort"	aria-hidden="false">
				...
				<div class="ajaxLoadWrapper" id="registryLoader">
					<bbbt:textArea key="txt_viewregistry_imageloader"
						language="${pageContext.request.locale.language}" />
				</div>
					...
					<dsp:oparam name="output">
						...
						<c:if test="${groupByFlag eq 'date' }">
							...
							<div class="accordionReg1 accordionReg accordion ui-accordion ui-widget ui-helper-reset ui-accordion-icons">
								<h2 class="pending ui-accordion-header ui-helper-reset ${header_class}">
									<span class="ui-icon ui-icon-triangle-1-e"></span><span class="accordionTitle block clearfix giftTitle"> <a href="#" class="fl accordionLink "><bbbl:label key="lbl_recom_date_sort" language ="${pageContext.request.locale.language}"/></a></span>
								</h2>
						...
						<dsp:droplet name="ForEach">
							...
							<dsp:oparam name="output">
								...
								<dsp:getvalueof var="skuListPriceCurrencey"
									param="element.skuListPrice" />
								<dsp:getvalueof var="recommendedQuantity"
									param="element.recommendedQuantity" />
								<dsp:getvalueof var="bazaarVoiceVo" param="element.bvProductVO" />
								<dsp:getvalueof var="skuImage"
									param="element.imageVO.smallImage" />
								<dsp:getvalueof var="productId" param="element.productId" />
								<dsp:getvalueof var="fname_profileId"
									param="element.fname_profileId" />
								...
								<c:set var="skuListPrice">
									<dsp:valueof converter="currency"
										value="${skuListPriceCurrencey}" />
								</c:set>
								<%--BBBH-4958 | incart price on recommnedation tab --%>
								<dsp:getvalueof var="skuIncartPrice"  param="element.skuIncartPrice" />
								<dsp:getvalueof var="skuIncartFlag"  param="element.sKUDetailVO.inCartFlag" />
	...
	<c:when test="${groupByFlag eq 'date' }">
		...
		<dsp:include
			page="pending_recommendation_tab_grpby_recommender_date.jsp"
			flush="true">
			...
			<dsp:param name="skuIncartPrice" value="${skuIncartPrice}" />
		</dsp:include>
	</c:when>
	</c:choose>
	</dsp:oparam>
	...
	<c:if test="${groupByFlag eq 'category' or groupByFlag eq 'price' or groupByFlag eq 'recommender'}">
		<dsp:include
			page="pending_recommendation_tab_grpby_category_price.jsp"
			flush="true">
	...
	<input type="hidden" name="prevRecommender_${pageNum}"
		value="${prevRecommender}" />
				<input type="hidden" name="sortOption" value='<dsp:valueof param="sortOption" />'/>
				<%-- <bbbt:textArea key="txt_pending_added_reg"
						language="${pageContext.request.locale.language}"></bbbt:textArea>
						    <bbbt:textArea key="txt_pending_declined_reg"
						language="${pageContext.request.locale.language}"></bbbt:textArea> --%>
	...
	<dsp:oparam name="error">
		<c:if test="${pageNum eq '0'}">
			<div class="emptyRecommenderMsg">
				<bbbt:textArea key="txt_empty_pending_recommendations"
					language="${pageContext.request.locale.language}" />
			</div>
		</c:if>
	</dsp:oparam>
