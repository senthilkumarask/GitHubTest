			...
			<c:choose>
				<c:when test="${kickStarterPage=='yes' && sizeValue>1 }">
					...
					<div class="addToRegistry cb noPad">	
						<a class="btnAddToRegistry" href="javascript:void(0);"><bbbl:label key='lbl_compare_add_to_registry' language="${pageContext.request.locale.language}" /></a>
					</div>
				</c:when>
				<c:when
					test="${sizeValue>1}">
					<c:choose>
						...
						<c:when test="${sizeValue>1 && not isInternationalCustomer && !isCustomizationRequired}">
								<div class="prodGridRegistry addToRegistry addItemToRegNoLogin marBottom_5 fr cb">
								<dsp:select bean="GiftRegistryFormHandler.registryId"
									id="addItemToRegMultipleRegID" name="registryId"
									iclass="addItemToRegis addItemToRegMultipleReg customSelectBoxCollection">
						...
						<c:otherwise>
							...
							<div class="fl addToRegistry">
								...
								<input class="btnAddToRegistry" name="btnAddToRegistry"
										type="button" value="${submitAddToRegistryBtn}" role="button" data-poboxflag="${poBoxAddress}" data-notify-reg="true" aria-pressed="false" <c:if test='${isInternationalCustomer || disableCTA ||( ltlFlag && fn:containsIgnoreCase(enableLTLRegForSite,"false"))}'>disabled="disabled"</c:if>/>
								</div>
							</div>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:otherwise>
							<div
								class="addToRegistry btnAddToRegNoRegistry marBottom_5 fr cb">
								<div
									class="button addToRegistrybtn btnAddToRegNoRegistry <c:if test='${isCustomizationRequired || isInternationalCustomer|| (ltlFlag && fn:containsIgnoreCase(enableLTLRegForSite,"false"))}'>button_disabled</c:if>">
									<input class="btnAddToRegNoRegistry"
										name="btnAddToRegNoRegistry" type="button"
										<c:if test='${isCustomizationRequired || isInternationalCustomer|| (ltlFlag && fn:containsIgnoreCase(enableLTLRegForSite,"false"))}'>disabled="disabled"</c:if>
										value="${submitAddToRegistryBtn}" />
								</div>
							</div>
						</c:otherwise>
		...
		<c:when test="${transient == 'true'}">
			<div class="addToRegistry clearfix">
				<div class="button <c:if test='${isCustomizationRequired || isInternationalCustomer|| (ltlFlag && fn:containsIgnoreCase(enableLTLRegForSite,"false"))}'>button_disabled</c:if>">
					<input class="addItemToRegNoLogin" name="addItemToRegNoLogin" type="button" <c:if test='${isCustomizationRequired || isInternationalCustomer|| (ltlFlag && fn:containsIgnoreCase(enableLTLRegForSite,"false"))}'>disabled="disabled"</c:if>  value="${submitAddToRegistryBtn}" />
				</div>
			</div>
		</c:when>
	</c:choose>
