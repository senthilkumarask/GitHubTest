   ...
   <dsp:page>
    <bbb:pageContainer section="${section}" pageWrapper="${pageWrapper}" titleString="${titleString}" pageVariation="${pageVariation}">
      ...
      <dsp:droplet name="GiftRegistryFlyoutDroplet">
       	 ...
       	 <dsp:param name="promoBoxTemplateLayout" param="LandingTemplateVO.promoBoxContentVO"/>
		<dsp:getvalueof var="promoBoxTemplate" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxContent" />
	   <dsp:getvalueof var="promoBoxJs" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxJsFilePath" />
	   <dsp:getvalueof var="promoBoxCss" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxCssFilePath" />
        ...
        <c:if test="${not interactiveCheckListFlag}">
       	...
       	<c:choose>
			...
			<c:set var="usOnlyCss">usOnly</c:set> 
			    	 <c:choose>
						<c:when test="${enableRegSearchById == 'true'}">
							 <div id="heroContent" class="${loggedStatusCss} usOnly serachByRegistryId">
						</c:when>
						<c:otherwise>
							 <div id="heroContent" class="${loggedStatusCss} usOnly">
						</c:otherwise>
					</c:choose>					
			</c:when>
			<c:otherwise>
				 <c:choose>
						<c:when test="${enableRegSearchById == 'true'}">							
							 <div id="heroContent" class="${loggedStatusCss} serachByRegistryId">
						</c:when>
						<c:otherwise>
          ...
          <dsp:getvalueof var="LandingTemplateVO" param="LandingTemplateVO"/>
			...
			<dsp:getvalueof param="LandingTemplateVO.bridalTool" var="bridalTool"/>
				 ...
				 <div class="clearfix cb">
					<div class="catTabsData prodIconsCarousal loggedOut usOnly">
			                 ...
			                 <c:set var="title"><dsp:valueof param="element.linkLabel" valueishtml="true"/></c:set>
								...
								<c:otherwise>
									<li class="noCarousel" ><a title="<dsp:valueof param="element.imageAltText"/>" href="<dsp:valueof param="element.linkUrl"/>">
									    <c:choose>
								        	<c:when test="${(fn:indexOf(iconImageURL, 'http') == 0) || (fn:indexOf(iconImageURL, '//') == 0) || (fn:indexOf(iconImageURL, '/_assets/') == 0)}">
												<img aria-hidden="true" src="${iconImageURL}" alt="<dsp:valueof param="element.imageAltText"/>" />
								            </c:when>
								            <c:otherwise>
												<img aria-hidden="true" src="${scene7Path}/${iconImageURL}" alt="<dsp:valueof param="element.imageAltText"/>" />
								            </c:otherwise>
										</c:choose>
				...
				<div id="content" class="container_12 clearfix bridalLanding" role="main">	
				 <c:choose>
			      	<c:when test="${currentSiteId eq 'BedBathUS' and loggedStatusCss eq 'loggedOut'}">
			      	<dsp:param name="promoBoxTemplateLayout" param="LandingTemplateVO.promoBoxContentVO"/>
			      	 <dsp:getvalueof var="promoBoxJs" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxJsFilePath" />
	   					<dsp:getvalueof var="promoBoxCss" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxCssFilePath" />
					 	 <c:if test="${not empty promoBoxJs}">
						 	<script type="text/javascript" src="${promoBoxJs}"></script>
						 </c:if>
						 <c:if test="${not empty promoBoxCss}">
							<link rel="stylesheet" type="text/css" href="${promoBoxCss}" />
						</c:if>
					<dsp:getvalueof var="promoBoxTemplate" vartype="java.lang.String" param="promoBoxTemplateLayout.promoBoxContent" />
				      		   <c:if test="${!empty promoBoxTemplate}">
				          			 <dsp:valueof param="promoBoxTemplateLayout.promoBoxContent" valueishtml="true"/>
			            		</c:if>
			      	</c:when>
	      			<c:otherwise>
