<dsp:page>
	...
	<dsp:getvalueof var="supplyBalance" param="supplyBalance" />
	<dsp:getvalueof var="localStorePDP" param="localStorePDP" />
	<ul class="clearfix">
		<dsp:getvalueof var="StoreDetailsVar" param="StoreDetails" scope="page"/>
			...
			<dsp:getvalueof var="productChkFlg"
				...
				scope="page" />
			<dsp:getvalueof var="favStore" param="favstorePresent" />
			<li class="column1">
				<div class="fl">
					<div class="clearfix">
							...
							<p class='<c:if test="${not empty favStore}">favouriteContact</c:if>'>
								${StoreDetailsVar.storePhone}
								<%-- <dsp:valueof param="StoreDetails.storePhone" /> --%>
	                       </p>
							<c:if test="${not empty favStore}">
							<c:choose>
						         <c:when test="${currentSiteId eq 
						            'BedBathCanada'}">
						         <a class="editFavouriteStore"
						            href="/store/selfservice/store/canada_store_locator.jsp?favStoreId=${StoreDetailsVar.postalCode}">
						         <bbbl:label 
						         ...
						         <c:otherwise>
						         <a class="editFavouriteStore"
						            href="/store/selfservice/store/find_store.jsp?favStoreId=${StoreDetailsVar.postalCode}">
						         <bbbl:label 
						            key="lbl_edit_favorite_store" language="${pageContext.request.locale.language}" />
						         </a>
						         </c:otherwise>
			...
			<c:choose>
				<c:when test="${(productChkFlg == 0 || productChkFlg == 2) && vdcPassed eq 'true'}">
				...
				<c:when test="${productChkFlg==0}">
					<div class="button button_active button_active_orange button_enabled">
						...
						<input type="submit" value="Add to Cart" name="changeStoreBtn" id="changeStoreBtn" role="button" aria-labelledby="changeStoreBtn"/>
					</div>
				</c:when>
				<c:when test="${productChkFlg==2}">
					<div class="button button_active button_active_orange button_enabled">
			...
			<c:choose>
				...
				<c:when test="${productChkFlg==0}">
					<div>
						<input type="hidden" name="storeId" value="${storeId}" data-change-store-storeid="storeId" class="addToCartSubmitData"/>
						<input type="submit" value="Reserve Now" class="button-Med btnPrimary storeAddtoCart" name="changeStoreBtn" id="changeStoreBtn" role="button" aria-labelledby="changeStoreBtn"/>
					</div>
				</c:when>
				<c:when test="${productChkFlg==2}">
					<div>
