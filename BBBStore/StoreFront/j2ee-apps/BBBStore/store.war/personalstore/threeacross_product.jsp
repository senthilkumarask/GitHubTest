<dsp:page>
	...
	<div class="grid_12 marBottom_20 threeAcross">

		<div class="grid_12 alpha omega marBottom_10">
					...
					<c:if test="${count < 4}">
						<c:choose>
							<c:when test="${count%3 == 0}">
								<div class="homeProd product grid_4 omega marBottom_10">
							</c:when>
							<c:otherwise>
								<div class="homeProd product grid_4 alpha marBottom_10">
							</c:otherwise>
						</c:choose>
						<div class="productContent">
							...
							<div class="prodData listDataItemsWrap">
								<div class="grid_3 alpha padBottom_10 fl quickViewAndCompare">
									<span
										class="quickView ${quickViewClass} fl marRight_30 padRight_5">
										${lblProductQuickView}</span>
									<div class="checkboxItem input clearfix noPad fl noBorder">
										...
										<div class="label">
											<label class="compareChkTxt" for="compareChkTxt_${productId}">
												${lblCompareProduct} </label>
										</div>
									</div>
								</div>
								...
								<c:if test="${BazaarVoiceOn}">
									...
									<c:choose>
										<c:when test="${ratingAvailable == true}">
											...
											<dsp:getvalueof param="productVO.bvReviews.averageOverallRating" var="averageOverallRating" />											<dsp:getvalueof param="productVO.bvReviews.totalReviewCount" var="totalReviewCount" />
											<c:choose>
										...
										<c:otherwise>
											...
											<span class="ratingTxt prodReview ratingsReviews"><span class="ariaLabel">${ratingsTitle}</span></span><span class="prodReviewSpanFont reviewTxt writeReview"> <c:set var="writeReviewLink"> ${lblGridWriteReviewLink} </c:set> <a href="${contextPath}${finalUrl}?writeReview=true" title="${writeReviewLink}">${lblGridWriteReviewLink} </a> </span></li>
										</c:otherwise>
									</c:choose>
								</c:if>
								<li class="prodPrice">
