<dsp:page>
	...
	<div class="scrollable3 scrollable marBottom_20 grid_12 padBottom_20 marTop_20 fiveAndHalfAcross">
		<div class="partial-matching-wrapper-five grid_12 noMar viewport">

			<div class="grid_12 noMar overview ">
				<ul class="clearfix prodGridRow" id="row1">
								...
								<div class="productContent">
									...
									<dsp:a href="${contextPath}${finalUrl}?strategyName=${strategyNameFromMap}" onclick="javascript:personalStore('${strategyNameFromMap}')">
										...
										<c:choose>
											...
											<c:otherwise>
												<img width="170" height="170" src="${scene7Path}/${imgSrc}"
									...
									<div class="prodData listDataItemsWrap">
										<div class="grid_2 alpha padBottom_10 fl quickViewAndCompare">
											<span class="quickView ${quickViewClass} fl marRight_5">${lblProductQuickView}</span>
											<div class="checkboxItem input clearfix noPad fl noBorder">
												<div class="checkbox noMar">
													<input type="checkbox" data-productid="${productId}"
														value="compareItem" class="compareChkTxt"
														name="Compare"
														style="opacity: 0;">
												</div>
												<div class="label">
													<label class="compareChkTxt"
														for="compareChkTxt_${productId}">${lblCompareProduct}</label>
												</div>
											</div>
											<input type="hidden" class="addItemToList addItemToRegis itemQuantity" value="1"
												name="qty" data-change-store-submit="qty" />
												 <input type="hidden" data-change-store-internaldata="true"
												...
												name="prodId" />
												 <input type="hidden" data-change-store-internaldata="true" data-change-store-errors="required" data-change-store-submit="skuId"
												class="addItemToRegis _skuId addItemToList changeStoreSkuId"
												value="${skuID}" name="skuId" /> 
												<input type="hidden" name="price" value="${omniPrice}"
												class="addItemToList addItemToRegis" /> 
												<input type="hidden" data-change-store-storeid="storeId" value=""
												name="storeId" class="addToCartSubmitData" />
												<input type="hidden" data-change-store-storeid="strategyName" value="${strategyNameFromMap}"
												name="strategyName"  />
												<input type="hidden" name="parentProductId" value="${productId}"
												class="addItemToList addItemToRegis" /> 
												<input type="hidden" value="" class="addToCartSubmitData"
												name="bts" data-change-store-storeid="bts" /> 
												<input type="hidden" value="true"
												class="addItemToList addItemToRegis" name="fromSearch" /> 
														...
														<c:choose>
															...
															<c:otherwise>
																<li class="prodReviews clearfix metaFeedback">
                                                                        <span class="ratingsReviews prodReviews<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/> ratingTxt"><span class="ariaLabel">${ratingsTitle}</span></span><span class="reviewTxt">(<c:out value="${totalReviewCount}" /> ${lblReviewsCount})</span></li>
															</c:otherwise>
														</c:choose>
													</c:when>
													<c:otherwise>
														<li class="prodReviews metaFeedback"><span class="ratingTxt ratingsReviews"><span class="ariaLabel">${ratingsTitle}</span></span><span class="prodReviewSpanFont reviewTxt writeReview"> <c:set var="writeReviewLink">${lblGridWriteReviewLink} </c:set> <a href="${contextPath}${finalUrl}?writeReview=true" title="${writeReviewLink}">${lblGridWriteReviewLink} </a> </span></li>
													</c:otherwise>
												</c:choose>
											</c:if>
											<li class="prodPrice">
