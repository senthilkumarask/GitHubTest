...
<dsp:getvalueof var="collectionFlag" param="BBBProductListVO.collectionFlag"/>
	...
	<c:otherwise>
		...
		<div class="grid_12 marBottom_20 twoAcross">
			...
			<dsp:oparam name="output">
        	 ...
        	 <c:if test="${count < 3}">
				<div class="product homeProd grid_6 alpha marBottom_10">
					...
					<dsp:droplet name="/atg/repository/seo/CanonicalItemLink">
						...
						<dsp:a href="${contextPath}${finalUrl}?strategyName=${strategyNameFromMap}" onclick="javascript:personalStore('${strategyNameFromMap}')">
							...
							<c:when test="${empty imgSrc}">
								<img width="478" height="478" src="${imagePath}/_assets/global/images/no_image_available.jpg" alt="${imageAltText}" title="${imageAltText}"/>
							</c:when>
							<c:otherwise>
								<img width="478" height="478" src="${scene7Path}/${imgSrc}" alt="${imageAltText}" class="noImageFound" title="${imageAltText}"/>
							</c:otherwise>
						</c:choose>
						...
						<div class="prodData listDataItemsWrap">
							<div class="grid_3 alpha padBottom_10 fl quickViewAndCompare"> 
								<span class="quickView ${quickViewClass} fl marRight_30 padRight_5">${lblProductQuickView}</span>
							...
							<c:otherwise>
								...
								<a href="${contextPath}${finalUrl}?writeReview=true" title="${writeReviewLink}">${lblGridWriteReviewLink}</a>
								</span></li>
							</c:otherwise>
							</c:choose>
							  </c:if>
