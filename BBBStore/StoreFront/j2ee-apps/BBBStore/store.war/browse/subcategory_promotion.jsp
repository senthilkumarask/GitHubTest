...
<dsp:page>
	<dsp:importbean
		bean="/atg/commerce/catalog/comparison/ProductComparisonList" />
	<dsp:importbean
		bean="/atg/commerce/catalog/comparison/ProductListHandler" />
	...
	<dsp:getvalueof var="queryString"
		bean="/OriginatingRequest.queryString" />
	...
	<c:set var="lblSelectCollection">
		<bbbl:label key="lblSelectCollection" language="${pageContext.request.locale.language}" />
	</c:set>
	...
	<c:set var="l2l3BoostFlag">
		<bbbc:config key="L2L3BoostFlag" configName="FlagDrivenFunctions" />
	</c:set>
	<c:set var="brandsBoostFlag">
		<bbbc:config key="BrandsBoostFlag" configName="FlagDrivenFunctions" />
	</c:set>
	<c:set var="defaultOmnitureVar">
		<bbbc:config key="DefaultEndecaOmnitureVariable"
			configName="OmnitureBoosting" />
	</c:set>
	<c:choose>
	...
	<c:set var="saSrc">
		<bbbc:config key="socialAnnexURL" configName="ThirdPartyURLs" />
	</c:set>
	<c:set var="lblWhatsNew">
		<bbbl:label key="lbl_whats_new"
			language="${pageContext.request.locale.language}" />
	</c:set>
	<c:set var="lblClearance">
		<bbbl:label key="lbl_clearance"
			language="${pageContext.request.locale.language}" />
	</c:set>
	<c:set var="view" scope="request">
		<c:out value="${param.view}" />
	</c:set>
	...
	<c:if test="${plpGridSize == '3'}">
		<c:if test="${defaultView == 'grid'}">
			<c:set var="pageGridClass" value="plp_view_3" />
				...
				<c:set var="facetClass" value="grid_4" />			
	</c:if>
	...
	<dsp:getvalueof var="currentPage"
		param="browseSearchVO.pagingLinks.currentPage" scope="request" />
	<dsp:getvalueof var="size" param="browseSearchVO.pagingLinks.pageSize"
		scope="request" />
	<dsp:getvalueof var="firstPageURL"
		param="browseSearchVO.pagingLinks.firstPage" scope="request" />
	<dsp:getvalueof var="filterApplied"
		param="browseSearchVO.pagingLinks.pageFilter" scope="request" />
	<dsp:getvalueof var="canonicalFilterApplied"
		param="browseSearchVO.pagingLinks.canonicalPageFilter" scope="request" />
	<dsp:getvalueof var="pageCount"
		param="browseSearchVO.pagingLinks.pageCount" scope="request" />
	<dsp:getvalueof var="nextPageParams"
		param="browseSearchVO.pagingLinks.nextPage" scope="request" />
	<dsp:getvalueof var="prevPageParams"
		param="browseSearchVO.pagingLinks.previousPage" scope="request" />
	...
	<c:if test="${empty frmBrandPage}">
		...
		<dsp:droplet name="/atg/repository/seo/CanonicalItemLink">
			...
			<dsp:oparam name="output">
				...
				<dsp:getvalueof var="seoUrl" vartype="java.lang.String" param="url"
					scope="request" />
			</dsp:oparam>
		</dsp:droplet>
	</c:if>
	...
	<dsp:getvalueof var="descriptorsList"
	...
	<c:choose>
		<c:when test="${not empty frmBrandPage and frmBrandPage eq true}">
			<c:if
				test="${(descriptorLength ge 1) && (descriptorsList[descriptorLength-1].rootName ne 'RECORD TYPE')}">
				<c:set var="isFilterApplied" value="true" scope="request" />
			</c:if>
		</c:when>
		<c:otherwise>
			<c:if
				test="${(descriptorLength ge 1) && (descriptorsList[descriptorLength-1].rootName ne 'DEPARTMENT')}">
				<c:set var="isFilterApplied" value="true" scope="request" />
			</c:if>
	...
	<c:choose>
		<c:when
			test="${not frmBrandPage && (not empty param.narrowDown || descriptorLength gt 1)}">
			<c:set var="isRefType" value="true"></c:set>
			<c:set var="subCategoryParamForLinkRel" scope="request"
				value="${subCategoryParamForLinkRel}&refType=false&view=${defaultView}" />
		</c:when>
		<c:otherwise>
			<c:set var="subCategoryParamForLinkRel" scope="request"
				value="${subCategoryParamForLinkRel}&view=${defaultView}" />
		</c:otherwise>
	</c:choose>
	...
	<c:forEach var="descriptor" items="${browseSearchVO.descriptors}">
		<c:if
			test="${(frmBrandPage and descriptor.rootName ne 'RECORD TYPE') or ((empty frmBrandPage or not frmBrandPage) and descriptor.rootName ne 'DEPARTMENT' and descriptor.rootName ne 'RECORD TYPE' )}">
			<!--  and count lt 4)}">-->
			<c:choose>
				<c:when test="${fn:containsIgnoreCase(descriptor.rootName,'COLOR')}">
					<c:choose>
						...
						<c:otherwise>
							<c:set var="colorstring"
								value="${colorstring}, ${descriptor.name}" />
						</c:otherwise>
					</c:choose>
				...
				<c:when test="${fn:containsIgnoreCase(descriptor.rootName,'BRAND')}">
					<c:choose>
						...
						<c:otherwise>
							<c:set var="brandstring"
								value="${brandstring}, ${descriptor.name}" />
						</c:otherwise>
					</c:choose>
					<c:set var="lastBrandstring" value="${descriptor.name}" />
				</c:when>
				<c:when
					test="${fn:containsIgnoreCase(descriptor.rootName,'DEPARTMENT')}">
					<c:set var="departmentstring"
						value="${departmentstring} ${descriptor.name }" />
				</c:when>
				<c:when
					test="${not empty titleFilterStringfacet && (not fn:containsIgnoreCase(descriptor.rootName,'Price'))}">
					<c:set var="titleFilterStringfacet"
						value="${titleFilterStringfacet} - ${descriptor.name}" />
				</c:when>
				<c:otherwise>
					<c:if
						test="${not fn:containsIgnoreCase(descriptor.rootName,'Price')}">
	...
	<c:if test="${not empty departmentstring}">
		<c:set var="stopWords">
			<bbbc:config key="stopcategoryintitle"
				configName="ContentCatalogKeys" />
		</c:set>
		<c:forTokens items="${stopWords}" delims="," var="mySplit">
			<c:if
				test="${departmentstring eq mySplit || browseSearchVO.currentCatName eq mySplit}">
				<c:set var="departmentstring"
					value="${fn:replace(departmentstring,mySplit,'')}" />
	...
	<c:choose>
		<c:when test="${not empty titleFilterString && not empty brandstring}">
			<c:set var="titleFilterString"
				value="${titleFilterString} ${brandstring}" />
		</c:when>
	...
	<c:choose>
		<c:when
			test="${not empty titleFilterString && not empty departmentstring}">
			<c:set var="titleFilterString"
				value="${titleFilterString} ${departmentstring}" />
		</c:when>
		<c:when
			test="${empty titleFilterString && not empty departmentstring}">
	...
	<c:choose>
		<c:when
			test="${(empty frmBrandPage or not frmBrandPage) && (not empty titleFilterString || not empty titleFilterStringfacet)}">
			<%-- <c:set var="titleFilterString" value="${titleFilterString} ${browseSearchVO.currentCatName}"/> --%>
			<c:if
				test="${empty categoryNameStopWord || categoryNameStopWord ne 'true' }">
				<c:set var="titleFilterString"
					value="${titleFilterString} ${browseSearchVO.currentCatName}" />
			</c:if>
			<c:if test="${not empty titleFilterStringfacet }">
				<c:set var="titleFilterString"
					value="${titleFilterString} - ${titleFilterStringfacet}" />
			</c:if>
		</c:when>
		<c:otherwise>
	...
	<c:if test="${not empty frmBrandPage and frmBrandPage eq true}">
		<c:choose>
			<c:when
				test="${not empty titleFilterString && (titleFilterString eq brandName &&  brandstringnull eq 'true')}">
				<c:set var="titleFilterString" value="${brandName} - " />
				<c:set var="titleFilterString">${titleFilterString}<bbbl:label
						key="lbl_metadetails_title"
						language="${pageContext.request.locale.language}" />
				</c:set>
			</c:when>
			<c:otherwise>
	...
	<bbb:pageContainer>
		...
		<jsp:attribute name="pageWrapper">subCategory newPlp useCertonaJs useStoreLocator ${pageGridClass} useScene7</jsp:attribute>
		...
		<jsp:body>
			...
			<c:if test="${fromBrandPage == 'true'}">
				...
				<input type="hidden" name="queryStringFromBrandPage"
					value="${queryString}" />
			</c:if>
			...
			<c:if test="${fromCatPage == 'true'}">
				<input type="hidden" name="fromPage" value="categoryPage" />
				<input type="hidden" name="inStoreValue" value="${inStore}" />
				<input type="hidden" name="queryStringFromCategoryPage"
					value="${queryString}" />
			</c:if>
			
			<input type="hidden" value="${radius_default_selected}"
				id="defaultRadius" name="defaultRadius">
			...
			<c:set var="department">
				<bbbl:label key="lbl_department"
					language="${pageContext.request.locale.language}" />
			</c:set>
			<c:set var="promoKeyTop">
				<bbbl:label key="lbl_promo_key_top"
					language="${pageContext.request.locale.language}" />
			</c:set>
			<c:set var="promoKeyRight">
				<bbbl:label key="lbl_promo_key_right"
					language="${pageContext.request.locale.language}" />
			</c:set>
			<c:set var="promoKeyCenter">
				<bbbl:label key="lbl_promo_key_center"
					language="${pageContext.request.locale.language}" />
			</c:set>
								...
								<dsp:oparam name="output">
									...
									<dsp:getvalueof var="count" param="count" />
										<c:if test="${count eq 2 }">
											...
											<c:set var="CatNameL2" scope="request">
										<dsp:valueof param="element.categoryName" />
									</c:set>
										</c:if>
										<c:if test="${count eq 3 }">
											<c:set var="CatFlagL3" scope="request" value="true"></c:set>
											<c:set var="CatNameL3" scope="request">
										<dsp:valueof param="element.categoryName" />
									</c:set>
										</c:if>
										<%-- End: Added for Scope # 81 H1 tags --%>
									<c:set var="categoryPath" scope="request">${categoryPath} > <dsp:valueof
										param="element.categoryName" />
								</c:set>
								</dsp:oparam>
							</dsp:droplet>
						</dsp:oparam>
			...
			<c:choose>
				<c:when test="${(not empty categoryVO.siblingCategories && empty categoryVO.subCategories) or 
									(empty categoryVO.siblingCategories && not empty categoryVO.subCategories)}">
					<c:set var="headerClasses" value="grid_8 alpha headerCont"></c:set>
				</c:when>
				<c:otherwise>
					<c:set var="headerClasses" value="grid_6 alpha headerCont"></c:set>
				</c:otherwise>
			</c:choose>
			
			<div class="alpha omega" id="dcNavCont">
			    <div class="alpha omega container_12 clearfix" id="dcNav">
					...
					<div class="${headerClasses}">
			      	...
			      	<c:if test="${not empty categoryVO.subCategories}">
						<div id="collectionDrp2" class="dcDropDownCont alpha">
				         <div class="dcDropDownTextReplace" aria-label="collection select dropdown">${lblSelectCollection}</div>
				        <ul id="collectionDrop" class="dcThemes">
							...
							<div class="overview" tabindex="0">
				         	...
				         	<dsp:oparam name="output">
								...
								<li class="subCategoryItem">
									<a href="/store${categoryURL}"><dsp:valueof param="element.categoryName" /></a>				
								</li>
							</dsp:oparam>
				         	</dsp:droplet>
				         </div>
				      ...
				      <c:if test="${not empty categoryVO.siblingCategories}">
						 <div id="collectionDrp1" class="dcDropDownCont omega">
							...
							<div class="overview" tabindex="0">
					            <dsp:droplet name="/atg/dynamo/droplet/ForEach">
					         	<dsp:param param="categoryVO.siblingCategories"
										name="array" />
					         	<dsp:oparam name="output">
									<dsp:getvalueof var="categoryName" param="element.categoryName" />
									<dsp:getvalueof var="categoryURL" param="element.seoURL" />								
<li class="subCategoryItem">
								<a href="/store${categoryURL}"><dsp:valueof
												param="element.categoryName" /></a>				
												</li>
					...
					<div id="subHeader" class="container_12 clearfix">
						<div class="grid_12 marTop_10">
							<dsp:getvalueof var="catBannerContent"
						param="categoryVO.bannerContent" />
							<%-- JIRA Defect # BBBSL-2 START --%>
							<c:if test="${not frmBrandPage}">
								<dsp:getvalueof var="subCatHeader"
							param="browseSearchVO.categoryHeader.name" />
								
								<dsp:getvalueof var="floatNode" value="false" />
								<c:set var="floatCatName">
							<bbbc:config key="float_node_cat_name"
								configName="DimNonDisplayConfig" />
						</c:set>
								<%--<c:if test="${subCatHeader eq 'Floating Nodes'}">--%>
								<c:if
							test="${fn:toLowerCase(subCatHeader) == fn:toLowerCase(floatCatName)}">
									<dsp:droplet name="/atg/commerce/catalog/CategoryLookup">
										<dsp:param name="id" param="catID" />
										<dsp:param name="elementName" value="mainCategory" />
										<dsp:oparam name="output">
						                	<dsp:getvalueof var="categoryName"
										param="mainCategory.displayName" />
						                </dsp:oparam>
						           	</dsp:droplet>
								...
								<dsp:getvalueof var="categoryVO" param="categoryVO" />
			                   	<dsp:getvalueof var="chatLinkPlaceholder"
							param="categoryVO.chatLinkPlaceholder" />
								<dsp:getvalueof var="chatEnabled"
							param="categoryVO.isChatEnabled" />
								<dsp:getvalueof var="chatLink" value="" />
								<c:choose>
									<c:when
								test="${(not empty chatEnabled)  && (chatEnabled eq 'true') && (chatLinkPlaceholder eq 'Top Right')}">
											...
											<dsp:oparam name="output">
												...
												<dsp:getvalueof param="count" id="count" />
													<c:if test="${(size eq 4) and (count eq 2)}">
														<dsp:droplet
												name="/com/bbb/commerce/browse/droplet/CategoryLandingDroplet">
															<dsp:param param="cat.repositoryId" name="id" />
															<dsp:param name="siteId" value="${applicationId}" />
															<dsp:oparam name="subcat">
																<dsp:getvalueof id="parentCatVO" param="categoryVO" />
																<dsp:getvalueof var="parentCatChatLinkPlaceholder"
														value="${parentCatVO.chatLinkPlaceholder}" />
																<dsp:getvalueof var="parentChatEnabled"
														value="${parentCatVO.isChatEnabled}" />
																<dsp:getvalueof id="chatEnabled"
														value="${parentCatVO.isChatEnabled}" />
																<c:if
														test="${(not empty parentChatEnabled)  && (parentChatEnabled eq 'true') && (parentCatChatLinkPlaceholder eq 'Top Right')}">
																	<dsp:getvalueof var="chatLink" value="true" />
																</c:if>
															</dsp:oparam>
														</dsp:droplet>
							...
							<dsp:droplet name="ForEach">
		                     	<dsp:param name="array"
							param="browseSearchVO.promoMap" />
		                        <dsp:oparam name="output">
		                        	<dsp:getvalueof var="elementList"
								param="element" />
		                            <dsp:getvalueof var="key1" param="key" />
		                            <%-- <c:choose>	    --%>
		                            <c:if
								test="${(empty frmBrandPage || !frmBrandPage) && (key1 eq promoKeyCenter || key1 eq promoKeyTop)}">
		                            	<c:set var="fromCatL2AndL3"
									scope="request" value="true"></c:set>
		                            	<dsp:droplet name="ForEach">
		                                	<dsp:param name="array"
										value="${elementList}" />
		                                    ...
		                                    <dsp:oparam name="output">
		                                    	<c:if test="${empty imageHREF}">
				                            		<dsp:getvalueof var="imageHREF"
												param="element.imageHref" />
				                            	</c:if>
				                            	<c:if test="${not empty imageHREF}">
				                            		<dsp:droplet
												name="AddContextPathDroplet">
				                            			<dsp:param name="InputLink"
													value="${imageHREF}" />
				                            			<dsp:oparam name="output">
				                            				<dsp:getvalueof var="imageHREF"
														param="OutputLink" />
				                            	...
				                            	<c:if test="${empty imageURL}">
							                    	<dsp:getvalueof var="imageURL"
												param="element.imageSrc" />
							                    </c:if>
							                    <c:if test="${empty imageAlt}">
							                    	<dsp:getvalueof var="imageAlt"
												param="element.imageAlt" />
							                    </c:if>
							                    <c:if test="${empty seoText}">
							                    	<dsp:getvalueof var="seoText"
												param="element.seoText" />
							                    </c:if>
							                </dsp:oparam>
							           	...
							           	<c:choose>
					                    	<c:when
										test="${(not empty seoText) && (not empty imageURL) }">
						                    	<div
											class="grid_6 omega marBottom_10 promoImage <c:if test="${key1 eq promoKeyTop}">promoHead</c:if>">
													<c:choose>
														<c:when test="${not empty imageHREF}">
															<a href="${imageHREF}" title="${imageAlt}"><img
														src="${scene7Path}/${imageURL}" alt="${imageAlt}" /></a>
														</c:when>
														<c:otherwise>
						                    	 ...
						                    	 <c:if test="${not subCatHeaderShown}">
						                    	 	<c:set var="subCatHeaderShown"
													scope="request" value="true" />
						                    	 	<c:choose>
						                    			<c:when test="${!(CatFlagL3 eq null)}">
						                    				
						                    				<c:set var="catHeaderValue" value="true"
															scope="request" />
						                    			</c:when>
						                    			<c:otherwise>
						                    				<c:choose>
						                    					<c:when
																test="${!(CatFlagL2 eq null) and (subCatHeader eq CatNameL2)}">
						                    					...
						                    					<c:otherwise>
						                    						<c:choose>
						                    							<c:when
																		test="${!(CatFlagL2 eq null) and (subCatHeader ne CatNameL2)}">
						                    							
						                    								<!-- <p class="catHeader"><dsp:valueof value="${subCatHeader}" valueishtml="true"/></p> -->
						                    								<c:set var="catHeaderValue"
					...
					class="grid_6 omega marBottom_10 promoImage <c:if test="${key1 eq promoKeyTop}">promoHead</c:if>">
								...
								alt="${imageAlt}" />
																	...
																	<div class="grid_7 alpha">
																	 <c:if test="${not subCatHeaderShown}">
																	 	<c:set var="subCatHeaderShown" scope="request"
							value="true" />
																	 	<c:choose>
											                    			<c:when test="${!(CatFlagL3 eq null)}">
						...
						value="${elementList}" />
			                        ...
			                        <c:if
				test="${(frmBrandPage and !isPromoContentAvailable) && key1 eq promoKeyTop &&  not empty elementList}">
								                    ...
								                    <c:choose>
								                    	<c:when test="${not empty imageHREF}">
								                    	 <div class="row promoHead">
								                    		<a href="${imageHREF}"
											title="<dsp:valueof param="element.imageAlt"/>">
			                                          			<img
											src="${scene7Path}/${imageURL}"
											alt="<dsp:valueof param="element.imageAlt"/>" />
			                                          		</a>
			                                          	 </div>	
								                    	</c:when>
								          	          	<c:otherwise>
						...
						scope="request" value="true"></c:set>
							...
							scope="request" value="true" />
											...
											scope="request" />
												...
												test="${!(CatFlagL2 eq null) and (subCatHeader ne CatNameL2)}">
													...
													scope="request" />
			                    							...
			                    							<c:otherwise>
			                    								<h1>
													<dsp:valueof value="${subCatHeader}" valueishtml="true" />
												</h1>
			                    								<c:set var="catHeaderValue" value="true"
													scope="request" />
			                    							</c:otherwise>
		                   	...
		                   	<c:if test="${!(catBannerContent eq null)}">
							    	...
							    	<dsp:getvalueof var="catJSFile"
					param="categoryVO.jsFilePath" />
				...
				name="/atg/commerce/catalog/CategoryLookup">
								...
								<dsp:oparam name="output">
									<dsp:droplet
						name="/atg/commerce/catalog/CatalogNavHistoryCollector">
				...
				param="categoryVO.isChatEnabled" />
							...
							test="${(not empty chatEnabled)  && (chatEnabled eq 'true') && (chatLinkPlaceholder eq 'Top Right')}">
								...
								test="${(not empty parentChatEnabled)  && (parentChatEnabled eq 'true') && (parentCatChatLinkPlaceholder eq 'Top Right')}">
			                   	 ...
			                   	 <c:if test="${fromCollege eq 'true' }">
			                   	  <bbbl:textArea key="txt_college_cat_image_grid"
						language="${pageContext.request.locale.language}" />
								 </c:if>
							</c:if>
						</div>
					...
					<dsp:getvalueof var="refinements"
				param="browseSearchVO.categoryHeader.categoryRefinement" />
					<dsp:getvalueof var="phantomCategory"
				param="browseSearchVO.categoryHeader.phantomCategory" />
					<c:if
				test="${(refinements eq null || empty refinements ) && phantomCategory}">
					...
					<div id="content" class="newtrendPlp container_12 clearfix">
					<dsp:getvalueof var="facets" param="browseSearchVO.facets" />
					<dsp:getvalueof var="descriptorList"
					param="browseSearchVO.descriptors" />
						<c:if
					test="${not empty facets or fromCatL2AndL3 or fn:length(swsTermsList)>0 or fn:length(descriptorList)>1 }">					    			
									<c:set var="subCategoryPageParamNewPlp"
						value="${subCategoryPageParam}" />	 												
							<div class="<c:out value="${facetClass}"/> ">								
								<c:set var="totSize" value="${descriptorLength}" />	 
								<c:if test="${categoryVO.bccManagedPromoCategoryVO.showFilters eq false}"> 								
						...
						<div id="prodGridContainer ddaa"
					class="<c:out value="${gridClass}"/> noMarTop clearfix ec_gridwall">
					
							<div id="pagTop"
						class="<c:out value="${gridClass}"/> alpha omega">
								<c:if test="${isRefType}">
									<dsp:getvalueof var="subCategoryPageParam"
								value="${subCategoryPageParam}&refType=false" />
								</c:if>		
										<dsp:include
									page="/_includes/modules/pagination_top_1bar_new_plp.jsp">
											<dsp:param name="subCategoryPageParam"
										value="${subCategoryPageParamNewPlp}" />
											<dsp:param name="view" value="${defaultView}" />
											<dsp:param name="seoUrl" value="${seoUrl}" />
											<dsp:param name="pagBot" value="false" />
											...
											<dsp:param name="showFilters" value="${categoryVO.bccManagedPromoCategoryVO.showFilters}" />
										</dsp:include>																	
							</div>
								<c:if
						test="${categoryVO.bccManagedPromoCategoryVO.showFilters eq true}">
							<c:set var="plpViewParam"
							value="&view=${defaultView}${fromCollegeUrl}" />
							<c:set var="swsTermsLength" value="${fn:length(swsTermsList)}" />
							<c:set var="facetCountForRefType" value="${swsTermsLength}" />
								<c:set var="facetCountForRefTypeParam"
							value="${browseSearchVO.facetCountForRefTypeParam}" />
								<c:set var="facetCountForRefTypeParam"
							value="${facetCountForRefTypeParam + facetCountForRefType}" />
					...
					<form id="frmSearchCriteria" class="frmnewsearch" method="post">
						<fieldset class="searchGroup">
							<div class="searchTitle">
								...
								<c:set var="facetCountForRefType" value="${swsTermsLength}" />
								<%--BBBSL-6423 END --%>
								<dsp:getvalueof var="facetCountForRefTypeParam"
											param="browseSearchVO.facetCountForRefTypeParam" />
								<c:set var="facetCountForRefTypeParam"
											value="${facetCountForRefTypeParam + facetCountForRefType}" />
								<c:if test="${facetCountForRefTypeParam > 1}">
									<c:set var="refTypeParam" value="&refType=true" />
							...
							<div class="searchContent noBorder">
								<dsp:droplet name="ForEach">
									...
									<dsp:oparam name="output">
										<dsp:getvalueof var="type"
													param="selectedFacetRefItem.rootName" />
										<dsp:getvalueof var="facetItemRemoveQuery"
													param="selectedFacetRefItem.removalquery" />
										<dsp:getvalueof var="facetDescFilter"
													param="selectedFacetRefItem.descriptorFilter" />
										
										<c:choose>
											<c:when test="${type eq 'DEPARTMENT'}">
												<dsp:getvalueof var="facetFilter"
															param="selectedFacetRefItem.descriptorFilter" />
												<dsp:getvalueof var="catIdFilter"
															param="selectedFacetRefItem.categoryId" />
												...
												<c:if test="${not empty facetDescFilter}">
													<li class="clearfix">
														<span class="searchListItem">
															...
															<a
																	href="${url}${facetDescFilter}/${pageSize}${storeIdParam}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}${refTypeParam}${inStoreParam}"
										...
										<c:if test="${not empty narrowDownFilter}">
			                               <li class="clearfix">
												<span class="searchListItem">
													...
													<a
																href="${flurl}${narrowDownFilterValue}${filterParam}${pageSize}${storeIdParam}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}${refTypeParam}${inStoreParam}"
																class="lnkSearchRemove" title="Remove"
													...
													<c:if test="${not empty hideDepartmentOnNullResultPage}">
														<c:set var="previousPage" scope="request">${flurl}${narrowDownFilterValue}${filterParam}${pageSize}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}&refType=true</c:set>
										...
										<c:if
											...
											<c:set var="clear">
                                   			...
                                   			<c:if test="${empty urlSe}">
                                   				<c:set var="urlSe">
												${flurl}${storeIdParamNoStartingSlash}?subCatPlp=true${fromCollegeUrl}&refType=true${inStoreParam}&clear=true
												</c:set>
                                   			</c:if>
											<a href="${urlSe}&clear=true" class="lnkSearchReset" title="${clear}"> ${clear} </a>
										</c:if>
									</li>
									</ul>
									</dsp:oparam>
					...
					<form id="frmSearchCriteria" method="post">
						<fieldset class="searchGroup">
							...
							<div class="searchContent noBorder">
												...
												<c:if test="${not empty facetDescFilter}">
													<li class="clearfix">
														<span class="searchListItem">
															...
															<a  href="${url}${facetDescFilter}/${pageSize}${storeIdParam}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}${refTypeParam}${inStoreParam}" class="lnkSearchRemove" title="Remove" data-submit-param-length="2" data-submit-param1-name="origSearchTerm" data-submit-param1-value="${origSearchTerm}" data-value="${swsterms}">X</a> 
															<a  href="${redirectUrl}" class="lnkSearchRemove" title="Remove" data-submit-param-length="2" data-submit-param1-name="origSearchTerm" data-submit-param1-value="${origSearchTerm}" data-value="${swsterms}">X</a>
														</span>
													</li>
												</c:if>
										...
										<c:if test="${not empty narrowDownFilter}">
			                               <li class="clearfix">
												<span class="searchListItem">
													...
													<a  href="${flurl}${narrowDownFilterValue}${filterParam}${pageSize}${storeIdParam}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}${refTypeParam}${inStoreParam}" class="lnkSearchRemove" title="Remove" data-value="${narrowDownFilter.removalValue}">X</a>
													<a  href="${redirectUrl}" class="lnkSearchRemove" title="Remove" data-value="${narrowDownFilter.removalValue}">X</a>
													<c:if test="${not empty hideDepartmentOnNullResultPage}">
														<c:set var="previousPage" scope="request">${flurl}${narrowDownFilterValue}${filterParam}${pageSize}?${facetItemRemoveQuery}${subCategoryPageParamNewPlp}${searchQueryParams}${plpViewParam}&refType=true</c:set>
														<c:set var="removalValue" scope="request">${narrowDownFilter.removalValue}</c:set>
								...
								<dsp:droplet name="ResultListDroplet">
									...
									test="${categoryVO.bccManagedPromoCategoryVO.showFilters eq true}"> --%>
													<dsp:include
										page="/_includes/modules/product_grid_5x4_new_plp.jsp">
														<dsp:param name="BBBProductListVO"
											param="BBBProductListVO" />
														<dsp:param name="promoSC" value="${promoSC}" />
														<dsp:param name="url" value="${url}" />
														<dsp:param name="plpGridSize" value="3" />
			                                   ...
			                                   <dsp:param name="array"
			                                    ...
			                                    <dsp:oparam name="output">
													...
													<div class="promo-2col">
					                                	<a href="${imageHREF}"
														title="${imageAlt}"><img src="${imagePath}${imageURL}"
														alt="${imageAlt}" /></a>
		                                            </div>
			                                    </dsp:oparam>
			                             	</dsp:droplet>
						...
						class="<c:out value="${gridClass}"/> alpha omega">
									...
									page="/_includes/modules/pagination_top_1bar_new_plp.jsp">
										...
										value="${subCategoryPageParamNewPlp}" />
											<dsp:param name="view" value="${defaultView}" />
											<dsp:param name="pagBot" value="true" />
											<dsp:param name="narrowDown" value="${param.narrowDown}" />
											<dsp:param name="categoryId" value="${categoryId}" />
	                    	...
	                    	<c:if test="${not empty seoFooterContent}">
								<div
							class="<c:out value="${gridClass}"/> alpha omega clearfix blurbTxt">
									<p>
								<c:out value="${seoFooterContent}" escapeXml="false" />
							<p>
			...
			<c:forEach var="conMap" items="${configMap}">
			    <c:if test="${conMap.key eq applicationId}">
					...
					<c:when test="${fn:contains(conMap.value, var1)}">
						...
						<dsp:getvalueof var="phantomCategory"
								param="browseSearchVO.categoryHeader.phantomCategory" />
						<dsp:getvalueof var="phantomCategoryName"
								param="browseSearchVO.categoryHeader.name" />
						<c:if
								test="${not empty phantomCategory && phantomCategory == 'true'}">
					...
					<c:otherwise>  
						...
						<c:when test="${empty var3}">
							<c:choose>
							<c:when
											test="${replacedVar1 == lblWhatsNew || var1 == lblClearance}">
						...
						<c:otherwise>
							<c:choose>
								...
								<c:otherwise>
								  <c:choose>
								    <c:when
													test="${var1 == lblClearance && (var2 == 1 || var2 == 2 || var2 == 1)}">
								      <c:set var="pageName">${var1} > ${var1} > All</c:set>
									  <c:set var="prop1Var">Category Page</c:set>
									  <c:set var="prop3Var">${var1} > ${var1} > All</c:set>
									  <c:set var="catL3Flag" scope="request" value="true"></c:set>
	...
	<c:if test="${not isRobotOn}">				 
		...
		<dsp:setvalue bean="ProductComparisonList.url"
				value="${url}${queryParam}" />
