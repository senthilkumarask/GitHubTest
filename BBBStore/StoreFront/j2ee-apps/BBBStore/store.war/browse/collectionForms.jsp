<dsp:page>
	...
	<c:if test="${childProducts ne null && not empty childProducts}">
		...
		<dsp:form name="collectionForm" id="collectionForm" method="post">
			<span class="incDec visuallyhidden" aria-live="assertive"
			...
			<div class="sortIcons fr">
				<c:choose>
					<c:when test="${view == 'list'}">
						...
						<a class="active" title="List View" role="link" id="listView"
				...
				<input type="hidden" name="parentProdId"
					...
					value="${parentProductId} " /> <input type="submit" class="hidden"
					value="" />
			</div>
			<a name="collectionItems"></a>
			<h2>
				<bbbl:label key='lbl_pdp_shop_items_collection'
