...
<c:set var="BazaarVoiceOn" value="${param.BazaarVoiceOn}"/>
<c:set var="MapQuestOn" value="${param.MapQuestOn}"/>
					...
					<dsp:param param="collectionVO.childProducts" name="array" />
								...
								<c:choose>
									<c:when test="${not empty CategoryId}">
										<dsp:a iclass="prodImg" page="${finalUrl}?poc=${parentProductId}&categoryId=${CategoryId}" title="${prodName}" onclick="${onClickEvent}">
											...
											<c:otherwise>
												<img class="fl lazyLoad noImageFound loadingGIF" src="${imagePath}/_assets/global/images/blank.gif" width="229" height="229" alt="${prodName}" data-lazyloadsrc="${scene7Path}/${prodImage}"/>
											</c:otherwise>
											</c:choose>
											</c:if>
										</dsp:a>
													...
													<c:when test="${ratingAvailable == true}">
														...
														<c:choose>
															<c:when test="${totalReviewCount == 1}">
																...
																<span class="ratingTxt ratingsReviews prodReviews<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/>"><span class="ariaLabel"><dsp:valueof param="element.bvReviews.ratingsTitle"/></span></span><span class="reviewTxt"><a href="${contextPath}${finalUrl}?skuId=${catalogRefId}&amp;categoryId=${CategoryId}&amp;showRatings=true" role="link" aria-label="${totalReviewCount} ${lblReviewCount} ${lblForThe} ${prodName}">
																${ReviewCount}<bbbl:label key='lbl_review_count' language="${pageContext.request.locale.language}" /></a></span></div>
															</c:when>
															<c:otherwise>
																<div class="prodReviewGrid prodReviews clearfix metaFeedback">
																
																<span class="ratingTxt ratingsReviews prodReviews<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/>"><span class="ariaLabel"><dsp:valueof param="element.bvReviews.ratingsTitle"/></span></span><span class="reviewTxt"><a href="${contextPath}${finalUrl}?skuId=${catalogRefId}&amp;categoryId=${CategoryId}&amp;showRatings=true" role="link" aria-label="${totalReviewCount} ${lblReviewsCount} ${lblForThe} ${prodName}"> ${ReviewCount}<bbbl:label key='lbl_reviews_count' language="${pageContext.request.locale.language}" /></a></span>
                                                                                                                                 </div>
															</c:otherwise>
														</c:choose>
													</c:when>
						...
						<%-- Attribute--%>
									   <div class="prodAttrGrid">
																				...
																				<c:choose>
																					...
																					<c:otherwise>
																						<c:choose>
																							...
																							<c:otherwise>
																								<span class="attribs noMar"><span><dsp:valueof
																											param="element.attributeDescrip"
																											valueishtml="true" /></span></span>
																							</c:otherwise>
																						</c:choose>
													...
													<span class="shippingRestrictionsLink bold<c:if test="${not shippingRestricted or empty shippingRestricted}"> hidden</c:if>">
									   ...
									   <c:choose>
										   ...
										   <c:otherwise>
											   	<div class="spinner fl noMarTop padRight_10 <c:if test="${not empty skuid && not empty isLtlItem && isLtlItem}">hidden</c:if>">
													<label id="lblcollGridDecreaseQty${skuid}" class="txtOffScreen" for="collGridDecreaseQty"><bbbl:label key="lbl_decrease_quantity" language="${pageContext.request.locale.language}" /></label>
													<a id="collGridDecreaseQty" class="scrollDown down" href="#" title="<bbbl:label key='lbl_decrease_quantity' language="${pageContext.request.locale.language}" />"><span class="txtOffScreen"><bbbl:label key="lbl_decrease_quantity" language="${pageContext.request.locale.language}" /></span></a>
													...
													<label id="lblcollGridIncreaseQty${skuid}" class="txtOffScreen" for="collGridIncreaseQty"><bbbl:label key="lbl_increase_quantity" language="${pageContext.request.locale.language}" /></label>
													<a  id="collGridIncreaseQty" class="scrollUp up" href="#" title="<bbbl:label key='lbl_increase_quantity' language="${pageContext.request.locale.language}" />"><span class="txtOffScreen"><bbbl:label key="lbl_increase_quantity" language="${pageContext.request.locale.language}" /></span></a>
												</div>
											</c:otherwise>
										</c:choose>
											...
											<c:choose>
												<c:when test="${ (isCustomizationRequired || customizationOffered) && not empty skuVO.customizableCodes && !isInternationalCustomer}">
													<div class="personalizationOffered clearfix cb fl marTop_5">
														<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="${contextPath}${finalUrl}?poc=${parentProductId}&openKatori=true&skuId=${skuid}"</c:otherwise></c:choose> class="collectionPersonalize ${customizeAttr} <c:if test="${not enableKatoriFlag}">disabled</c:if>" role="button" data-product="${productId}" data-sku="${skuid}" data-vendor-name="${vendorName}" data-refnum="" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
															${personalizationTxt}
														</a>
														<c:choose>
													...
													<c:choose>
														<c:when test="${isInternationalCustomer}">
															<c:choose>
																<c:when test="${(isCustomizationRequired || customizationOffered) && not empty skuVO.customizableCodes}">
																	<div class="personalizationOffered clearfix cb fl marTop_5">
																		<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="#"</c:otherwise></c:choose> class="personalize ${customizeAttr} disabled" role="button" data-sku="${pDefaultChildSku}" data-product="${productId}" data-refnum="" data-vendor-name="${vendorName}" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
																			${personalizationTxt}
																		</a>
																...
																<div class="personalizationOffered clearfix cb fl hidden marTop_5">
																	<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="#"</c:otherwise></c:choose> class="personalize ${customizeAttr} disabled" role="button" data-product="${productId}" data-sku="${pDefaultChildSku}" data-vendor-name="${vendorName}" data-refnum="" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
																		${personalizationTxt}
																	</a>
																	<span class="personalizationIntlShippingMsg"><bbbt:textArea key="txt_pdp_per_unavailable_intl" language="${pageContext.request.locale.language}" /></span>
																</div>
														...
														<c:otherwise>
															<div class="personalizationOffered clearfix cb fl hidden marTop_5">
																...
																<a <c:choose><c:when test="${enableKatoriFlag eq false}">href="javascript:void(0)"</c:when><c:otherwise>href="${contextPath}${finalUrl}?poc=${parentProductId}&openKatori=true&skuId=${skuid}"</c:otherwise></c:choose> class="collectionPersonalize ${customizeAttr} <c:if test="${not enableKatoriFlag}">disabled</c:if>" role="button" data-vendor-name="${vendorName}" data-sku="${skuid}" data-refnum="" data-product="${productId}" data-quantity="" <c:if test="${enableKatoriFlag}">onclick="customLinks('${omniPersonalizeButtonClick}')"</c:if>;>
																	${personalizationTxt}
																</a>
																<c:choose>
																...
																<div class="collectionCart addToCart <c:if test="${not empty skuid && not empty isLtlItem && isLtlItem}">hidden</c:if>">
																	<div class="button button_active button_active_orange <c:if test="${isCustomizationRequired || not inStock || (isInternationalCustomer && isIntlRestricted) }">button_disabled</c:if>">
																		<input type="submit" name="btnAddToCart"
																			...
																			role="button"
																			<c:if test="${not inStock ||(isInternationalCustomer && isIntlRestricted) || isCustomizationRequired}">disabled="disabled"</c:if>/>
																	</div>
																</div>
																<c:choose>
																	...
																	<c:otherwise>
																		...
																		<div class="button button_active">
																			<input type="button" class="showOptionMultiSku" name="btnSelectOption" id="btnSelectOption${skuid}" value="<bbbl:label key='lbl_pdp_grid_choose_options' language="${pageContext.request.locale.language}" />" role="button" aria-pressed="false" onclick=""/>
																		</div>
																	</div>
																<%--LTL 498 end--%>	
															</c:otherwise>
											...
											<c:choose>
												...
												<c:otherwise>
													<div class="btnPDLinks padTop_10 clearfix cb">
														<div class="addToList fl padRight_5" id="btnAddToList${skuid}">
													<%--Adding customization disable flag check for grid view --%>	
													<c:choose>
														...
														<c:otherwise>
															...
															<%--Adding customization disable flag check for grid view --%>	
																<c:choose>
                                       					 			<c:when test="${isCustomizationRequired}">
                                       					  				<a class="disableText" href="javascript:void(0);" role="link" onclick="pdpOmnitureProxy('${productID}', 'findinstore')"><bbbl:label key='lbl_compare_find_in_store' language="${pageContext.request.locale.language}" /></a>
                                       						 		</c:when>
																	<c:otherwise>
