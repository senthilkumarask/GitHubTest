		...
		<dsp:oparam name="output">
			...
			<c:if test="${not empty childSKU}">
				<li class="bdrBot collectionItems clearfix">
					<fieldset class="registryDataItemsWrap listDataItemsWrap" itemscope
						<c:if test="${empty fromAjax}"> itemtype="http://schema.org/Product"</c:if> >
						...
						<c:if test="${not empty collFirstChildSKU && count eq 1}">
							<div id="firstproduct" style="display: none">
								LT_F_PRD_ID:=${productID} LT_F_SKU_ID:=${collFirstChildSKU}</div>
						...
						<div class="grid_2 alpha marRight_20">
							<c:choose>
								...
								<c:otherwise>
									<dsp:a iclass="prodImg"
										page="${finalUrl}?poc=${parentProductId}" title="${prodName}"
						...
						<div class="grid_6 omega">
							<div
								class="priceQuantityNotAvailable grid_6 alpha omega <c:if test="${inStock==true}">hidden</c:if> fl">
							...
							<div
								...
								<div
									class="rebateContainer collectionRebateContainer prodAttribWrapper fr">
																	...
																	<c:when test="${null ne imageURLTop}">
																		<span class="attribs  ${sep}"><img
																			src="${imageURLTop}" alt="" /><span><dsp:valueof
																					param="element.attributeDescrip" valueishtml="true" /></span></span>
																	</c:when>
																	<c:otherwise>
																		<c:choose>
																			...
																			<c:otherwise>
																				<span class="attribs  ${sep}"><span><dsp:valueof
																							param="element.attributeDescrip"
												...
												<c:otherwise>
													...
													<c:when test="${prodType eq 'FINISH'}">
														<div class="prodFinish fl">
															...
															<div class="pfinish">
																<select id="selProdFinish" name="selProdFinish" class="customSelectBoxCollection" aria-required="false" >
																	<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																	<dsp:param param="element" name="array" />
																		<dsp:oparam name="outputStart">
																			<option value=""><bbbl:label key='lbl_pdp_product_select' language="${pageContext.request.locale.language}" /></option>
							...
							<input type="hidden" class="addItemToList" value="${fn:escapeXml(view)}"
								name="pageView" />
							<%--<div class="fr btnPD">
