...
<div title="<bbbl:label key='lbl_notifyme_email_me_when_instock' language="${pageContext.request.locale.language}" />">
	<p class="noMarTop"><bbbl:label key='lbl_notifyme_placeholder' language="${pageContext.request.locale.language}" /></p>
...
<dsp:param name="id" param="productId" />
			...
			<div class="product clearfix">
					...
					<c:choose>
						<c:when test="${not empty productMediumImageUrl}">
							...
							<img class="productImage noImageFound" width="146" height="146"
								src='${scene7Path}/${productMediumImageUrl }'
								title="${getDetailsTitleText}" alt="${imageAltText}"/>
						</c:when>
						<c:otherwise>
							<c:choose>
  ...
  <form id="frmNotifyOOS" method="post" action="${contextroot}/browse/notifyMeConfirmPopup.jsp" >
    ...
    <input name="productName" type="hidden" value="${fn:escapeXml(productName)}"/>
			...
			<div class="formRow clearfix">
				<div class="button button_active">
					<input type="submit" value="SUBMIT" aria-pressed="false" role="button" />
				</div>
				<a href="#" class="buttonTextLink close-any-dialog" role="link"><bbbl:label key='lbl_notifyme_cancel' language="${pageContext.request.locale.language}" /></a>
			</div>
		</fieldset>
         </form>
</div>
