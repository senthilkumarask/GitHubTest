<dsp:page>
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/EverLivingDetailsDroplet" />
<dsp:importbean bean="/atg/userprofiling/Profile" />
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/ProductDetailDroplet" />
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/EverLivingPDPDroplet" />
	<dsp:importbean bean="/com/bbb/selfservice/ChatAskAndAnswerDroplet"/>
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/BreadcrumbDroplet" />
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/ProductViewedDroplet" />
	<dsp:importbean
	...
	<dsp:importbean
		bean="/com/bbb/commerce/browse/droplet/BrandDetailDroplet" />
	<dsp:importbean bean="/atg/multisite/Site"/>
	...
	<c:set var="submitAddToRegistryBtn">
		...
		<dsp:param name="isMainProduct" value="true" />
					<dsp:oparam name="output">
						...
						<a
							href="/store/browse/print/everLivingPDP_print.jsp?productId=${productId}"
							class="print" title="Print"></a> <a href="#" class="email"
							title="Email"></a>
	                            </div>
                            </div>
							<dsp:getvalueof var="isLeadSKU" param="collectionVO.leadSKU"/>
							<dsp:getvalueof var="collection" param="productVO.collection"/>
				<dsp:getvalueof var="linkStringNonRecproduct"
					param="linkStringNonRecproduct" />
							<dsp:getvalueof var="rkgProductList" param="rkgCollectionProdIds" />
				<dsp:getvalueof id="showFlag" idtype="java.lang.String"
					param="showFlag" />
							<dsp:getvalueof var="prodGuideId" param="productVO.shopGuideId"/>
							
							<div  itemscope itemtype="http://schema.org/Product">
					<div
						class="grid_12 marTop_10 <c:if test="${showFlag == '1'}">hasProdNextCollection</c:if>">
						<dsp:getvalueof var="refinedNameProduct"
							param="productVO.refinedName" scope="request" />
							
							 <c:set var="warningHeader">
							<bbbl:label key='lbl_everLivingPDP_warning_header'
								language="${pageContext.request.locale.language}" />
                            </c:set>
                             <c:set var="warningDetails">
							<bbbt:textArea key='lbl_everLivingPDP_warning_details'
								language="${pageContext.request.locale.language}" />
                            </c:set>
                            
							 <div class="alert  alert-alert-everliving">
							 <br/>
							 <c:choose>
                            <c:when test="${(appid eq BuyBuyBabySite)}">
									<span
										style="color: #444444; font-family: American Typewriter; font-size: 18px; line-height: 26px; text-transform: capitalize;">"${refinedNameProduct }"
										${warningHeader}</span>
									<br />
							 </c:when>
							 <c:otherwise>
									<span
										style="color: #444444; font-family: FuturaStdHeavy; font-size: 18px; line-height: 26px; text-transform: capitalize;">"${refinedNameProduct }"
										${warningHeader}</span>
									<br />
							 </c:otherwise>
							 </c:choose>
							<span
								style="color: #444444; font-family: Arial; font-size: 14px; line-height: 20px;">${warningDetails}</span>
							<br />
							<br />
						</div>
							...
							<dsp:getvalueof var="largeImage"
								param="productVO.productImages.largeImage" />
							<dsp:getvalueof var="smallImage"
								param="productVO.productImages.smallImage" />
							<dsp:getvalueof var="thumbnailImage"
								param="productVO.productImages.thumbnailImage" />
							<dsp:getvalueof var="basicImage"
								param="productVO.productImages.basicImage" />
							...
							<%-- <div>${productName}</div> --%>
							<div id="s7ProductImageWrapper"
								<c:if test="${zoomFlag && SceneSevenOn && (not empty largeImage)}"> class="bbbS7ImageWrapper" data-s7ImgsetID="${largeImage} " </c:if>
								data-parentProdId="${parentProductId}">
                                    <div class="bbbS7Main ">
									<div id="s7ProductImageWrapperInner"
										class="easyzoom easyzoom--adjacent ">
                                        <c:choose>
                                            <c:when test="${empty largeImage || !SceneSevenOn}">
												<img itemprop="image" id="mainProductImg"
													src="${imagePath}/_assets/global/images/no_image_available.jpg"
													class="mainProductImage noImageFound" height="380"
													width="380" alt="${productName}"
													onClick="_gaq.push(['_trackEvent', 'zoom', 'hero image', '${refinedNameProduct}']);" />
                                            </c:when>
											<c:when
												test="${!zoomFlag && SceneSevenOn && (not empty largeImage)}">
												<img itemprop="image" id="mainProductImg" src="${scene7Path}/${largeImage}"
													class="mainProductImage noImageFound" height="380"
													width="380" alt="${productName}"
													onClick="_gaq.push(['_trackEvent', 'zoom', 'hero image', '${refinedNameProduct}']);" />
                                            </c:when>
                                            <c:otherwise>
												<a id="mainProductImgZoom"
													data-zoomhref="${scene7Path}/${basicImage}?hei=2000&wid=2000&qlt=50,1"
													href="javascript:void(0)"> <img itemprop="image" id="mainProductImg"
													src="${scene7Path}/${largeImage}" class="mainProductImage"
													alt="${productName}"
													onClick="_gaq.push(['_trackEvent', 'zoom', 'hero image', '${refinedNameProduct}']);" />
		                                             </a>
				...
				<div class="grid_6 omega productDetails">
					<dsp:form name="prodForm" method="post" id="prodForm">
								...
								<div class="registryDataItemsWrap listDataItemsWrap"
                        ...
                        <dsp:droplet name="BreadcrumbDroplet">
                            ...
                            <c:set var="backToCircular">
													<bbbl:label key='lbl_pdp_breadcrumb_circular'
														language="${pageContext.request.locale.language}" />
                            </c:set>
												<dsp:a iclass="backToCircular"
													href="/store/browse/gadgets/circular.jsp?${output_circular}"
													title="${backToCircular}">
                           	${backToCircular}
                            </dsp:a>
                            </c:if>
                            </dsp:oparam>
                        ...
                        <dsp:droplet name="ChatAskAndAnswerDroplet">
							...
							<dsp:oparam name="output">
								<dsp:getvalueof var="PDPAttributesVo" param="PDPAttributesVo"/>
											<c:set var="chatEnabled"
												value="${PDPAttributesVo.chatEnabled}" scope="request" />
											<c:set var="askAndAnswerEnabled"
												value="${PDPAttributesVo.askAndAnswerEnabled}"
												scope="request" />
											<c:set var="chatCode" value="${PDPAttributesVo.categoryId}"
												scope="request" />
							</dsp:oparam>
						</dsp:droplet>
						<%-- End for Chat and Ask and Answer 2.1.1 --%>
									...
									<input type="hidden" class="addToCartSubmitData"
										value="${btsValue}" name="bts" />
						...
						<dsp:param name="array" value="${attribs}"/>
							...
							<dsp:getvalueof var="placeHolderTop" param="key"/>
												<c:if
													test="${(not empty placeHolderTop) && (placeHolderTop eq AttributePDPTOP)}">
									<dsp:droplet name="/atg/dynamo/droplet/ForEach">
										<dsp:param param="element" name="array" />
										<dsp:param name="sortProperties" value="+priority"/>
											<dsp:oparam name="output">
															<dsp:getvalueof var="placeHolderTop"
													...
													<c:when test="${null ne attributeDescripTop}">
														<c:choose>
														    <c:when test="${null ne imageURLTop}">
																				<a href="${actionURLTop}" class="newOrPopup"><img
																					src="${imageURLTop}"
																					alt="<dsp:valueof param="element.attributeDescrip" valueishtml="true"/>" /></a>
														    </c:when>
														    <c:otherwise>
													            <c:choose>
														            <c:when test="${null ne actionURLTop}">
																						<a href="${actionURLTop}" class="newOrPopup"><span><dsp:valueof
																									param="element.attributeDescrip"
																									valueishtml="true" /></span></a>
														            </c:when>
														            <c:otherwise>
																						<span><dsp:valueof
																								param="element.attributeDescrip"
						...
						<c:choose>
							...
							<c:otherwise>
											<img class="hidden productImage noImageFound" height="83"
												width="83" src="${scene7Path}/${thumbnailImage}"
												alt="${productName}" />
							</c:otherwise>
						</c:choose>
						<dsp:getvalueof var="productName" param="productVO.name"/>
									<h1 id="productTitle" class="productDetails" itemprop="name">
										<dsp:valueof param="productVO.name" valueishtml="true" />
									</h1>
						<div class="bulletsReset productDesc" itemprop="description">
										<dsp:valueof param="productVO.shortDescription"
											valueishtml="true" />
						...
						<c:set var="component_ID">
										<bbbc:config key="${client_ID}_WVLink"
											configName='BBBLiveClicker' />
						</c:set>
									<div
										class="productLinksWrapper rebateContainer prodAttribWrapper clearfix">
							<div class="pdpAttributeContainer fl clearfix">
								...
								<c:when test="${null ne mediaVO}">
										...
										<c:if test="${null ne mediaURL}">
														<div class="productLinks watchVideo clearfix">
															<div>
																<a href="${mediaURL}"
																	onClick="javascript:rkg_micropixel('${currentSiteId}','video'); openmedia(); pdpOmnitureProxy('${parentProductId}', 'videoplay'); return true;"
																	target="module35892976"> <img
																	src="/_assets/global/images/watch_video.png"
																	alt="Watch Video" title="Watch Video" width="119"
								...
								<c:otherwise>
										<c:if test="${LiveClickerOn}">
														<div id="liveClickerWatchVideo"
															class="productLinks watchVideo clearfix"
															data-parentProdId="${parentProductId}">
															<script id="tag" type="text/javascript"
																src="<bbbc:config key="liveclicker_pdp" configName="ThirdPartyURLs" />client_id=${client_ID}&component_id=${component_ID}&dim6=${parentProductId}&widget_id="></script>
										</div>
										</c:if>
								</c:otherwise>
								</c:choose>
								<c:if test="${not empty prodGuideId}">
									<dsp:droplet name="/atg/repository/seo/GuideItemLink">
													<dsp:param name="repositoryName"
														value="/com/bbb/cms/repository/GuidesTemplate" />
										...
										<dsp:oparam name="output">
											...
											<div id="viewProductGuides"
												class="productLinks viewProductGuides clearfix">
													<a href="${contextPath}${finalUrl}"
														onclick="pdpOmnitureProxy('${parentProductId}', 'prodguide')"
														title="${guideTitle}" class="productGuideImg">${guideTitle}</a>
											</div>
								</c:if>
								<c:if test="${collection==true}">
									<c:if test="${isLeadSKU==true}">
										<div class="productLinks">
														<a class="lnkCollectionItems viewAccessories"
															data-smoothscroll-topoffset="65" href="#collectionItems"><bbbl:label
																key='lbl_pdp_view_accessories'
							...
							<%-- Rebate Container --%>
								...
								<c:if test="${not empty hasRebate && hasRebate}">
											<dsp:getvalueof var="chkEligibleRebates"
												param="pSKUDetailVO.eligibleRebates" />
											<c:if
												test="${(null != chkEligibleRebates) && (fn:length(chkEligibleRebates) == 1 )}">
								...
								<dsp:droplet name="/atg/dynamo/droplet/ForEach">
									...
									<dsp:getvalueof var="placeHolderPrice" param="key"/>
												<c:if
													test="${(not empty placeHolderPrice) && (placeHolderPrice eq AttributePDPPrice)}">
											<dsp:droplet name="/atg/dynamo/droplet/ForEach">
												<dsp:param param="element" name="array" />
												<dsp:param name="sortProperties" value="+priority"/>
													<dsp:oparam name="output">
													...
													<c:if test="${chkCount == chkSize}">
														...
														<c:otherwise>
															...
															<dsp:getvalueof var="placeHolderPrice"
																param="element.placeHolder" />
															<dsp:getvalueof var="attributeDescripPrice"
																param="element.attributeDescrip" />
															<dsp:getvalueof var="imageURLPrice"
																param="element.imageURL" />
															<dsp:getvalueof var="actionURLPrice"
																param="element.actionURL" />
													<c:choose>
														<c:when test="${null ne attributeDescripPrice}">
															<c:choose>
																<c:when test="${null ne imageURLPrice}">
																			<span class="rebateAttribs attribs ${sep}"><a
																				href="${actionURLPrice}" class="newOrPopup"><img
																					src="${imageURLPrice}"
																					alt="<dsp:valueof param="element.attributeDescrip" valueishtml="true"/>" /></a></span>
																</c:when>
																<c:otherwise>
																	<c:choose>
																		<c:when test="${null ne actionURLPrice}">
																					<span class="rebateAttribs attribs ${sep}"><a
																						href="${actionURLPrice}" class="newOrPopup"><span><dsp:valueof
																									param="element.attributeDescrip"
																									valueishtml="true" /></span></a></span>
																		</c:when>
																		<c:otherwise>
																					<span class="rebateAttribs attribs ${sep}"><dsp:valueof
																							param="element.attributeDescrip"
																							valueishtml="true" /></span>
																		</c:otherwise>
																	</c:choose>
															...
															<c:if test="${null ne imageURLPrice}">
																<c:choose>
																	<c:when test="${null ne actionURLPrice}">
																				<span class="rebateAttribs attribs ${sep}"><a
																					href="${actionURLPrice}" class="newOrPopup"><img
																						src="${imageURLPrice}" alt="" /></a></span>
																	</c:when>
																	<c:otherwise>
																				<span class="rebateAttribs attribs ${sep}"><img
																					src="${imageURLPrice}" alt="" /></span>
																	</c:otherwise>
																</c:choose>
															</c:if>
														</c:otherwise>
									...
									<dsp:param name="array" value="${eligibleRebates}"/>
										<dsp:oparam name="output">
											...
											<dsp:getvalueof var="rebate" param="element"/>
													<span class="rebateAttribs attribs ${sep1}"><a
							...
							<div class="partCollection clearfix cb">
								<dsp:droplet name="/atg/repository/seo/CanonicalItemLink">
									...
									<dsp:param name="repositoryName"
										value="/atg/commerce/catalog/ProductCatalog" />
									<dsp:oparam name="output">
													<dsp:getvalueof var="collectionFinalUrl"
														vartype="java.lang.String" param="url" />
									</dsp:oparam>
								</dsp:droplet>
											<dsp:a page="${collectionFinalUrl}"
												title="${collectionProductName}">
									<c:choose>
										<c:when test="${empty collectionSmallImage}">
														<div class="partCollectionImg">
															<img class="collectionSmallImage" height="63" width="63"
																src="${imagePath}/_assets/global/images/no_image_available.jpg"
																alt="${collectionProductName}" />
														</div>
										</c:when>
										<c:otherwise>
														<div class="partCollectionImg">
															<img class="collectionSmallImage noImageFound"
																height="63" width="63"
																src="${scene7Path}/${collectionSmallImage}"
																alt="${collectionProductName}" />
														</div>
										</c:otherwise>
									</c:choose>
										<div class="collectionName">
											<table class="noMar" border="0">
												<tr>
															<td><bbbl:label key='lbl_pdp_product_part_of'
																	language="${pageContext.request.locale.language}" />
																&nbsp;<c:out value="${collectionProductName}"
																	escapeXml="false" /> &nbsp;<bbbl:label
																	key='lbl_pdp_product_part_of_collection'
																	language="${pageContext.request.locale.language}" /></td>
												</tr>
											</table>
										</div>
									</dsp:a>
								...
								<c:if test="${BazaarVoiceOn}">
									<div id="BVRRSummaryContainer">
													<img width="20" height="20" alt=""
														src="/_assets/global/images/widgets/small_loader.gif">
	                                </div>
							...
							<div class="fbLikePrintMessage share marTop_10">
                                ...
                                <!--[if !IE 7]><!-->
												<div class="fb-like" data-layout="button_count"
													data-send="false" data-width="90" data-show-faces="false"></div>
                                <!--<![endif]-->
                                </c:if>
               ...
               <dsp:droplet name="/atg/dynamo/droplet/ForEach">
				...
				<dsp:getvalueof var="placeHolderMiddle" param="key"/>
											<c:if
												test="${(not empty placeHolderMiddle) && (placeHolderMiddle eq AttributePDPMiddle)}">
						<dsp:droplet name="/atg/dynamo/droplet/ForEach">
							<dsp:param param="element" name="array" />
							<dsp:param name="sortProperties" value="+priority"/>
								<dsp:oparam name="output">
														<dsp:getvalueof var="placeHolderMiddle"
															param="element.placeHolder" />
														<dsp:getvalueof var="attributeDescripMiddle"
															param="element.attributeDescrip" />
														<dsp:getvalueof var="imageURLMiddle"
															param="element.imageURL" />
														<dsp:getvalueof var="actionURLMiddle"
															param="element.actionURL" />
									...
									<c:choose>
										<c:when test="${null ne attributeDescripMiddle}">
											<c:choose>
												<c:when test="${null ne imageURLMiddle}">
																			<a href="${actionURLMiddle}" class="newOrPopup">
																				<img src="${imageURLMiddle}"
																				alt="<dsp:valueof param="element.attributeDescrip" valueishtml="true"/>" />
																			</a>
												</c:when>
												<c:otherwise>
													<c:choose>
														<c:when test="${null ne actionURLMiddle}">
																					<a href="${actionURLMiddle}" class="newOrPopup"><span><dsp:valueof
																								param="element.attributeDescrip"
																								valueishtml="true" /></span></a>
														</c:when>
														<c:otherwise>
										...
										<c:otherwise>
											<c:if test="${null ne imageURLMiddle}">
												<c:choose>
													<c:when test="${null ne actionURLMiddle}">
																				<a href="${actionURLMiddle}" class="newOrPopup"><img
																					src="${imageURLMiddle}" alt="" /></a>
													</c:when>
													<c:otherwise>
				...
				<c:if test="${skuVO != null}">
									<dsp:getvalueof var="pDefaultChildSku"
										param="pSKUDetailVO.skuId" />
				</c:if>
			...
			<c:set var="everLivingAlert">
									<bbbl:label key='lbl_everLivingPDP_error'
										language="${pageContext.request.locale.language}" />
								</c:set>
			<span>${everLivingAlert }</span>
				...
				<%--<div class="priceQuantity noPadTop <c:if test="${inStock==false}">priceQuantityNotAvailable</c:if>">
					<span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
						...
						<%--<div class="btnPD clearfix">
							<div class="fl addToList noMarLeft">
								<div class="button button_disabled">
									<input type="button" name="btnAddToList" id="btnAddToList" value="Save for Later" role="button"  onclick="rkg_micropixel('${appid}','wish')"/>
								</div>
							</div>
								    ...
								    <dsp:getvalueof var="bopusAllowed" param="pSKUDetailVO.bopusAllowed"/>
								   	<c:choose>
									   	<c:when test="${bopusAllowed}">
										     ...
										     <input type="button" name="btnFindInStore" value="Find in Store" disabled="disabled" class="changeStore" role="button" />
										   </div>
										</c:when>
										<c:otherwise>
										   <div class="button changeStoreItemWrap button_active_orange">
									     <input type="button" name="btnFindInStore" value="Find in Store" role="button" onclick="pdpOmnitureProxy('${parentProductId}', 'findinstore');rkg_micropixel('${appid}','findstore');"  class="changeStore"/>
										   </div>
										</c:otherwise>
									</c:choose>
								</div>
							...
							<div class="fl addToCart button_disabled">
								<div class="button button_disabled">
									<input type="submit" name="btnAddToCart" value="Add to Cart" onclick="rkg_micropixel('${appid}','cart')" role="button" disabled="disabled" <c:if test="${inStock==true}">class="enableOnDOMReady"</c:if> />
								</div>
							</div>
						...
						<c:if test="${MapQuestOn && (appid eq BuyBuyBabySite)}">
							<bbbt:textArea key="txt_pdp_pickupinstore"
								language="${pageContext.request.locale.language}" />
                        </c:if>
						...
						<dsp:getvalueof var="brandId"
							param="productVO.productBrand.brandId" />
						<dsp:getvalueof var="brandName"
							param="productVO.productBrand.brandName" />
						...
						<c:if test="${null ne brandId }">
								<div class="fl shopAll" itemprop="brand" itemscope
                            ...
                            <script type="text/javascript">
                                ...
                                // -->
											...
											<a href="${brandUrl}" title="${brandName}"
												onclick="_gaq.push(['_trackEvent', 'Product_BrandLink', 'click', _ga_refinedBrandName]);">
												<bbbl:label key='lbl_pdp_shop_all'
													language="${pageContext.request.locale.language}" /> <span itemprop="name">${brandName}</span>
											</a>
						</c:when>
						<c:otherwise>
											<a href="${brandUrl}" title="${brandName}"> <bbbl:label
													key='lbl_pdp_shop_all'
													language="${pageContext.request.locale.language}" /> <span itemprop="name">${brandName}</span></a>
						</c:otherwise>
						</c:choose>
						</div>
						</c:if>
							...
							<c:if test="${isLeadSKU==false}">
								<div class="productLinks noMarTop clearfix marBottom_5">
									<div class="button button_active">
												<a class="lnkCollectionItems smoothScrollTo"
													data-smoothscroll-topoffset="65" href="#collectionItems"><bbbl:label
														key='lbl_pdp_show_collection'
														language="${pageContext.request.locale.language}" /></a>
									</div>
								</div>
							</c:if>
					...
					<c:if test="${tipsOn}">
						<dsp:droplet name="TargetingRandom">
								<dsp:param
									bean="/atg/registry/RepositoryTargeters/TargeterPDPSecond"
									name="targeter" />
						...
						<div class="width_6 returnByStore">
							<h3> ${tipsTitle} </h3>
									<div class="desclight2">${tipsTxt}</div>
			...
			<c:set var="certona_on">
				<bbbc:config key="certona_no_search_results_flag"
			...
			<dsp:getvalueof var="linkStringNonRecproduct"
				param="linkStringNonRecproduct" />
			<c:set var="linkStringNonRecproduct" value="${linkStringNonRecproduct}" scope="request"/>
             <%--Ajax Call --%>
             <%--BBBSL-8186 | Passing parentProductID to populate value of certona context --%>
			 <div id="certonaBottomTabs" class="clearfix loadAjaxContent" 
                            data-ajax-url="${contextPath}/browse/certona_everliving_slots.jsp" 
                            data-ajax-target-divs="#certonaBottomTabs" 
				data-ajax-params-count="5" data-ajax-param1-name="scheme"
				data-ajax-param1-value="${SchemeName}"
				data-ajax-param2-name="certonaSwitch"
				data-ajax-param2-value="${CertonaOn}"
				data-ajax-param3-name="productId"
				data-ajax-param3-value="${productId}" data-ajax-param4-name="number"
				data-ajax-param4-value="${custAlsoViewedProdMax};${frequentlyBuyProdMax}}"
				data-ajax-param5-name="parentProductId"
				data-ajax-param5-value="${parentProductId}"
				data-ajax-param6-name="linkStringNonRecproduct"
				data-ajax-param6-value="${linkStringNonRecproduct}}"
                	role="complementary">
				<div class="grid_12 clearfix">
					<img width="20" height="20" alt="small loader"
						src="/_assets/global/images/widgets/small_loader.gif" />
             </div>
			</div>
	...
	var resx = new Object();
			 ...
			 <%-- <span class="fl"><bbbl:label key='lbl_pdp_product_explore' language="${pageContext.request.locale.language}" /></span> --%>
				...
				<div class="categoryProductTabs marTop_10">
					<ul class="categoryProductTabsLinks" role="tablist">
						<li id="prodViewDefault-tab1" role="tab"><div class="arrowSouth"></div>
							<a
							title='<bbbl:label key='lbl_pdp_product_info' language="${pageContext.request.locale.language}" />'
							href="#prodExplore-tabs1" role="tab" aria-controls="prodExplore-tabs1" aria-selected="true"
							onclick="pdpOmnitureProxy('${parentProductId}', 'productspec')"><bbbl:label
									key='lbl_pdp_product_info'
						...
						<dsp:droplet name="/atg/dynamo/droplet/ForEach">
							...
							<dsp:oparam name="output">
								...
								<li  id="prodViewDefault-tab<c:out value='${count+1}'/>" role="tab"><div class="arrowSouth"></div>
									<a title="<dsp:valueof param="element.tabName"/>"
									href="#prodExplore-tabs<c:out value='${count}'/>" role="tab" aria-controls="prodExplore-tabs<c:out value='${count}'/>" aria-selected="false"
									onclick="pdpOmnitureProxy('${parentProductId}', 'videoplay')"><dsp:valueof
											param="element.tabName" /></a></li>
	                    ...
	                    <c:if test="${BazaarVoiceOn}">
							<li  id="prodViewDefault-tab<c:out value='${count}'/>" role="tab"><div class="arrowSouth"></div> <a
								title="<bbbl:label key='lbl_pdp_product_ratings_reviews' language="${pageContext.request.locale.language}" />"
								href="#prodExplore-tabs<c:out value='${count}'/>" role="tab" aria-controls="prodExplore-tabs<c:out value='${count}'/>" aria-selected="false" 
								onclick="pdpOmnitureProxy('${parentProductId}', 'ratingreviews')"><bbbl:label
										key='lbl_pdp_product_ratings_reviews'
										language="${pageContext.request.locale.language}" /></a></li>
	                    	<%-- Start for Chat and Ask and Answer 2.1.1 --%>
							<c:if test="${askAndAnswerEnabled}">
								<li id="prodViewDefault-tab<c:out value='${count+1}'/>" role="tab"><div class="arrowSouth"></div> <a
									title="<bbbl:label key='lbl_pdp_product_ask_answer' language="${pageContext.request.locale.language}" />"
									class="aaBeta"
									href="#prodExplore-tabs<c:out value='${count+1}'/>" role="tab" aria-controls="prodExplore-tabs<c:out value='${count+1}'/>" aria-selected="false"
									onclick="pdpOmnitureProxy('${parentProductId}', 'askAndAnswer')"><span><bbbl:label
												key='lbl_pdp_product_ask_answer'
												language="${pageContext.request.locale.language}" /></span><span
										class="aaBetaImg"></span><span class="clear"></span></a></li>
					...
					<div id="prodExplore-tabs1" role="tabpanel" aria-labelledby="prodViewDefault-tab1"  class="categoryProductTabsData" aria-hidden="false">
						...
						<dsp:getvalueof var="harmonLongDescription"
							param="productVO.harmonLongDescription" />
                           	<c:if test="${not empty harmonLongDescription}">
                           		<div class="harmonDesc grid_12 noMar">
								<dsp:valueof param="productVO.harmonLongDescription"
									valueishtml="true" />
                           		</div>
                           	</c:if>
                        <%-- R 2.2.1 || Harmon END   --%>
                  ...
                  <%-- <c:if test="${LiveClickerOn}">
					...
					<c:set var="count" value="2" />
						...
						<dsp:oparam name="output">
							...
							<div id="prodExplore-tabs<c:out value='${count}'/>" aria-hidden="true" aria-labelledby="prodViewDefault-tab<c:out value='${count}'/>" role="tabpanel"
								class="categoryProductTabsData">
					...
					<c:if test="${BazaarVoiceOn}">
						<div id="prodExplore-tabs<c:out value='${count}'/>" aria-labelledby="prodViewDefault-tab<c:out value='${count}'/>" role="tabpanel" 
							class="categoryProductTabsData clearfix divRatings" aria-hidden="true">
						...
						<div id="BVRRContainer">
							...
							<c:if test="${SmartSEOOn}">
									<dsp:droplet
										name="/com/bbb/commerce/browse/droplet/BVReviewContentDroplet">
				...
				<c:if test="${showQA}">
						<div id="prodExplore-tabs<c:out value='${count+1}'/>" aria-labelledby="prodViewDefault-tab<c:out value='${count+1}'/>" role="tabpanel"
							class="categoryProductTabsData clearfix divQuestions" aria-hidden="true">
			...
			<c:when test="${isLeadSKU==true}">
							...
							<div class="loadCollectionAccesories grid_12 clearfix"
								data-ajax-url="${contextPath}/browse/accessoriesForms.jsp"
								data-ajax-params-count="10" data-ajax-param1-name="color"
								data-ajax-param1-value="${color}"
								data-ajax-param2-name="parentProductId"
								data-ajax-param2-value="${parentProductId}"
								data-ajax-param3-name="crossSellFlag"
								data-ajax-param3-value="true" data-ajax-param4-name="desc"
								data-ajax-param4-value="Accessories (pdp)"
								data-ajax-param5-name="BazaarVoiceOn"
								data-ajax-param5-value="${BazaarVoiceOn}"
								data-ajax-param6-name="isEverLivingProd"
								data-ajax-param6-value="true"
								data-ajax-param7-name="showAccessories"
								data-ajax-param7-value="true"
								data-ajax-param8-name="OutOfStockOn"
								data-ajax-param8-value="${OutOfStockOn}"
								data-ajax-param9-name="view" data-ajax-param9-value="${view}"
			...
			<c:otherwise>
							<div class="loadCollectionAccesories grid_12 clearfix"
								...
								data-ajax-params-count="9" data-ajax-param1-name="color"
								data-ajax-param1-value="${color}"
								data-ajax-param2-name="parentProductId"
								data-ajax-param2-value="${parentProductId}"
								data-ajax-param3-name="crossSellFlag"
								data-ajax-param3-value="true" data-ajax-param4-name="desc"
								data-ajax-param4-value="Collection (pdp)"
								data-ajax-param5-name="isEverLivingProd"
								data-ajax-param5-value="true"
								data-ajax-param6-name="BazaarVoiceOn"
								data-ajax-param6-value="${BazaarVoiceOn}"
								data-ajax-param7-name="OutOfStockOn"
								data-ajax-param7-value="${OutOfStockOn}"
								data-ajax-param8-name="view" data-ajax-param8-value="${view}"
								data-ajax-param9-name="fromAjax" data-ajax-param9-value="true"
								></div>
			...
			<div id="botCrossSell" class="marTop_20 container_12"
				role="complementary">
			...
			<%-- Shashank Start2 --%>
				...
				<div class="categoryProductTabs marTop_20"> 
							<c:set var="lastViewedTab">
								<bbbl:label key='lbl_pdp_product_last_viewed'
									language="${pageContext.request.locale.language}" />
							</c:set>
					...
					<c:if test="${not empty lastviewedProductsList}">
						...
						<div id="botCrossSell-tabs3" class="categoryProductTabsData">
							<dsp:include page="last_viewed.jsp" >
										<dsp:param name="lastviewedProductsList"
											value="${lastviewedProductsList}" />
		...
		<div class="grid_12 clearfix marTop_20">
					<h3>
						<span class="error"> <bbbl:label
								key='lbl_pdp_product_not_available'
								language="${pageContext.request.locale.language}" />
						</span>
					</h3>
		</div>
