...
<c:set var="imagePath"><bbbc:config key="image_host_gs" configName="ThirdPartyURLs" /></c:set>
		...
		<dsp:param name="elementName" value="productVO"/>
			...
			<a class="prodImg" href="${contextPath}${finalUrl}" title="${productName}" onclick="${onClickEvent}" style="width:inherit;">
				
					<c:choose>
						<c:when test="${empty basicImage}">
							<img src="${imagePath}/_assets/global/images/no_image_available.jpg" height="170" width="170" alt=" ${productName}" /> 
						</c:when>
						<c:otherwise>
							<img class="productImage noImageFound" src="/_assets/global/images/image-preloader.gif" alt=" ${productName}" data-src-url="${scene7Path}/${basicImage}" height="170" width="170"/>
						</c:otherwise>
					</c:choose>
					...
					<noscript aria-hidden='true'>
						...
						<c:otherwise>
							<img src="${scene7Path}/${basicImage}" height="170" width="170" alt=" ${productName}" class="noImageFound"/>
						</c:otherwise>
						</c:choose>
					</noscript>
						  ...
						  <c:when test="${not empty kickStrtWasPriceRangeDescripMX}">
							<span>
									...
									<span class="was"><bbbl:label key='lbl_was_text' language="${pageContext.request.locale.language}" /></span>
										 <dsp:valueof converter="currency" param="productVO.kickStarterPrice.kickStrtWasPriceRangeDescripMX"/>
									</span>
									</span>
									</c:when>
									<c:otherwise>
						           ...
						           <c:when test="${not empty kickStrtWasPriceRangeDescrip}">
									...
									<span class="wasPrice">
									<span class="was"><bbbl:label key='lbl_was_text' language="${pageContext.request.locale.language}" /></span>
										 <dsp:valueof converter="currency" param="productVO.kickStarterPrice.kickStrtWasPriceRangeDescrip"/>
									</span>
									</span>
									</c:when>
									<c:otherwise>
