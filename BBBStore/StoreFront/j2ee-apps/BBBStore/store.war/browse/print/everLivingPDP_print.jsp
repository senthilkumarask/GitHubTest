...
<jsp:attribute name="pageWrapper">productDetails ${pageWrapperClass} useMapQuest printerFriendly</jsp:attribute>
<jsp:body>
									...
									<div class="width_5 swatches">
										<dsp:droplet name="/atg/dynamo/droplet/ForEach">
												...
												<c:choose>
													...
													<a href="#" class="size fl selected" data-attr="${attribute}" title="${attribute}"><span><c:out value="${attribute}" escapeXml="false"/></span></a>
													</c:when>
													<c:otherwise>
													<a href="#" class="size fl" data-attr="${attribute}" title="${attribute}"><span><c:out value="${attribute}" escapeXml="false"/></span></a>
													</c:otherwise>
												</c:choose>
												</dsp:oparam>	
											</dsp:droplet>
										...
										<c:when test="${menu eq 'COLOR'}">
											...
											<label class="fl"><dsp:valueof param="key"/>:</label>
												...
												<dsp:param param="element" name="array" />											
													...
													<a href="#" class="fl selected" data-attr="${fn:toLowerCase(attribute)}" title="${attribute}" >
													<span>
													<c:choose>
													...
													<c:otherwise>
													<a href="#" class="fl" data-attr="${fn:toLowerCase(attribute)}" title="${attribute}" >
													<span>
													<c:choose>
													<c:when test="${(colorImagePath != null) && (colorImagePath ne '')}">
														<img src="${scene7Path}/${colorImagePath}" height="20" width="20" alt="${attribute}" class="noImageFound" />
										...
										<c:otherwise>
											...
											<label class="fl"><dsp:valueof param="key"/>:</label>
												...
												<dsp:param param="element" name="array" />											
													...
													<c:choose>
													<c:when test="${(colorParam != null) && (colorParam eq attribute)}">
													<a href="#" class="${fn:toLowerCase(attribute)} fl selected" data-attr="${attribute}" title="${attribute}" >
													<span>
													<c:choose>
													...
													<c:otherwise>
													<a href="#" class="${fn:toLowerCase(attribute)} fl" data-attr="${attribute}" title="${attribute}" >
													<span>
													<c:choose>
													<c:when test="${(finishImagePath != null) && (finishImagePath ne '')}">
														<img src="${scene7Path}/${finishImagePath}" height="20" width="20" alt="${attribute}" class="noImageFound" />
			...
			<dsp:getvalueof var="showTab" param="tabLookUp"/>
						...
						<%--<div class="priceQuantity <c:if test="${inStock==false}">priceQuantityNotAvailable</c:if>">
								...
								<c:choose>
									...
									<c:otherwise>
										<c:choose>
											<c:when test="${not empty productVO.salePriceRangeDescription}">
											...
											<dsp:droplet name="/atg/dynamo/droplet/ForEach">
												...
												<dsp:param name="sortProperties" value="+priority"/>
													...
													<dsp:getvalueof var="actionURLPrice" param="element.actionURL"/>
																<%--removing shipping details for ever living print page --%>
																<%--	<c:choose>
			...
			<c:if test="${CertonaOn}">
				...
				<div class="categoryProductTabs marTop_20">
					<ul class="categoryProductTabsLinks">
						<li><h2><bbbl:label key='lbl_pdp_product_related_items' language="${pageContext.request.locale.language}" /></h2></li>
					</ul>
