					...
					<dsp:oparam name="output">
						<li class="bdrBot collectionItems clearfix">
							<fieldset class="registryDataItemsWrap listDataItemsWrap"
								...
								<c:if
									test="${(null != childSKU) && (fn:length(childSKU) == 1 )}">
									<dsp:droplet name="/atg/dynamo/droplet/ForEach">
										...
										<c:when test="${not empty CategoryId}">
											<dsp:a iclass="prodImg"
												...
												<c:choose>
													...
													<c:otherwise>
														<img class="noImageFound" src="${scene7Path}/${prodImage}"
															width="146" height="146" alt="${prodName}" />
													</c:otherwise>
												</c:choose>
											</dsp:a>
								...
								<div class="grid_4 alpha">
									...
									<ul class="prodInfo">
										<li class="prodName"><c:choose>
												...
												<c:otherwise>
													<dsp:a page="${finalUrl}?poc=${parentProductId}"
														title="${prodName}" onclick="${onClickEvent}">
														${prodName}
													</dsp:a>
												</c:otherwise>
										...
										<%-- Rating --%>
										<dsp:getvalueof var="ReviewCountvar"
												...
												<c:when test="${ratingAvailable == true}">
													...
													<c:choose>
														<c:when test="${totalReviewCount == 1}">
															<li
																class="prodReview clearfix prodReview<fmt:formatNumber value="${fltValue * 10}" maxFractionDigits="0"/>"
								...
								<div class="grid_6 omega">
									<div
										class="priceQuantityNotAvailable grid_6 alpha omega <c:if test="${inStock==true}">hidden</c:if> fl width_12">
											...
											<c:if test="${OutOfStockOn}">
												<c:choose>
													<c:when
														...
														<div class="info">
															<a class="info fl marLeft_10 lnkNotifyOOS" href="#"><bbbl:label
									...
									<div
										class="fr grid_6 prodAttributeContainer alpha omega <c:if test="${inStock==false}">hidden</c:if>">
									...
									<div
										class="fr prodInfoContainer alpha omega listViewQtyWrapper">
										<dsp:droplet name="/atg/dynamo/droplet/ForEach">
												...
												<c:choose>
													...
													<c:when test="${prodType eq 'SIZE'}">
														<div class="prodSize fl">
															...
															<div class="psize fl">
																<select id="selProdSize" name="selProdSize"
																	class="customSelectBoxCollection" aria-required="false">
																	<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																		<dsp:param param="element" name="array" />
																		<dsp:oparam name="outputStart">
																			<option value=""><bbbl:label
																					key='lbl_pdp_product_select'
																					language="${pageContext.request.locale.language}" /></option>
																		</dsp:oparam>
																		<dsp:oparam name="output">
																			<dsp:getvalueof var="elementValue"
																				param="element.rollupAttribute" />
																			<option value="${elementValue}"><c:out
																					value="${elementValue}" escapeXml="false" /></option>
																		</dsp:oparam>
																	</dsp:droplet>
																</select>
															</div>
															<input type="hidden" name="prodSize"
																class="_prodSize addItemToRegis addItemToList" value="" />
														</div>
													</c:when>
													<c:otherwise>
														<c:choose>
															<c:when test="${prodType eq 'FINISH'}">
																<div class="prodFinish fl">
																	<label for="selProdFinish"><dsp:valueof
																			param="key" />:</label>
																	<div class="pfinish fl">
																		<select id="selProdFinish" name="selProdFinish"
																			class="customSelectBoxCollection"
																			aria-required="false">
																			<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																				<dsp:param param="element" name="array" />
																				<dsp:oparam name="outputStart">
																				...
																				<dsp:oparam name="output">
																					<dsp:getvalueof var="elementValue"
																						param="element.rollupAttribute" />
																					<dsp:getvalueof var="smallImagePath"
																						param="element.smallImagePath" />
																					<dsp:getvalueof var="thumbnailImagePath"
																						param="element.thumbnailImagePath" />
																					...
																					<c:choose>
																						<c:when test="${color eq elementValue}">
																							<option value="${elementValue}"
																								selected="selected"
																								data-imgSrc="${scene7Path}/${swatchImagePath}"
																								data-imgURLSmall="${scene7Path}/${smallImagePath}"
																								data-imgURLThumb="${scene7Path}/${thumbnailImagePath}">${elementValue}</option>
																						</c:when>
																						<c:otherwise>
																							<option value="${elementValue}"
																								data-imgSrc="${scene7Path}/${swatchImagePath}"
																	...
																	<input type="hidden" name="prodFinish"
																		class="_prodFinish addItemToRegis addItemToList"
																		value="" />
																</div>
															</c:when>
															<c:otherwise>
																<c:if test="${prodType eq 'COLOR'}">
																	<div class="prodColor fl">
																		<label for="selProdColor"><dsp:valueof
																				param="key" />:</label>
																		<div class="pcolor fl">
																			<select id="selProdColor" name="selProdColor"
																				class="customSelectBoxCollection"
																				aria-required="false">
																				<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																					<dsp:param param="element" name="array" />
																					<dsp:oparam name="outputStart">
																						<option value=""><bbbl:label
																								key='lbl_pdp_product_colour_select'
																								language="${pageContext.request.locale.language}" /></option>
																					</dsp:oparam>
																					<dsp:oparam name="output">
																						<dsp:getvalueof var="elementValue"
																		...
																		<input type="hidden" name="prodColor"
																			class="_prodColor addItemToRegis addItemToList"
																			value="" />
																	</div>
																</c:if>
															</c:otherwise>
									...
									<c:choose>
										<c:when test="${appid eq 'BuyBuyBaby'}">
											<div
												class="fr grid_6 prodInfoContainer alpha omega marTop_10">
										</c:when>
										<c:otherwise>
											<div
												class="fr grid_6 prodInfoContainer alpha omega marTop_20">
										</c:otherwise>
									</c:choose>
									<input type="hidden" class="addItemToList" value="${view}"
										name="pageView" />
									<%--<div class="fr btnPD">
											...
											<div class="findInStore noMar fl noPad <c:if test="${appid eq BedBathCanadaSite}">invisible</c:if>">
												...
												<c:choose>
													...
													<div class="button changeStoreItemWrap button_disabled">
													     <input type="button" name="btnFindInStore" value="Find in Store" disabled="disabled"  role="button"  <c:if test="${appid ne BedBathCanadaSite}">class="changeStore"</c:if> />
													</div>
													</c:when>
													<c:otherwise>
													<div class="button changeStoreItemWrap">
													     <input type="button" onclick="pdpOmnitureProxy('${parentProductId}', 'findinstore');"  role="button" name="btnFindInStore" value="Find in Store" <c:if test="${appid ne BedBathCanadaSite}">class="changeStore"</c:if> />
													</div>
													</c:otherwise>
												</c:choose>
											</div>
										...
										<div class="fl addToCart">
											<div class="button button_active button_active_orange  <c:if test="${not inStock}">button_disabled</c:if>">
												<input type="submit" name="btnAddToCart" value="Add to Cart" onclick="rkg_micropixel('${appid}','cart')"  role="button"  <c:if test="${not inStock}">disabled="disabled"</c:if>/>
											</div>
										</div>
								...
								<div class="fr priceQtyFloatFix">
									...
									<span itemprop="offers" itemscope
										itemtype="http://schema.org/Offer"> <input
										type="hidden" name="price" value="${inputFieldPrice}"
										class="addItemToList addItemToRegis" /> <span
										class="prodAvailabilityStatus hidden"> <c:choose>
												<c:when test="${inStock==false}">
													<span itemprop="availability"
														href="http://schema.org/OutOfStock"><bbbl:label
															key='lbl_pdp_out_of_stock'
															language="${pageContext.request.locale.language}" /></span>
												</c:when>
												<c:otherwise>
													<span itemprop="availability"
														href="http://schema.org/InStock"><bbbl:label
															key='lbl_pdp_in_stock'
															language="${pageContext.request.locale.language}" /></span>
												</c:otherwise>
											</c:choose>
									</span>
