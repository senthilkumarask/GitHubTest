<dsp:page>
   ...
   <dsp:importbean bean="/com/bbb/profile/session/SessionBean" />
    ...
    <bbb:pageContainer section="checkout" index="false" follow="false" pageWrapper="billing billingCoupons billingPayment useGoogleAddress">
        ...
        <div id="content" class="container_12 clearfix" role="main">
            <div id="leftCol" class="grid_8">
                ...
                <%-- End: 258 - Verified by visa - refresh back button story - display error message and reset the payment state --%>
				 ...
				 <dsp:getvalueof var="allCoupons" bean="ShoppingCart.current.couponMap"></dsp:getvalueof>
				 <dsp:getvalueof var="selectedPromo" bean="Profile.selectedPromotionsList"></dsp:getvalueof>
				 <c:choose>
				 <c:when test="${not empty allCoupons}">
				 	<c:choose>
				 	<c:when test="${not empty selectedPromo}">
				 		<c:set var="couponText">info_coupon_applied - ${selectedPromo}</c:set>
				 	</c:when>
				 	<c:otherwise>
				 		<c:set var="couponText">info_coupon_found</c:set>
				 	</c:otherwise>
				 	</c:choose>
				 </c:when>
				 <c:otherwise>
				 	<c:set var="couponText">info_no_coupon_found</c:set>
				 </c:otherwise>
				 </c:choose>
                ...
                <dsp:droplet name="BBBPaymentGroupDroplet">
                    ...
                    <dsp:oparam name="output">
                        ...
                        <%-- #Scope 83 A Start R2.2 pay pal button dependeding upon its flag in bcc   --%>
                          ...
                          <c:if test = "${not isOrderAmtCoveredVar}"> 
                             <c:if test="${paypalButtonEnable}"> 
                             	<c:choose>
									 <c:when test="${paypalOn}">
											<c:choose >
													<c:when test="${(orderType eq 'BOPUS_ONLY') or (orderType eq 'HYBRID')}">
														<div id="paymentRadioGroup" class="radioGroup marTop_10">
																<div class="radioItem noBorder clearfix"> 
															<div class="radio"> 
															 <input id="paypalPaymentOpt" aria-hidden='false' type="radio" style="opacity: 0;" name="paymentOpt" value="paypal" disabled="disabled"/> 
															</div> 
														<div class="label"> 															
															<span class="fl"><bbbt:textArea key="txt_paypal_disable_image" language ="${pageContext.request.locale.language}"/></span>
															<div class="fl block padLeft_5 padTop_5">
															<label class="sectionHeading noBorder" for="paypalPaymentOpt"><bbbl:label key="lbl_payment_section_paypal" language="${pageContext.request.locale.language}"/>
															<span class="textPayPal"><bbbl:label key="lbl_paypal_title" language="${pageContext.request.locale.language}"/> </span> 
															</label>
															...
															<span class="amountCharged marTop_10 marLeft_20"><bbbl:label key="lbl_payment_amtCharged_paypal" language="${pageContext.request.locale.language}"/> 
													...
													<div class="radioItem noBorder clearfix"> 
														...
														<c:choose >
															<c:when test="${(orderType eq 'BOPUS_ONLY')}">
															  <div class="label"> 
																	<label class="sectionHeading noBorder"><bbbl:label key="lbl_pay_withCreditCard" language="${pageContext.request.locale.language}" />
																	<c:if test="${currentSiteId eq 'BedBathCanada'}"><sup>${placeHolderMap.creditCardDisclaimer}</sup></c:if></label>
																	<br><p class="normal ropuselement" style="font-size:12px;"><bbbl:label key="lbl_ropus_message" language="${pageContext.request.locale.language}" /></p>
																	<span class="amountChargedropus bold marTop_10 marLeft_20"><span><bbbl:label key="lbl_amount_with_store" language="${pageContext.request.locale.language}" /></span>
																			<dsp:droplet name="CurrencyFormatter">
																				<dsp:param name="currency" value="${storeSubtotal}"/>
																				<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																				<dsp:oparam name="output">
																					<dsp:valueof param="formattedCurrency"><bbbl:label key="lbl_gc_label3" language="${pageContext.request.locale.language}" /></dsp:valueof>
																				</dsp:oparam>
															...
															<c:otherwise>
															<label class="sectionHeading noBorder"><bbbl:label key="lbl_pay_withCreditCard" language="${pageContext.request.locale.language}" /><br>
																<span class="amountChargedropus marTop_10 marLeft_20"><span class="bold"><bbbl:label key="lbl_payment_amtCharged" language="<c:out param='${language}'/>"/></span>
																<dsp:droplet name="CurrencyFormatter">
																	<dsp:param name="currency" value="${priceValue}"/>
																	<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																	<dsp:oparam name="output">
																...
																</dsp:droplet><hr></span></label>
																<c:if test="${not empty storeSubtotal && storeSubtotal > 0}">
																	<div class="label"> 
																		<p class="normal ropuselement" style="font-size:12px;"><bbbl:label key="lbl_ropus_message" language="${pageContext.request.locale.language}" /></p>
																	<c:if test="${currentSiteId eq 'BedBathCanada'}"><sup>${placeHolderMap.creditCardDisclaimer}</sup></c:if>
																	<span class="amountChargedropus marTop_10 marLeft_20 bold"><span><bbbl:label key="lbl_amount_with_store" language="${pageContext.request.locale.language}" /></span>
																			<dsp:droplet name="CurrencyFormatter">
																				<dsp:param name="currency" value="${storeSubtotal}"/>
																				<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																				<dsp:oparam name="output">
													...
													<c:otherwise>													
													   <div id="paymentRadioGroup" class="radioGroup marTop_10">
														...
														<c:when test="${payPalOrder eq true}">
															<div class="radio"> 
																<input id="paypalPaymentOpt" type="radio" aria-hidden='false' style="opacity: 0;" name="paymentOpt" value="paypal" checked="true"/> 
															</div> 
														</c:when>
														<c:otherwise>
															<div class="radio"> 
																<input id="paypalPaymentOpt" type="radio" aria-hidden='false' style="opacity: 0;" name="paymentOpt" value="paypal"/> 
															</div> 
														</c:otherwise>
														</c:choose>
														<div class="label"> 
															<img src="<bbbt:textArea key="txt_paypal_image" language ="${pageContext.request.locale.language}"/>" aria-hidden='true'>
															 <label class="sectionHeading noBorder" for="paypalPaymentOpt"><bbbl:label key="lbl_payment_section_paypal" language="${pageContext.request.locale.language}"/> <span class="textPayPal"><bbbl:label key="lbl_paypal_title" language="${pageContext.request.locale.language}"/> </span></label>
															<span class="amountCharged marTop_10 marLeft_20"><bbbl:label key="lbl_payment_amtCharged_paypal" language="${pageContext.request.locale.language}"/> 
																 <dsp:droplet name="CurrencyFormatter">
																	<dsp:param name="currency" value="${priceValue}"/>
																	<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
														...
														<div class="radioItem noBorder clearfix"> 
															...
															<div class="label"> <label for="creditCartPaymentOpt" class="sectionHeading noBorder"><bbbl:label key="lbl_payment_sectionTitle" language="${pageContext.request.locale.language}"/><c:if test="${currentSiteId eq 'BedBathCanada'}"><sup>${placeHolderMap.creditCardDisclaimer}</sup></c:if>
															</label> 
															<span class="amountCharged marTop_10 marLeft_20"><bbbl:label key="lbl_payment_amtCharged" language="<c:out param='${language}'/>"/> 
																<dsp:droplet name="CurrencyFormatter">
																	<dsp:param name="currency" value="${priceValue}"/>
																	<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																	<dsp:oparam name="output">
									 ...
									 <div id="paymentRadioGroup" class="radioGroup marTop_10">
                                 			<div class="radioItem noBorder clearfix"> 
		                                        ...
		                                        <div class="label"> 
													...
													<div class="error"><bbbl:label key="lbl_paypal_not_available" language="${pageContext.request.locale.language}" /></div></span>
											...
											<div id="payPalInfo" style="z-index: 1002; outline: 0px none; height: auto; width: 440px; top:100px; left: 229px; display: none;" class="ui-dialog ui-widget ui-widget-content ui-corner-all changeStoreDialog" tabindex="-1" role="dialog" aria-labelledby="ui-dialog-title-changeStoreDialogWrapper">
			                                 ...
			                                 <div class="radioItem noBorder clearfix"> 
			                                     <div class="radio">
			                                      ...
			                                      <div class="label"> <label class="sectionHeading noBorder" for="creditCartPaymentOpt"><bbbl:label key="lbl_payment_sectionTitle" language="${pageContext.request.locale.language}"/><c:if test="${currentSiteId eq 'BedBathCanada'}"><sup>${placeHolderMap.creditCardDisclaimer}</sup></c:if></label>
				                                     <span class="amountCharged marTop_10 marLeft_20"><bbbl:label key="lbl_payment_amtCharged" language="<c:out param='${language}'/>"/> 
				                                         <dsp:droplet name="CurrencyFormatter">
				                                             <dsp:param name="currency" value="${priceValue}"/>
				                                             <dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
                        ...
                        <dsp:form name="frmAddCreditCard" iclass="clearfix" id="frmAddCreditCard" formid="frmAddCreditCard" action="${pageContext.request.requestURI}" method="post">   
                             <c:choose>
                             <c:when test="${isOrderAmtCoveredVar and paypalButtonEnable and paypalOn}">
                             	...
                             	<div class="clearfix padTop_20 highlightSection marTop_20">
									<div class="button button_active button_disabled">
									 ...
									 <c:choose>
										 <c:when test="${not empty storeSubtotal && storeSubtotal >0}">
										 <p class="amountChargedropus bold marTop_10"><span><bbbl:label key="lbl_amount_with_store" language="${pageContext.request.locale.language}" /></span>
																			<dsp:droplet name="CurrencyFormatter">
																				<dsp:param name="currency" value="${storeSubtotal}"/>
																				<dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																				<dsp:oparam name="output">
										 	...
										 	<%-- <p class="amountCharged marTop_10 ">Amount Due in store:${storeSubtotal}</p> --%>
										 </c:when>
										 <c:otherwise>
										 	<p class="marTop_10"><bbbl:label key="lbl_gift_card_cover_total_paypal" language="<c:out param='${language}'/>"/></p>
										 </c:otherwise>
									 </c:choose>
                                   	
                                   	<input type="hidden" name="IsOrderAmtCoveredByGC" value="${isOrderAmtCoveredVar}"/>
                                   	<div class="button button_active button_disabled">
                                        <c:set var="lbl_preview_order" scope="page">
                                            <bbbl:label key="lbl_preview_order" language="${pageContext.request.locale.language}" />
                                        </c:set>
                                        <dsp:input type="submit" value="${lbl_preview_order}" disabled="true" iclass="enableOnDOMReady" bean="PaymentGroupFormHandler.payment" id="paymentNextBtn">
                             ...
                             <c:otherwise>
                                ...
                                <div class="payWithPayPal marTop_15 highlightSection padTop_25"> 
                                	
                                		<c:set var="lbl_continue_to_paypal" scope="page">
										   <bbbl:label key="lbl_continue_to_paypal" language="${pageContext.request.locale.language}" />
										</c:set>
                                       	<div class="button button_active">
												<dsp:a href="" iclass="checkoutPaypal">${lbl_continue_to_paypal}
													<dsp:property bean="CartModifierFormHandler.payPalErrorURL" value="/checkout/payment/billing_payment.jsp" priority="10"/>
													<dsp:property bean="CartModifierFormHandler.checkoutWithPaypal" value="true" priority="0"/>
												</dsp:a>
										 </div>
                            ...
                            <div class="grid_8 alpha omega payWithCreditCard">
                                <c:if test="${param.fromCardinalError eq 'true'}">
									<div class="error customError"><bbbe:error key='err_caridinal_payment_error' language='${language}'/></div>
							    </c:if>
                                <c:if test="${not paypalButtonEnable}">    
                                	<h3 class="sectionHeading noBorder"><bbbl:label key="lbl_payment_sectionTitle" language="<c:out param='${language}'/>"/></h3>
                                </c:if> 
                                <%-- For logged in user and any credit card in order or profile --%>
                                   ...
                                   <dsp:droplet name="Switch">
                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <h3 class="sectionHeading noBorder marTop_20"><bbbl:label key="lbl_payment_savedProfile" language="<c:out param='${language}'/>"/></h3>
                                                <div class="highlightSection fl">
                                                    <div class="clearfix">
                                                        <%-- Start of pay with a credit card Header --%>
                                                        <div class="productsListHeader noBorderTop">
                                                            <div class="cardName marLeft_10">Card</div>
                                                            <div class="grid_2 textLeft"><bbbl:label key="lbl_payment_nameOnCard" language="<c:out param='${language}'/>"/></div>
                                                            <div class="grid_1 textLeft omega"><bbbl:label key="lbl_payment_expires" language="<c:out param='${language}'/>"/></div>
                                                            <div class="grid_2 textRight omega"><bbbl:label key="lbl_payment_cvv" language="<c:out param='${language}'/>"/></div>
                                                        </div>
                                                        ...
                                                        <div id="frmPayUsingSavedCard">
                                                            <fieldset class="radioGroup">
                                                                <legend class="hidden"><bbbl:label key="lbl_payment_savedCard" language="<c:out param='${language}'/>"/></legend>
                                                                                                ...
                                                                                                <c:if test="${disabledFlag != true}">
                                                                                                    ...
                                                                                                    <div class="cvvPayUsingSavedCardContainer">
                                                                                                        <div class="grid_2 fr textRight omega input cvvPayUsingSavedCard subForm">
                                                                                                            ...
                                                                                                            <div class="whatsThis omega">
	                                                                                                             <a class="info" id="cvvInfo" href="#">
                                                                                                                    <b><bbbl:label key="lbl_payment_txt" language="<c:out param='${language}'/>"/></b> 
                                                                                                                    <span><bbbl:textArea key="txt_payment_dynamicText" language="<c:out param='${language}'/>"/></span> 
                                                                                                                </a>
                                                                                                            </div>
                                                                                        ...
                                                                                        <div class="input grid_3 omega">
                                                                                            ...
                                                                                            <div class="text grid_1 noMarBot">
                                                                                                <dsp:input bean="PaymentGroupFormHandler.creditCardInfo.cardVerificationNumber" type="text" name="securityCode" id="securityCode" maxlength="4" autocomplete="off" iclass="textRight securityCodeCVV" >
                                                                                                    ...
                                                                                                    <a class="info"> <span><bbbl:textArea key="txt_payment_dynamicText" language="<c:out param='${language}'/>"/></span> 
                                                                                            ...
                                                                                            <div class="whatsThis grid_2 omega"><a class="info" href="#" > <b><bbbl:label key="lbl_payment_txt" language="<c:out param='${language}'/>"/></b> <span><bbbl:textArea key="txt_payment_dynamicText" language="<c:out param='${language}'/>"/></span> </a></div>
                                                                                            <div class="cb">
                                        ...
                                        <div class="grid_4 alpha billingAddress">
                                            <h3 class="noBorder">  <bbbl:label key="lbl_spc_preview_billingaddress" language="${pageContext.request.locale.language}" /></h3>
                                                    ...
                                                    <dsp:oparam name="false">
                                                        <c:choose>
                                                            <c:when test="${internationalCCFlag}">
                                                                <span class="countryName internationalCountry"><dsp:valueof param="order.billingAddress.countryName" /></span>
                                                            </c:when>
                                                            <c:otherwise>
                                                                <span class="countryName hidden"><dsp:valueof param="order.billingAddress.countryName" /></span>
                                                            </c:otherwise>
                                                        </c:choose>
                                    ...
                                    <dsp:oparam name="true">
                                        <p class="noMar"><bbbl:label key="lbl_gift_card_covers_order_total" language="<c:out param='${language}'/>"/></p>
                                    </dsp:oparam>
                                </dsp:droplet>
                                
                                <div class="grid_8 alpha marTop_20 omega">
                                     <dsp:droplet name="Switch">
                                        <dsp:param name="value" value="${isOrderAmtCoveredVar}"/>
                                        <dsp:oparam name="true">
                                            <input type="hidden" name="IsOrderAmtCoveredByGC" value="${isOrderAmtCoveredVar}"/>
