	                    ...
	                    <dsp:oparam name="output">
                    	  ...
                    	  <dt>
							...
							<c:if test="${shippingGroupItemsTotal gt 0.0}">
								<dd class="bold itemsPrice"><dsp:valueof value="${shippingGroupItemsTotal}" converter="defaultCurrency"/></dd>
							</c:if>
							
							<dsp:getvalueof var="rawShippingTotal" param="priceInfoVO.rawShippingTotal"/>
							
							<c:if test="${totalEcoFeeAmount gt 0.0}">
								<dt class="bold"><bbbl:label key="lbl_eco_fees" language="<c:out param='${language}'/>"/></dt>
								<dd class="bold"><dsp:valueof value="${totalEcoFeeAmount}" converter="defaultCurrency"/></dd>
							</c:if>
							<c:if test="${giftWrapTotal gt 0.0}">
								<dt class="bold"><bbbl:label key="lbl_preview_giftpackaging" language="<c:out param='${language}'/>"/></dt>
								<dd class="bold"><dsp:valueof value="${giftWrapTotal}" converter="defaultCurrency"/></dd>
							</c:if>								
							
							<c:choose>
								<c:when test="${rawShippingTotal gt 0.0}">
									<dt><span class="bold"><bbbl:label key="lbl_cartdetail_estimatedshipping" language="<c:out param='${language}'/>"/>: </span>
									<c:choose>
								...
								<c:otherwise>
									<dt class=""><span class="bold"><bbbl:label key="lbl_cartdetail_estimatedshipping" language="<c:out param='${language}'/>"/>: </span>
									<c:choose>
										 <c:when test="${finalShippingCharge eq 0.0}">
										 <a href="${contextPath}/static/shippingexclusionsinclusions" class="popup"><bbbl:label key="lbl_exclusion_inclusion_details" language="<c:out param='${language}'/>"/></a></dt>
										 	<dd class="bold"><bbbl:label key="lbl_shipping_free" language="<c:out param='${language}'/>"/></dd>
							...
							<c:if test="${surchargeSavings gt 0.0}">
								...
								<dd class="highlight bold">(<dsp:valueof value="${surchargeSavings}" converter="defaultCurrency"/>)</dd>
							</c:if>
							...
							<dt class="hidden">LTL items summary</dt>
								...
								<%-- <dl class="LTLDetails clearfix"> --%>
									...
									<dt class=""> <span class="bold"><bbbl:label key="lbl_cart_delivery_surcharge" language="<c:out param='${language}'/>"/>: </span><a href="${contextPath}/static/shippingexclusionsinclusions" class="popup"><bbbl:label key="lbl_exclusion_inclusion_details" language="<c:out param='${language}'/>"/></a></dt>
									...
									<c:if  test ="${deliverySurchargeSaving gt 0.0}">
										...
										<dd class="parentheses highlightRed bold">(<dsp:valueof value="${deliverySurchargeSaving}" number="0.00" converter="defaultCurrency"/>)</dd>
									</c:if>
									<c:if  test ="${assemblyFee gt 0.0}">
										<dt class="bold"><bbbl:label key="lbl_cart_assembly_fee" language="<c:out param='${language}'/>"/>:</dt>
										<dd class="bold"><dsp:valueof value="${assemblyFee}" number="0.00" converter="defaultCurrency"/></dd>
									</c:if>
								<%-- </dl> --%>
							...
							<%-- Additional info for LTL items summary --%>
							 ...
							 <c:choose>
								...
								<dd class="bold"><dsp:valueof value="${shippingLevelTax}" converter="defaultCurrency"/></dd>	
								</c:when>
								<c:otherwise>
								<c:choose>
							...
							<c:if test="${totalAmount gt 0.0}">
								...
								<span class="bold"><dsp:valueof value="${totalAmount}" converter="defaultCurrency"/></span>
								</div></dt>
							</c:if>
							<c:if test="${(totalSavedAmount + deliverySurchargeSaving) gt 0}">
								...
								<span class="highlight bold"><dsp:valueof value="${totalSavedAmount+deliverySurchargeSaving}" converter="defaultCurrency"/></span></div></dt>
							</c:if>
						</dsp:oparam>
					</dsp:droplet>	
				</dl>
