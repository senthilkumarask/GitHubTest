				...
				<dsp:droplet name="/com/bbb/commerce/pricing/droplet/BBBPriceDisplayDroplet">
					 ...
					 <li class="grid_1 textCenter">
							<dsp:droplet name="/atg/dynamo/droplet/NotZero">
							  ...
							  <dsp:oparam name="empty"><strong tabindex="0" aria-label="Our Price:<dsp:valueof param="commerceItemRelationship.commerceItem.priceInfo.listPrice" converter="defaultCurrency"/>"><dsp:valueof param="commerceItemRelationship.commerceItem.priceInfo.listPrice" converter="defaultCurrency"/></strong></dsp:oparam>
							  <dsp:oparam name="output">
							  <strong class="highlight" tabindex="0" aria-label="Our Price:<dsp:valueof param="commerceItemRelationship.commerceItem.priceInfo.salePrice" converter="defaultCurrency"/>">
									<dsp:valueof param="commerceItemRelationship.commerceItem.priceInfo.salePrice" converter="defaultCurrency"/>
								</strong>
							  </dsp:oparam>
							</dsp:droplet>
							<c:if test="${unitSavedAmount gt 0.0}">
                        <span class="smallText marTop_5" style="display: block;">
                                (<bbbl:label key="lbl_preview_reg" language="<c:out param='${language}'/>"/> <dsp:valueof value="${unitListPrice}" converter="defaultCurrency"/>)
                         </span>
                     </c:if>
					</li>
					 ...
					 <c:choose>
                                     ...
                                     <ul class="productPriceContainer">
        								...
        								<c:choose>
        				                    <c:when test="${totalSavedAmount gt 0.0}">
											 <li class="highlight">
												 <strong tabindex="0" aria-label="Your Price:<dsp:valueof value="${undiscountedItemsCount}" /> <bbbl:label key="lbl_cart_multiplier" language="${language}"/>
        				                         <dsp:valueof value="${unitSalePrice}" converter="defaultCurrency"/>"><dsp:valueof value="${undiscountedItemsCount}" /> <bbbl:label key="lbl_cart_multiplier" language="${language}"/>
        				                         <dsp:valueof value="${unitSalePrice}" converter="defaultCurrency"/>
											  </strong></li>
        				                     </c:when>
        				                     <c:otherwise>
												 <li><strong tabindex="0" aria-label="Your Price:<dsp:valueof value="${undiscountedItemsCount}" /> <bbbl:label key="lbl_cart_multiplier" language="${language}"/>
        				                         <dsp:valueof value="${unitListPrice}" converter="defaultCurrency"/>"><dsp:valueof value="${undiscountedItemsCount}" /> <bbbl:label key="lbl_cart_multiplier" language="${language}"/>
        				                         <dsp:valueof value="${unitListPrice}" converter="defaultCurrency"/>
        				                     </strong></li></c:otherwise>
        								</c:choose> 
        							
                                  
                                    <c:if test="${totalSavedAmount gt 0.0}">
                                        <li class="smallText"><bbbl:label key="lbl_cartdetail_yousave" language="<c:out param='${language}'/>"/> 
                                            <dsp:valueof value="${totalSavedAmount}" converter="defaultCurrency"/>
                                        </li>
                                    </c:if>
                                 </ul>							
							</c:when>
                        ...
                        <dsp:getvalueof var="couponApplied" value=""/>
    						...
    						<dsp:droplet
    							...
    							<dsp:param name="shippingMethod" param="shippingGroup.shippingMethod" />
    							...
    							<dsp:getvalueof var="shippingMethodDescription" param="shippingMethodDescription"/>
									<dsp:getvalueof var="ecoFeeAmountTemp" param="ecoFeeAmount" />
									 <c:if test="${ecoFeeAmountTemp gt '0.0'}">
										  ...
										  <li class="smallText youPay"> <bbbl:label key="lbl_eco_fee" language="${language}"/>: 
											<dsp:valueof param="ecoFeeAmount" converter="defaultCurrency"/></li>
					...
					<li class="grid_2 alpha omega textRight">
						<ul class="productPriceContainer">
		                    ...
		                    <c:when test="${priceInfoVO.undiscountedItemsCount ne commerceItemRelationship.quantity}">
		                         <li class="highlight">
									<strong tabindex="0" aria-label="Total:<dsp:valueof param="priceInfoVO.shippingGroupItemTotal" converter="defaultCurrency"/>"><dsp:valueof param="priceInfoVO.shippingGroupItemTotal" converter="defaultCurrency"/></strong>					
								</li>
		                     </c:when>
		                     <c:otherwise>
		                         <li><strong tabindex="0" aria-label="Total:<dsp:valueof param="priceInfoVO.shippingGroupItemTotal" converter="defaultCurrency"/>"><dsp:valueof param="priceInfoVO.shippingGroupItemTotal" converter="defaultCurrency"/></strong></li>
		                     </c:otherwise>
							</c:choose> 
						...
						<c:if test="${isSkuLtl}">
							<dl class="LTLDetails clearfix">
								<dt>+ ${shippingMethodDescription}</dt>
								<dd>
								  <c:if test="${deliverySurcharge eq 0.0}"> FREE</c:if>
				                   <c:if test="${deliverySurcharge gt 0.0}"><dsp:valueof value="${deliverySurcharge}" number="0.00" converter="defaultCurrency"/></c:if>
				                 </dd>
								<c:if  test ="${deliverySurchargeSaving gt 0.0}">
									<dt class="highlightRed">- <bbbl:label key="lbl_cart_delivery_surcharge_saving" language="<c:out param='${language}'/>"/></dt>
									<dd class="highlightRed">(<dsp:valueof value="${deliverySurchargeSaving}" number="0.00" converter="defaultCurrency"/>)</dd>
								</c:if>
								<c:if  test ="${assemblyFee gt 0.0}">
									<dt>+ <bbbl:label key="lbl_cart_assembly_fee" language="<c:out param='${language}'/>"/>:</dt>
									<dd><dsp:valueof value="${assemblyFee}" number="0.00" converter="defaultCurrency"/></dd>
								</c:if>
							</dl>
						</c:if>
						<%-- Additional info for LTL items --%>
