<dsp:page>
    ...
    <script type="text/javascript">
		...
		<ul class="productInfoContainer">
			...
			<div class="prodAttribsCart">
				<c:if test='${not empty color}'>
					 <bbbl:label key="lbl_item_color" language ="${pageContext.request.locale.language}"/> : <dsp:valueof value="${color}" valueishtml="true" />
					</c:if>
				<c:if test='${not empty size}'>
					 <c:if test='${not empty color}'><br/></c:if><bbbl:label key="lbl_item_size" language ="${pageContext.request.locale.language}"/> : <dsp:valueof value="${size}" valueishtml="true" />
				</c:if>
				<c:if test='${not empty personalizationOptions}'>
					<br>${eximCustomizationCodesMap[personalizationOptions]} :  <c:out value="${personalizationDetails}" escapeXml="false" />
				</c:if>
			</div>
				<dsp:getvalueof var="isFromPreviewPage" param="isFromPreviewPage"/>
				<c:if test="${isFromPreviewPage and giftWrapEligible}">
					<br/><bbbl:label key="lbl_cartdetail_elegibleforgiftpackaging" language="<c:out param='${language}'/>"/>&#32;<bbbt:textArea key="txt_giftpackaging_popup_link" language="<c:out param='${language}'/>"/>
				</c:if>
		...
		<c:choose>
			...
			<c:otherwise>
			<dl class="expectedDelivery cb padTop_5">
				...
				<dsp:oparam name="equal">
					<dsp:droplet
						...
						<dsp:oparam name="expectedDeliveryDateOutput">
							<dt class="bold">
								<bbbl:label key="lbl_preview_expecteddelivery"
									language="<c:out param='${language}'/>" />
							</dt>
							<dd class="padBottom_5">
