...
<bbb:pageContainer index="false" follow="false">
    ...
    <jsp:body>
        ...
        <c:set var="language" value="${pageContext.request.locale.language}" scope="page"/>
  		<c:set var="IsFromRegistry" value="false"/>
  
        <div id="content" class="container_12 clearfix" role="main">
            <h3 class="sectionHeading noBorder marLeft_10"><bbbl:label key="lbl_multi_gift_select" language="<c:out param='${language}'/>"/></h3>
                    ...
                    <dsp:form id="formShippingSingleLocation"  name="form" formid="formShippingSingleLocation" action="${pageContext.request.requestURI}" method="post">
                            ...
                            <dsp:droplet name="GiftWrapCheckDroplet">
                                ...
                                <dsp:oparam name="output">
												...
												<c:choose>
													<c:when test="${not empty tempShippingGroup.registryId}">
															<span><strong>${tempShippingGroup.registryInfo}</strong></span>	
																<c:set var="IsFromRegistry" value="true"/>		
													</c:when>
													<c:otherwise>
                                                   ...
                                                   <c:set var="IsFromRegistry" value="${IsFromRegistry}"/>     
                                            </label>
                                    ...
                                    <div class="viewOrderContentContainer">
                                        <div class="marBottom_5"><a class="viewOrderContents iconExpand" role="button" href="#" title="Click here to expand and view cart contents" aria-expanded="false" aria-controls="orderContentDetails" aria-live="assertive"><bbbl:label key="lbl_multi_gift_view_order" language="<c:out param='${language}'/>"/></a></div>
                                        <div>
                                            <div class="orderContents clearfix" id="orderContentDetails">
                                                <div class="carousel clearfix">
                                                    <div class="carouselBody grid_8 marTop_20">
                                                        <div class="grid_1 carouselArrow omega carouselArrowPrevious clearfix"> &nbsp; <a class="carouselScrollPrevious" title="Previous" href="#"><bbbl:label key="lbl_multi_gift_previous" language="<c:out param='${language}'/>"/></a> </div>
                                                        ...
                                                        <div class="grid_1 carouselArrow alpha carouselArrowNext clearfix"> &nbsp; <a class="carouselScrollNext" title="Next" href="#"><bbbl:label key="lbl_multi_gift_next" language="<c:out param='${language}'/>"/></a> </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <fieldset class="noMarTop">                                        
										<dsp:include page="frag/giftWrapMessage.jsp">
                                            ...
                                            <dsp:param name="IsFromRegistry" value="${IsFromRegistry}"/>    
                                        </dsp:include>
                                    </fieldset>
                        ...
                        <div class="button button_active">
                            <c:set var="lbl_continue_to_billing" scope="page">
                                <bbbl:label key="lbl_continue_to_billing" language="${pageContext.request.locale.language}" />
                            </c:set>
                            <dsp:input type="submit" value="${lbl_continue_to_billing}" id="submitShippingSingleLocationBtn" bean="BBBShippingGroupFormhandler.giftMessaging" >
	...
	<jsp:attribute name="footerContent">
			...
			<script type="text/javascript">
			           if(typeof s !=='undefined') {
			        	s.pageName='Check Out>Shipping Gift Options';
			        	s.channel='Check Out';
			        	s.prop1='Check Out';
			        	s.prop2='Check Out';
			        	s.prop3='Check Out';
