...
<dsp:input bean="BBBShippingGroupFormhandler.BBBShippingInfoBeanList[${count}].shippingGroupId" type="hidden" value="${shipGroupId}"/>
                    ...
                    <div class="checkboxItem input clearfix">
                        <div class="checkbox">
                        <c:choose>
						<c:when test="${IsFromRegistry && giftWrapFlag || giftWrapFlag && not empty shipGroupGiftMessage || shipGroupParam.containsGiftWrap}">
							<c:set var="showchecked" value="true"/>
						</c:when>
						<c:otherwise>
							<c:set var="showchecked" value="false"/>
						</c:otherwise>
					</c:choose>
          	<dsp:input type="checkbox" name="shippingOption2_${count}" id="shippingOption3_${count}" bean="BBBShippingGroupFormhandler.BBBShippingInfoBeanList[${count}].giftingFlag" checked="${showchecked}">
                ...
                <dsp:tagAttribute name="aria-labelledby" value="lblshippingOption3_${count}"/>
                        ...
                        <div class="label">
                            ...
                            <label id="lblshippingOption3_${count}" for="shippingOption3_${count}">
                            ...
                            <strong><bbbl:label key="lbl_multi_gift_include_gifts" language="<c:out param='${language}'/>" /> </strong> <bbbl:label key="lbl_multi_gift_packing_slip_info" language="<c:out param='${language}'/>" placeHolderMap="${GiftPriceCounter}"/></label>
                        </div>
                        <div class="clear"></div>
                            ...
                            <div class="clearfix ">
                                <div class="fl">
                                    <div class="giftPackaging clearfix">
                                        <div class="label">
                                            <label for="shippingGiftMessage_${count}"> <bbbl:label key="lbl_multi_gift_add_message" language="<c:out param='${language}'/>"/> </label>
                                        </div>
                                        <div class="text">
                                            <div class="width_5">
										...
										<dsp:droplet name="GiftWrapGreetingsDroplet">
											...
											<dsp:oparam name="output">
											 <div class="fl marLeft_20 marTop_20 addCommonGreetings"> <a href="#" class="bold shippingAddCommonGreetMsgs" id="shippingAddCommonGreetMsgs_${count}"> <bbbl:label key="lbl_multi_gift_symbol_plus" language="<c:out param='${language}'/>"/> <bbbl:label key="lbl_multi_gift_common_greeting" language="<c:out param='${language}'/>"/></a>
										...
										<dsp:oparam name="true">
											 <div class="checkboxItem input clearfix includeGiftPackagingContainer noPadBot">
												   <div class="checkbox">
													 ...
													 <div class="label">
														<label id="lblincludeFigtPackaging_${count}" for="includeFigtPackaging_${count}"><bbbl:label key="lbl_multi_gift_include_packaging" language="<c:out param='${language}'/>"/> <bbbl:label key="lbl_multigift_include_wrap_start" language="<c:out param='${language}'/>"/><span class="bold">
														<dsp:droplet name="CurrencyFormatter">
																			   <dsp:param name="currency" param="giftWrapPrice"/>
																			   <dsp:param name="locale" bean="/OriginatingRequest.requestLocale.locale"/>
																			   <dsp:oparam name="output">
														...
														</dsp:droplet></span><bbbl:label key="lbl_multigift_include_wrap_end" language="<c:out param='${language}'/>"/>
													 ...
													 <div class="giftPackagingMessage"><bbbl:label key="lbl_gift_msg_some_items" language="<c:out param='${language}'/>"/> <c:out value="${nonGiftWrapSkus}" /> <bbbl:label key="lbl_gift_msg_not_eligible" language="<c:out param='${language}'/>"/></div>
													 </c:if>

											 </div>
										</dsp:oparam>
										<dsp:oparam name="false"><div class="giftPackagingMessage"><bbbl:label key="lbl_gift_no_item_available" language="<c:out param='${language}'/>"/></div>
										</dsp:oparam>
