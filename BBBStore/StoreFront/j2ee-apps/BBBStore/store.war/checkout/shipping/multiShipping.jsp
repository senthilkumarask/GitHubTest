...
<dsp:page>
 ...
 <c:set var="isUpdateOrderSummaryAjax">
	 <bbbc:config key="updateOrderSummaryAjax" configName="FlagDrivenFunctions" />
 </c:set>
 ...
 <bbb:pageContainer index="false" follow="false" >
    ...
    <jsp:attribute name="pageWrapper">billing shippingWrapper useMapQuest multiShippingPage useStoreLocator useGoogleAddress usePorch</jsp:attribute>
	 ...
	 <c:set var="errMessageShown" value="false" scope="page"/>
	  ...
	  <dsp:droplet name="BBBPackNHoldDroplet">
		...
		<dsp:droplet name="BBBBeddingKitsAddrDroplet">
						...
						<c:if test="${hasAllPackNHoldItems && beddingShipAddrVO.shippingEndDate gt currentDate}">
							<div class="shopNowContainer">
								<h3 class="shopNowMsg"><bbbl:label key="lbl_pack_hold_eligibility_ship_page" language="${pageContext.request.locale.language}" /></h3>
								<span><bbbt:textArea key="txt_pack_hold_eligibilitymessage_ship_page" language="${pageContext.request.locale.language}" /></span>
							</div>
						</c:if>
         ...
         <div id="leftCol" class="grid_8 highlightSection">
         	...
         	<c:if test="${fn:length(commerceItems) != 0}" >
				...
				<input type="hidden" name="cisiShipGroupName" value="" />
					...
					<c:choose>
						<c:when test="${sessionScope.giftsAreIncludedInOrder eq 'true'}">
							...
							<div class="label">
								...
								<jsp:useBean id="GiftPriceCounter" class="java.util.HashMap" scope="request"/>
									...
									<dsp:droplet name="GiftWrapCheckDroplet">
										...
										<dsp:param name="onlyGiftWrapProductDetails" value="true" />
										<dsp:oparam name="output">
					...
					<c:choose>
						...
						<c:choose>
							<c:when test="${disableButton eq 'yes'}">
								<dsp:input type="submit" disabled="true" id="submitShippingMultipleLocationBtn" value="${lbl_continue_to_billing}" bean="BBBShippingGroupFormhandler.multipleShipping" >
							...
							<c:otherwise>
								<dsp:input type="submit" id="submitShippingMultipleLocationBtn" value="${lbl_continue_to_billing}" bean="BBBShippingGroupFormhandler.multipleShipping" >
                                    <dsp:tagAttribute name="aria-pressed" value="false"/>
                                    <dsp:tagAttribute name="aria-labelledby" value="submitShippingMultipleLocationBtn"/>
                                    <dsp:tagAttribute name="role" value="button"/>
                                </dsp:input>
						...
						<c:if test="${isUpdateOrderSummaryAjax}">
							<dsp:input type="hidden" id="updateOrderSummaryAjax" bean="BBBShippingGroupFormhandler.updateOrderSummaryAjax" value="false"/>
						</c:if>
						<dsp:input type="hidden" id="ajaxAddressFilled" bean="BBBShippingGroupFormhandler.ajaxAddressFilled" value="false"/>
            ...
            <dsp:include page="/checkout/order_summary_frag.jsp">
				...
				<dsp:param name="displayShippingDisclaimer" value="${!isUpdateOrderSummaryAjax}"/>
			</dsp:include>
      </div>
				...
				<div id="addNewAddressDialogWrapper" class="addNewAddressDialogWrapper">
					...
					<dsp:form name="addNewAddressFrm" id="addNewAddressFrm" action="#" method="post">
						...
						<div class="container_6 clearfix">
								...
								<div class="input">
									<div class="label">
										<label id="lblcheckoutfirstName" for="checkoutfirstName">
											<bbbl:label key="lbl_shipping_new_first_name" language="<c:out param='${language}'/>"/>
								...
								<div class="input">
									<div class="label">
										<label id="lblcheckoutlastName" for="checkoutlastName">
											<bbbl:label key="lbl_shipping_new_last_name" language="<c:out param='${language}'/>"/>
											<span class="required">*</span>
										</label>
									</div>
									<div class="text">
								...
								<div class="input">
									<div class="label">
										<label id="lblcompany" for="company">
											<bbbl:label key="lbl_shipping_new_company" language="<c:out param='${language}'/>"/>
										</label>
									</div>
									<div class="text">
										<div>
											<dsp:input type="text" name="company" value="" id="company" bean="BBBShippingGroupFormhandler.address.companyName" >
								...
								<div class="input ltlField">
									<div class="label">
										<label id="lblaltphone" for="altphone">
											<bbbl:label key="lbl_shipping_alternate_phone" language="<c:out param='${language}'/>"/>
										</label>
									</div>
									<div class="text">
										<div>
	                            ...
	                            <dsp:droplet name="/atg/dynamo/droplet/Switch">
	                                ...
	                                <dsp:oparam name="false">
											...
											<div class="checkboxItem input clearfix">
												...
												<div class="label">
													<label id="lblsaveToAccount" for="saveToAccount"> <bbbl:label key="lbl_shipping_new_save_account" language="<c:out param='${language}'/>"/></label>
												</div>
											</div>
									</dsp:oparam>
								</dsp:droplet>
								...
								<div class="input">
									<div class="label">
										<label id="lbladdress1" for="address1">
											<bbbl:label key="lbl_shipping_address1" language="<c:out param='${language}'/>"/>
											<span class="required">*</span>
										</label>
									</div>
									<div class="text">
								...
								<div class="input">
									<div class="label">
										<label id="lbladdress2" for="address2">
											<bbbl:label key="lbl_shipping_address2" language="<c:out param='${language}'/>"/>
										</label>
									</div>
									<div class="text">
										<div>
											<dsp:input type="text" name="address2" value="" iclass="${iclassValue }" id="address2" bean="BBBShippingGroupFormhandler.address.address2" >
                                                <dsp:tagAttribute name="aria-required" value="false"/>
								...
								<div class="input">
									<div class="label">
										<label id="lblcityName" for="cityName">
											<bbbl:label key="lbl_shipping_new_city" language="<c:out param='${language}'/>"/>
											<span class="required">*</span>
										</label>
									</div>
									<div class="text">
									...
									<div class="label">
										...
										<c:choose>
											 <c:when test="${currentSiteId eq 'BedBathCanada'}">
											  ...
											  <c:otherwise>
												  <bbbl:label key="lbl_shipping_new_state" language="<c:out param='${language}'/>"/>
											  </c:otherwise>
										  </c:choose>
										...
										<label id="lblzip" for="zip">
											 <c:choose>
												 <c:when test="${currentSiteId eq 'BedBathCanada'}">
												  ...
												  <c:otherwise>
													  <bbbl:label key="lbl_shipping_new_zip" language="<c:out param='${language}'/>"/>
												  </c:otherwise>
										   </c:choose>
											<span class="required">*</span>
										</label>
							...
							<dsp:input type="hidden" name="poBoxStatus" bean="BBBShippingGroupFormhandler.poBoxStatus"  value=""/>
	                        ...
	                        <div class="ui-dialog-buttonset">
	                            <div class="button button_active">
	                                <c:set var="lbl_button_create" scope="page">
		                               <bbbl:label
											key="lbl_shipping_button_create"
											language="${pageContext.request.locale.language}" />
	                                </c:set>
...
<c:if test="${isUpdateOrderSummaryAjax}">
	<dsp:form id="formUpdateOrderSummary" action="${contextPath}/checkout/shipping/shipping.jsp?shippingGr=multi" style="display:none">
		<dsp:include page="frag/multi_shipping_line_item.jsp">
			<dsp:param name="ajaxParam" value="true"/>
		</dsp:include>
		<dsp:input bean="BBBShippingGroupFormhandler.fromPage" type="hidden" value="multiShipping" />
		<dsp:input type="submit" iclass="hidden" id="saveMultiShippingChanges" value="true" bean="BBBShippingGroupFormhandler.multipleShipping" />
	</dsp:form>
</c:if>
