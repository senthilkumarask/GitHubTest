<dsp:page>
	...
	<bbb:pageContainer index="false" follow="false" >
		...
		<jsp:attribute name="pageWrapper">billing chekoutReview chekoutConfirm useCertonaJs usePorch</jsp:attribute>
	    ...
	    <%-- BBBSL-4343  <c:if test="${DoubleClickOn}">
	    ...
	    <%-- oracle responsys code ends --%>
	   		...
	   		<div class="container_12 clearfix" id="content">
				<div class="topContent clearfix">
					<div id="subHeader" class="grid_10 clearfix">
						...
						<dsp:droplet name="/atg/dynamo/droplet/Switch">
							...
							<dsp:oparam name="true">
								<div id="accountCallout" class="grid_4 alpha">
							<div class="grid_12 expressProfile marTop_20 alpha omega">
       							<dsp:form name="frmCheckoutExpressProfile" id="frmCheckoutExpressProfile" action="${contextPath}/account/frags/checkout_registration_json.jsp" method="post">
                                    ...
                                    <h3 class="sectionHeading"><bbbl:label key="lbl_checkoutconfirmation_createaccount" language="${language}"/></h3>
									<p class="noMar padBot padRight_30"><bbbl:label key="lbl_checkoutconfirmation_wecansavetheinformationyouenteredandcreateanaccount" language="${language}"/></p>
									<p class="noMar padBot padRight_30"><bbbl:label key="lbl_checkoutconfirmation_advantagesofcreatinganaccount" language="${language}"/></p>
									...
									<div class="formRow marTop_10">
                                        <div class="createAccountForm hidden">
                                            <div class="clearfix">
                                                <div class="input">
                                                    <div class="label width_3 posRel">
														<label id="lblpassword" for="password">All fields are required
														</label>
                                                ...
                                                <div class="formRow clearfix noMarBot">
                                                    <div class="optIn input">
                                                      ...
                                                      <label for="emailOptIn" class="textDgray11 wrapTextAfterCheckBox"><bbbl:label key="lbl_email_optin_bedbath_canada" language="${pageContext.request.locale.language}"/></label>
                                                    ...
                                                    <%-- Baby Canada --%>
	                                                    <c:if test="${currentSiteId == BedBathCanadaSite}">
		                                                    <div class="optIn input">
		                                                      ...
		                                                      <label for="emailOptIn_BabyCA" class="textDgray11 wrapTextAfterCheckBox"><bbbl:label key="lbl_email_optin_baby_canada" language="${pageContext.request.locale.language}"/></label>
		                                                    </div>
		                                                </c:if>
                                                    <%-- Baby Canada --%>
                                                </div>
                                              ...
                                              <c:set var="checkSharedAccountByDefault" scope="request">
                                                ...
                                                <c:if test="${currentSiteId ne BedBathCanadaSite}">
                                                    <div class="formRow shareAccount clearfix noMarBot">
                                                        <dsp:input bean="ProfileFormHandler.sharedCheckBoxEnabled" type="checkbox" checked="${checkSharedAccountByDefault}" iclass="fl" id="shareAccount" />
                                                        <label for="shareAccount" class="textDgray11 wrapTextAfterCheckBox"><bbbl:label key="lbl_profile_shareaccount" language="${pageContext.request.locale.language}"/></label>
                                            ...
                                            <c:if test="${internationalCCFlag ne 'true' && isCreditCard}">
                                            	<div class="formRow shareAccount clearfix">
									            	...
									            	<label class="textDgray11 wrapTextAfterCheckBox" for="saveCreditCard"><bbbl:label key="lbl_checkout_confirmation_donot_save_cc" language="${pageContext.request.locale.language}"/></label>
									            </div>
                                            </c:if>
                                            <c:if test="${isTransient}">
                                                <c:set var="profile_private_policy_title"><bbbl:label key="lbl_private_policy_account" language="${pageContext.request.locale.language}"/></c:set>
                                                 <div class="clearfix formRow clearfix viewPrivacyPolicy noMarTop">
                                                    <a href="/store/static/PrivacyPolicy?showAsPopup=true" class="privacyPolicyModal" title="${profile_private_policy_title}"><bbbl:label key="lbl_private_policy_account" language="${pageContext.request.locale.language}"/></a>
                                                	 <c:if test="${currentSiteId eq BedBathCanadaSite}">
									             	 	<bbbt:textArea key="txt_ca_address" language="${pageContext.request.locale.language}"/>
									             	</c:if> 
                                                 </div>
                                            </c:if>
                                        </div>
										...
										<c:choose>
											<c:when test="${currentSiteId == BuyBuyBabySite}">
												<div class="button-Med btnPrimary">
											</c:when>
											<c:otherwise>
                                               ...
                                               <c:set var="submitKey">
                                                ...
                                                <dsp:input bean="ProfileFormHandler.registration" id="createAccountBtn" type="Submit" value="${submitKey}" disabled="true" iclass="enableOnDOMReady" />
                                            </div>
								...
								<div id="successMessaging" class="grid_5 omega">
						<h2 class="section fl"><bbbl:label key="lbl_checkoutconfirmation_confirmation" language="${language}"/></h2>
                        <div class="clear"></div>
                        <%--Start : Porch Integration --%>						
						<c:if test="${hasPorchService}">
							<p><bbbl:label key="lbl_bbb_porch_order_confirmation" language="${language}"/></p>
							</c:if>
                        <div class="marTop_10 clearfix">
                            <p class="noMar bold"><bbbl:label key="lbl_checkoutconfirmation_orderconfirmationsuccessmsg" language="${language}"/></p>
                            <p class="noMar marTop_5 "><bbbl:label key="lbl_checkoutconfirmation_confirmationemailsentto" language="${language}"/></p>
						<p class="noMar marTop_5 "><dsp:valueof bean="ShoppingCart.last.billingAddress.email"/></p>
					</div>

						<p class="marBottom_5 lblOrderId marTop_10"><bbbl:label key="lbl_checkoutconfirmation_order_date" language="${language}"/></p>
						<span>
							<c:choose>
						 ...
						 <c:if test="${not empty onlineOrderNumber}">
						 	<p class="marBottom_5 lblOrderId"><bbbl:label key="lbl_checkoutconfirmation_delivery_order" language="${language}"/></p>
						 	<c:choose>
					            <c:when test="${CommonConfiguration.displayOrderXML eq false}">
					              	<span class="">${onlineOrderNumber}</span>
					            </c:when>
					            <c:otherwise>
						 ...
						 <c:if test="${not empty bopusOrderNumber}">
						 	<p class="marBottom_5"><bbbl:label key="lbl_checkoutconfirmation_instorepickup_order" language="${language}"/></p>
						 	<c:choose>
					            <c:when test="${CommonConfiguration.displayOrderXML eq false}">
					...
					<div class="share grid_2 omega clearfix">
						<div id="socialShare" class="fr">
							...
							<h2 class="section fl"><bbbl:label key="lbl_checkoutconfirmation_confirmation_loggedin" language="${language}"/></h2>
                        			<div class="clear"></div>
                        				<%--Start : Porch Integration --%>		
											<c:if test="${hasPorchService}">
							<p><bbbl:label key="lbl_bbb_porch_order_confirmation" language="${language}"/></p>
                        				...
                        				<c:choose>
                        				<c:when test="${currentSiteId == BuyBuyBabySite}">
                        				<p class="noMar marTop_5 ">
                        				<dsp:valueof bean="ShoppingCart.last.billingAddress.email"/></p>
                        				</c:when>
                        				<c:otherwise>
                        				<p class="noMar marTop_5 "><bbbl:label key="lbl_checkoutconfirmation_confirmationemailsentto" language="${language}"/>                          				
											<dsp:valueof bean="ShoppingCart.last.billingAddress.email"/></p>
                        				</c:otherwise>
                        				</c:choose>
							...
							<p class="a marBottom_5 lblOrderId marTop_10 upperCase"><bbbl:label key="lbl_checkoutconfirmation_order_date" language="${language}"/></p>
						<span class="bold">
							<c:choose>
								<c:when test="${currentSiteId == BedBathCanadaSite}">
									<dsp:valueof value="${orderDate}" date="dd/MM/yyyy"/>
						 ...
						 <c:if test="${not empty onlineOrderNumber}">
						 	<p class="marBottom_5"><bbbl:label key="lbl_checkoutconfirmation_delivery_order" language="${language}"/></p>
						 	<c:choose>
					            <c:when test="${CommonConfiguration.displayOrderXML eq false}">
					...
					<div class="share grid_2 omega clearfix">
						<div id="socialShare" class="fr">
							<a class="facebook shareIcon" href="#facebook" data-socialshare-role="icon" data-socialshare-network="facebook" title="Share this order on Facebook">Share this order on Facebook</a>
							<a class="twitter shareIcon" href="#twitter" data-socialshare-role="icon" data-socialshare-network="twitter" title="Share this order on Twitter">Share this order on Twitter</a>
							<a class="pinterest shareIcon" href="#pinterest" data-socialshare-role="icon" data-socialshare-network="pinterest" title="Share this order on Pinterest">Share this order on Pinterest</a>
							<a class="print" href="#" title="Print confirmation"><bbbl:label key="lbl_checkoutconfirmation_printconfirmation" language="${language}"/></a>
						</div>
					</div>
				</dsp:oparam>
				...
				<dsp:getvalueof var="order" bean="ShoppingCart.last" />
					<dsp:getvalueof var="shippingGroup" value="${order.shippingGroups[0]}"/>
					<dsp:getvalueof var="shippingMethod" value="${order.shippingGroups[0].shippingMethod}" />
					<c:choose>
					<c:when test="${shippingMethod eq 'SDD'}">
						...
						<dsp:oparam name="output">
							...
							<dsp:getvalueof var="carrierTrackUrl" value="${carrierURL[trackKey]}"/>
								...
								<div class="ssdt-contaner">
								<div class="box-img"></div>
								<div class="ssdt-text">
						    	<bbbt:textArea key="txt_deliv_tracking" language="${pageContext.request.locale.language}"/>
							    <div class="num-contaner">
								    <span class="track-text"><bbbl:label key="lbl_deliv_tracking_no" language="${language}"/></span>
								    <c:choose>
										<c:when test="${carrierTrackUrl  ne null && not empty carrierTrackUrl}">
											<a href="${trackURL}" class="track-num" target="_blank"><img class="track-img" alt="${carrierName}" src="${imagePath}${carrierImageURL}"/>${trackingNumber}</a>
										</c:when>
										<c:otherwise>
											${trackingNumber}
										</c:otherwise>
								...
								<c:set target="${placeHolderSignature}" property="signatureThreshold">${sddSignatureThreshold}</c:set>
							   	<bbbt:textArea key="txt_sdd_signature_text" placeHolderMap="${placeHolderSignature}" language="${pageContext.request.locale.language}"/>
								</div>
								</div>
						</dsp:oparam>
						</dsp:droplet>
				...
				<!-- BBBH-3030 SDD Order Preview and Confirmation Page Changes end-->
                    ...
                    <div class="clearfix">
                         <a class="backToHomepage fl" href="/"><bbbl:label key="lbl_checkoutconfirmation_backtohomepage" language="${language}"/></a>
                     </div>
			</div>
