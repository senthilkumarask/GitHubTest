...
<div
    ...
    <c:set var="bedBathCanadaSiteCode"><bbbc:config key="BedBathCanadaSiteCode" configName="ContentCatalogKeys" /></c:set>
		<h1 class="SPCSectionHeading noBorder">
			<bbbl:label key="lbl_spc_preview_billinginformation"
				...
				<div class="grid_4 omega">
					<dsp:droplet name="/atg/dynamo/droplet/IsEmpty">
						...
						<dsp:oparam name="false">
							<p class="previewText">
								<strong><bbbl:label key="lbl_spc_preview_billingmethod"
							...
							<dl class="clearfix">
								...
								<dt><bbbl:label key="lbl_spc_preview_exp" language="<c:out param='${language}'/>" /></dt>
						...
						<dsp:oparam name="false">
							...
							<dsp:droplet name="/atg/dynamo/droplet/ForEach">
								...
								<dsp:getvalueof id="count" param="count"/>
									<dl class="clearfix">
										<dt><bbbl:label key="lbl_spc_preview_giftcard" language="<c:out param='${language}'/>"/></dt>
					...
					<div class="grid_4 alpha omega marBottom_20">
						<c:if test="${not empty billingAddress.city}">
							<p class="previewText">
								<strong><bbbl:label key="lbl_spc_preview_billingaddress"
										language="<c:out param='${language}'/>" /> </strong>
							</p>
							<p class="marTop_5">
				...
				<h3 class="subHeading"><bbbl:label key="lbl_spc_preview_delivery" language="<c:out param='${language}'/>"/></h3>
					...
					<dl
						...
						<c:if test="${ECOFee gt 0.0}">
							<dt class="bold">
								<bbbl:label key="lbl_spc_preview_ecofee"
									language="<c:out param='${language}'/>" />
								<c:set var="ecofeeFootNote" scope="page">
									<bbbl:label key="lbl_spc_footnote_ecofee"
										language="<c:out param='${language}'/>" />
								</c:set>
						...
						<c:if test="${totalShippingAmount gt 0.0}">
							<dt><span class="bold">
								<bbbl:label key="lbl_spc_cartdetail_estimatedshipping"
									language="<c:out param='${language}'/>" />:</span> <a href="${contextPath}/static/shippingexclusionsinclusions" class="popup"><bbbl:label key="lbl_spc_exclusion_inclusion_details" language="<c:out param='${language}'/>"/></a> 
								<c:set var="shippingFootNote" scope="page">
									<bbbl:label key="lbl_spc_footnote_shipping"
										language="<c:out param='${language}'/>" />
								</c:set>
						...
						<c:if test="${giftWrapPrice gt 0.0}">
							<dt class="bold">
								<bbbl:label key="lbl_spc_preview_giftpackaging"
									language="<c:out param='${language}'/>" />
								<c:set var="giftWrapFootNote" scope="page">
									<bbbl:label key="lbl_spc_footnote_giftWrap"
										language="<c:out param='${language}'/>" />
								</c:set>
						...
						<%-- <c:if test="${totalTax gt 0.0}">
							<dt>
								<bbbl:label key="lbl_spc_preview_tax"
									language="<c:out param='${language}'/>" />
							</dt>
							<dd>
						...
						<c:when test="${currentSiteId eq bedBathCanadaSiteCode}">
							<c:choose>
								<c:when test="${stateLevelTax eq 0.0 && countyLevelTax ne 0.0}">
									<dt class="bold">
										<bbbl:label key="lbl_spc_preview_state_tax"
											language="<c:out param='${language}'/>" />
									</dt>
									<dd class="bold">
									...
									<dt class="bold">
										<bbbl:label key="lbl_spc_preview_county_tax"
											language="<c:out param='${language}'/>" />
									</dt>
									<dd class="bold">
					...
					<%-- <c:if test="${preTaxAmout gt 0.0}">
							<dt class="preTax">
								<strong><bbbl:label key="lbl_spc_preview_pretaxtotal"
										language="<c:out param='${language}'/>" /> </strong>
							</dt>
							<dd class="preTax">
					...
					<div class="fl">
						<c:if test="${storePickupShippingGroupItemCount gt 0}">
							<h3 class="subHeading borderTop">
								<bbbl:label key="lbl_spc_shipping_store_pickup"
									language="<c:out param='${language}'/>" />
							</h3>
						</c:if>
						<dl
							...
							<c:if test="${storePickupShippingGroupItemCount gt 0}">
								<dt>
									...
									<bbbl:label key="lbl_spc_preview_items"
										language="<c:out param='${language}'/>" />
								</dt>
								<dd>
							...
							<dt class="total">
								...
								<c:set var="totalFootNote" scope="page">
									<bbbl:label key="lbl_spc_footnote_total"
										language="<c:out param='${language}'/>" />
								</c:set>
		...
		<div class="clearfix marTop_20">
			...
			<div class="placeOrderInformation grid_6 omega marLeft_25">
				<bbbl:label key="lbl_spc_preview_placeordercaption"
					language="<c:out param='${language}'/>" />
			</div>
			</c:if>
			<div class="grid_8 footNotes marLeft_25">
				<ol>
					<c:if test="${not empty ecofeeFootNote}">
						<li class="smallText"><bbbl:label key="lbl_spc_footnote_ecofee"
								language="<c:out param='${language}'/>"
								placeHolderMap="footNotesMap" />
						</li>
					</c:if>
					<c:if test="${not empty shippingFootNote}">
						<li class="smallText"><bbbl:label key="lbl_spc_footnote_shipping"
								language="<c:out param='${language}'/>"
								placeHolderMap="footNotesMap" />
						</li>
					</c:if>
					<c:if test="${not empty giftWrapFootNote}">
						<li class="smallText"><bbbl:label key="lbl_spc_footnote_giftWrap"
								language="<c:out param='${language}'/>"
								placeHolderMap="footNotesMap" />
						</li>
					</c:if>
					<c:if test="${not empty totalFootNote}">
						<li class="smallText"><bbbl:label key="lbl_spc_footnote_total"
								language="<c:out param='${language}'/>"
								placeHolderMap="footNotesMap" />
						</li>
					</c:if>
