...
<dsp:page>
   ...
   <dsp:importbean bean="/atg/commerce/ShoppingCart" var="shoppingcart" />
    ...
    <c:set var="url" scope="request">${pageContext.request.scheme}://${pageContext.request.serverName}/store/checkout/singlePage/preview/cCAuthenticate.jsp</c:set>
    ...
    <c:choose>
        ...
        <c:otherwise>
			...
			<c:choose>
			<c:when test="${shoppingcart.currentEmpty eq 'true'}">
            <c:set var="url" scope="request">${pageContext.request.scheme}://${pageContext.request.serverName}/store/checkout/checkout_single.jsp</c:set>
			</c:when>
			  <c:otherwise>
           <c:set var="url" scope="request">${pageContext.request.scheme}://${pageContext.request.serverName}/store/checkout/singlePage/payment/billing_payment.jsp</c:set>
			  </c:otherwise>
			</c:choose>
