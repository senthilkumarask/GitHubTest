...
<dsp:page>
    <dsp:importbean bean="com/bbb/commerce/common/BBBVBVSessionBean"/>
     <dsp:importbean bean="/com/bbb/profile/session/SessionBean" />
      <dsp:getvalueof var="isSinglePageCheckout" bean="SessionBean.singlePageCheckout" />
    ...
    <c:set var="centinelTimeout"><bbbc:config key="Centinel_timeout" configName="ThirdPartyURLs" /></c:set>
       <input type="hidden" value="${centinelTimeout}" id="centinelTimeout"/>
       <c:choose>
              <c:when test="${isSinglePageCheckout}">
                     <c:set var="redirectPage" value="${scheme}://${serverName}${contextPath}/checkout/checkout_single.jsp?#vbvMsg"></c:set>
              </c:when>
              <c:otherwise>
                     <c:set var="redirectPage" value="${scheme}://${serverName}${contextPath}/checkout/payment/billing_payment.jsp"></c:set>
              </c:otherwise>
       </c:choose>
    ...
    <dsp:getvalueof var="scheme" bean="/OriginatingRequest.scheme"/>
       ...
       <dsp:getvalueof var="serverName" bean="/OriginatingRequest.serverName"/>
    <c:set var="centinelTimeout"><bbbc:config key="Centinel_timeout" configName="ThirdPartyURLs" /></c:set>
    <c:set var="centinelActive" ><bbbc:config key="cardinal_timeout_key" configName="FlagDrivenFunctions" /></c:set>
       <input type="hidden" value="${centinelTimeout}" id="centinelTimeout"/>
       <c:choose>
              <c:when test="${isSinglePageCheckout}">
                     <c:set var="redirectPage" value="${scheme}://${serverName}${contextPath}/checkout/checkout_single.jsp?fromCardinalError=true"></c:set>
              </c:when>
              <c:otherwise>
                     <c:set var="redirectPage" value="${scheme}://${serverName}${contextPath}/checkout/payment/billing_payment.jsp?fromCardinalError=true"></c:set>
              </c:otherwise>
       </c:choose>
       <input type="hidden" value="${redirectPage}" id="redirectPage"/>
            ...
            <compress:html removeComments="false">
                ...
                <html lang="${pageContext.request.locale.language}" xml:lang="${pageContext.request.locale.language}">
                    ...
                    <body id="themeWrapper" class="${themeName}">
                        <noscript><bbbt:textArea key="txt_spc_js_disabled_msg"  language="${pageContext.request.locale.language}" /></noscript>
                            ...
                            <div id="content" class="container_12 clearfix">
                                <div class="grid_12 clearfix">
                                    <div class="bvbAuthenticate">
                                        ...
                                        <bbbl:label key="lbl_spc_vbv_wait_temporarily" language="${pageContext.request.locale.language}" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <c:if test="${centinelActive eq 'true'}">         
                            <script type="text/javascript">
                          
                            var counterIframe=0;
                            function redirectPage(){
                             if(counterIframe<=1){
                               window.location.href=document.getElementById('redirectPage').value;
                              } 
                            }

                            setTimeout(redirectPage, document.getElementById('centinelTimeout').value);

                            var refFrameObj = document.getElementsByTagName("iframe")[0];
                            refFrameObj.onload = function(){
                                counterIframe++;
                                console.log(counterIframe);
                            };
                           </script>
                        </c:if>
                    </body>
