<dsp:page>
	...
	<legend class="hidden"><bbbl:label key="lbl_spc_checkout_billing_contact" language="${pageContext.request.locale.language}" /></legend>
				...
				<div class="inputPhone clearfix phoneFieldWrap requiredPhone">
                    <fieldset class="phoneFields">
						...
						<label id="lblbasePhoneFull" for="basePhoneFull">
							<bbbl:label key="lbl_spc_checkout_phone" language="${pageContext.request.locale.language}" /><span class="required">&nbsp;*</span>
						</label>					
						<%--R2.2 Story - AY - Disable field if order is of type Paypal and continue to paypal is selected --%>
						<c:choose>
							<c:when test = "${payPalOrder eq true}">
								...
								aria-labelledby="lblbasePhoneFull errorbasePhoneFull" disabled="disabled" data-paypalMobile="${mobileNumber}"/>
							</c:when>
							<c:otherwise>
								<input id="basePhoneFull" role="textbox" type="text" value="" name="basePhoneFull" class="phoneField required" maxlength="10" aria-required="false" aria-labelledby="lblbasePhoneFull errorbasePhoneFull"/>
							</c:otherwise>
						</c:choose>
