...
<dsp:droplet name="/com/bbb/commerce/checkout/droplet/BBBBeddingKitsAddrDroplet">
	 	  ...
	 	  <dsp:param name="" bean="/atg/commerce/ShoppingCart.current"/>
        ...
        <dsp:droplet name="BBBPackNHoldDroplet">
			...
			<c:if test="${hasSingleCollegeItem}">
                <div class="checkboxItem input clearfix" id="packHoldCheck">
				<c:choose>
					<c:when test="${hasSingleCollegeItem && !hasAllPackNHoldItems}">
						...
						<div class="label maxWidth70">
							<span class="icon-calendar disabled half-opacity icon-font" aria-hidden="true"></span>
							<label for="shippingOption1pack" class="disabled half-opacity floatNone"><strong><bbbl:label key="lbl_spc_shipping_pack_hold" language="${pageContext.request.locale.language}" /></strong> <bbbl:label key="lbl_shipping_pack_hold_unavailable_msg" language="${pageContext.request.locale.language}" /></label>
					...
					<c:otherwise>
                    		...
                    		<dsp:param name="value" param="isPackHold"/>
								<c:choose>
									<c:when test="${formExceptionFlag eq 'true'}">
											<dsp:input bean="BBBSPShippingGroupFormhandler.packNHold" type="checkbox" name="shippingOption1pack" id="shippingOption1pack"/>
									</c:when>
									<c:otherwise>
										<dsp:oparam name="true">
										  		<dsp:input bean="BBBSPShippingGroupFormhandler.packNHold" type="checkbox" onfocus="blur();" name="shippingOption1pack" id="shippingOption1pack" checked="false"/>
										</dsp:oparam>
										<dsp:oparam name="false">
										 		<dsp:input bean="BBBSPShippingGroupFormhandler.packNHold" type="checkbox" name="shippingOption1pack" id="shippingOption1pack" checked="false"/>
									  </dsp:oparam>
									</c:otherwise>
								</c:choose>                   
							</dsp:droplet>			 	
						...
						<div class="label">
							<span class="icon-calendar icon-font" aria-hidden="true"></span>
							<label for="shippingOption1pack"><strong><bbbl:label key="lbl_spc_shipping_pack_hold" language="${pageContext.request.locale.language}" /></strong> <bbbl:label key="lbl_spc_shipping_pack_hold_msg" language="${pageContext.request.locale.language}" /></label>
						</div>
                    <div class="clear"></div>
                        ...
                        <div class="input shippingDate clearfix">
                            ...
                            <div class="label">
                                <label id="lbl${shippinDateIDName}" for="${shippinDateIDName}">
                                    <bbbl:label key="lbl_spc_shipping_pack_hold_date" language="${pageContext.request.locale.language}" />
                                </label>
                            </div>
                            <div class="clear"></div>
                            <div class="text">
								...
								<c:choose>
								  ...
								  <c:choose>
									<c:when test="${formExceptionFlag eq 'true'}">
										<dsp:input bean="BBBSPShippingGroupFormhandler.packNHoldDate" type="text"
									...
									<c:otherwise>									
										<c:choose>
										  <c:when test="${beddingKit eq true || weblinkOrder eq true}">
											...
											<dsp:input bean="BBBSPShippingGroupFormhandler.packNHold" type="hidden" name="shippingOption1pack" id="shippingOption1packhidden" value=""/>
										  	  <dsp:input bean="BBBSPShippingGroupFormhandler.packNHoldDate" type="hidden"
		                                        name="${shippinDateIDName}" id="${shippinDateIDName}" value="${PreFilledDate}"/>
										  </c:when>
										  <c:otherwise>
											  <dsp:input bean="BBBSPShippingGroupFormhandler.packNHoldDate" type="text"
                                ...
                                <div class="grid_1 omega">
                                  ...
                                  <span class="icon-calendar icon-font" id="shippingDateButton" aria-hidden="true"></span>
                                </div>
                                <div class="cb">
