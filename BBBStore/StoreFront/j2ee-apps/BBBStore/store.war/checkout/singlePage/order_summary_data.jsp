<dsp:page>
    ...
    <dsp:droplet name="/com/bbb/commerce/pricing/droplet/BBBPriceDisplayDroplet">
        ...
        <dsp:param name="profile" bean="Profile"/>
			...
			<h3 class="subHeading"><bbbl:label key="lbl_spc_preview_delivery" language="<c:out param='${language}'/>"/></h3>
            <dl class="clearfix <c:if test='${storePickupShippingGroupItemCount gt 0}'>inStorePickup</c:if> noBorder summaryNew">
            	<c:choose>
			        ...
			        <c:otherwise>
               		<dt class="bold"><bbbl:label key="lbl_spc_order_subtotal" language="<c:out param='${language}'/>"/> <span class="summaryCount">${hardgoodShippingGroupItemCount} <bbbl:label key="lbl_spc_preview_items" language="<c:out param='${language}'/>"/></span></dt>
               		</c:otherwise>
                </c:choose>
                ...
                <c:if test="${hardgoodShippingGroupItemCount gt 0.0}">
                    ...
                    <c:if test="${onlineEcoFeeTotal gt 0.0}">
                        <dt class="bold"><bbbl:label key="lbl_spc_preview_ecofee" language="<c:out param='${language}'/>"/>
                            <c:set var="ecofeeFootNote" scope="request">
                                <bbbl:label key="lbl_spc_footnote_ecofee" language="<c:out param='${language}'/>"/>
                            </c:set>
                    ...
                    <c:if test="${giftWrapTotal gt 0.0}">
                        <dt class="bold"><bbbl:label key="lbl_spc_preview_giftpackaging" language="<c:out param='${language}'/>"/>
                            <c:set var="giftWrapFootNote" scope="request">
                                <bbbl:label key="lbl_spc_footnote_giftWrap" language="<c:out param='${language}'/>"/>
                            </c:set>
                    ...
                    <c:choose>
                        <c:when test="${freeShipping ne true}">
                        		 ...
                        		 <c:choose>
										...
										<dt><span class="bold"><bbbl:label key="lbl_spc_cartdetail_estimatedshipping" language="<c:out param='${language}'/>"/>:
											<c:set var="shippingFootNote" scope="request">
												<bbbl:label key="lbl_spc_footnote_shipping" language="<c:out param='${language}'/>"/>
											</c:set>
                        ...
                        <c:otherwise>
                            <dt class="fl"><span class="bold"><bbbl:label key="lbl_spc_cartdetail_estimatedshipping" language="<c:out param='${language}'/>"/>:</span> 
							 <c:choose>
							 	...
							 	<c:otherwise>
								 	<c:choose>
										...
										<a href="${contextPath}/static/shippingexclusionsinclusions" class="popup"><bbbl:label key="lbl_spc_exclusion_inclusion_details" language="<c:out param='${language}'/>"/></a></dt>
											<dd class="fl bold"><bbbl:label key="lbl_spc_shipping_free" language="<c:out param='${language}'/>"/></dd>
										</c:when>
										<c:otherwise>
										</dt>
					...
					<c:if  test ="${orderHasLtl}">
						...
						<c:if  test ="${totalDeliverySurcharge gt 0.0 and !orderContainsEmptySG and orderHasLtl}">
							<dt><span class="bold"><bbbl:label key="lbl_spc_cart_delivery_surcharge" language="<c:out param='${language}'/>"/>: </span><a href="${contextPath}/static/shippingexclusionsinclusions" class="popup"><bbbl:label key="lbl_spc_exclusion_inclusion_details" language="<c:out param='${language}'/>"/></a></dt>
						...
						<c:if  test ="${maxDeliverySurchargeReached}"> 
									<c:choose>
										<c:when test="${orderHasLtl and orderContainsEmptySG}">
									 <dt>
			                            <span class="bold"><bbbl:label key="lbl_spc_cart_max_surcharge_reached" language="<c:out param='${language}'/>"/>:</span> <br>
					                            <a href="${contextPath}/static/whatthismean" class="maxSurcharges popup"><bbbl:label key="lbl_spc_what_this_mean" language="<c:out param='${language}'/>"/></a>
					                        </dt>
	                ...
	                <c:if test="${not empty order.specialInstructions['CYBERSOURCE_TAX_FAILURE']}">
				    	<c:set var="taxFailureFootNote" scope="request">
				            <bbbl:label key="lbl_spc_preview_tax_unavailable" language="<c:out param='${language}'/>"/>
				        </c:set>
	                </c:if>       
                    <c:choose>
						...
						<c:otherwise>
							...
							<c:set var="totalCountyTax"
			                 ...
			                 <c:if  test ="${totalGCAmt gt 0.0}"> 
							<dt class="bold"><bbbl:label key="lbl_Gift_Card_Amount_Applied" language="${pageContext.request.locale.language}"/></dt>
									<dd>-<span class="bold"> 
							...
							<dt class="total bold">
							<c:choose>
							<c:when test="${totalGCAmt gt 0.0 }">
							<div class="totalAllign"><strong><bbbl:label key="lbl_spc_preview_total" language="<c:out param='${language}'/>"/>:</strong>
								<span><dsp:valueof value="${onlineTotal - totalGCAmt}" converter="defaultCurrency"/> </span></div>
							</c:when>
							<c:otherwise>
							<div class="totalAllign"><strong><bbbl:label key="lbl_spc_preview_total" language="<c:out param='${language}'/>"/>:</strong>
								<span><dsp:valueof value="${onlineTotal}" converter="defaultCurrency"/> </span></div>
								 ...
								 <c:choose>
										<c:when
											...
											<bbbl:label key="lbl_spc_cartdetail_totalsavings"
										...
										<c:if  test ="${totalSavedAmount gt 0.0}"> 
											<dt class="flatform fr highlight summarySaving bold"><span>
											<bbbl:label key="lbl_spc_cartdetail_totalsavings" language="<c:out param='${language}'/>" />
												: 
												<dsp:valueof value="${totalSavedAmount}" converter="defaultCurrency" /></span>
														
                                                       <bbbl:textArea key="txt_total_savings_tooltip" language="<c:out param='${language}'/>"/>
                        ...
                        <c:otherwise>          
                            ...
                            <dt class="total bold">
	                            <c:choose>
							   		<c:when test ="${orderHasLtl and orderContainsEmptySG and totalGCAmt == 0.0}">
							   			<div class="totalAllign"><strong><bbbl:label key="lbl_spc_cartdetail_cart_total" language="<c:out param='${language}'/>"/>:</strong>
							   		...
							   		<c:when test="${totalGCAmt gt 0.0}">
							   		<div class="totalAllign"><strong><bbbl:label key="lbl_spc_cartdetail_cart_total" language="<c:out param='${language}'/>"/>:</strong>
									<span><dsp:valueof value="${storeAmount + onlinePurchaseTotal + totalSurcharge + totalAssemblyFee - totalGCAmt}" converter="defaultCurrency"/></span></div>							   		
							   		</c:when>
							   		<c:otherwise>
							   			<div class="totalAllign"><strong><bbbl:label key="lbl_spc_preview_pretaxtotal" language="<c:out param='${language}'/>"/>:</strong>
			...
			<c:if test="${storePickupShippingGroupItemCount gt 0}">
                <h3 class="subHeading"><bbbl:label key="lbl_spc_shipping_store_pickup" language="<c:out param='${language}'/>"/> <span class="smallText"><bbbl:label key="lbl_spc_store_amountdue" language="<c:out param='${language}'/>"/></span></h3>
                ...
                <dl class="clearfix summaryNew">
                    ...
                    <c:if test="${storeEcoFeeTotal gt 0.0}">
                        <dt class="bold"><bbbl:label key="lbl_spc_preview_ecofee" language="<c:out param='${language}'/>"/></dt>
