<dsp:page>
    ...
    <c:if test="${param.bExternalJSCSS == 'false'}">
    	<c:set var="bExternalJSCSS" value="${false}" scope="request"/>
    </c:if>

    <c:if test="${param.bAnalytics == 'false'}">
        <c:set var="bAnalytics" value="${false}" scope="request"/>
    </c:if>

    <c:if test="${param.bPlugins == 'false'}">
        <c:set var="bPlugins" value="${false}" scope="request"/>
    </c:if>
    ...
    <c:set var="QASOnAsync" value="${false}" scope="request" />
	...
	<c:choose>
		<c:when test="${currentSiteId eq BedBathUSSite}">
			...
			<c:set var="PInterestOn" scope="request"><tpsw:switch tagName="PInterestTag_us"/></c:set>
			<c:set var="QASOn" scope="request"><tpsw:switch tagName="QASTag_us"/></c:set>
		...
		<c:when test="${currentSiteId eq BuyBuyBabySite}">
			...
			<c:set var="PInterestOn" scope="request"><tpsw:switch tagName="PInterestTag_baby"/></c:set>
			<c:set var="QASOn" scope="request"><tpsw:switch tagName="QASTag_baby"/></c:set>
		...
		<c:otherwise>
			...
			<c:set var="PInterestOn" scope="request"><tpsw:switch tagName="PInterestTag_ca"/></c:set>
			<c:set var="QASOn" scope="request"><tpsw:switch tagName="QASTag_ca"/></c:set>
	...
	<c:set var="FindAStoreOn" scope="request"><bbbc:config key="FindAStoreOn" configName="ContentCatalogKeys" /></c:set>
	...
	<%-- adding this flag Jan release ship to PO box  --%>
    ...
    <%-- set lazy-load var based on value defined in config --%>
    <c:set var="disableLazyLoadS7Images" scope="request"><bbbc:config key="disableLazyLoadS7ImagesFlag" configName="FlagDrivenFunctions" /></c:set>
	...
	<c:set var="yourAmigoSiteId" scope="request">
		<bbbc:config key="YourAmigo_site_id" configName="ThirdPartyURLs" />
	</c:set>
	
	<%-- adding this flag for BPSI-3221 --%>
	...
	<%-- mobile redirect JS flag BBBSL-7474 --%>
