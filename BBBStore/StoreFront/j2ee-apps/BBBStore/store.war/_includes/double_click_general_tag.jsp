...
<dsp:getvalueof var="url" value="${pageContext.request.requestURL}"/>
<dsp:getvalueof id="currentSiteId" bean="Site.id" />
			<c:if test="${(currentSiteId eq BedBathUSSite)}">
    		   <c:set var="cat"><bbbc:config key="cat_general_bedBathUS" configName="RKGKeys" /></c:set>
    		 ...
    		 <c:if test="${(currentSiteId eq BuyBuyBabySite)}">
    		   <c:set var="cat"><bbbc:config key="cat_general_baby" configName="RKGKeys" /></c:set>
    		...
    		<c:if test="${(currentSiteId eq BedBathCanadaSite)}">
    		   <c:set var="cat"><bbbc:config key="cat_general_bedbathcanada" configName="RKGKeys" /></c:set>
			 ...
			 <dsp:setvalue param="doubleClickParam" value="src=${src};type=${type};cat=${cat};u4=null;u5=null"/>

<dsp:getvalueof id="requestParam" param="doubleClickParam"/>
<c:if test="${DoubleClickOn}">
		<c:if test="${currentSiteId eq 'BedBathUS'}">
			<c:set var="doubleClickUrl">
				<bbbc:config key="doubleClickUrlBedBathUS"
					configName="ThirdPartyURLs" />
			</c:set>
		...
		<c:if test="${currentSiteId eq 'BuyBuyBaby'}">
			<c:set var="doubleClickUrl">
				<bbbc:config key="doubleClickUrlBaby" configName="ThirdPartyURLs" />
			</c:set>
			<c:if test="${empty doubleClickUrl}">
		...
		<c:if test="${currentSiteId eq 'BedBathCanada'}">
			<c:set var="doubleClickUrl">
				<bbbc:config key="doubleClickUrlBedBathCA"
					configName="ThirdPartyURLs" />
			</c:set>
			<c:if test="${empty doubleClickUrl}">
				<c:set var="doubleClickUrl">//3948870.fls.doubleclick.net</c:set>
	...
	--%>
	<c:choose>
	<c:when test="${fn:contains(url, 'cart/cart.jsp') || fn:contains(url, 'checkout/checkout_confirmation.jsp')}">
	</c:when>
	<c:otherwise>
		<c:choose>
			<c:when test="${ord eq null}">
				...
				document.write('<iframe type="some_value_to_prevent_js_error_on_ie7" src="${doubleClickUrl}/activityi;${requestParam};ord=' + a + '?"  width="1" height="1" frameborder="0" style="display:none"></iframe>');
			</script>
				<noscript>
					<iframe type="some_value_to_prevent_js_error_on_ie7" src="${doubleClickUrl}/activityi;${requestParam};ord=1?"
						width="1" height="1" frameborder="0" style="display: none"></iframe>
				</noscript>
			</c:when>
			<c:otherwise>
				<iframe  type="some_value_to_prevent_js_error_on_ie7"  src="${doubleClickUrl}/activityi;${requestParam}" width="1"
					height="1" frameborder="0" style="display: none"></iframe>
			</c:otherwise>
		</c:choose>
		</c:otherwise>
		</c:choose>
