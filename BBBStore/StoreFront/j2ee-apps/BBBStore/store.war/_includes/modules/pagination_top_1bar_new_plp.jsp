...
<dsp:getvalueof var="showFilters" param="showFilters"/>
...
<dsp:getvalueof var="queryString" bean="/OriginatingRequest.queryString"/>
	...
	<dsp:getvalueof var="pagBot" param="pagBot"/>
	<dsp:getvalueof var="narrowDown" param="narrowDown"/>
...
<c:set var="filterSelected" value="false"/>
...
<input type="hidden" value="newSubCategoryDetail" id="newTrendplp"> 
...
<%-- R2.2 - Story 116E Start --%>
		...
		<c:if test="${not empty param.narrowDown}">
           ...
           <input type="hidden" name="windowUrl" class="windowUrl" value="${url}"/>
           <input type="hidden" name="urlNarrowDown" class="urlNarrowDown" value="${param.narrowDown}"/>
           <input type="hidden" name="filtersonLoad" class="filtersonLoad" value="${filtersonLoad}"/>
<%-- R2.2 - Story 116E END --%>
		...
		<c:if test="${showViewOptions}">
		 <script type="text/javascript">
			...
			<%-- <div class="pagGroupings clearfix">
				...
				<div id="pagViewMode" class="searchViewMode clearfix" aria-labelledby="lblpagViewMode">
				  <span class="incDec visuallyhidden" aria-live="assertive" aria-atomic="true"></span>
					<c:choose>
						<c:when test="${plpView == 'list'}">
						     <a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid4${inStoreParam}" aria-selected="false" class="defaultView  redirPage dynFormSubmit" title="${lbl_grid_view}" aria-label="Product Grid View - 3 Across" >${lbl_grid_view}</a>
							<a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid${inStoreParam}" aria-selected="false" class="gridView redirPage dynFormSubmit" title="${lbl_grid_view}" aria-label="Product Grid View - 4 Across">${lbl_grid_view}</a>
						...
						<c:when test="${plpView == 'grid4'}">
						     <a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid4${inStoreParam}" aria-label="Product Grid View - 3 Across" class="defaultView active redirPage dynFormSubmit" aria-selected="true" title="${lbl_grid_view}">${lbl_grid_view}</a>
							<a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid${inStoreParam}" aria-label="Product Grid View - 4 Across" aria-selected="false" class="gridView redirPage dynFormSubmit" title="${lbl_grid_view}">${lbl_grid_view}</a>
						...
						<c:otherwise>
						     <a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid4${inStoreParam}" aria-label="Product Grid View - 3 Across" aria-selected="false" class="defaultView redirPage dynFormSubmit" title="${lbl_grid_view}">${lbl_grid_view}</a>
							<a data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" href="${url}${filterAppliedWithPrefix}${currentPage}-${size}${storeIdParam}?${subCategoryPageParam}<dsp:valueof param="browseSearchVO.searchQuery"/>${searchQueryParamsWithoutView}&view=grid${inStoreParam}" aria-selected="true" aria-label="Product Grid View - 4 Across"  class="gridView active redirPage dynFormSubmit" title="${lbl_grid_view}">${lbl_grid_view}</a>
			...
			<fieldset class="pagControls clearfix cd">
				...
				<div class="pagSort">
					<ul class="pagLevel1UL">
						...
						<div class="sfWrap">
							...
							<a href="javascript:void(0);" aria-label="sort by link" id="sortlink" class="srtlink" data-menu-content="sortbySel" onclick="" data-target="sortbySel">sort</a>
							<ul	class="sortflyout" id="sortbySel" name="pagSortOpt" aria-required="false" aria-live="assertive" data-submit-param-length="1"  data-submit-param1-name="swsterms" data-submit-param1-value="${swsterms}" >	  <dsp:droplet name="/atg/dynamo/droplet/ForEach">
													...
													<c:if test="${(defaultUserCountryCode ne 'MX' && sortOption.sortUrlParam ne 'PriceMX'  ) || (defaultUserCountryCode eq 'MX' && sortOption.sortUrlParam ne 'Price')}">							
				                                        <c:choose>
															<c:when test="${pagSortOpt == currentSortCode}">
																<li class="">	
																	<a selected href="${url}${filterAppliedWithoutPrefix}${storeIdParamForSortBy}?pagSortOpt=${currentSortCode}${searchQueryParams}${plpViewParam}${subCategoryPageParam}${inStoreParam}" >${sortOption.sortValue}</a>
																</li>
															</c:when>
															<c:otherwise>
																<li class="">	
																	<a href="${url}${filterAppliedWithoutPrefix}${storeIdParamForSortBy}?pagSortOpt=${currentSortCode}${searchQueryParams}${plpViewParam}${subCategoryPageParam}${inStoreParam}" onclick="javascript:sortTrack('${sortOption.sortValue}')">${sortOption.sortValue}</a>
																</li>
															</c:otherwise>
														</c:choose>		
													</c:if>							
						...
						<c:if test="${showFilters eq true}">
							<li class="filtrDroplist">
								<a href="javascript:void(0);" class="filterDropdwnn" aria-label="filter" id="filterDrop" data-menu-content="#filterBox" onclick="" data-target="filterBox">filter</a>
							</li>
						</c:if>
						</ul>
						</div>
						...
						<c:if test="${showFilters eq true}">
							<dsp:include page="/_includes/modules/ajaxifyFilters_new_plp.jsp" flush="true">
								<dsp:param name="paginationPage" value="true"/>
