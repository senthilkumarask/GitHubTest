<dsp:page>
    ...
    <c:set var="attributeFlag" value="false"/>
    <c:set var="productAttributesNew" value="|"/>
	<c:set var="pipe" value="|"/>
	<c:choose>
	<c:when test="${not empty categoryVO.bccManagedPromoCategoryVO }">
	<c:set var="showQuickViewLink" value="${categoryVO.bccManagedPromoCategoryVO.showQuickViewLink }"/>
	<c:set var="showProductCompare" value="${categoryVO.bccManagedPromoCategoryVO.showProductCompare }"/>
	<c:set var="showColorSwatches" value="${categoryVO.bccManagedPromoCategoryVO.showColorSwatches }"/>
	<c:set var="showTitle" value="${categoryVO.bccManagedPromoCategoryVO.showTitle }"/>
	<c:set var="showReviewRatings" value="${categoryVO.bccManagedPromoCategoryVO.showReviewRatings}"/>
	<c:set var="showPrice" value="${categoryVO.bccManagedPromoCategoryVO.showPrice}"/>
	<c:set var="showGlobalAttributes" value="${categoryVO.bccManagedPromoCategoryVO.showGlobalAttributes }"/>
	<c:set var="showFilters" value="${categoryVO.bccManagedPromoCategoryVO.showFilters }"/>
		<c:forEach var="productAttribute" items="${categoryVO.bccManagedPromoCategoryVO.productAttributes }" >
		<c:set var="productAttributesNew" value="${productAttributesNew }${productAttribute}${pipe}"/>
		</c:forEach>
	</c:when>
	<c:otherwise>
	<c:set var="showQuickViewLink" value="true"/>
	<c:set var="showProductCompare" value="true"/>
	<c:set var="showColorSwatches" value="true"/>
	<c:set var="showTitle" value="true"/>
	<c:set var="showReviewRatings" value="true"/>
	<c:set var="showPrice" value="true"/>
	<c:set var="showGlobalAttributes" value="true"/>
	<c:set var="showFilters" value="true"/>
	<c:set var="attributeFlag" value="true"/>
	</c:otherwise>
	</c:choose>
...
<form method="post" id="hiddenFacetedForm" action="#">
			                ...
			                <input type="hidden" name="swsterms" value="">
	...
	<c:choose>
	   <%-- Adding condition for new 4x4 view |List View Redesign Story |Sprint2 |START --%>
		...
		<%-- Adding condition for new 4x4 view  |List View Redesign Story |Sprint2 |End FILTERChange--%> 
			...
			<div class="<c:out value="${liGridClass}"/> product <c:out value="${custClass}"/> registryDataItemsWrap listDataItemsWrap">
				...
				<dsp:getvalueof var="seoUrl" param="element.seoUrl"/>
					...
					<c:if test="${is_gridview_3x3}">
						<dsp:getvalueof var="imageURL" param="element.productImageUrlForGrid3x3_newPlp"/>
					</c:if>
					<c:if test="${plpGridSize == '4'}">
		        ...
		        <%--defect fixed by @psin52 --%>
					...
					<c:choose>
						<c:when test="${plpGridSize == 3}">
	                    	...
	                    	<c:if test="${showQuickViewLink}"><span role="button" aria-label="${lbl_product_quick_view} ${lblOfThe} ${prodName}" class="quickView ${quickViewClass} fl marRight_30 padRight_5">${lbl_product_quick_view}</span></c:if>
		                    <c:if test="${showProductCompare }"><div class="checkboxItem input clearfix noPad noMar fl noBorder">
	                  ...
	                  <c:if test="${showQuickViewLink}">  <div class="padTop_10"><span tabindex="0" role="link" aria-label="${lbl_product_quick_view} ${lblOfThe} ${prodName}" class="quickView ${quickViewClass} ">${lbl_product_quick_view}</span></div></c:if>
	                    <c:if test="${showProductCompare }">
	                     <c:if test="${compareProducts eq true}">
	                        <dsp:getvalueof var="productId" param="element.productID"/>
							<dsp:getvalueof var="inCompareDrawer" param="element.inCompareDrawer"/>
								<c:choose>
	               ...
	               <c:if test="${showColorSwatches}">
	               <c:set var="colorSetSize">${fn:length(element.colorSet) }</c:set>
					<c:if test="${swatchFlag == '1' and colorSetSize gt 0}">
					<fieldset>
					<legend class="hidden"><bbbl:label key="lbl_colors_dropdown" language ="${pageContext.request.locale.language}"/></legend>
						...
						<label class="hidden"> <bbbl:label key="lbl_colors_dropdown" language ="${pageContext.request.locale.language}"/></label>
							...
							<ul class="prodSwatches hideSwatchRows" role="radiogroup" > 
								<dsp:droplet name="ForEach">
							...
							<c:if test="${(plpGridSize==4 && colorSetSize > 5) ||  (plpGridSize==3 && colorSetSize > 7)}">	
				...
				<c:set var="lblPriceTBD"><bbbl:label key="lbl_price_is_tbd" language="${pageContext.request.locale.language}"/></c:set>
							...
							<dsp:droplet name="/atg/dynamo/droplet/ForEach">
								...
								<dsp:oparam name="output">
								<dsp:getvalueof var="skuAttributeId" param="attributeVO.skuAttributeId"  />
									<c:set var="skuAttributeId" value="${pipe}${skuAttributeId }${pipe}"/>
									<c:if test="${fn:containsIgnoreCase(productAttributesNew, skuAttributeId)}">
									<c:set var="attributeFlag" value="true"/>
									</c:if>
									<c:if test="${attributeFlag }">
				...
				<%--  BBB AJAX 2.3.1  Was-Is price change on PLP,Search,Brand ends--%>	
						...
						<c:if test="${showGlobalAttributes}">
					...
					<c:if test="${inStore eq true && (mswpFlag eq true || collectionFlag eq 1)}">
							<div class="inStoreMayNotBeAvailable">
								<c:choose>
							<c:when test="${vdcFlag eq true}">
