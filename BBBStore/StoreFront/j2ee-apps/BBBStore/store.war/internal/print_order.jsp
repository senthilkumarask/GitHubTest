			...
			<c:choose>
				<c:when test="${not empty orderId}">
					...
					<dsp:droplet name="/atg/commerce/order/OrderLookup">
				        ...
				        <dsp:oparam name="empty"><bbbl:label key="lbl_not_valid_atg_order_id" language="${pageContext.request.locale.language}"/> </dsp:oparam>
				        <dsp:oparam name="output">
								   		...
								   		<div class="container_12 clearfix" id="content" role="main">
											<div class="topContent clearfix">
												<div id="subHeader" class="grid_10 clearfix">
													<p class="noMar marTop_5"><bbbl:label key="lbl_ATG_Order_ID" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.id}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_spc_checkoutconfirmation_order_date" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.submittedDate}" date="M/dd/yyyy hh:ss" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_trackorder_status" language="${pageContext.request.locale.language}"/>: <span class="bold"><dsp:valueof value="${order.stateAsString}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_Profile_ID" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.profileId}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_IP_Address" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.userIP}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_Affiliate" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.affiliate}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_School" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.schoolId}" /></span></p>
													<p class="noMar marTop_5"><bbbl:label key="lbl_School_Promo" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${order.schoolCoupon}" /></span></p>
												</div>
											</div>
																	...
																	<dsp:oparam name="equal">
																		...
																		<c:if test="${empty showLinks}">
																			<c:if test="${not empty onlineOrderNumber}">
																				...
																				<h3 class="sectionHeading noBorderBot"><bbbl:label key="lbl_Delivery_Order" language="${pageContext.request.locale.language}"/> <span class="normal">${onlineOrderNumber}</span></h3>
																				<p class="noMar marTop_5"><bbbl:label key="lbl_Shipping_Group_ID" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${shippingGroup.id}" /></span></p>
																				<p class="noMar marTop_5"><bbbl:label key="lbl_Shipping_Group_Type" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof value="${shippingGroup.shippingGroupClassType}" /></span></p>
																				<p/>
																				<dsp:getvalueof var="billingAddress" param="result.billingAddress"/>
																				<h3 class="sectionHeading">${bbbItemcount} <bbbl:label key="lbl_item(s)_shipping_to" language="${pageContext.request.locale.language}"/> <span>${billingAddress.firstName} ${billingAddress.lastName}</span></h3>
																		...
																		<c:if test="${empty showLinks}">
																			<c:if test="${not empty bopusOrderNumber}">															
																				<div class="marBottom_10 grid_8 alpha">
																					...
																					<p/>																	
																					<h3 class="sectionHeading">${bbbStoreItemcount} <bbbl:label key="lbl_Item(s)_to_be_picked_up" language="${pageContext.request.locale.language}"/></h3>
																					<p><bbbl:label key="lbl_Status_Item(s)_ready_to_be_picked" language="${pageContext.request.locale.language}"/> <fmt:formatDate value="${order.submittedDate}" pattern="dd/MM/yyyy"/></p>
																				</div>
																			</c:if>
																		</c:if>
																			...
																			<dsp:oparam name="output">													
																				<dsp:droplet name="Switch">
																					...
																					<dsp:oparam name="creditCard">
																						<p class="noMar marTop_5"><bbbl:label key="lbl_Payment_Group_ID" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof param="paymentGroup.id" /></span></p>
																						<p class="noMar marTop_5"><bbbl:label key="lbl_Payment_Group_Type" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof param="paymentGroup.paymentMethod" /></span></p>																	
																						<p class="noMar marTop_5"><bbbl:label key="Payment_Group_Auth_Code" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof param="paymentGroup.authorizationStatus[0].authorizationCode" /></span></p>
																						<p>
																			...
																			<dsp:droplet name="BBBPaymentGroupDroplet">
																			  ...
																			  <dsp:oparam name="output">															  
																				<dsp:droplet name="/atg/dynamo/droplet/ForEach">
																					...
																					<dsp:oparam name="output">
																						<p class="noMar marTop_5"><bbbl:label key="lbl_Payment_Group_ID" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof param="giftcard.id" /></span></p>
																						<p class="noMar marTop_5"><bbbl:label key="lbl_Payment_Group_Type" language="${pageContext.request.locale.language}"/> <span class="bold"><dsp:valueof param="giftcard.paymentMethod" /></span></p>
																						<p>																	
