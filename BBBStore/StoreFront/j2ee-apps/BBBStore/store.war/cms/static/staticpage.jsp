...
<dsp:droplet name="StaticTemplateDroplet">
		...
		<dsp:getvalueof var="showRegistryRibbon" param="staticTemplateData.registryRibbonFlag" scope="request" />
        ...
        <dsp:getvalueof var="seoUrl" param="staticTemplateData.seoUrl" />
        <c:if test="${fn:containsIgnoreCase(seoUrl,'/images')}">
            <input type="hidden" id="isFromImages" value="true"/>
        </c:if>
		<c:if test="${not empty omnitureData}">
			...
			<c:set var="sa_page">${omnitureData['sa_page'] }</c:set>			
		</c:if>
...
<c:if test="${pageName ne 'SameDayDelivery'}">
	<script type="text/javascript">
	    ...
	    if (typeof s !== 'undefined') {
	        ...
	        if(omni_omniPageName.length>0){
	            ...
	            s.prop4=omni_prop4;
	            s.prop5=omni_prop5;
	            if($('#advisor-container')[0]){
	            	s.eVar1='smart assistant:'+omni_evar1;
	            } else{
		            s.eVar1=omni_evar1;	            
	            }
	            s.eVar2=omni_evar2;
	            ...
	            s.pageName='Content Page>'+omni_pageTitleNew;
	            s.channel='Guides & Advice';
	            if(geturlparams('icid') && $('#advisor-container')[0]){
	                s.eVar1='smart assistant:'+omni_pageTitleNew.toLowerCase();
	            }
	            ...
	            else if (document.getElementById("isFromImages")){
	                s.prop1='UGC Content Page';
	                s.pageName='UGC>'+omni_pageTitleNew;
	            }else{
	            }
	        }
	        fixOmniSpacing();
