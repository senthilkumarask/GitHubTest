	...
	<dsp:droplet name="GuidesTemplateDroplet">
		...
		<div id="cmsPageContent" class="grid_9 clearfix">
			...
			<dsp:getvalueof var="seeAll" param="seeAll" scope="request"/>
	  	    <dsp:getvalueof param="emptyResults" var="emptyResults" />
	  	    <c:choose>
	  	    <c:when test="${emptyResults ne true}" >
	 	  ...
	 	  <c:when test="${emptyResults eq true }">
	 	  <div>No ${contentType } Found</div>
	 	  </c:when>
	 	  </c:choose>
    </div>		
    </jsp:body>
      ...
      <script type="text/javascript">   if(typeof s !=='undefined') {
   			...
   			if(s_code)document.write(s_code);		
				...
				<dsp:oparam name="error">
					<dsp:include page="/404.jsp" flush="true" ></dsp:include>
