<dsp:page>
				...
				<div id="editors">	
					...
					<div class="textEditor" id="textEditorOne" >
						...
						<div class="textEditorFieldsWrapper">
							...
							<div id="fontSizeContainer">
								<label><bbbl:label key="lbl_Font_Size" language="${pageContext.request.locale.language}" /></label>
								<div class="select">
							...
							<div id="fontColorContainer">
								<label><bbbl:label key="lbl_Font_Color" language="${pageContext.request.locale.language}" /></label>
					...
					<div class="textEditor" id="textEditorTwo" >
						...
						<div class="textEditorFieldsWrapper">
							...
							<label><bbbl:label key="lbl_Select_Font" language="${pageContext.request.locale.language}" /></label>
							<div class="select">
					...
					<dsp:oparam name="outputStart">		
					 ...
					 <dsp:oparam name="output">	
					 	...
					 	<dsp:getvalueof var="cardId" param="element.Id"/>
					      ...
					      ${fn:replace(filterCardContent,"${imageServer}" ,"")}
					</dsp:oparam>				
		            <dsp:oparam name="outputEnd">
					      	</div>
		            </dsp:oparam>
  ...
  <head>
	...
	.cardContainer img{display: block; position: absolute;}
	.cardContainer span{position: absolute; border:2px solid red; display: block;  top:0 !important; margin-top: 80px; display: block; width: 100%; border:1px solid green;width: 378px;padding:10px;clear:both;}
	.cardContainer p{ width: 100%; margin:0; padding: 0;}
	.cardContainer p.cardNameOne{ width: 100%; margin:0; padding: 0;}
	.cardContainer p.cardNameTwo{ width: 100%;}
	#cardContainer {float:left;}
	#cardContainer2 {float:right;}
	#cardContainer3 {float:left;}
	#cardContainer4 {float:right;}
	#cardContainer5 {float:left;}
	#cardContainer6 {float:right;}
	.cardContainer p{ width: 100%;}
...
BBB.printAtHome = (function($) { 		
		...
		$('#submitPrintCard').click(function(e){
            //e.preventDefault();
            var pdfHtml = $('<div/>').html($cardAreaHtml.val()); 
            //add closing image tags for pdf parser
			var imageURL = pdfHtml.find(".cardBackground").attr("src");
				
			pdfHtml.find(".cardBackground").remove();
			pdfHtml.find(".cardContainer").css({'background-image':'url("{{backReplace}}")',  "background-size": "100% 100%"});              

			pdfHtml = pdfHtml.html();
			pdfHtml = pdfHtml.replace(/span\>/g, "p>");
			pdfHtml = pdfHtml.replace(/\<span/g, "<p");     
			...
			pdfHtml = $("<div>").text($('#cardHeaderHtml').val().replace("$html",pdfHtml)).html();

			 pdfHtml = pdfHtml.replace(/\{\{backReplace\}\}/g, imageURL);               
			 $cardAreaFull.val(pdfHtml);
