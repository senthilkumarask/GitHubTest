	...
	<dsp:getvalueof id="currentSiteId" bean="/atg/multisite/Site.id" />
		...
		<dsp:getvalueof id="applicationId" bean="Site.id" scope="session"/>	
<%--	<dsp:getvalueof var="applicationId" param="applicationId"/>	--%>
	 	<dsp:getvalueof var="appIdCertona" param="appIdCertona"/>
	    <dsp:getvalueof var="RegistryContext" param="RegistryContext"/>
	    <dsp:getvalueof var="language" param="language"/>
	    <dsp:getvalueof var="CertonaContext" param="CertonaContext"/>
		...
		<dsp:droplet
			...
			<dsp:oparam name="output">
				...
				<dsp:droplet name="/com/bbb/commerce/order/purchase/OrderHasErrorPersonalizedItemDroplet">
				 <dsp:param name="items" param="giftList"/>
				 <dsp:oparam name="output">
					<dsp:getvalueof var="orderHasErrorPrsnlizedItem" param="orderHasErrorPrsnlizedItem" />
				 </dsp:oparam>
				 </dsp:droplet>
				 <c:if test="${orderHasErrorPrsnlizedItem eq true}">
					<div id="saveForLaterTopMessaging" class="clearfix grid_10 alpha omega">
						<div class="genericRedBox savedItemChangedWrapper clearfix closeWinWrapper marBottom_10 changeStoreItemWrap">
							 <h2 class="heading"><bbbl:label key="lbl_sfl_top_personalized_err_message" language="${pageContext.request.locale.language}" /></h2>
						</div>
					</div>
				</c:if>
	            <div class="clear"></div>
				...
				<div id="saveForLaterBody"
					class="savedItems clearfix grid_10 alpha omega">
					<div id="saveForLaterContent"
						...
						<div class="productsListHeader noBorderBot clearfix" aria-hidden="true" style="display:none">
							<div class="wishlistItemDetails clearfix"><strong><bbbl:label
										key="lbl_sfl_item" language="<c:out param='${language}'/>" /></strong></div>
							<div class="wishlistQuantityDetails clearfix"><strong><bbbl:label
										key="lbl_sfl_quantity" language="<c:out param='${language}'/>" /></strong></div>
							<div class="wishlistTotalDetails clearfix"><strong><bbbl:label
										key="lbl_sfl_total" language="<c:out param='${language}'/>" /></strong></div>
						</div>
						<div class="clear"></div>
						<dsp:include page="../saveForlaterCartForms.jsp" />
						<dsp:form formid="frmSavedItems"
							iclass="frmAjaxSubmit clearfix frmSavedItems" method="post"
							action="../ajax_handler_cart.jsp">
							<ul class="productsListContent clearfix">
								...
								<dsp:droplet name="CertonaDroplet">
									...
									<dsp:oparam name="output">
										<dsp:getvalueof var="pageIdCertona"
											param="certonaResponseVO.pageId" scope="request" />
									</dsp:oparam>
								...
								<dsp:droplet name="/atg/dynamo/droplet/ForEach">
									...
									<dsp:oparam name="output">
										...
										<dsp:getvalueof var="eximErrorExists"
										    param="element.eximErrorExists"/>
										<c:choose>
										...
										<c:if
											test="${currentCount == displaySize+1 && arraySize > displaySize}">
												...
												<input type="hidden" name="ltlFlag" value="${isLtlFlag}"/>
													<div class="button">
														<input id="showAllBtn" type="button"
															value="Show all ${arraySize} items" />
													</div>
												</div>
											</li>
										</c:if>
										...
										<dsp:droplet name="SKUWishlistDetailDroplet">
											...
											<dsp:oparam name="output">
												...
												<dsp:getvalueof var="isIntlRestricted"
													param="pSKUDetailVO.intlRestricted" />
												...
												<dsp:getvalueof var="shipMsgFlag"
										...
										<li>
											<%-- LTL Alert  --%> <c:if
												test="${isLtlFlag && empty ltlShipMethod}">
												<div class="clear"></div>
										...
										<li id="savedItemID_${wishListItemId}"
											class=" ${lastRow} <c:if test="${not empty registryId}">registeryItem</c:if> savedItemRow changeStoreItemWrap clearfix <c:if test="${currentCount > displaySize && arraySize > displaySize}">hidden</c:if>">
											...
											data-dest-class="productId" class="productId"
											data-change-store-submit="prodId" name="productId"> <input
											type="hidden" value="${skuID}" data-dest-class="skuId"
											class="changeStoreSkuId" data-change-store-submit="skuId"
											name="skuId"> <c:if test="${flagOff}">
												<input type="hidden" data-dest-class="itemQuantity"
													class="itemQuantity" data-change-store-submit="itemQuantity"
													value="${quantity}" name="itemQuantity">
											</c:if> <dsp:getvalueof var="productVO" param="element.productVO" />
											<dsp:droplet name="/atg/dynamo/droplet/ForEach">
											...
											</dsp:droplet> <dsp:getvalueof var="prodImage" value="${skuImageURL}" /> <dsp:getvalueof
												var="pName" value="${skuDisplayName}" /> <dsp:include
												page="../itemLinkWishlist.jsp">
												...
												<c:set var="registryUrl"
													value="../giftregistry/view_registry_guest.jsp" />
												<c:if test='${fn:contains(userActiveRegList, registryId)}'>
													<c:set var="registryUrl"
														value="../giftregistry/view_registry_owner.jsp" />
												</c:if>
												...
												<div class="registeryItemHeader clearfix">
													<span><bbbl:label key="lbl_cart_registry_from_text"
											...
											<div class="clear"></div> <dsp:getvalueof
												var="shipMethodUnsupported"
												param="element.shipMethodUnsupported" /> <%-- LTL Alert  for Ship method unsupported--%>
											<c:if test="${isLtlFlag && shipMethodUnsupported}">
												...
												<c:set target="${placeHolderMapServiceLevel}"
													property="ShippingMethodDesc" value="${shippingMethodDesc}" />
												<div class="clear"></div>
											...
											<input type="hidden" name="ltlFlag" value="${isLtlFlag}"/>
													...
													<dsp:droplet name="/atg/repository/seo/CanonicalItemLink">
														...
														<c:choose>
															<c:when test="${not flagOff}">
																...
																<dsp:a iclass="prodImg clearfix block" title="${pName}"
																	page="${url}">
															...
															<c:otherwise>
															<dsp:a iclass="prodImg clearfix block" title="${prodName}" page="${finalUrl}?skuId=${catalogRefId}"> <c:choose>
																		<c:when test="${not empty referenceNumber}">
																			<c:choose>
																				<c:when test="${not empty fullImagePath && !eximErrorExists && enableKatoriFlag}">
																					  <img class="fl productImage" src="${fullImagePath}" alt="${pName}" title="${pName}" height="146" width="146" />
														...
														<c:if test="${empty pName}">
														...
														</c:if> <span class="prodInfo block breakWord"> <span
															class="prodName"> <c:choose>
																	<c:when test="${not flagOff}">
																		<c:choose>
																			<%-- Start :: BPSI-407 | DSK -Remember LTL Service Option if user clicks LTL item from cart page, save for later section--%>
																		...
																		<dsp:a page="${url}" iclass="productName"
																			title="${productVO.name}">${pName}</dsp:a>
																		<%-- end :: BPSI-407 | DSK -Remember LTL Service Option if user clicks LTL item from cart page, save for later section--%>
																	</c:when>
																	<c:otherwise>
																		<span class="productName disableText">${pName}</span>
																	</c:otherwise>
																</c:choose> <dsp:getvalueof var="pName" value="" /> <dsp:getvalueof
																	var="prodImage" value="" />
														</span> <span class="prodAttribsSavedItem"> <c:if
																	test='${not empty skuColor}'>
																	<c:choose>
																	...
																	test='${enableKatoriFlag && not empty personalizationOptions && personalizationOptions != null}'>
																	${eximCustomizationCodesMap[personalizationOptions]} :  ${personalizationDetails}</c:if>
																<c:set var="rollupAttributesFinish" value="false" />
														</span> <c:if
																test="${enableKatoriFlag && not empty referenceNumber && referenceNumber != null}">
																<c:if
																	test="${not empty personalizationOptions && personalizationOptions != null}">
																	<span class="personalizationAttr katoriPrice"> 
																	<%--BBBSL-8154 --%>
																	<%-- <span aria-hidden="true">${personalizationOptionsDisplay}
																	</span>  --%>
															...
															</c:if> <span class="skuNum"><bbbl:label key="lbl_sfl_sku"
																	language="<c:out param='${language}'/>" /> #${skuID}</span>
															...
															<ul class="prodDeliveryInfo squareBulattedList">
																...
																<dsp:droplet name="IsProductSKUShippingDroplet">
																	...
																	<dsp:oparam name="true">
																		...
																		<dsp:getvalueof var="restrictedAttributes"
															...
															</ul> <c:if test="${BazaarVoiceOn}">
																<dsp:getvalueof var="reviews"
																	value="${productVO.bvReviews.totalReviewCount}" />
																<dsp:getvalueof var="ratings"
																	value="${productVO.bvReviews.averageOverallRating}"
																...
																<c:choose>
																	...
																	<c:otherwise>
																		<dsp:getvalueof var="totalReviewCount"
																			value="${productVO.bvReviews.totalReviewCount}"></dsp:getvalueof>
																	</c:otherwise>
																</c:choose>
																<c:if test="${not flagOff}">
																	<c:choose>
																		<c:when
																			test="${ratings ne null && ratings ne '0' && (reviews eq '1' || reviews gt '1') }">
																			<c:choose>
																				<c:when
																					test="${isLtlFlag && not empty ltlShipMethod}">
																			<span
																				class="prodReview clearfix marBottom_10 prodReview<fmt:formatNumber value="${rating}" pattern="#0" />">
																				<a
																							href="${contextPath}${finalUrl}?skuId=${skuID}&categoryId=${CategoryId}&sopt=${ltlShipMethod}&showRatings=true"  role="link" aria-label="${reviews} ${lblReviewsCount} ${lblForThe} ${productVO.name}">(${totalReviewCount}) <bbbl:label
																								key="lbl_sfl_reviews"
																								language="<c:out param='${language}'/>" />
																						</a>
																					</span>
																				...
																				<c:otherwise>
																					<span
																						class="prodReview clearfix marBottom_10 prodReview<fmt:formatNumber value="${rating}" pattern="#0" />">
																						<a
																					href="${contextPath}${finalUrl}?skuId=${skuID}&categoryId=${CategoryId}&showRatings=true" role="link" aria-label="${reviews} ${lblReviewsCount} ${lblForThe} ${productVO.name}">(${totalReviewCount}) <bbbl:label
																								key="lbl_sfl_reviews"
																								language="<c:out param='${language}'/>" />
																						</a>
																					</span>
																			...
																			<c:choose>
																				<c:when
																					...
																					<span
																						class="prodReview clearfix marBottom_10 prodReview<fmt:formatNumber value="${rating}" pattern="#0" />">
																						<a
																							href="${contextPath}${finalUrl}?skuId=${skuID}&sopt=${ltlShipMethod}&showRatings=true" role="link" aria-label="${reviews} ${lblReviewsCount} ${lblForThe} ${productVO.name}">(${totalReviewCount}) <bbbl:label
																						key="lbl_sfl_reviews"
																						language="<c:out param='${language}'/>" />
																				</a>
																			</span>
																		...
																		<c:otherwise>
																			<span
																				class="prodReview clearfix marBottom_10 prodReview<fmt:formatNumber value="${rating}" pattern="#0" />">
																				<a
																					href="${contextPath}${finalUrl}?skuId=${skuID}&showRatings=true" role="link" aria-label="${reviews} ${lblReviewsCount} ${lblForThe} ${productVO.name}">(${totalReviewCount}) <bbbl:label
																						key="lbl_sfl_reviews"
																						language="<c:out param='${language}'/>" />
																				</a>
																			</span>
															...
															<div class="disPriceString"> <bbbl:label key="lbl_discounted_incart_text" language="${pageContext.request.locale.language}" /> </div>
															 ...
															 <c:set var="itemPrice" /> <dsp:droplet
																name="ListPriceSalePriceDroplet">
															...
															</dsp:droplet> <c:choose>
																<c:when
																	test="${(not empty salePriceD) && (salePriceD gt 0.0)}">
															...
															</c:choose> <c:set var="PersonalizedPrice" value="${PersonalPrice}" />
															<c:choose>
																			<c:when test="${not empty referenceNumber}">
																					 <c:choose>
													...
													<c:set var="omnitemPrice">
														<dsp:valueof value="${itemPriceD}"
															converter="unformattedCurrency" />
													</c:set>
													<div class="wishlistQuantityDetails clearfix">
														<table class="prodDeliveryInfo">
															...
															<c:if test="${not flagOff}">
																	...
																	<td>
																		<label id="lblquantity_text_${skuID}"
																			for="quantity_text_${skuID}" class="hidden"><bbbl:label key="lbl_sfl_quantity" language="<c:out param='${language}'/>"/> ${quantity} of ${skuDisplayName} 
																		</label>
																	<div class="spinner">
																		
																		<input name="${wishListItemId}" type="text"
																			value="${quantity}" id="quantity_text_${skuID}"
																			maxlength="2"
																			title="<bbbl:label key='lbl_quantity_input_box' language="${pageContext.request.locale.language}" />"
																			class="moveToCartData itemQuantity fl"
																			aria-required="true"
																			aria-labelledby="lblquantity_text_${skuID}" />
																		<div class="clear"></div>
																		</div>
																		</td>
																	</tr>
															</c:if>
															<dsp:input bean="GiftlistFormHandler.fromCartPage"
																type="hidden" value="true" />
															<dsp:input bean="GiftlistFormHandler.productId"
																type="hidden" value="${prodID}" />
															<dsp:input bean="GiftlistFormHandler.catalogRefIds"
																type="hidden" value="${skuID}" />
															<%-- BBBH-391 | Client DOM XSRF
															<dsp:input
																bean="GiftlistFormHandler.updateGiftlistItemsSuccessURL"
																type="hidden" value="/store/cart/ajax_handler_cart.jsp" />
															<dsp:input
															...
															<dsp:input bean="GiftlistFormHandler.giftlistId"
																value="${giftListId}" type="hidden" />
															<dsp:input bean="GiftlistFormHandler.currentItemId"
																value="${wishListItemId}" type="hidden" />
															<c:if test="${not flagOff}">
																	<tr class="lnkUpdate">
																		<td>
																			<a href="#" class="triggerUpdate" role="link" aria-label="Update quantity ${lblForThe} ${productVO.name}"
																	data-submit-button="btnUpdateSFL0${currentCount}"
																	title="Update Quantity"
																	onclick="updateWishList('${prodID}','${skuID}');"><strong><bbbl:label
																				key="lbl_sfl_update"
																							language="<c:out param='${language}'/>" /></strong>
																			</a>
																		</td>
															...
															<input type="hidden" name="removeGiftitemId"
																class="frmAjaxSubmitData" value="${wishListItemId}" />
															<input type="hidden" name="giftlistId"
																class="frmAjaxSubmitData" value="${giftlistId}" />
																<tr class="lnkRemove">
																	<td>
																		<a href="#" data-ajax-frmID="frmSaveItemRemove"
																class="btnAjaxSubmitSFL" role="link" aria-label="Remove quantity ${lblForThe} ${productVO.name}"
																data-submit-button="btnRemoveSFL0${currentCount}"
																title="Remove Product from Save For Later"
																onclick="removeWishList('${prodID}','${skuID }');"><strong><bbbl:label
																			key="lbl_sfl_remove"
																						language="<c:out param='${language}'/>" /></strong>
																		</a>
																	</td>
														...
														<div class="clear"></div> <dsp:input
															bean="GiftlistFormHandler.updateGiftlistItems"
															name="btnUpdate" id="btnUpdateSFL0${currentCount}"
															type="submit" value="Update" iclass="hidden" />
													</div>

													<div class="wishlistTotalDetails clearfix">
		
													   <div class="clearfix marBottom_5">
														  <div class="prodInfo">
															 <span
																class="padLeft_20 fl prodPrice">
															 ...
															 <span class="prodPrice">
																 <c:choose>
	                                                               <c:when test="${(!eximErrorExists && not empty referenceNumber && enableKatoriFlag) || empty referenceNumber}">
														...
														<c:if test="${isLtlFlag}">
															 <div class="cartShipMethodInfo padBottom_20 padTop_10">
																	<dsp:getvalueof var="ltlShipMethodDesc"
																		param="element.ltlShipMethodDesc" />
																	<dsp:getvalueof var="deliverySurcharge"
																		param="element.deliverySurcharge" />
																	<dsp:getvalueof var="assemblyFee"
																		param="element.assemblyFees" />
																	<c:choose>
																		<c:when
																			test="${ltlShipMethod == null || shipMethodUnsupported}">
																			<dt class="padLeft_20 fl noPadBot">
																				+
																				<bbbl:label key="lbl_cart_delivery_surcharge"
																					language="<c:out param='${language}'/>" />
																				:
																			</dt>
																	...
																	<c:if
																		test="${assemblyFee gt 0.0 && !shipMethodUnsupported}">
															 <div class="cartShipMethodInfo padBottom_35">
																			<dt class="padLeft_20 fl noPadBot">
																				+
																				<bbbl:label key="lbl_cart_assembly_fee"
																					language="<c:out param='${language}'/>" />
																				:
																			</dt>
																			<dd class="fr noPadBot padRight_10">
																				<dsp:valueof value="${assemblyFee}" number="0.00"
																					converter="currency" />
																			</dd>
															 </div>
																	</c:if>
															...
															<dsp:getvalueof id="wishListItemId"
																param="element.wishListItemId" />
															<dsp:getvalueof var="ltlShipMethodToSend"
																param="element.ltlShipMethod" />
															<input type="hidden" name="prodID"
																class="frmAjaxSubmitData" value="${prodID}" />
															<input type="hidden" name="skuID"
																class="frmAjaxSubmitData" value="${skuID}" />
															<input type="hidden" name="cartQuantity"
																class="frmAjaxSubmitData" value="${cartQuantity}" />
															<input type="hidden" name="cartWishListItemId"
																class="frmAjaxSubmitData" value="${wishListItemId}" />
															<input type="hidden" name="iCount"
																class="frmAjaxSubmitData" value="${currentCount}" />
															<input type="hidden" name="registryId"
																class="frmAjaxSubmitData" value="${registryId}" />
															<input type="hidden" name="saveBts"
																class="frmAjaxSubmitData" value="${btsValue}" />
															<input type="hidden" name="shipMethodUnsupported"
																class="frmAjaxSubmitData"
																value="${shipMethodUnsupported}" />
															<input type="hidden" name="refNum"
																class="frmAjaxSubmitData" value="${referenceNumber}" />
														  ...
														  <c:set var="OmniItemPrice">
															 <dsp:valueof value="${Price * quantity}" number="0.00" converter="unformattedCurrency" />
														  </c:set>
															...
															<c:if
																test="${(empty priceMessageVO) or ((not empty priceMessageVO) and (priceMessageVO.inStock))}">
																<c:set var="mveToCart" value="${true}" />
															 <div>
																	<div
																class="button fr marBottom_10 padRight_10 
																<c:if test="${(isInternationalCustomer && isIntlRestricted) or (isPersonalizationIncomplete) or (isInternationalCustomer && isPersonalized) or eximErrorExists
																   or (not empty referenceNumber && !enableKatoriFlag)}">button_disabled</c:if>
																">
																			...
																			<dsp:oparam name="false">
																					...
																					<c:choose>
																						<c:when test="${not empty cusDet}">
																							<input name="addItemToOrder" type="button"
																					data-ajax-frmID="frmSaveForLater"
																					value="MOVE TO CART" aria-label="Move to cart"
																					class="btnAjaxSubmitSFL moveToCart"																					
																					role="button"
																					<c:if test="${(isInternationalCustomer && isIntlRestricted) or (isPersonalizationIncomplete) or (isInternationalCustomer && isPersonalized) or eximErrorExists
													...
													<c:if
																		test="${isInternationalCustomer && isIntlRestricted or (isInternationalCustomer && isPersonalized)}">
																		<div
																			class="notAvailableIntShipMsg saveForLaterIntShipMsg fr padBottom_10 cb clearfix">
																			<bbbl:label key="lbl_cart_intl_restricted_message"
																				language="${pageContext.request.locale.language}" />
																		</div>
																	</c:if>
																	<div class="clear"></div>
															...
															<dsp:getvalueof id="defaultCountry"
																bean="Site.defaultCountry" />
															<c:if
																test="${bopusoff and mapQuestOn and (not mveToCart)}">
													<div>
																	<div
																		class="button fr marBottom_10 padRight_10 <c:if test='${isInternationalCustomer eq true or  not empty referenceNumber && referenceNumber != null}'>disabled ptrEvtNone</c:if> ">
																		<dsp:getvalueof id="id" param="element.wishListItemId" />
																		<input type="hidden" value="${prodID}"
																			data-dest-class="productId" class="productId"
																			data-change-store-submit="productId" name="productId">
																		<input type="hidden" value="${skuID}"
																			data-dest-class="skuId" class="changeStoreSkuId"
																			data-change-store-submit="skuId" name="skuId">
																		<input type="hidden" data-dest-class="itemQuantity"
																			class="itemQuantity"
																			data-change-store-submit="itemQuantity"
																			value="${quantity}" name="itemQuantity"> <input
																			type="hidden" name="count" value="${currentCount}"
																			data-dest-class="count"
																			data-change-store-submit="count" /> <input
																			type="hidden" name="pageURL"
																			value="${contextPath}/cart/ajax_handler_cart.jsp"
																			data-change-store-submit="pageURL" />
																		<c:if test="${not empty registryId}">
																			<input type="hidden" value="${registryId}"
																				data-dest-class="registryId" class="registryId"
																				data-change-store-submit="registryId"
																				name="registryId">
																		</c:if>
																		<input type="hidden" name="check" value="check"
																			data-dest-class="check"
																			data-change-store-submit="check" /> <input
																			type="hidden" name="wishlistItemId" value="${id}"
																			data-dest-class="wishlistItemId"
																			data-change-store-submit="wishlistItemId" />
																		<!--changes for defect 439    -->
																		<c:choose>
																			<c:when test="${isInternationalCustomer eq true}">
																				<input type="button" class="changeStore"
																					value="Find in Store" role="button"
																					disabled="disabled" />
																			</c:when>
																			<c:when test="${not empty referenceNumber && referenceNumber != null}">
		                                                                            <input type="button" class="changeStore" value="Find in Store" role="button" disabled="disabled"/>
																			</c:when>
																			<c:otherwise>
																				<input type="button" class="changeStore"
																					value="Find in Store" role="button" />
													...
													<div><input type="hidden" name="prodID"
													   class="frmAjaxSubmitData" value="${prodID}" />
													<input
													   type="hidden" name="skuID" class="frmAjaxSubmitData" value="${skuID}" />
													...
													<dsp:getvalueof var="appid"
													   bean="Site.id" /> <c:if test="${not flagOff}">
													<dsp:getvalueof var="transient" bean="Profile.transient" />
													<c:choose>
													<c:when test="${transient == 'false'}">
													<c:choose>
													<c:when
													   test="${isLtlFlag && fn:containsIgnoreCase(enableLTLRegForSite,'false')}">
													</c:when>
													<c:otherwise>
													<dsp:droplet name="AddItemToGiftRegistryDroplet">
													<dsp:param name="siteId" value="${appid}" />
													<c:set var="submitAddToRegistryBtn">
													<bbbl:label key='lbl_move_to_registry'
													   language="${pageContext.request.locale.language}"></bbbl:label>
													</c:set>
													<dsp:getvalueof bean="SessionBean.values"
													   var="sessionMapValues" />
													<dsp:getvalueof
													   value="${sessionMapValues.registrySkinnyVOList}"
													   var="registrySkinnyVOList" />
													<dsp:getvalueof var="sizeValue"
													   value="${fn:length(registrySkinnyVOList)}" />
													<c:choose>
													<c:when
													   test="${sizeValue>1 && (isInternationalCustomer || isPersonalizationIncomplete || (isLtlFlag && shipMethodUnsupported)) || (not empty referenceNumber && !enableKatoriFlag)}">
													<div class="fr btnAddToRegistrySel padRight_10">
													<div
													   class="upperCase addToRegistry addToRegistrySel">
													...
													<dsp:param name="array"
													   value="${registrySkinnyVOList}" />
													<dsp:oparam name="output">
													<dsp:param name="futureRegList"
													   param="element" />
													...
													<dsp:getvalueof param="futureRegList.poBoxAddress" var="poBoxAddress"/>
													<option value="${regId}"
													   class="${event_type}" data-poboxflag="${poBoxAddress}" data-notify-reg="true" >
													<dsp:valueof param="element.eventType"></dsp:valueof>
													<dsp:valueof param="element.eventDate"></dsp:valueof>
													</option>
													</dsp:oparam>
													</dsp:droplet>
													<dsp:tagAttribute name="disabled"
													   value="disabled" />
													</dsp:select>
													<c:choose>
													...
													<c:when
													   test="${sizeValue>1 && not isInternationalCustomer}">
													<div class="fr btnAddToRegistrySel padRight_10">
													...
													<input type="hidden" value="" name="alternateNum" class="frmAjaxSubmitData"/>
													<dsp:setvalue
													   bean="GiftRegistryFormHandler.registryId" value="" />
													...
													<input type="hidden" value="${regId}"
													   name="registryId" class="addItemToRegis" />
													<dsp:setvalue
													   bean="GiftRegistryFormHandler.registryId"
													   value="${regId}" />
													<input type="hidden" value="${registryName}"
													   name="registryName"
													   class="addItemToRegis omniRegName" />
													<dsp:setvalue
													   bean="GiftRegistryFormHandler.registryName"
													   value="${registryName}" />
													</dsp:oparam>
													</dsp:droplet>
													<div class="fr addToRegistry">
													<div class="button <c:if test="${isInternationalCustomer or (isPersonalizationIncomplete) or (isLtlFlag && shipMethodUnsupported) or (not empty referenceNumber && !enableKatoriFlag)}">button_disabled</c:if>">
													<input type="hidden" name="reqRegistryId" class="frmAjaxSubmitData omniRegId" value="${regId}" <c:if test='${isInternationalCustomer || isPersonalizationIncomplete || (not empty referenceNumber && !enableKatoriFlag)}'>disabled="disabled"</c:if> />
													<c:choose>
													<c:when test="${not empty referenceNumber && personalizationOptions!=null && not empty personalizationOptions}">
													<input class="btnAjaxSubmitSFL moveToReg" name="123reqRegistryId" type="button" value="Move to Registry" data-ajax-frmID="frmRegSaveForLater" data-poboxflag="${poBoxAddress}" data-notify-reg="true" role="button" <c:if test='${isInternationalCustomer || isPersonalizationIncomplete}'>disabled="disabled"</c:if> aria-label="Move to Registry" />
													</c:when>
													<c:otherwise>
													<input class="btnAjaxSubmitSFL moveToReg" name="123reqRegistryId" type="button" value="Move to Registry" data-ajax-frmID="frmRegSaveForLater" data-poboxflag="${poBoxAddress}" data-notify-reg="true" role="button" <c:if test='${isInternationalCustomer || isPersonalizationIncomplete}'>disabled="disabled"</c:if> aria-label="Move to Registry" />
													</c:otherwise>
