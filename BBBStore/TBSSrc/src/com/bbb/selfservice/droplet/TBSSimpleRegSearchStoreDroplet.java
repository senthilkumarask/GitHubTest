...
public class TBSSimpleRegSearchStoreDroplet extends BBBDynamoServlet {
	/**
	 * 
	 */
	...
	public static final String CANADA_SITE = "TBS_BedBathCanada";
		...
		try {
			...
			if (!BBBUtility.isEmpty(storeId)) {
				...
				if (!BBBUtility.isEmpty(searchType)	&& !BBBUtility.isEmpty(searchString)) {
					...
					if (searchType.equals(SelfServiceConstants.ZIPCODE_BASED_STORE_SEARCH) || searchType.equals(SelfServiceConstants.ADDRESS_BASED_STORE_SEARCH)) {
						if(searchType.equals(SelfServiceConstants.ZIPCODE_BASED_STORE_SEARCH)){
							...
							logDebug(new StringBuilder(logMessage).append(" Store Search based on address. . ").append(originalSearchString).toString());
						}
					...
					if (null != objStoreDetailsWrapper) {
						if (objStoreDetailsWrapper.getStoreDetails() != null && !objStoreDetailsWrapper.getStoreDetails().isEmpty()) {
							...
							if (null != objStoreDetailsWrapper && null != objStoreDetailsWrapper.getStoreDetails()) {
