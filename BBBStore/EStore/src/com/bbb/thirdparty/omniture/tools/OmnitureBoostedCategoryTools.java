...
/**
 * Implements methods used in omniture report API call for L2 category boosting
 * processing
 * 
 * @author Sapient
 *
 */
public class OmnitureBoostedCategoryTools extends OmnitureReportAPIToolsImpl {
	
	String CLS_NAME = "OmnitureBoostedCategoryTools";
	/**
	...
	public OmnitureQueueRequestVO prepareInputForQueueRequest(QueueRequestVO queueRequestVO) throws BBBSystemException, BBBBusinessException {
		...
		String searchKeywordString = getBbbCatalogTools().getConfigKeyValue(BBBCoreConstants.OMNITURE_BOOSTING, BBBCoreConstants.L2_BOOSTING_KEYWORDS_TO_OMIT, BBBCoreConstants.BLANK);
		...
		//int noOfTopSearchedItems = getBbbCatalogTools().getValueForConfigKey(BBBCoreConstants.OMNITURE_BOOSTING, BBBCoreConstants.L2_BOOSTING_TOP_SEARCHED_TERMS, BBBCoreConstants.FOUR);
		int noOfTopSearchedItems=queueRequestVO.getOmnitureReportConfigVo().getProductCount();
		String classificationCategory = getBbbCatalogTools().getConfigKeyValue(BBBCoreConstants.OMNITURE_BOOSTING, BBBCoreConstants.L2_BOOSTING_SEARCH_CLASSIFICATION, BBBCoreConstants.BLANK);
				
		elementVO.setClassification(classificationCategory);
		...
		// Setting Metric ID
		String omnitureReportMetricId = getBbbCatalogTools().getConfigKeyValue(BBBCoreConstants.OMNITURE_BOOSTING, BBBCoreConstants.L2_BOOSTING_REPORT_METRIC_ID, BBBCoreConstants.BLANK);
		...
		reportDescription.setSortBy(omnitureReportMetricId);
