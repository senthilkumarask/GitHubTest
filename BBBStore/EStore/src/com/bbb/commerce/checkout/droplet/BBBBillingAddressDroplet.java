 ...
 * @version 1.0
 */
public class BBBBillingAddressDroplet extends BBBDynamoServlet {

	private BBBCheckoutManager mCheckoutMgr;
	private BBBShippingGroupManager mShippingGroupManager;
	private BBBAddressAPI mAddressAPI;
	private BBBCatalogTools catalogTools;
	private Entry element;
	public static final String FROM_SPC="fromSPC";

	
	public BBBAddressAPI getAddressAPI() {
		return mAddressAPI;
	}

	public void setAddressAPI(BBBAddressAPI pAddressAPI) {
		this.mAddressAPI = pAddressAPI;
	}
	 ...
	 * @param mCheckoutMgr
	 */
	public void setCheckoutMgr(final BBBCheckoutManager checkoutMgr) {
		this.mCheckoutMgr = checkoutMgr;
	}
	...
	public void service(final DynamoHttpServletRequest pRequest,
		...
		//PS-65358 | addressing billing issue
		 String fromSPC = pRequest.getParameter(FROM_SPC);

		if (order != null && null == order.getBillingAddress()) {
		...
		if (isUserAuthenticated(profile)) {
			try {
				...
				if (null != getAddressAPI()) {
					...
					if(null != defaultProfileAddress && !extrachIsNullAddress(defaultProfileAddress) && StringUtils.isBlank(selectedAddrKey) ){
	...
	protected boolean extrachIsNullAddress(BBBAddress defaultProfileAddress) {
		return BBBAddressTools.isNullAddress((Address)defaultProfileAddress);
	}
