 ...
 *
 */
public class SendWeeklyRecommEmailScheduler extends
	...
	public void doScheduledTask(Scheduler paramScheduler,
		...
		logDebug("SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate);
		if(!isEnabled()) {
			logDebug("SendWeeklyRecommEmailScheduler is disabled. Please enable the scheduler "+schedulerStartDate);
			return;
		}
		BBBPerformanceMonitor.start("SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate);
		try {
			getGiftRegistryTools().refreshMattView(BBBGiftRegistryConstants.EMAIL_OPT_WEEKLY);
			getGiftRegistryTools().refreshRepoItemCache(BBBGiftRegistryConstants.RECOMMENDATION_EMAIL_WEEKLY);
			boolean breakLoop=false;
			int counter=0;
			while(!breakLoop) {
				RepositoryItem[] recomEmails = getGiftRegistryTools().fetchRecomEmail(counter*getBatchSize(),getBatchSize(),
						BBBGiftRegistryConstants.RECOMMENDATION_EMAIL_WEEKLY);
				if(null == recomEmails) {
					logDebug("No Recommendations for SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate);
					breakLoop = true;
					break;
				}
				else {
					logDebug("Total Recommendations:"+recomEmails.length +" for SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate);
			...
			getGiftRegistryTools().logBatchJobStatus(BBBGiftRegistryConstants.EMAIL_OPT_WEEKLY,schedulerStartDate);
		} catch (BBBSystemException e) {
			logError("SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate+" Failed",e);
		} catch (BBBBusinessException e) {
			logError("SendWeeklyRecommEmailScheduler Started at "+schedulerStartDate+" Failed",e);
		} catch (SQLException e) {
			if (e.getMessage() != null
					&& e.getMessage().contains("Registry not found")) {
				logInfo("SendWeeklyRecommEmailScheduler doScheduledTask started :: Registry not found");
			} else {
				logError("SendWeeklyRecommEmailScheduler Started at "
						+ schedulerStartDate + " Failed", e);
			}
		}
		BBBPerformanceMonitor.end("SendWeeklyRecommEmailScheduler Ended");
	}
	...
	public void sendRegistrantEmail(RepositoryItem[] dailyRecomEmails) throws BBBSystemException {
		...
		for(RepositoryItem dailyRecomEmail : dailyRecomEmails) {
			...
			req.setParameter(BBBGiftRegistryConstants.EVENT_TYPE_CONFIGURABLE, BBBGiftRegistryConstants.SCHEDULED_RECOMMENDATION_EMAIL_FREQ_WEEKLY);
	...
	public void executeDoScheduledTask() {
		...
		doScheduledTask(null,new ScheduledJob("SendWeeklyRecommEmailScheduler", "", "", null, null, false));
