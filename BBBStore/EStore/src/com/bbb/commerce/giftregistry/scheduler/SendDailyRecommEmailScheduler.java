 ...
 *
 */
public class SendDailyRecommEmailScheduler extends
	...
	public void doScheduledTask(Scheduler paramScheduler,
		...
		logDebug("SendDailyRecommEmailScheduler Started at "+schedulerStartDate);
		if(!isEnabled()) {
			logDebug("SendWeeklyRecommEmailScheduler is disabled. Please enable the scheduler "+schedulerStartDate);
			return;
		}
		BBBPerformanceMonitor.start("SendDailyRecommEmailScheduler Started at "+schedulerStartDate);
		try {
			getGiftRegistryTools().refreshMattView(BBBGiftRegistryConstants.EMAIL_OPT_DAILY);
			getGiftRegistryTools().refreshRepoItemCache(BBBGiftRegistryConstants.RECOMMENDATION_EMAIL_DAILY);
			boolean breakLoop=false;
			int counter=0;
			while(!breakLoop) {
				RepositoryItem[] recomEmails = getGiftRegistryTools().fetchRecomEmail(counter*getBatchSize(),getBatchSize(),
						BBBGiftRegistryConstants.RECOMMENDATION_EMAIL_DAILY);
				if(null == recomEmails) {
					breakLoop = true;
					break;
				}
			...
			getGiftRegistryTools().logBatchJobStatus(BBBGiftRegistryConstants.EMAIL_OPT_DAILY,schedulerStartDate);
		} catch (BBBSystemException e) {
			logError("SendDailyRecommEmailScheduler Started at "+schedulerStartDate+" Failed",e);
		} catch (BBBBusinessException e) {
			logError("SendDailyRecommEmailScheduler Started at "+schedulerStartDate+" Failed",e);
		} catch (SQLException e) {
			if (e.getMessage() != null
					&& e.getMessage().contains("Registry not found")) {
				logInfo("SendDailyRecommEmailScheduler doScheduledTask started :: Registry not found");
				
			} else {
				logError("SendDailyRecommEmailScheduler Started at "
						+ schedulerStartDate + " Failed", e);
			}
		}
		BBBPerformanceMonitor.end("SendDailyRecommEmailScheduler Ended");
	}
	
	/**
	 * This method is used to generate email for registraints.
	 * @param dailyRecomEmails
	 * @throws BBBSystemException
	 */
	public void sendRegistrantEmail(RepositoryItem[] dailyRecomEmails) throws BBBSystemException {
		logDebug("Entry sendRegistrantEmail");
		...
		for(RepositoryItem dailyRecomEmail : dailyRecomEmails) {
			...
			req.setParameter(BBBGiftRegistryConstants.EVENT_TYPE_CONFIGURABLE, BBBGiftRegistryConstants.SCHEDULED_RECOMMENDATION_EMAIL_FREQ_DAILY);
	...
	public void executeDoScheduledTask() {
		...
		doScheduledTask(null,new ScheduledJob("SendDailyRecommEmailScheduler", "", "", null, null, false));
