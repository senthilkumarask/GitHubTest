...
public class ProcSaveTBSShippingInfo extends SavedProperties implements PipelineProcessor{
	...
	public int runProcess(Object pParamObject,
		...
		for (ShippingGroup shipGrp = null; iter.hasNext(); ) {
			shipGrp = (ShippingGroup) iter.next();

			if (shipGrp instanceof BBBHardGoodShippingGroup) {
				BBBHardGoodShippingGroup bbbShipGrp = (BBBHardGoodShippingGroup) shipGrp;
				
				
				TBSShippingInfo tbsShipInfoInfo = bbbShipGrp.getTbsShipInfo();
				if(tbsShipInfoInfo != null){
					if (getSaveChangedPropertiesOnly()
							&& tbsShipInfoInfo instanceof ChangedProperties
							&& (! ((ChangedProperties) tbsShipInfoInfo).getSaveAllProperties())) {
						savedProperties =
								((ChangedProperties) tbsShipInfoInfo).getChangedProperties().toArray();
					}
					else {
					...
					mutItem = null;
					if (tbsShipInfoInfo instanceof ChangedProperties)
						mutItem = ((ChangedProperties) tbsShipInfoInfo).getRepositoryItem();

					if (mutItem == null) {
						mutItem = mutRep.getItemForUpdate(tbsShipInfoInfo.getId(),
								orderTools.getMappedItemDescriptorName(tbsShipInfoInfo.getClass().getName()));
						if (tbsShipInfoInfo instanceof ChangedProperties)
							((ChangedProperties) tbsShipInfoInfo).setRepositoryItem(mutItem);
					}
					...
					for (int i = 0; i < savedProperties.length; i++) {
						...
						if (! OrderRepositoryUtils.hasProperty(order, tbsShipInfoInfo, mappedPropName))
							continue;

						try {
							value = OrderRepositoryUtils.getPropertyValue(order, tbsShipInfoInfo, mappedPropName);
						}
						...
						if (isLoggingDebug())
							logDebug("save property[" + (String) savedProperties[i] + ":" + value + ":" +
									tbsShipInfoInfo.getClass().getName() + ":" + tbsShipInfoInfo.getId() + "]");
					...
					if (tbsShipInfoInfo instanceof ChangedProperties) {
						ChangedProperties cp = (ChangedProperties) tbsShipInfoInfo;
						if (cp.isChanged())
							changed = true;
