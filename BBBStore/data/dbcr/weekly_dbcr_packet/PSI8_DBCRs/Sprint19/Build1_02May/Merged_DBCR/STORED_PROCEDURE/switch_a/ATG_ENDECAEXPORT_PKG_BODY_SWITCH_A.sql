...
PROCEDURE Export_Category_Products ( p_filename varchar2, p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  BEGIN
	  ...
	  useOldCatProductSeq:='TRUE';
       ...
       select count(*) into nCountConf from BBB_SWITCH_A.BBB_CONFIG_KEY_VALUE where config_key ='IS_USE_OLD_CAT_PRODUCT_SEQ';
     IF(nCountConf > 0)THEN
        select config_value into useOldCatProductSeq from BBB_SWITCH_A.BBB_CONFIG_KEY_VALUE where config_key ='IS_USE_OLD_CAT_PRODUCT_SEQ';
...
--Search Config
  ...
  query:='SELECT DISTINCT ''*#*'',GRP1.GROUP_NAME AS GROUP_NAME,PROP_ITM1.PROPERTY_NAME,SRCH_ITM1.PRIORITY AS PRIORITY
			FROM BBB_SWITCH_A.BBB_SEARCH_CONFIG_GROUPS GRP1, BBB_SWITCH_A.BBB_SEARCH_GROUP_ITEMS GRP_ITEM1, BBB_SWITCH_A.BBB_SEARCHABLE_ITEM SRCH_ITM1,
			BBB_SWITCH_A.BBB_SEARCH_PROPERTY_ITEM PROP_ITM1, BBB_SWITCH_A.BBB_SEARCH_PROPERTY_REF_ITEM PROP_REF_ITM1 WHERE GRP_ITEM1.GROUP_ID=GRP1.GROUP_ID 
			...
			FROM BBB_SWITCH_A.BBB_SEARCH_CONFIG_GROUPS GRP2, BBB_SWITCH_A.BBB_FACET_TYPES_VER FCT_TYP1, BBB_SWITCH_A.BBB_SEARCHABLE_ITEM SRCH_ITM2, BBB_CORE.BBB_FACET_TYPES FCT_TYPE,
			BBB_SWITCH_A.BBB_SEARCH_GROUP_ITEMS SRCH_GRP2 WHERE GRP2.GROUP_ID=SRCH_GRP2.GROUP_ID AND SRCH_GRP2.SEARCH_ITEM_ID=SRCH_ITM2.SEARCH_ITEM_ATTR_ID and
...
EXCEPTION
  ...
  query:='SELECT DISTINCT ''*#*'',GRP1.GROUP_NAME,GRP1.GROUP_RELRANK AS RELRANK
			FROM BBB_SWITCH_A.BBB_SEARCH_CONFIG_GROUPS GRP1'; 
...
-- Remove Procedure export_store_only.  RSP 2/24/2016
...
--        -- New RSP 11/17/2014
--        nvl((
--            select regexp_substr(translation_id, '.{1}$') from bbb_switch_a.bbb_sku_site_translations st
...
is
  ...
  CURSOR cur_ss is
    select category_id, rank from BBB_CORE.BBB_DIM_ORDER
