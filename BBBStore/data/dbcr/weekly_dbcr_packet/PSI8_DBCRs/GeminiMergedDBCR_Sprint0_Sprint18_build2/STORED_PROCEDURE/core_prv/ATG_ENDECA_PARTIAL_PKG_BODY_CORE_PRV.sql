...
PROCEDURE Export_Controller (p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  BEGIN
        ...
        ATG_EndecaExport_Pkg.export_store_only('MIESkuProduct.txt', p_lastModifiedDate, p_feedType); -- New RSP 8/27/2014
...
BEGIN
    ...
    WHEN OTHERS THEN  -- handles all other errors
          ...
          v_message := RTRIM(SUBSTR(SQLERRM||' '||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE||' Export_Full_Controller: FATAL ERROR. Please check logs for SPs getting executed in sequence.',1,512));
		  v_message_type :='I';
		  ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
