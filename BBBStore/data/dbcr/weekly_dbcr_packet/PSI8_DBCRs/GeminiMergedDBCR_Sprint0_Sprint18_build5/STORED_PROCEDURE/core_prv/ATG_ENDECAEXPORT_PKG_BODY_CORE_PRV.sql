...
--------------------------------------------------------
CREATE OR REPLACE PACKAGE BODY BBB_CORE_PRV.ATG_ENDECAEXPORT_PKG AS
...
dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_PRODUCTS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_Colleges_States(
  ...
  endeca_query:='select distinct(a.school_ver_id),b.school_name,a.college_logo,b.state,c.descrip from BBB_SCHOOLS_VER a,BBB_CORE_PRV.BBB_SCHOOLS b, bbb_states c         
  ...
  BEGIN
   var_num1:= 69999;
    ...
    v_process_sub_cd:='Exp_Col_Tax';
      ...
      endeca_query:='select 69999+rownum,''69999'',b.school_name,a.college_logo from BBB_SCHOOLS_VER a,BBB_CORE_PRV.BBB_SCHOOLS b
...
PROCEDURE Export_Features(
  ...
  endeca_query:='SELECT FACET_ID,DESCRIPTION from BBB_CORE_PRV.BBB_FACET_TYPES';
     v_message :='Export_Features: '||endeca_query;
     Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
...
PROCEDURE Export_SKU_Features(
  ...
  BEGIN
      ...
      endeca_query:='SELECT ITEM_ID, FACET_ID, FACET_VALUE, FACET_VALUE_ID from BBB_CORE_PRV.BBB_FACET_SKU';
...
PROCEDURE Export_Product_Item_Attributes(
  ...
  BEGIN
   ...
   IF(nCount > 0)THEN
    execute immediate 'truncate table BBB_CORE_PRV.ENDECA_PRODUCT_ITEM_ATTR'; 
   END IF;  
	 
      select count(*) into nCountConfig from BBB_CORE_PRV.BBB_CONFIG_KEY_VALUE where config_key ='EXTRACT_PRD_ATTR_SITES_DATA';
     IF(nCountConfig > 0)THEN
        select config_value into useAttrSitesTbl from BBB_CORE_PRV.BBB_CONFIG_KEY_VALUE where config_key ='EXTRACT_PRD_ATTR_SITES_DATA';
   ...
   ELSIF (useAttrSitesTbl='false' or useAttrSitesTbl='FALSE')THEN
        ...
        insert into BBB_CORE_PRV.ENDECA_PRODUCT_ITEM_ATTR (product_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, 
      ...
      dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_PRODUCT_ITEM_ATTR', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);

	endeca_query :='SELECT distinct A.PRODUCT_ID,A.SKU_ATTRIBUTE_ID,A.SITE_ID,A.START_DATE,A.END_DATE,A.MISC_INFO  from BBB_CORE_PRV.ENDECA_PRODUCT_ITEM_ATTR A';
        IF p_feedType   = 'PARTIAL' THEN
        endeca_query := 'SELECT distinct A.PRODUCT_ID,A.SKU_ATTRIBUTE_ID,A.SITE_ID,A.START_DATE,A.END_DATE,A.MISC_INFO  from BBB_CORE_PRV.ENDECA_PRODUCT_ITEM_ATTR A, BBB_CORE_PRV.ENDECA_EXPORT_PRODUCTS E  
...
PROCEDURE Export_Reviews(
  ...
  BEGIN
      ...
      Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
  endeca_query:='SELECT PRODUCT_ID,AVERAGE_OVERALL_RATING,TOTAL_REVIEW_COUNT,SITE_ID FROM BBB_CORE_PRV.BBB_BAZAAR_VOICE';
...
PROCEDURE Export_Collection_SKUs(
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_COLLECTION_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_EXIM_Customization_Code (
 ...
 endeca_query:='select customization_code ,description from BBB_CORE_PRV.BBB_EXIM_CUSTOMIZATION_CODES';
...
--Lokesh insert collection stats here ENDECA_EXPORT_PROD_SKUS
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_PROD_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, BBB_SCHOOLS f,bbb_prd_keywords g
...
PROCEDURE Export_Accessories(
  ...
  'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.INTL_RESTRICTED,b.email_out_of_stock,            
            ...
            case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id,g.keywords_string
from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, BBB_CORE_PRV.BBB_SCHOOLS f, bbb_prd_keywords g            
            where a.product_id = b.product_id and a.product_id=c.product_id(+)
...
PROCEDURE Export_Lead_Products(
  ...
  'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.INTL_RESTRICTED,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,      
        ...
        anywhere_zoom, case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end,
        case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id, g.keywords_string
        from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, BBB_CORE_PRV.BBB_SCHOOLS f, bbb_prd_keywords g 
...
and b.college_id=f.school_id(+) and a.product_id=g.product_id(+) and b.lead_prd_flag=''1'' '
  ...
  IF p_feedType  = 'PARTIAL' THEN
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.INTL_RESTRICTED,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom, case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id, k.keywords_string from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, BBB_CORE_PRV.BBB_SCHOOLS f, dcs_prd_chldsku g, bbb_sku h, dcs_cat_chldprd i, bbb_category j, bbb_prd_keywords k where a.product_id = b.product_id and a.product_id = r.product_id and a.product_id=g.product_id(+) and g.sku_id=h.sku_id(+) and a.product_id=c.product_id(+) and a.product_id=d.product_id(+) and a.product_id=i.child_prd_id (+) and a.product_id = k.product_id(+) and i.category_id = j.category_id and b.college_id=f.school_id(+) and b.lead_prd_flag=''1''  and ( b.last_mod_date > to_date('''
    || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') or h.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')  or j.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') ) ' ;
  END IF;
...
and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists(select 1 from dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id) '
  ...
  IF p_feedType  = 'PARTIAL' THEN
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.INTL_RESTRICTED,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id, k.keywords_string from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, BBB_CORE_PRV.BBB_SCHOOLS f, dcs_prd_chldsku g, bbb_sku h, dcs_cat_chldprd i, bbb_category j, bbb_prd_keywords k where a.product_id = b.product_id and a.product_id=c.product_id(+) and a.product_id=d.product_id(+)  and a.product_id=g.product_id(+) and g.sku_id=h.sku_id(+) and a.product_id = k.product_id(+) and a.product_id=i.child_prd_id (+) and i.category_id = j.category_id and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists (select 1 from dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id)  and ( b.last_mod_date > to_date('''
    || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') or h.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')  or j.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') ) ' ;
  END IF;
...
and b.college_id=f.school_id(+) and b.collection_flag=''1'' '
  ...
  IF p_feedType = 'PARTIAL' THEN
		...
		Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.INTL_RESTRICTED,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id, m.keywords_string from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, BBB_CORE_PRV.BBB_SCHOOLS f, dcs_price h, dcs_price i, bbb_price h1, dcs_cat_chldprd k, bbb_category l, bbb_prd_keywords m where a.product_id = b.product_id  and a.product_id = r.product_id(+)   and a.product_id=c.product_id(+)  and a.product_id=d.product_id(+)   and b.sku_low_price = h.sku_id (+)  and b.sku_high_price = i.sku_id (+)  and h.price_id = h1.price_id and b.college_id=f.school_id(+) and a.product_id = m.product_id(+) and a.product_id=k.child_prd_id (+) and k.category_id = l.category_id (+) and b.collection_flag=''1''  and ( b.last_mod_date > to_date('''
...
PROCEDURE Export_SKU_Item_Attributes(
  ...
  BEGIN
	...
	select count(*) into nCount from all_tables where table_name ='ENDECA_SKU_ITEM_ATTR';
	 IF(nCount > 0)THEN
		execute immediate 'truncate table BBB_CORE_PRV.ENDECA_SKU_ITEM_ATTR'; 
	 END IF;
	
	-- Insert site specific data into temporary Table Start	  
	insert into BBB_CORE_PRV.ENDECA_SKU_ITEM_ATTR (sku_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, MISC_INFO)
...
--Lokesh insert collection stats here ENDECA_SKU_ITEM_ATTR
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_SKU_ITEM_ATTR', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);

  endeca_query   :=	'select sku_id, sku_attribute_id, site_id, start_date, end_date, misc_info from BBB_CORE_PRV.ENDECA_SKU_ITEM_ATTR';
  If P_Feedtype   = 'PARTIAL' Then
    endeca_query :=	'select a.sku_id, a.sku_attribute_id, a.site_id, a.start_date, a.end_date, a.misc_info from BBB_CORE_PRV.ENDECA_SKU_ITEM_ATTR a, BBB_CORE_PRV.BBB_SKU d ,
					endeca_export_skus es where a.sku_id = es.sku_id and a.sku_id=d.sku_id';
  END IF;
...
PROCEDURE Export_Category_Products ( p_filename varchar2, p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  BEGIN
	  ...
	  useOldCatProductSeq:='TRUE';
       ...
       select count(*) into nCountConf from BBB_CORE_PRV.BBB_CONFIG_KEY_VALUE where config_key ='IS_USE_OLD_CAT_PRODUCT_SEQ';
     IF(nCountConf > 0)THEN
        select config_value into useOldCatProductSeq from BBB_CORE_PRV.BBB_CONFIG_KEY_VALUE where config_key ='IS_USE_OLD_CAT_PRODUCT_SEQ';
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_CAT_PRODS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_Prices(
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_PRICES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
--Lokesh  insert collection stats here ENDECA_EXPORT_PROD_PROPERTIES
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_PROD_PROPERTIES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
-- Lokesh Insert collection stats here ENDECA_EXPT_SKU_SITE_PROPTS
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPT_SKU_SITE_PROPTS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
EXCEPTION
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_PROD_SITE_RELATION', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);

	
	endeca_query:='select distinct ''*#*'', eps.product_id, nvl(bsd.UNIT_SALES,''0''), nvl(bsd.ORDER_SALES,''0''), nvl(bsd.TOTAL_SALES,''0''), eps.site_id from endeca_prod_site_relation eps, BBB_CORE_PRV.BBB_SALES_DATA bsd where eps.product_id = bsd.product_id(+) and eps.site_id = bsd.site_id(+) order by eps.product_id';
	
	select count(*) into nTotal_Sales_Rows_Count from BBB_CORE_PRV.BBB_SALES_DATA bsd where bsd.total_sales > 0;     
...
-- Lokesh insert collection stats here ENDECA_EXPORT_PRICES_UPDATES	
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPORT_PRICES_UPDATES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
--Search Config
  ...
  query:='SELECT DISTINCT ''*#*'',GRP1.GROUP_NAME AS GROUP_NAME,PROP_ITM1.PROPERTY_NAME,SRCH_ITM1.PRIORITY AS PRIORITY
			FROM BBB_CORE_PRV.BBB_SEARCH_CONFIG_GROUPS GRP1, BBB_CORE_PRV.BBB_SEARCH_GROUP_ITEMS GRP_ITEM1, BBB_CORE_PRV.BBB_SEARCHABLE_ITEM SRCH_ITM1,
			BBB_CORE_PRV.BBB_SEARCH_PROPERTY_ITEM PROP_ITM1, BBB_CORE_PRV.BBB_SEARCH_PROPERTY_REF_ITEM PROP_REF_ITM1 WHERE GRP_ITEM1.GROUP_ID=GRP1.GROUP_ID 
			...
			FROM BBB_CORE_PRV.BBB_SEARCH_CONFIG_GROUPS GRP2, BBB_CORE_PRV.BBB_FACET_TYPES_VER FCT_TYP1, BBB_CORE_PRV.BBB_SEARCHABLE_ITEM SRCH_ITM2, BBB_CORE_PRV.BBB_FACET_TYPES FCT_TYPE,
			BBB_CORE_PRV.BBB_SEARCH_GROUP_ITEMS SRCH_GRP2 WHERE GRP2.GROUP_ID=SRCH_GRP2.GROUP_ID AND SRCH_GRP2.SEARCH_ITEM_ID=SRCH_ITM2.SEARCH_ITEM_ATTR_ID and
...
EXCEPTION
  ...
  query:='SELECT DISTINCT ''*#*'',GRP1.GROUP_NAME,GRP1.GROUP_RELRANK AS RELRANK
			FROM BBB_CORE_PRV.BBB_SEARCH_CONFIG_GROUPS GRP1'; 
...
-- Remove export_store_only 3/2/2016
...
----         bbb_switch_a.GET_PRODUCT_ID(bs.SKU_ID) as MIE_PRODUCT_ID,
...
----   from bbb_switch_a.bbb_sku bs inner join bbb_switch_a.dcs_sku ds
----   on bs.SKU_ID = ds.SKU_ID
----   left outer join bbb_bbb_switch_a.item_excluded i on
----   i.SKU = bs.SKU_ID
...
--         ds.DISPLAY_NAME as MIE_DISPLAY_NAME,
--         ds.DESCRIPTION as MIE_DESCRIPTION,
--         BBB_CORE_PRV.GET_PRODUCT_ID(bs.SKU_ID) as MIE_PRODUCT_ID,
--         --i.SITE_FLAG as MIE_ITEM_EXCLUDE_SITE_FLAG,
--         BBB_CORE_PRV.GET_ITEM_EXCLUDED_SITE_FLAG(bs.SKU_ID) as MIE_ITEM_EXCL_SITE_FLAG_RES_CD ,
--         'sku_'||bs.SKU_ID as MIE_SKU_PRODUCT_ID,
--         bs.IS_STORE_SKU as MIE_IS_STORE_SKU,
...
--        -- New RSP 11/17/2014
--        nvl((
--            select regexp_substr(translation_id, '.{1}$') from BBB_CORE_PRV.bbb_sku_site_translations st
...
--   from BBB_CORE_PRV.bbb_sku bs inner join BBB_CORE_PRV.dcs_sku ds
--   on bs.SKU_ID = ds.SKU_ID
...
--begin
--    IF p_feedtype = 'PARTIAL' THEN
--        v_dir := 'ATG_APP_PARTIAL_STAGING';
--        ELSE
--        v_dir:= 'ATG_APP_FULL_STAGING';
--    END IF;
...
-- New Procedure export_category_rank  RSP 2/24/2016
  procedure export_category_rank(p_filename varchar2)
/*
    New Procedure to create a feed file of Category Ranks.  There will only be FULL feeds, no PARTIAL.
    
    Modification History: 
    Programmer  Date        Modification
    RSP         03/03/2016  Subract the Rank value from 1000 to get the actual rank.  If rank is >= 1000 then make it 1.
...
is
  ...
  l_cnt         NUMBER DEFAULT 0;
  v_dir         VARCHAR2(40);
  l_headr       varchar2(100) := 'CATEGORY_ID|RANK';
  v_total       number := 0;

  -- Error Handling
  v_process_cd    BBB_PIM_STG.process_support.process_cd%TYPE := 'ATG_ENDECA';
  v_process_sub_cd  BBB_PIM_STG.process_support.process_sub_cd%TYPE := 'exp_cat_rank';
  v_start_dt   DATE                     := SYSDATE;
  v_end_dt     DATE                     := NULL;
  v_err_dt     date      := NULL;
  v_err_cd     number(10);
  ...
  v_text       VARCHAR2(1000);
  v_partial_dt date;
  max_rank  number(4) := 1000;
  rank_num  number;

  -- Rank cannot be null and must contain only numerals, no characters.
  CURSOR cur_ss is
    select category_id, rank from BBB_CORE_PRV.BBB_DIM_ORDER
    where rank is not null and not REGEXP_LIKE (category_id, '[^0-9]') and not REGEXP_LIKE (rank, '[^0-9]')
...
begin
    ...
    l_output := utl_file.fopen( v_dir, p_filename, 'w', 32767);
    --  utl_file.put( l_output, l_headr );
    UTL_FILE.PUT_LINE(l_output, l_headr);
    ...
    LOOP
      ...
      rank_num := to_number(rec_ss.rank);

      if rank_num >= 1000 then
        rec_ss.rank := '1';
      else
        rec_ss.rank := to_char(max_rank - rank_num);
      end if;


      v_text := rec_ss.category_id                              ||'|'
            ||rec_ss.rank;
...
exception
  ...
  WHEN OTHERS THEN
        ...
        v_err_msg := RTRIM(SUBSTR(SQLERRM||' '||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE,1,200));
        v_event_dt := SYSDATE;
        v_message := SUBSTR('Error in ' || v_process_cd ||' Category ID: ' || rec_ss.category_id || ' Rank - ' || rec_ss.rank || ' - Error Code:' ||  v_err_cd || ' - Error Msg: ' || v_err_msg, 1, 512);
...
BEGIN
  IF p_dir = 'PARTIAL' THEN
    l_dir := 'ATG_APP_PARTIAL_STAGING';
    ELSE 
    l_dir:= 'ATG_APP_FULL_STAGING';
    END IF;
