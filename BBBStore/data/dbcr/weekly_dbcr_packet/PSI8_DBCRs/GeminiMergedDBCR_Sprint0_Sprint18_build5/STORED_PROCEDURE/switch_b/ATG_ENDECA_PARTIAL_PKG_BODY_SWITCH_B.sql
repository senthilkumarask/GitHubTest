...
SET SERVEROUTPUT ON;
  ...
  CREATE OR REPLACE PACKAGE BODY BBB_SWITCH_B.ATG_ENDECAEXPORT_PARTIAL_PKG 
AS
...
PROCEDURE Export_Controller (p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  BEGIN
        ...
        if rowcount_cat > 0 then
            ...
            v_message := 'Export_Controller: Category Products: From BBB_CATEGORY TABLE : START';
            ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
            ATG_EndecaExport_Pkg.Export_Category_Products ('Products_Category.txt', p_lastModifiedDate, p_feedType);
            v_message := 'Export_Controller: Category Products: From BBB_CATEGORY TABLE : END';
            ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
        end if;
        ...
        if rowcount_cat > 0 then
           ...
           --If it called for category no need to call it again
            ...
            v_cat_prod_relation :=1;
            --category product relation
            v_message := 'Export_Controller: collection product price changes : START';
            ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
            ATG_EndecaExport_Pkg.Export_Category_Products ('Products_Category.txt', p_lastModifiedDate, p_feedType);
            v_message := 'Export_Controller: collection product price changes : END';
            ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
            end if;
        end if;
        ...
        if rowcount_prod > 0 then
            ...
            if v_cat_prod_relation=0 then
                ...
                v_message := 'Export_Controller: Category Products: From BBB_PRODUCT TABLE : START';
                ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
                ATG_EndecaExport_Pkg.Export_Category_Products ('Products_Category.txt', p_lastModifiedDate, p_feedType);
                v_message := 'Export_Controller: Category Products: From BBB_PRODUCT TABLE : END';
                ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
            end if;
        ...
        -- Remove export_store_only. RSP 3/1/2016
        -- Remove export_store_only. RSP 3/1/2016
        --ATG_EndecaExport_Pkg.export_store_only('MIESkuProduct.txt', sysdate, p_feedType); -- New RSP 8/27/2014
        -- export_category_rank is only executed in the full so does not need date or feed_type.  RSP 3/2/2016
        -- ATG_EndecaExport_Pkg.export_category_rank('bedbathbeyond_category_dval_ranks.txt');
        --mandatory file for partial generation: As per Divakar's discussion with Alan
...
BEGIN
    ...
    IF p_feedType = 'FULL' THEN
        ...
        THEN
			...
			ATG_EndecaExport_Pkg.Export_Products_And_Skus (sysdate, p_feedType); -- in full
            ATG_EndecaExport_Pkg.export_category_rank('bedbathbeyond_category_dval_ranks.txt'); -- New RSP 3/2/2016
			...
			----- Search Config
			ATG_EndecaExport_Pkg.Export_Search_Config_Items ('SearchInterfaceMem.txt', sysdate, p_feedType);
			ATG_EndecaExport_Pkg.Export_EXIM_Customization_Code ('EXIM_Customization_Code.txt', sysdate, p_feedType);
			ATG_EndecaExport_Pkg.Export_SearchGroup_RelRanks ('SearchInterfaceRelRanks.txt', sysdate, p_feedType);
			ATG_EndecaExport_Pkg.Export_OPB ('Bedbathbeyond_OPB_Data.txt',sysdate, p_feedType);
			----- Release 2.2.1 Harmon Tab Sites File START
    ...
    WHEN OTHERS THEN  -- handles all other errors
          ...
          --ATG_EndecaExport_Pkg.Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
            ...
            v_event_dt := SYSDATE;
            v_message := SUBSTR('Error in ' || v_process_cd || ' - Error Code:' ||  v_err_cd || ' - Error Msg: ' || v_err_msg, 1, 512);
