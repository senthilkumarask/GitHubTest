...
BEGIN
    ...
    IF p_vOnlyIncludeLegacyReg = 'N' THEN
        ...
        AND NVL(H.IS_PUBLIC, (SELECT NVL(MAX(PARAM_VALUE), 'Y') FROM ECOM_GLOBAL_PARAMS WHERE PARAM_NAME = 'IS_PUBLIC')) = 'Y'
...
BEGIN
    ...
    IF (p_itemType = 'PER' OR p_itemType = 'LTL') THEN
		...
		-- Get the existing reference_id for the delivery_service, assembly_selected, or create a new one

                SELECT NVL(MAX(reference_id), NVL(v_ltl_delivery_service || v_assembly_selected, '-1')) INTO v_reference_id
                FROM ecomadmin.Reg_Details2
                WHERE registry_num = v_registry_num                    
               	AND sku = v_sku
		AND item_type = 'LTL'
		AND action_cd != 'D'
                AND NVL(ltl_delivery_service, ' ') = NVL(v_ltl_delivery_service, ' ')
		AND NVL(assembly_selected, ' ') = NVL(v_assembly_selected, ' ');
    ...
    IF v_reqQty > 0 THEN
        ...
        BEGIN
			...
			v_reference_id := NVL(v_ltl_delivery_service || v_assembly_selected, '-1');
                	...
                	AND ROWID != p_rowID
            		AND action_cd != 'D'
                	AND ((reference_id = v_reference_id) OR
			(NVL(ltl_delivery_service, ' ') = NVL(v_ltl_delivery_service, ' ')
			AND NVL(assembly_selected, ' ') = NVL(v_assembly_selected, ' ')));
...
BEGIN
    ...
    BEGIN
        ...
        ( REGISTRY_NUM, EVENT_TYPE, EVENT_DT, EMAIL_FLAG,
            ...
            SHOWER_DATE, OTHER_DATE, NETWORK_AFFILIATE_FLAG, NUMBER_OF_GUEST, LAST_TRANS_STORE, LAST_MAINT_PROG, IS_PUBLIC
        )
        VALUES
        ( v_regNum, p_EventType, p_EventDate, p_PromoEmailFlag,
            ...
            TO_DATE(TO_CHAR(p_showerDate),'YYYYMMDD'), TO_DATE(TO_CHAR(p_otherDate),'YYYYMMDD'), p_NetworkAffFlag, p_numGuests, p_LastTransStore, p_LastMaintProg, 'Y'
        );
...
BEGIN
    ...
    BEGIN
        ...
        VALUES
        ( v_regNum, p_EventType, DECODE(p_EventDate, 0, NULL, p_EventDate), p_PromoEmailFlag,
            NULL, NULL,
            'A', 'A', 'N', v_annc_cards_cnt, NULL, '2',
...
BEGIN
    ...
    UPDATE REG_HEADERS
            SET
            EVENT_DT = DECODE(p_EventDate, 0, NULL, p_EventDate),
            EMAIL_FLAG = p_PromoEmailFlag,
            LAST_MAINT_DT_TM = v_JDADateNow,
            ROW_XNG_USR = p_RowXngUser,
            LAST_MAINT_USER = p_RowXngUser,
...
--  END;
    ...
    TO_CHAR(OTHER_DATE,'YYYYMMDD') OTHER_DATE, TO_CHAR(SHOWER_DATE,'YYYYMMDD') SHOWER_DATE,
    (SELECT REGISTRY_NUM FROM BT_PLANNER WHERE REGISTRY_NUM = RH.REGISTRY_NUM AND ROWNUM = 1) BRIDAL_TOOLKIT,
    NVL(IS_PUBLIC, 'N') IS_PUBLIC, CREATE_COUNTRY, REGISTRANT_BG, COREGISTRANT_BG
...
IS
    ...
    v_time_zone_offset varchar2(10) := '-05:00';
    v_time_zone_abbrev varchar2(3) := 'EDT';
    --v_sys_time DATE := (SYSDATE + 10)  + 07/24;
