...
CREATE OR REPLACE PACKAGE BODY BBB_SWITCH_A.ATG_GS_ENDECAEXPORT_PKG 
	AS
	-- Log Message SP
	...
	BEGIN
		...
		endeca_query :='SELECT a.ITEM_ID, a.FACET_ID, a.FACET_VALUE, a.FACET_VALUE_ID from BBB_CORE.BBB_FACET_SKU a, BBB_SWITCH_A.bbb_sku b where a.item_id = b.sku_id and ((b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'') OR (b.GS_CA_WEB_OFFERED = ''1'' and b.GS_CA_DISABLED = ''0'') OR (b.GS_BAB_WEB_OFFERED = ''1'' and b.GS_BAB_DISABLED = ''0''))';
	...
	BEGIN
		...
		GS_Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
		endeca_query:='SELECT a.PRODUCT_ID,a.AVERAGE_OVERALL_RATING,a.TOTAL_REVIEW_COUNT FROM BBB_CORE.BBB_BAZAAR_VOICE a, BBB_SWITCH_A.BBB_PRODUCT b where a.product_id = b.product_id and ((b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'') OR (b.GS_CA_WEB_OFFERED = ''1'' and b.GS_CA_DISABLED = ''0'') OR (b.GS_BAB_WEB_OFFERED = ''1'' and b.GS_BAB_DISABLED = ''0''))';
	...
	BEGIN
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPT_GS_SKUS ';   
		END IF;

		execute immediate 'insert into BBB_SWITCH_A.endeca_expt_GS_SKUs (sku_id) select b.sku_id from BBB_SWITCH_A.bbb_sku a, BBB_SWITCH_A.dcs_sku b where a.sku_id=b.sku_id and ((a.GS_BBB_WEB_OFFERED = ''1'' and  a.GS_BBB_DISABLED = ''0'') OR (a.GS_CA_WEB_OFFERED = ''1'' and a.GS_CA_DISABLED = ''0'') OR (a.GS_BAB_WEB_OFFERED = ''1'' and a.GS_BAB_DISABLED = ''0''))';
		execute immediate 'update BBB_SWITCH_A.endeca_expt_GS_SKUs t set t.gsPLPImage = (select t1.scene7_url from BBB_SWITCH_A.BBB_GS_SKU_IMAGES t1 where t.sku_id=t1.sku_id and t1.SHOT_TYPE = ''GSC''), t.gsPDPImage = (select t2.scene7_url from BBB_SWITCH_A.BBB_GS_SKU_IMAGES t2 where t.sku_id=t2.sku_id and t2.SHOT_TYPE = ''GSP'')';
		...
		endeca_query:='select ''*#*'',a.sku_id, replace(a.display_name,''"'',''&quot;''), replace(a.description,''"'',''&quot;''), 
			...
			from BBB_SWITCH_A.dcs_sku a, BBB_SWITCH_A.bbb_sku b, BBB_SWITCH_A.dcs_sku_media c, BBB_SWITCH_A.bbb_sku_media d, BBB_SWITCH_A.endeca_expt_GS_SKUs e
		...
		IF p_feedType = 'PARTIAL' THEN
			endeca_query := 'select distinct ''*#*'',a.sku_id, replace(a.display_name,''"'',''&quot;''), replace(a.description,''"'',''&quot;''), 
				...
				d.zoom_index, d.anywhere_zoom, b.jda_dept_id, b.jda_sub_dept_id, b.jda_class, b.gift_cert_flag, b.college_id, b.sku_type,b.email_out_of_stock,b.color,b.color_group,replace(b.sku_size,''"'',''&quot;''),b.gift_wrap_eligible,b.vdc_sku_type,b.vdc_sku_message,b.upc,
				case when NVL(e.gsPDPImage,b.scene7_url) is not null then NVL(e.gsPDPImage,b.scene7_url)||''?$146v$'' else '''' end as vert_image_id, case when e.gsPLPImage is not null then e.gsPLPImage else b.scene7_url end as gsPLPImage
				from BBB_SWITCH_A.dcs_sku a, BBB_SWITCH_A.bbb_sku b, BBB_SWITCH_A.dcs_sku_media c, BBB_SWITCH_A.bbb_sku_media d, BBB_SWITCH_A.dcs_price i, BBB_SWITCH_A.bbb_price j, BBB_SWITCH_A.endeca_expt_GS_SKUs e
	...
	BEGIN
		...
		endeca_query:='select a.product_id,a.sku_id,a.sequence_num from BBB_SWITCH_A.dcs_prd_chldsku a, BBB_SWITCH_A.BBB_SKU s where a.sku_id=s.sku_id and ((s.GS_BBB_WEB_OFFERED = ''1'' and  s.GS_BBB_DISABLED = ''0'') OR (s.GS_CA_WEB_OFFERED = ''1'' and s.GS_CA_DISABLED = ''0'') OR (s.GS_BAB_WEB_OFFERED = ''1'' and s.GS_BAB_DISABLED = ''0'')) and a.product_id in (select product_id from BBB_SWITCH_A.bbb_product where product_id not in (select product_id from BBB_SWITCH_A.bbb_prd_reln where like_unlike=1) union select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd) order by a.product_id,a.sequence_num';
		if p_feedType = 'PARTIAL' then
		...
		endeca_query:='select distinct a.product_id,a.sku_id,a.sequence_num from BBB_SWITCH_A.dcs_prd_chldsku a, BBB_SWITCH_A.BBB_PRODUCT b, BBB_SWITCH_A.BBB_SKU s where a.product_id = b.product_id and a.sku_id=s.sku_id and s.web_offered_flag =''1'' and s.disable_flag =''0'' and b.last_mod_date > '''|| p_lastModifiedDate ||''' and a.product_id in ( select product_id from BBB_SWITCH_A.bbb_product where product_id not in ( select product_id from BBB_SWITCH_A.bbb_prd_reln where like_unlike=1 ) ) order by product_id,sequence_num';
		end if;
	...
	BEGIN
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPT_GS_ACCESSORIES ';   
		END IF;
		
		execute immediate 'insert into BBB_SWITCH_A.endeca_expt_GS_Accessories (product_id) select a.product_id from BBB_SWITCH_A.bbb_product b,BBB_SWITCH_A.dcs_product a where a.product_id=b.product_id and ((b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'') OR (b.GS_CA_WEB_OFFERED = ''1'' and b.GS_CA_DISABLED = ''0'') OR (b.GS_BAB_WEB_OFFERED = ''1'' and b.GS_BAB_DISABLED = ''0''))';
		execute immediate 'update BBB_SWITCH_A.endeca_expt_GS_Accessories t set t.gsPLPImage = (select t1.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t1 where t.product_id=t1.product_id and t1.SHOT_TYPE = ''GSC''), t.gsPDPImage = (select t2.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t2 where t.product_id=t2.product_id and t2.SHOT_TYPE = ''GSP'')';
        endeca_query:='select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,
            ...
            from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b, BBB_SWITCH_A.dcs_prd_media c, BBB_SWITCH_A.bbb_prd_media d, BBB_CORE.BBB_SCHOOLS f, BBB_SWITCH_A.endeca_expt_GS_Accessories e
            ...
            and a.product_id in ( select distinct c.product_id from BBB_SWITCH_A.bbb_product a, BBB_SWITCH_A.bbb_prd_prd_reln b, BBB_SWITCH_A.bbb_prd_reln c  where a.product_id=b.product_id
	...
	BEGIN
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPT_LEAD_PRODUCTS ';   
		END IF;

		IF p_feedType = 'PARTIAL' THEN
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_Lead_Products (product_id) select a.product_id from BBB_SWITCH_A.bbb_product b,BBB_SWITCH_A.dcs_product a where b.product_id=a.product_id and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
		ELSE
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_Lead_Products (product_id) select a.product_id from BBB_SWITCH_A.bbb_product b,BBB_SWITCH_A.dcs_product a where b.product_id=a.product_id and ((b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'') OR (b.GS_CA_WEB_OFFERED = ''1'' and b.GS_CA_DISABLED = ''0'') OR (b.GS_BAB_WEB_OFFERED = ''1'' and b.GS_BAB_DISABLED = ''0''))';
		END IF;
		execute immediate 'update BBB_SWITCH_A.endeca_expt_Lead_Products t set t.gsPLPImage = (select t1.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t1 where t.product_id=t1.product_id and t1.SHOT_TYPE = ''GSC''), t.gsPDPImage = (select t2.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t2 where t.product_id=t2.product_id and t2.SHOT_TYPE = ''GSP'')';
        ...
        endeca_query:='select distinct ''*#*'',a.product_id,     gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,
			...
			anywhere_zoom, case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end,
			case when NVL(e.gsPDPImage,b.scene7_url) is not null then NVL(e.gsPDPImage,b.scene7_url)||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id, case when e.gsPLPImage is not null then e.gsPLPImage else b.scene7_url end as gsPLPImage
			from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b, BBB_SWITCH_A.dcs_prd_media c, BBB_SWITCH_A.bbb_prd_media d,BBB_SWITCH_A.endeca_expt_Lead_Products e, BBB_SWITCH_A.bbb_prd_prd_reln r, BBB_CORE.BBB_SCHOOLS f 
	...
	BEGIN
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPT_SIMPLE_PRODUCTS ';   
		END IF;

		IF p_feedType = 'PARTIAL' THEN
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_Simple_Products (product_id) select a.product_id from BBB_SWITCH_A.bbb_product b,BBB_SWITCH_A.dcs_product a where b.product_id=a.product_id and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
		ELSE
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_Simple_Products (product_id) select a.product_id from BBB_SWITCH_A.bbb_product b,BBB_SWITCH_A.dcs_product a where b.product_id=a.product_id and ((b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'') OR (b.GS_CA_WEB_OFFERED = ''1'' and b.GS_CA_DISABLED = ''0'') OR (b.GS_BAB_WEB_OFFERED = ''1'' and b.GS_BAB_DISABLED = ''0''))';
		END IF;
		execute immediate 'update BBB_SWITCH_A.endeca_expt_Simple_Products t set t.gsPLPImage = (select t1.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t1 where t.product_id=t1.product_id and t1.SHOT_TYPE = ''GSC''), t.gsPDPImage = (select t2.scene7_url from BBB_SWITCH_A.BBB_GS_PRODUCT_IMAGES t2 where t.product_id=t2.product_id and t2.SHOT_TYPE = ''GSP'')';
		endeca_query:='select distinct ''*#*'',a.product_id, gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,
			case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id, 
			case when NVL(e.gsPLPImage,b.scene7_url) is not null then NVL(e.gsPLPImage,b.scene7_url)||''?$146$'' else '''' end as medium_image_id,
			case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id, 
			...
			anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end,
			case when NVL(e.gsPDPImage,b.scene7_url) is not null then NVL(e.gsPDPImage,b.scene7_url)||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id,case when e.gsPLPImage is not null then e.gsPLPImage else b.scene7_url end as gsPLPImage
			from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b, BBB_SWITCH_A.dcs_prd_media c, BBB_SWITCH_A.bbb_prd_media d, BBB_CORE.BBB_SCHOOLS f ,BBB_SWITCH_A.endeca_expt_Simple_Products e
			...
			and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists(select 1 from BBB_SWITCH_A.dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id) ';

		IF p_feedType = 'PARTIAL' THEN
			endeca_query:='select distinct ''*#*'',a.product_id, gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when NVL(e.gsPLPImage,b.scene7_url) is not null then NVL(e.gsPLPImage,b.scene7_url)||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when NVL(e.gsPDPImage,b.scene7_url) is not null then NVL(e.gsPDPImage,b.scene7_url)||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id,case when e.gsPLPImage is not null then e.gsPLPImage else b.scene7_url end as gsPLPImage from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b, BBB_SWITCH_A.dcs_prd_media c, BBB_SWITCH_A.bbb_prd_media d, BBB_CORE.BBB_SCHOOLS f,BBB_SWITCH_A.endeca_expt_Simple_Products e, BBB_SWITCH_A.dcs_prd_chldsku g, BBB_SWITCH_A.bbb_sku h, BBB_SWITCH_A.dcs_cat_chldprd i, BBB_SWITCH_A.bbb_category j where a.product_id = b.product_id and a.product_id = e.product_id and a.product_id=c.product_id(+) and a.product_id=d.product_id(+)  and a.product_id=g.product_id(+) and g.sku_id=h.sku_id(+) and a.product_id=i.child_prd_id (+) and i.category_id = j.category_id and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists (select 1 from BBB_SWITCH_A.dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id)  and ( b.last_mod_date > '''|| p_lastModifiedDate ||''' or h.last_mod_date > '''|| p_lastModifiedDate ||'''  or j.last_mod_date > '''|| p_lastModifiedDate ||''' ) ' ;
		END IF;
	...
	BEGIN
		...
		endeca_query:='select ''*#*'',a.brand_id,brand_descrip,brand_image,site_id from BBB_SWITCH_A.BBB_BRANDS a, BBB_SWITCH_A.bbb_brand_sites b where a.brand_id=b.brand_id and b.site_id like ''GS_%''';
		dbms_output.put_line('Export_GS_Brands: '||endeca_query);     
		v_message :='Export_GS_Brands: STARTS: endeca_query:'||endeca_query;
		GS_Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
		IF p_feedType = 'PARTIAL' then
			endeca_query := 'select distinct ''*#*'',a.brand_id,brand_descrip,brand_image,site_id from BBB_SWITCH_A.BBB_BRANDS a, BBB_SWITCH_A.bbb_brand_sites b,BBB_SWITCH_A.bbb_product c where a.brand_id=b.brand_id and b.site_id like ''GS_%'' and c.brand_id=a.brand_id and (a.last_mod_date > '''|| p_lastModifiedDate ||''' or c.last_mod_date > '''|| p_lastModifiedDate ||''')' ;
		END IF;
		dbms_output.put_line('Export_GS_Brands: '||endeca_query);  
	...
	BEGIN
		...
		endeca_query:='select ''*#*'',a.sku_attribute_id, replace (display_descrip, ''"'',''''''''), image_url,action_url, place_holder, priority, site_id, start_date, end_date,
			...
			from BBB_SWITCH_A.BBB_SKU_ATTRIBUTES_INFO a, BBB_SWITCH_A.BBB_SKU_ATTRIBUTE_SITES b where a.sku_attribute_id = b.sku_attribute_id and b.site_id like ''GS_%''
		...
		IF p_feedType  = 'PARTIAL' THEN
			endeca_query:='select distinct ''*#*'',c.sku_attribute_id, replace (display_descrip, ''"'',''''''''), a.image_url, a.action_url, a.place_holder, a.priority, b.site_id, a.start_date, a.end_date,
				CASE WHEN INSTR( display_descrip, ''>'') > 1 THEN SUBSTR( display_descrip, INSTR( display_descrip, ''>'') + 1, INSTR( display_descrip, ''</'') - INSTR( display_descrip, ''>'') - 1 ) ELSE display_descrip END ATTRIBUTE_DISPLAY_NAME
				from BBB_SWITCH_A.BBB_SKU_ATTRIBUTES_INFO a, BBB_SWITCH_A.BBB_SKU_ATTRIBUTE_SITES b, BBB_SWITCH_A.BBB_ATTR_RELN c,BBB_SWITCH_A.BBB_PRODUCT d,BBB_SWITCH_A.BBB_PRD_ATTR_RELN f                
	...
	BEGIN
	 ...
	 IF(nCount > 0)THEN
		execute immediate 'truncate table BBB_SWITCH_A.ENDECA_GS_PRODUCT_ITEM_ATTR'; 
	 END IF;
	 
		--- Insert Data into temp table Start
		insert into BBB_SWITCH_A.ENDECA_GS_PRODUCT_ITEM_ATTR (product_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, MISC_INFO,attr_last_modified_date, product_last_modified_date)
			(SELECT distinct A.PRODUCT_ID,B.SKU_ATTRIBUTE_ID,'GS_BedBathUS',B.START_DATE,B.END_DATE,B.MISC_INFO,B.last_mod_date, d.last_mod_date             
					FROM BBB_SWITCH_A.BBB_PRD_ATTR_RELN A, BBB_SWITCH_A.BBB_ATTR_RELN B, BBB_SWITCH_A.BBB_PRODUCT D            
		...
		--- Insert Data into temp table End
			...
			endeca_query :='SELECT distinct PRODUCT_ID,SKU_ATTRIBUTE_ID,SITE_ID,START_DATE,END_DATE,MISC_INFO from BBB_SWITCH_A.ENDECA_GS_PRODUCT_ITEM_ATTR';
		IF p_feedType   = 'PARTIAL' THEN
	...
	BEGIN
	 ...
	 IF(nCount > 0)THEN
		execute immediate 'truncate table BBB_SWITCH_A.ENDECA_GS_SKU_ITEM_ATTR'; 
	 END IF;
		
		--- Insert Data into temp table Start
		insert into BBB_SWITCH_A.ENDECA_GS_SKU_ITEM_ATTR (sku_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, MISC_INFO, attr_last_modified_date)
			(select a.sku_id,b.sku_attribute_id,'GS_BedBathUS',b.start_date,b.end_date,b.misc_info, b.last_mod_date
					from BBB_SWITCH_A.BBB_SKU_ATTR_RELN a, BBB_SWITCH_A.BBB_ATTR_RELN b  
		...
		endeca_query:='select sku_id, sku_attribute_id, site_id, start_date, end_date, misc_info from BBB_SWITCH_A.ENDECA_GS_SKU_ITEM_ATTR';
		IF p_feedType = 'PARTIAL' THEN
			endeca_query := 'select sku_id, sku_attribute_id, site_id, start_date, end_date, misc_info from BBB_SWITCH_A.ENDECA_GS_SKU_ITEM_ATTR a where a.attr_last_modified_date > '''|| p_lastModifiedDate ||'' ;
		END IF;
	...
	BEGIN
		...
		endeca_query:='SELECT distinct channel.channel_id, a.child_cat_id,b.display_name FROM BBB_SWITCH_A.dcs_cat_chldcat a, BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BedBathUS'') channel WHERE CONNECT_BY_ISLEAF=1 and a.child_cat_id=b.category_id START WITH a.category_id = channel.category_id CONNECT BY PRIOR a.child_cat_id = a.category_id and PRIOR channel.channel_id=channel.channel_id union SELECT distinct category_channel.channel_id, b.category_id,b.display_name FROM BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BedBathUS'') category_channel where b.category_id=category_channel.category_id and b.category_id not in (select category_id from BBB_SWITCH_A.dcs_cat_chldcat)';
	...
	BEGIN
		...
		GS_Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
		endeca_query:='SELECT distinct channel.channel_id, a.child_cat_id,b.display_name FROM BBB_SWITCH_A.dcs_cat_chldcat a, BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BuyBuyBaby'') channel WHERE CONNECT_BY_ISLEAF=1 and a.child_cat_id=b.category_id START WITH a.category_id = channel.category_id CONNECT BY PRIOR a.child_cat_id = a.category_id and PRIOR channel.channel_id=channel.channel_id union SELECT distinct category_channel.channel_id, b.category_id,b.display_name FROM BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BuyBuyBaby'') category_channel where b.category_id=category_channel.category_id and b.category_id not in (select category_id from BBB_SWITCH_A.dcs_cat_chldcat)';
	...
	BEGIN
		...
		GS_Log_message(v_process_cd, v_process_sub_cd, v_message, v_message_type);
		endeca_query:='SELECT distinct channel.channel_id, a.child_cat_id,b.display_name FROM BBB_SWITCH_A.dcs_cat_chldcat a, BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BedBathCanada'') channel WHERE CONNECT_BY_ISLEAF=1 and a.child_cat_id=b.category_id START WITH a.category_id = channel.category_id CONNECT BY PRIOR a.child_cat_id = a.category_id and PRIOR channel.channel_id=channel.channel_id union SELECT distinct category_channel.channel_id, b.category_id,b.display_name FROM BBB_SWITCH_A.dcs_category b,(select d.category_id as category_id, a.channel_id as channel_id from BBB_SWITCH_A.BBB_CHANNEL_INFO a, BBB_SWITCH_A.BBB_GS_CHAN_CATS b, BBB_SWITCH_A.BBB_GS_CATEGORY_CONTNR c, BBB_SWITCH_A.BBB_GS_CONT_CATS d where a.channel_id=b.channel_id and b.cat_container_id=c.cat_container_id and c.cat_container_id=d.cat_container_id and b.site_id=''GS_BedBathCanada'') category_channel where b.category_id=category_channel.category_id and b.category_id not in (select category_id from BBB_SWITCH_A.dcs_cat_chldcat)';
	...
	BEGIN
		...
		endeca_query:='select a.category_id, c.category_id, display_name, b.is_college, b.node_type, b.phantom_category, b.shop_guide_id 
			from BBB_SWITCH_A.dcs_category a, BBB_SWITCH_A.bbb_category b, BBB_SWITCH_A.dcs_cat_chldcat c where a.category_id = b.category_id 
		...
		IF p_feedType = 'PARTIAL' THEN
			insert into ENDECA_EXP_GS_CAT_PRODS (cat_id, prod_id, seq_num) 
				select distinct a.category_id, a.child_prd_id, a.sequence_num
					from BBB_SWITCH_A.dcs_cat_chldprd a, BBB_SWITCH_A.BBB_CATEGORY b, BBB_SWITCH_A.BBB_PRODUCT c, BBB_SWITCH_A.dcs_prd_chldsku g, BBB_SWITCH_A.bbb_sku h, BBB_SWITCH_A.dcs_price h1, BBB_SWITCH_A.dcs_price i, BBB_SWITCH_A.bbb_price h2, BBB_SWITCH_A.bbb_price i1 
	...
	BEGIN
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_GS_EXPORT_PRICES ';   
		END IF;

		insert into BBB_SWITCH_A.endeca_gs_export_prices (sku_id, was_price,last_mod_date) select t1.sku_id, t1.list_price,t2.last_mod_date from BBB_SWITCH_A.dcs_price t1, BBB_SWITCH_A.bbb_price t2 , BBB_SWITCH_A.bbb_sku a where t1.price_id = t2.price_id(+) and t1.sku_id = a.sku_id and t1.price_list = 'plist100005' and ((a.GS_BBB_WEB_OFFERED = '1' and  a.GS_BBB_DISABLED = '0') OR (a.GS_CA_WEB_OFFERED = '1' and a.GS_CA_DISABLED = '0') OR (a.GS_BAB_WEB_OFFERED = '1' and a.GS_BAB_DISABLED = '0')) order by t1.sku_id ;

		MERGE INTO BBB_SWITCH_A.endeca_gs_export_prices t USING (select distinct sku_id,list_price from BBB_SWITCH_A.dcs_price  where price_list='plist100004') t1 on (t.sku_id = t1.sku_id) WHEN MATCHED THEN UPDATE SET t.is_price =t1.list_price;
		MERGE INTO BBB_SWITCH_A.endeca_gs_export_prices t USING (select distinct sku_id,list_price from BBB_SWITCH_A.dcs_price  where price_list='plist100003') t1 on (t.sku_id = t1.sku_id) WHEN MATCHED THEN UPDATE SET t.ca_is_price =t1.list_price;
		MERGE INTO BBB_SWITCH_A.endeca_gs_export_prices t USING (Select distinct sku_id,list_price from BBB_SWITCH_A.dcs_price  where price_list='plist100002') t1 on (t.sku_id = t1.sku_id) WHEN MATCHED THEN UPDATE Set t.ca_was_price =t1.list_price;
		endeca_query:='select sku_id,LTRIM(to_char(was_price,999999999999.99)),LTRIM(to_char(case when is_price is null or is_price = '''' then was_price else is_price end,999999999999.99)),LTRIM(to_char(ca_was_price,999999999999.99)),LTRIM(to_char(case when ca_is_price is null or ca_is_price = '''' then ca_was_price else ca_is_price end,999999999999.99)) from BBB_SWITCH_A.endeca_gs_export_prices';
	...
	BEGIN 
		...
		IF(nCount > 0)THEN
			EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPORT_GS_PROD_PROP ';
		END IF; 
		IF p_feedType = 'PARTIAL' THEN 
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id,web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price, enable_date, site_id) select a.product_id, GS_BBB_WEB_OFFERED, GS_BBB_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BedBathUS'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id, web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price,enable_date, site_id) select a.product_id, GS_BAB_WEB_OFFERED, GS_BAB_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BuyBuyBaby'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_BAB_WEB_OFFERED = ''1'' and  b.GS_BAB_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id,web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price,enable_date, site_id) select a.product_id, GS_CA_WEB_OFFERED, GS_CA_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BedBathCanada'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_CA_WEB_OFFERED = ''1'' and  b.GS_CA_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
		ELSE 
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id,web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price,enable_date, site_id ) select a.product_id, GS_BBB_WEB_OFFERED, GS_BBB_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BedBathUS'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) ';
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id,web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price,enable_date, site_id ) select a.product_id, GS_BAB_WEB_OFFERED, GS_BAB_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BuyBuyBaby'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_BAB_WEB_OFFERED = ''1'' and  b.GS_BAB_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) ';
			execute immediate 'insert into BBB_SWITCH_A.endeca_export_gs_prod_prop(product_id,web_offered_flag, disable_flag, product_title, short_description, long_description, price_range_descrip, sku_low_price, sku_high_price,enable_date, site_id ) select a.product_id, GS_CA_WEB_OFFERED, GS_CA_DISABLED, display_name, dbms_lob.substr( long_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,b.enable_date,''GS_BedBathCanada'' from BBB_SWITCH_A.dcs_product a, BBB_SWITCH_A.bbb_product b where a.product_id=b.product_id and b.GS_CA_WEB_OFFERED = ''1'' and  b.GS_CA_DISABLED = ''0'' and a.product_id in (select child_prd_id from BBB_SWITCH_A.dcs_cat_chldprd union all select product_id from BBB_SWITCH_A.bbb_prd_reln where NVL(like_unlike, 0) = 0) ';
		END IF; 

		--Web CA Site
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t2.product_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_prod_site_translations t2, BBB_SWITCH_A.bbb_prod_translations t1 where t2.translation_id=t1.translation_id  and t1.site_id=''BedBathCanada'' and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.product_id=tt1.product_id and t.site_id=''GS_BedBathCanada'') WHEN MATCHED THEN UPDATE SET t.product_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.product_title)'; 
		...
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t2.product_id,t1.attribute_value_date from BBB_SWITCH_A.bbb_prod_site_translations t2, BBB_SWITCH_A.bbb_prod_translations t1 where t2.translation_id=t1.translation_id  and t1.site_id=''BedBathCanada'' and t1.attribute_name = ''enableDate'' and t1.attribute_value_date is not null) tt1 on (t.product_id=tt1.product_id and t.site_id=''GS_BedBathCanada'') WHEN MATCHED THEN UPDATE SET t.enable_date =NVL(tt1.attribute_value_date, t.enable_date)';
		--Web Baby Site
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t2.product_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_prod_site_translations t2, BBB_SWITCH_A.bbb_prod_translations t1 where t2.translation_id=t1.translation_id  and t1.site_id=''BuyBuyBaby'' and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.product_id=tt1.product_id and t.site_id=''GS_BuyBuyBaby'') WHEN MATCHED THEN UPDATE SET t.product_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.product_title)'; 
		...
		--GS Sites
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t1.site_id,t2.product_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_prod_site_translations t2, BBB_SWITCH_A.bbb_prod_translations t1 where t2.translation_id=t1.translation_id   and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.product_id=tt1.product_id and t.site_id=tt1.site_id) WHEN MATCHED THEN UPDATE SET t.product_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.product_title)'; 
		...
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t1.site_id,t2.product_id,t1.attribute_value_clob from BBB_SWITCH_A.bbb_prod_site_translations t2, BBB_SWITCH_A.bbb_prod_translations t1 where t2.translation_id=t1.translation_id   and t1.attribute_name = ''longDescription'' and t1.attribute_value_clob is not null) tt1 on (t.product_id=tt1.product_id and t.site_id=tt1.site_id) WHEN MATCHED THEN UPDATE SET t.long_description =NVL(replace(tt1.attribute_value_clob,''"'',''&quot;''), t.long_description)';
		...
		--GS Enable Dates
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t1.product_id,t1.gs_bbb_enable_date from BBB_SWITCH_A.bbb_product t1
		...
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t1.product_id,t1.gs_bab_enable_date from BBB_SWITCH_A.bbb_product t1
		...
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_export_gs_prod_prop t USING (select t1.product_id,t1.gs_ca_enable_date from BBB_SWITCH_A.bbb_product t1
		...
		execute immediate 'delete from BBB_SWITCH_A.endeca_export_gs_prod_prop where product_id in ( select distinct e1.product_id from BBB_SWITCH_A.endeca_export_gs_prod_prop e1, BBB_SWITCH_A.bbb_product p1 where p1.product_id = e1.product_id and ( p1.collection_flag = 1 or p1.lead_prd_flag = 1) and e1.product_id not in (  select distinct e.product_id from BBB_SWITCH_A.endeca_export_gs_prod_prop e, BBB_SWITCH_A.bbb_product p, BBB_SWITCH_A.bbb_prd_prd_reln r where p.product_id = e.product_id and p.product_id = r.product_id and e.product_id = r.product_id and ( p.collection_flag = 1 or p.lead_prd_flag = 1) ) )';

		endeca_query:='select distinct ''*#*'',product_id, web_offered_flag, disable_flag, product_title, dbms_lob.substr( short_description,8000,1), dbms_lob.substr( long_description,8000,1), price_range_descrip, sku_low_price, sku_high_price,TO_CHAR (enable_date, ''DD-MON-YY''), site_id from BBB_SWITCH_A.endeca_export_gs_prod_prop order by product_id'; 
	...
	BEGIN 
		...
		IF(nCount > 0)THEN
		EXECUTE immediate 'truncate table BBB_SWITCH_A.ENDECA_EXPT_GS_SKU_SITE_PROPTS ';
		END IF;      
		IF p_feedType = 'PARTIAL' THEN
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_gs_sku_site_propts (sku_id, web_offered_flag, disable_flag,sku_title, short_description, site_id,last_mod_date) select b.sku_id, b.GS_BBB_WEB_OFFERED, b.GS_BBB_DISABLED, a.display_name, a.description,''GS_BedBathUS'',b.last_mod_date from BBB_SWITCH_A.bbb_sku b,BBB_SWITCH_A.dcs_sku a where b.sku_id=a.sku_id and b.GS_BBB_WEB_OFFERED = ''1'' and  b.GS_BBB_DISABLED = ''0'' and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_gs_sku_site_propts (sku_id, web_offered_flag, disable_flag,sku_title, short_description, site_id,last_mod_date) select b.sku_id, b.GS_BAB_WEB_OFFERED, b.GS_BAB_DISABLED, a.display_name, a.description,''GS_BuyBuyBaby'',b.last_mod_date from BBB_SWITCH_A.bbb_sku b,BBB_SWITCH_A.dcs_sku a where b.sku_id=a.sku_id and b.GS_BAB_WEB_OFFERED = ''1'' and  b.GS_BAB_DISABLED = ''0'' and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
			execute immediate 'insert into BBB_SWITCH_A.endeca_expt_gs_sku_site_propts (sku_id, web_offered_flag, disable_flag,sku_title, short_description, site_id,last_mod_date) select b.sku_id, b.GS_CA_WEB_OFFERED, b.GS_CA_DISABLED, a.display_name, a.description,''GS_BedBathCanada'',b.last_mod_date from BBB_SWITCH_A.bbb_sku b,BBB_SWITCH_A.dcs_sku a where b.sku_id=a.sku_id and b.GS_CA_WEB_OFFERED = ''1'' and  b.GS_CA_DISABLED = ''0'' and b.last_mod_date > '''|| p_lastModifiedDate ||'''';
		ELSE
		...
		END IF;
		--Web Baby
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_expt_gs_sku_site_propts t USING (select t2.sku_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_sku_site_translations t2, BBB_SWITCH_A.bbb_sku_translations t1 where t2.translation_id=t1.translation_id  and t1.site_id=''BuyBuyBaby'' and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.sku_id=tt1.sku_id and t.site_id=''GS_BuyBuyBaby'') WHEN MATCHED THEN UPDATE SET t.sku_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.sku_title)';
		...
		--Web CA
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_expt_gs_sku_site_propts t USING (select t2.sku_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_sku_site_translations t2, BBB_SWITCH_A.bbb_sku_translations t1 where t2.translation_id=t1.translation_id  and t1.site_id=''BedBathCanada'' and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.sku_id=tt1.sku_id and t.site_id=''GS_BedBathCanada'') WHEN MATCHED THEN UPDATE SET t.sku_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.sku_title)';
		...
		--GS properties
		execute immediate 'MERGE INTO BBB_SWITCH_A.endeca_expt_gs_sku_site_propts t USING (select t1.site_id,t2.sku_id,t1.attribute_value_string from BBB_SWITCH_A.bbb_sku_site_translations t2, BBB_SWITCH_A.bbb_sku_translations t1 where t2.translation_id=t1.translation_id  and t1.attribute_name = ''displayName'' and t1.attribute_value_string is not null) tt1 on (t.sku_id=tt1.sku_id and t.site_id=tt1.site_id) WHEN MATCHED THEN UPDATE SET t.sku_title =NVL(replace(tt1.attribute_value_string,''"'',''&quot;''), t.sku_title)';
		...
		endeca_query:='select ''*#*'',sku_id, web_offered_flag,disable_flag, sku_title, short_description, site_id from BBB_SWITCH_A.endeca_expt_gs_sku_site_propts order by sku_id'; 
