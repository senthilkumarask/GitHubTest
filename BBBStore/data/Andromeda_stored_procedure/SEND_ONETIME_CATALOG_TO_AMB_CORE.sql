...
CREATE OR REPLACE PROCEDURE BBB_CORE.SEND_ONETIME_CATALOG_TO_AMB (bbuMerchId IN VARCHAR2, bbbyMerchId IN VARCHAR2, dataCenter IN VARCHAR2)
IS
...
BEGIN
	...
	VALUES 
     ...
     DELETE FROM bbb_ambassador_usr.e4x_categories
     WHERE merchant_id = bbuMerchId
     AND parent_id NOT IN (SELECT id FROM bbb_ambassador_usr.e4x_categories WHERE merchant_id = bbuMerchId);
     -- Delete all orphan categories - bbbyMerchId
     DELETE FROM bbb_ambassador_usr.e4x_categories
     WHERE merchant_id = bbbyMerchId
     AND parent_id NOT IN (SELECT id FROM bbb_ambassador_usr.e4x_categories WHERE merchant_id = bbbyMerchId);
	...
	VALUES 
     ...
     DELETE FROM bbb_ambassador_usr.e4x_products
     WHERE merchant_id = bbuMerchId
     AND parent_id NOT IN (SELECT id FROM bbb_ambassador_usr.e4x_categories WHERE merchant_id = bbuMerchId);
