...
CREATE OR REPLACE PROCEDURE BBB_CORE.SEND_DELTA_CATALOG_TO_AMB (bbuMerchId IN VARCHAR2, bbbyMerchId IN VARCHAR2, dataCenter IN VARCHAR2)
IS
...
FROM 
  BBB_SWITCH_A.BBB_DEPT dept,
  bbb_ambassador_usr.BBB_SCHEDULER_PROCESS sch
WHERE
  dept.descrip is not NULL
  AND sch.data_center = dataCenter
  AND sch.PROCESS_NAME='CatalogFeed'
  AND dept.last_mod_date > sch.last_run
UNION  
SELECT 
...
FROM 
  BBB_SWITCH_A.BBB_SUB_DEPT sdept,
  bbb_ambassador_usr.BBB_SCHEDULER_PROCESS sch
WHERE
  sdept.descrip is not NULL 
  AND sch.data_center = dataCenter
  AND sch.PROCESS_NAME='CatalogFeed'
  AND sdept.last_mod_date > sch.last_run  
UNION  
SELECT 
  bbbyMerchId MERCHANT_ID,
  sdept.jda_sub_dept_id ID,
...
FROM 
  BBB_SWITCH_A.BBB_CLASS cls,
  bbb_ambassador_usr.BBB_SCHEDULER_PROCESS sch
WHERE
  cls.descrip is not NULL
  AND sch.data_center = dataCenter
  AND sch.PROCESS_NAME='CatalogFeed'
  AND cls.last_mod_date > sch.last_run  
UNION  
SELECT 
  bbbyMerchId MERCHANT_ID,
  cls.jda_dept_id || '_' || cls.jda_sub_dept_id || '_' || cls.jda_class ID,
...
FROM 
  BBB_SWITCH_A.DCS_SKU sku,
  BBB_SWITCH_A.BBB_SKU bsku,
  BBB_SWITCH_A.DCS_PRICE price,
  BBB_SWITCH_A.BBB_PRICE bprice,
  BBB_SWITCH_A.DCS_SITE site,
  BBB_SWITCH_A.DCS_PRD_CHLDSKU chld,
  BBB_SWITCH_A.BBB_PRODUCT prd,
  BBB_CORE.BBB_ITEM_MISC misc,
  bbb_ambassador_usr.BBB_SCHEDULER_PROCESS sch
WHERE 
  sku.sku_id = bsku.sku_id 
  AND site.list_pricelist_id = price.price_list
  AND bsku.sku_id = price.sku_id
  AND price.price_id = bprice.price_id (+)
  ...
  AND sch.data_center = dataCenter
  AND sch.PROCESS_NAME='CatalogFeed'
  AND ( (bsku.last_mod_date > sch.last_run) OR 
        (bprice.last_mod_date > sch.last_run) OR 
        (prd.last_mod_date > sch.last_run) OR 
        (misc.last_mod_date > sch.last_run)
      )
UNION
SELECT 
  DECODE(site.id, 'BedBathUS', bbuMerchId, 'BuyBuyBaby', bbbyMerchId) MERCHANT_ID,
...
FROM 
  ...
  BBB_SWITCH_A.DCS_PRD_CHLDSKU chld,
  BBB_SWITCH_A.BBB_PRODUCT prd,
  BBB_SWITCH_A.BBB_SKU_SITE_TRANSLATIONS skuSite,
  BBB_CORE.BBB_ITEM_MISC misc,
  bbb_ambassador_usr.BBB_SCHEDULER_PROCESS sch
WHERE 
  sku.sku_id = bsku.sku_id 
...
BEGIN
	...
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Inserting categories into ambassador schema.', sysdate, 'AMB', 'A');
	
    FOR nextCat IN category_cursor  
     LOOP
        BEGIN
            Insert into bbb_ambassador_usr.E4X_CATEGORIES (MERCHANT_ID,ID,NAME,PARENT_ID,DESCRIPTION) 
        ...
        EXCEPTION
		  ...
		  INSERT into 
			BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
		  VALUES 
			(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Duplicate Category found. Skipping Insert.', sysdate, 'AMB', 'I');
		  CONTINUE;
        END;
     END LOOP;
	...
	INSERT into 
		BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Finished inserting categories into ambassador schema.', sysdate, 'AMB', 'A');
     
    -- Insert SKUs to be send to BorderFree
	INSERT into 
		BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Inserting SKUs into ambassador schema.', sysdate, 'AMB', 'A');
		
    FOR nextSku IN sku_cursor  
     LOOP
        BEGIN
            Insert into bbb_ambassador_usr.E4X_PRODUCTS (MERCHANT_ID,SKU,NAME,DESCRIPTION,PARENT_ID,DUTIABLE_PRICE,URL,IMAGE_URL,UPC,COS,PRODUCT_CONDITION,MANUFACTURER,DEFAULT_LOCALE,COO,BOX_LENGTH,BOX_WIDTH,BOX_HEIGHT,SIZE_UNITS,NET_WEIGHT,WEIGHT_UNITS) 
        ...
        EXCEPTION
		  ...
		  INSERT into 
			BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
		  VALUES 
			(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Duplicate SKU found. Skipping Insert.', sysdate, 'AMB', 'I');		  
		  CONTINUE;
        END;
     END LOOP;
	...
	INSERT into 
		BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Finished inserting SKUs into ambassador schema.', sysdate, 'AMB', 'A');
		
	INSERT into 
		BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Inserting flush string into ambassador schema.', sysdate, 'AMB', 'A');		
     ...
     INSERT INTO bbb_ambassador_usr.E4X_TRIGGERS(TASK) VALUES('FlushCatalog');     
     
     -- Commit in the end when all inserts are successful
     COMMIT;
     
     -- Update last run date for the catalog feed
     UPDATE bbb_ambassador_usr.BBB_SCHEDULER_PROCESS set status = 'SUCCESS', data_center= dataCenter, LAST_RUN = sysdate WHERE PROCESS_NAME='CatalogFeed';
     COMMIT;
	 
	INSERT into 
		BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
	VALUES 
		(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Delta catalog feed is successful. Commit Done.', sysdate, 'AMB', 'A');	 
	COMMIT;
     ...
     -- Exception Block
      ...
      WHEN others THEN
        ...
        ROLLBACK;
        UPDATE bbb_ambassador_usr.BBB_SCHEDULER_PROCESS set status = 'FAIL', data_center= dataCenter WHERE PROCESS_NAME='CatalogFeed';
		INSERT into 
			BBB_PIM_STG.process_support (ID, PROCESS_CD, PROCESS_SUB_CD, MESSAGE, ROW_XNG_DT, ROW_XNG_USR, ROW_STATUS) 
		VALUES 
			(BBB_PIM_STG.process_support_seq.nextval, 'AMB_DELTA', 'BF_CatFeed', 'Delta catalog feed failed. Rolled-back everything.', sysdate, 'AMB', 'I');		
        COMMIT;
END;
/
