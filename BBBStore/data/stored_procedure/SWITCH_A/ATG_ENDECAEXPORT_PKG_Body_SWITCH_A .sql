...
create or replace 
PACKAGE BODY    BBB_SWITCH_A.ATG_ENDECAEXPORT_PKG
AS
...
PROCEDURE Export_Product_Item_Attributes(
  ...
  BEGIN
      ...
      v_message :='Export_Product_Item_Attributes: STARS';
      Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
	  	
		 select count(*) into nCount from all_tables where table_name ='ENDECA_PRODUCT_ITEM_ATTR';
	 IF(nCount > 0)THEN
		execute immediate 'truncate table BBB_SWITCH_A.ENDECA_PRODUCT_ITEM_ATTR'; 
	 END IF;
 
	 
	  -- Insert BedBathUS data in temp table
	insert into BBB_SWITCH_A.ENDECA_PRODUCT_ITEM_ATTR (product_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, 
	...
	endeca_query :='SELECT distinct PRODUCT_ID,SKU_ATTRIBUTE_ID,SITE_ID,START_DATE,END_DATE,MISC_INFO  from BBB_SWITCH_A.ENDECA_PRODUCT_ITEM_ATTR';
  IF p_feedType   = 'PARTIAL' THEN
	endeca_query := 'SELECT distinct A.PRODUCT_ID,A.SKU_ATTRIBUTE_ID,A.SITE_ID,A.START_DATE,A.END_DATE,A.MISC_INFO  from BBB_SWITCH_A.ENDECA_PRODUCT_ITEM_ATTR A, BBB_SWITCH_A.ENDECA_EXPORT_PRODUCTS E  
  ...
  BEGIN
      ...
      Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
			...
			endeca_query:='select ''*#*'',a.sku_id,
                ...
                case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id
...
where a.sku_id=b.sku_id and a.sku_id=c.sku_id(+) and a.sku_id=d.sku_id(+) '
  ;
      IF p_feedType = 'PARTIAL' THEN
			endeca_query := 'select distinct ''*#*'',a.sku_id,
                            case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id, 
                            case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,
                            case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, 
                            case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id,
                            ...
                            d.zoom_index, d.anywhere_zoom, b.jda_dept_id, b.jda_sub_dept_id, b.jda_class, b.gift_cert_flag, b.college_id, b.sku_type,b.email_out_of_stock,b.color,b.color_group,replace(b.sku_size,''"'',''&quot;''),b.gift_wrap_eligible,b.vdc_sku_type,b.vdc_sku_message,b.upc,
                            case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id
							from dcs_sku a, bbb_sku b, dcs_sku_media c, bbb_sku_media d, dcs_price i, bbb_price j, endeca_export_skus es
							where a.sku_id=b.sku_id and a.sku_id=c.sku_id(+) and a.sku_id=d.sku_id(+) and a.sku_id = i.sku_id and i.price_id = j.price_id
and a.sku_id = es.sku_id' ;
      END IF;
      SELECT dump_csv( endeca_query, '|', p_feedType, p_filename, NULL, 'NEW_RECORD|SKU_ID|THUMBNAIL_IMAGE_ID|SMALL_IMAGE_ID|LARGE_IMAGE_ID|MEDIUM_IMAGE_ID|SWATCH_IMAGE_ID|ZOOM_IMAGE_ID|ZOOM_INDEX|ANYWHERE_ZOOM|JDA_DEPT_ID|JDA_SUB_DEPT_ID|JDA_CLASS|GIFT_CERT_FLAG|COLLEGE_ID|SKU_TYPE|EMAIL_OUT_OF_STOCK|COLOR|COLOR_GROUP|SKU_SIZE|GIFT_WRAP_ELIGIBLE|VDC_SKU_TYPE|VDC_SKU_MESSAGE|UPC|VERT_IMAGE_ID' )
	  INTO l_rows
      FROM dual;
...
PROCEDURE Export_Collection_SKUs(
  ...
  endeca_query:='select distinct PRODUCT_ID,SKU_ID, SEQUENCE_NUM, SKU_SITE_ID from ENDECA_EXPORT_COLLECTION_SKUS where CHILDPRD_WEBOFFERED_FLAG=''1'' and SKU_WEBOFFERED_FLAG =''1'' order by product_id,sku_id,sequence_num';
     ...
     Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
      execute immediate 'drop index GBL_TMP_IDX6';
	  execute immediate 'drop index GBL_TMP_IDX7';
      execute immediate 'drop table endeca_export_collection_skus'; 
  EXCEPTION
      WHEN OTHERS THEN  -- handles all other errors
...
PROCEDURE Export_Product_SKUs ( p_filename varchar2, p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  BEGIN
	...
	END IF;
      ...
      execute immediate 'create global temporary table endeca_export_prod_skus (product_id varchar2(40),sku_id varchar2(40), site_id varchar2(40),sequence_num varchar2 (20), translation_id varchar2(80), web_offered_flag number(1), disable_flag number(1) ) ON COMMIT DELETE ROWS';
      execute immediate 'create index GBL_TMP_IDX6 on endeca_export_prod_skus (translation_id)';
      ...
      execute immediate 'drop index GBL_TMP_IDX6';
      execute immediate 'drop table endeca_export_prod_skus'; 
...
PROCEDURE Export_UnlikeProducts(
  ...
  'select distinct ''*#*'', a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,
            ...
            case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id
from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, BBB_CORE.BBB_SCHOOLS f            
...
PROCEDURE Export_Lead_Products(
  ...
  'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,      
        ...
        anywhere_zoom, case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end,
        case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id
        from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, BBB_CORE.BBB_SCHOOLS f 
...
and b.college_id=f.school_id(+) and b.lead_prd_flag=''1'' '
  ...
  IF p_feedType  = 'PARTIAL' THEN
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom, case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, BBB_CORE.BBB_SCHOOLS f, dcs_prd_chldsku g, bbb_sku h, dcs_cat_chldprd i, bbb_category j where a.product_id = b.product_id and a.product_id = r.product_id and a.product_id=g.product_id(+) and g.sku_id=h.sku_id(+) and a.product_id=c.product_id(+) and a.product_id=d.product_id(+)  and a.product_id=i.child_prd_id (+) and i.category_id = j.category_id and b.college_id=f.school_id(+) and b.lead_prd_flag=''1''  and ( b.last_mod_date > to_date('''
    || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') or h.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')  or j.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') ) ' ;
  END IF;
...
and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists(select 1 from dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id) '
  ...
  IF p_feedType  = 'PARTIAL' THEN
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, BBB_CORE.BBB_SCHOOLS f, dcs_prd_chldsku g, bbb_sku h, dcs_cat_chldprd i, bbb_category j where a.product_id = b.product_id and a.product_id=c.product_id(+) and a.product_id=d.product_id(+)  and a.product_id=g.product_id(+) and g.sku_id=h.sku_id(+) and a.product_id=i.child_prd_id (+) and i.category_id = j.category_id and b.college_id=f.school_id(+) and b.collection_flag=''0'' and b.lead_prd_flag = ''0'' and exists (select 1 from dcs_cat_chldprd a1 where a1.child_prd_id = a.product_id)  and ( b.last_mod_date > to_date('''
    || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') or h.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')  or j.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'') ) ' ;
  END IF;
...
and b.college_id=f.school_id(+) and b.collection_flag=''1'' '
  ...
  IF p_feedType = 'PARTIAL' THEN
		...
		Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
    endeca_query:=
    'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code, case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id, case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id,  case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id, d.swatch_image_id as swatch_image_id, d.zoom_image_id as zoom_image_id, d.zoom_index,  case when b.scene7_url is not null then b.scene7_url||''?$83$'' else '''' end as collection_thumbnail, anywhere_zoom,case when b.seo_url like ''%.jsp%'' then '''' else replace(b.seo_url,''%'',''%25'') end, case when b.scene7_url is not null then b.scene7_url||''?$146v$'' else '''' end as vert_image_id, b.shop_guide_id from dcs_product a, bbb_product b, dcs_prd_media c, bbb_prd_media d, bbb_prd_prd_reln r, oct04_qa_replica1_core.BBB_SCHOOLS f, dcs_price h, dcs_price i, bbb_price h1, dcs_cat_chldprd k, bbb_category l where a.product_id = b.product_id  and a.product_id = r.product_id(+)   and a.product_id=c.product_id(+)  and a.product_id=d.product_id(+)   and b.sku_low_price = h.sku_id (+)  and b.sku_high_price = i.sku_id (+)  and h.price_id = h1.price_id and b.college_id=f.school_id(+) and a.product_id=k.child_prd_id (+) and k.category_id = l.category_id (+) and b.collection_flag=''1''  and ( b.last_mod_date > to_date('''
  ...
  BEGIN
      ...
      endeca_query:='select ''*#*'',a.brand_id,brand_descrip,brand_image,site_id from BBB_BRANDS a, bbb_brand_sites b where a.brand_id=b.brand_id';
       dbms_output.put_line('Export_Brands: '||endeca_query);     
  	  ...
  	  Log_message(v_process_cd, v_process_sub_cd, RTRIM(SUBSTR( v_message,1,512)), v_message_type);
  IF p_feedType   = 'PARTIAL' THEN
    endeca_query := 'select distinct ''*#*'',a.brand_id,brand_descrip,brand_image,site_id from BBB_BRANDS a, bbb_brand_sites b,bbb_product c where a.brand_id=b.brand_id and c.brand_id=a.brand_id and (a.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')or (c.last_mod_date > to_date(''' || p_lastModifiedDate ||''',''YYYY-MM-DD HH24:MI:SS'')))' ;
  END IF;
      dbms_output.put_line('Export_Brands: '||endeca_query);  
...
PROCEDURE Export_SKU_Item_Attributes(
  ...
  BEGIN
	 ...
	 IF(nCount > 0)THEN
		execute immediate 'truncate table BBB_SWITCH_A.ENDECA_SKU_ITEM_ATTR'; 
	 END IF; 
 
	 
	-- Insert site specific data into temporary Table Start	  
	insert into BBB_SWITCH_A.ENDECA_SKU_ITEM_ATTR (sku_id, SKU_ATTRIBUTE_ID, SITE_ID, start_date, END_DATE, MISC_INFO)
  ...
  endeca_query   :=	'select sku_id, sku_attribute_id, site_id, start_date, end_date, misc_info from BBB_SWITCH_A.ENDECA_SKU_ITEM_ATTR';
  If P_Feedtype   = 'PARTIAL' Then
    endeca_query :=	'select a.sku_id, a.sku_attribute_id, a.site_id, a.start_date, a.end_date, a.misc_info from BBB_SWITCH_A.ENDECA_SKU_ITEM_ATTR a, BBB_SWITCH_A.BBB_SKU d ,
					BBB_SWITCH_A.endeca_export_skus es where a.sku_id = es.sku_id and a.sku_id=d.sku_id';
  END IF;
...
PROCEDURE Export_SKU_Site_Properties(
  ...
  BEGIN 
	...
	commit;
	endeca_query:='select distinct ''*#*'', eps.product_id, nvl(bsd.UNIT_SALES,''0''), nvl(bsd.ORDER_SALES,''0''), nvl(bsd.TOTAL_SALES,''0''), eps.site_id from endeca_prod_site_relation eps, oct04_qa_replica1_core.BBB_SALES_DATA bsd where eps.product_id = bsd.product_id(+) and eps.site_id = bsd.site_id(+) order by eps.product_id';
