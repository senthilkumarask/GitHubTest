...
create or replace PACKAGE BODY  BBB_SWITCH_A.ATG_ENDECAEXPORT_PKG AS
...
dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_PRODUCTS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_Product_Item_Attributes(
  ...
  BEGIN
	 ...
	 END IF;
      ...
      select count(*) into nCountConfig from BBB_SWITCH_A.BBB_CONFIG_KEY_VALUE where config_key ='EXTRACT_PRD_ATTR_SITES_DATA';
     IF(nCountConfig > 0)THEN
        select config_value into useAttrSitesTbl from BBB_SWITCH_A.BBB_CONFIG_KEY_VALUE where config_key ='EXTRACT_PRD_ATTR_SITES_DATA';
...
dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_PRODUCT_ITEM_ATTR', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);

	endeca_query :='SELECT distinct A.PRODUCT_ID,A.SKU_ATTRIBUTE_ID,A.SITE_ID,A.START_DATE,A.END_DATE,A.MISC_INFO  from BBB_SWITCH_A.ENDECA_PRODUCT_ITEM_ATTR A';
        IF p_feedType   = 'PARTIAL' THEN
...
PROCEDURE Export_Collection_SKUs(
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_COLLECTION_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
--Lokesh insert collection stats here ENDECA_EXPORT_PROD_SKUS
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_PROD_SKUS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
--Lokesh insert collection stats here ENDECA_SKU_ITEM_ATTR
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_SKU_ITEM_ATTR', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_Category_Products ( p_filename varchar2, p_lastModifiedDate varchar2, p_feedType varchar2)
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_CAT_PRODS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
PROCEDURE Export_Prices(
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_PRICES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
--Lokesh  insert collection stats here ENDECA_EXPORT_PROD_PROPERTIES
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_PROD_PROPERTIES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
-- Lokesh Insert collection stats here ENDECA_EXPT_SKU_SITE_PROPTS
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPT_SKU_SITE_PROPTS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
EXCEPTION
  ...
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_PROD_SITE_RELATION', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
...
-- Lokesh insert collection stats here ENDECA_EXPORT_PRICES_UPDATES	
  dbms_stats.gather_table_stats(ownname => 'BBB_SWITCH_A', tabname => 'ENDECA_EXPORT_PRICES_UPDATES', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
