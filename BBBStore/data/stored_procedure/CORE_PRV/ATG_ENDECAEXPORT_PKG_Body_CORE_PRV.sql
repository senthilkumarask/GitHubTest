...
SET SERVEROUTPUT ON;
 ...
 --Revision History :
 --R2.2 Story - 92-A. Product Meta Keywords Endeca SP's changes in following procedures : 
 --Export_UnlikeProducts
 --Export_Accessories
 --Export_Lead_Products
 --Export_Simple_Products
 --Export_Collection_Products
...
PROCEDURE Export_UnlikeProducts(
  ...
  endeca_query:=
  'select distinct ''*#*'', a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,show_images_in_collection,f.school_name,rollup_type_code,
            case when b.scene7_url is not null then b.scene7_url||''?$63$'' else '''' end as small_image_id,
            case when b.scene7_url is not null then b.scene7_url||''?$146$'' else '''' end as medium_image_id,
            case when b.scene7_url is not null then b.scene7_url||''?$'' else '''' end as thumbnail_image_id, 
            case when b.scene7_url is not null then b.scene7_url||''?$478$'' else '''' end as large_image_id,
...
PROCEDURE Export_Accessories(
  ...
  endeca_query:=
  'select distinct ''*#*'',a.product_id,gift_cert,collection_flag,swatch_flag,brand_id,b.email_out_of_stock,            
...
and (c.last_mod_date > to_date(''' || p_lastModifiedDate ||
  ...
  END IF;
  SELECT dump_csv( endeca_query, '|', p_feedType, p_filename, NULL, 'NEW_RECORD|SKU_ATTRIBUTE_ID|DISPLAY_DESCRIP|IMAGE_URL|ACTION_URL|PLACE_HOLDER|PRIORITY|SITE_ID|START_DATE|END_DATE|ATTRIBUTE_DISPLAY_NAME' )
  INTO l_rows
  FROM dual;
...
PROCEDURE Export_SKU_Site_Properties(
  ...
  BEGIN 
    ...
    If P_Feedtype = 'PARTIAL' Then
     insert into endeca_expt_sku_site_propts (sku_id, site_id, web_offered_flag,last_mod_date) select bbb_sku_site_translations.sku_id, bbb_sku_translations.site_id,  bbb_sku_translations.attribute_value_boolean,bbb_sku_translations.last_mod_date from dcs_sku,bbb_sku,bbb_sku_site_translations,bbb_sku_translations,endeca_export_skus es where dcs_sku.sku_id=bbb_sku_site_translations.sku_id and dcs_sku.sku_id=bbb_sku.sku_id and bbb_sku_site_translations.translation_id=bbb_sku_translations.translation_id and bbb_sku_translations.attribute_name = 'webOffered' and bbb_sku.sku_id = es.sku_id;
  ELSE
    insert into endeca_expt_sku_site_propts (sku_id, site_id, web_offered_flag,last_mod_date) select bbb_sku_site_translations.sku_id, bbb_sku_translations.site_id,  bbb_sku_translations.attribute_value_boolean,bbb_sku_translations.last_mod_date from dcs_sku,bbb_sku_site_translations,bbb_sku_translations where dcs_sku.sku_id=bbb_sku_site_translations.sku_id and bbb_sku_site_translations.translation_id=bbb_sku_translations.translation_id and bbb_sku_translations.attribute_name = 'webOffered';
  END IF;
  commit;
-- Lokesh Insert collection stats here ENDECA_EXPT_SKU_SITE_PROPTS
  ...
  IF p_feedType = 'PARTIAL' THEN
    insert into endeca_expt_sku_site_propts (sku_id, web_offered_flag, disable_flag,sku_title, short_description, site_id,last_mod_date) select bbb_sku.sku_id, bbb_sku.web_offered_flag, bbb_sku.disable_flag, replace(dcs_sku.display_name,'"','&quot;'), replace(dcs_sku.description,'"','&quot;'),'BedBathUS',bbb_sku.last_mod_date from bbb_sku,dcs_sku,endeca_export_skus es where bbb_sku.sku_id=dcs_sku.sku_id and bbb_sku.sku_id = es.sku_id;
  Else
    insert into endeca_expt_sku_site_propts (sku_id, web_offered_flag, disable_flag,sku_title, short_description, site_id,last_mod_date) select bbb_sku.sku_id, bbb_sku.web_offered_flag, bbb_sku.disable_flag, replace(dcs_sku.display_name,'"','&quot;'),  replace(dcs_sku.description,'"','&quot;'),'BedBathUS',bbb_sku.last_mod_date from bbb_sku,dcs_sku where bbb_sku.sku_id=dcs_sku.sku_id;
  END IF;
  commit;
-- Lokesh Insert collection stats here ENDECA_EXPT_SKU_SITE_PROPTS
  dbms_stats.gather_table_stats(ownname => 'BBB_CORE_PRV', tabname => 'ENDECA_EXPT_SKU_SITE_PROPTS', estimate_percent => 100, method_opt => 'FOR ALL COLUMNS SIZE AUTO', cascade => TRUE);
  endeca_query:='select ''*#*'',sku_id, web_offered_flag,disable_flag, replace(sku_title,''"'',''&quot;''), replace(short_description,''"'',''&quot;''), site_id from endeca_expt_sku_site_propts where sku_id NOT IN (''GiftWrapBaby'',''GiftWrapCanada'',''GiftWrapUS'') order by sku_id';
  SELECT dump_csv( endeca_query, '|', p_feedType, p_filename, NULL, 'NEW_RECORD|SKU_ID|WEB_OFFERED_FLAG|DISABLE_FLAG|SKU_TITLE|SHORT_DESCRIPTION|SITE_ID' )
  INTO l_rows
  FROM dual;
