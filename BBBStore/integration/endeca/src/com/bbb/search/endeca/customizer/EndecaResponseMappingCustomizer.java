...
/**
 * This is a customizer Implementation of  EndecaResponseCustomizer, which 
 * contains methods to create the facet-descriptor mapping, create facet 
 * List from descriptors in case facet list is empty, set refined string 
 * for facet, descriptors and refinement name.
 * 
 * @author rjain40
 * 
 */
public class EndecaResponseMappingCustomizer extends BBBGenericService implements EndecaResponseCustomizer{
	...
	private static final String CHECKLIST_CATEGORY = "Checklist Category";
	...
	/**
	 * This is overridden method to customize the Endeca Response. This method calls the method to 
	...
	public void customizeResponse(final SearchResults browseSearchVO, final SearchQuery pSearchQuery){
		if(isLoggingDebug()){
			logDebug("[START] EndecaResponseMappingCustomizer.customizeResponse() method. Parameters :: Search Keyword -" 
		...
		// Set these variables only for browse PLP
		if(null != descriptorVOs && BBBUtility.isNotEmpty(catalogRefId)){
			String descRootName = BBBCoreConstants.BLANK;
			String descName = BBBCoreConstants.BLANK;
			final Iterator<CurrentDescriptorVO> descIterator = descriptorVOs.iterator();
			while(descIterator.hasNext()){
				final CurrentDescriptorVO descVO = (CurrentDescriptorVO)descIterator.next();
				...
				if(!descRootName.equals(DEPARTMENT_FACET) && !descRootName.equals(RECORD_TYPE_FACET) && !descRootName.equals(CHECKLIST_CATEGORY)){
		...
		if (!BBBUtility.isListEmpty(facetParentVOs)) {
			...
			while(facetItr.hasNext()){
				...
				final FacetParentVO facetVO = (FacetParentVO)facetItr.next();
				...
				if(facetName.equals(DEPARTMENT_FACET)){
					try {
						getCatalogTools().removePhantomCategory(facetVO.getFacetRefinement(), siteId);
					} catch (BBBSystemException | BBBBusinessException e) {
						logError("EndecaResponseMappingCustomizer.customizeResponse error in calling getCatalogTools().removePhantomCategory", e);
					}
					deptAvail = true;
				}
			...
			if(isLoggingDebug()){
				logDebug("[END] EndecaResponseMappingCustomizer.customizeResponse() method");
			}
	...
	{	
		if(isLoggingDebug()){
			logDebug("[START] EndecaResponseMappingCustomizer.createEmptyFacetList() method");
		}
		final List<FacetParentVO> emptyFacetParentVOs = new ArrayList<FacetParentVO>();
		String facetName = "";
		...
		if(null != descriptorVOs){
			...
			while(descItr.hasNext()){
				...
				final CurrentDescriptorVO descVO = (CurrentDescriptorVO)descItr.next();
				...
				if(!RECORD_TYPE_FACET.equals(descVO.getRootName()) && descVO.isMultiSelect()){
					...
					while(emptyfacetsItr.hasNext()){
						final FacetParentVO itrFacetVO = (FacetParentVO)emptyfacetsItr.next();
					...
					if(!isFacetExist){
						...
						final List<CurrentDescriptorVO> finalDescriptorVOs = new ArrayList<CurrentDescriptorVO>();
						...
						while(descVOItr.hasNext()){
							final CurrentDescriptorVO finalDescVO = (CurrentDescriptorVO)descVOItr.next();
		...
		if(isLoggingDebug()){
			logDebug("[END] EndecaResponseMappingCustomizer.createEmptyFacetList() method");
		}
		return emptyBox;
	}
	...
	private void createFacetDescriptorMapping(final List<CurrentDescriptorVO> descriptorVOs, final FacetParentVO facetVO, final String facetName){
		if(isLoggingDebug()){
			logDebug("[START] EndecaResponseMappingCustomizer.createFacetDescriptorMapping() method");
		}
		
		final List<CurrentDescriptorVO> facetdescVOs = new ArrayList<CurrentDescriptorVO>();
		final Iterator<CurrentDescriptorVO> descItrerator = descriptorVOs.iterator();
		while(descItrerator.hasNext()){
			final CurrentDescriptorVO descVO = (CurrentDescriptorVO)descItrerator.next();
		...
		if(isLoggingDebug()){
			logDebug("[END] EndecaResponseMappingCustomizer.createFacetDescriptorMapping() method");
		}
	}
}