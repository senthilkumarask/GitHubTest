...
/**
 * <p>
 * The Property Accessor for retrieving LIST_PRICE from DCS_PRICE Table(for the SKU_ID & PRICE_LIST_ID(MexicoListPrice) to a given PRODUCT_ID)
 * </p>
 * @author dkhadka
 *
 */

public class SkuWasHighPriceMxPropertyAccessor extends PropertyAccessorImpl implements IndexConstants {
	...
	protected Object getTextOrMetaPropertyValue(Context pContext, RepositoryItem pItem, String pPropertyName, PropertyTypeEnum pType) {
		BBBPerformanceMonitor.start(BBBPerformanceConstants.PRICE_ACCESSOR_CALL , "SkuWasHighPriceMxPropertyAccessor");
		Double listPrice=null;
		if(isLoggingDebug()){
			logDebug("RepositoryItem\t:"+pItem);			
		...
		BBBPerformanceMonitor.end(BBBPerformanceConstants.PRICE_ACCESSOR_CALL , "SkuWasHighPriceMxPropertyAccessor");
		return listPrice;
	}
