...
public class SkuWasLowPricePropertyAccessor extends PropertyAccessorImpl implements IndexConstants {
	...
	protected Object getTextOrMetaPropertyValue(Context pContext, RepositoryItem pItem, String pPropertyName, PropertyTypeEnum pType) {
		BBBPerformanceMonitor.start(BBBPerformanceConstants.PRICE_ACCESSOR_CALL , "SkuWasLowPricePropertyAccessor");
		...
		if (null != pItem) {
			...
			listPrice = getAccessorHelper().getWasSkuLowPrice(pItem, AccessorConstants.SKU_LOW_PRICE_PROPERTY_NAME, priceListId);
			if (isLoggingDebug()) {
				logDebug("\nFor SKU_LOW_PRICE...SiteID:\t"+siteId+"\nProductID:\t"+pItem.getRepositoryId()+"\nPrice_ListID US(plist100004) or Canada(plist100003):\t"+priceListId+"\nList Price Retrieved:\t"+listPrice);				

			}
		}
		BBBPerformanceMonitor.end(BBBPerformanceConstants.PRICE_ACCESSOR_CALL , "SkuWasLowPricePropertyAccessor");
		return listPrice;
	}
