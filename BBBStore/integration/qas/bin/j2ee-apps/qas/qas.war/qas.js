...
var QAS_Variables = {
    ...
    PROXY_PATH: "/qas/qas_atg_proxy.jsp", //Standard Pro Web proxy file
    ADD_PROXY_PATH: "/qas/add_atg_proxy.jsp", //Additional Countries proxy file
    TD_PROXY_PATH: "/qas/email_atg_proxy.jsp", //Email and Phone validation proxy file

    //QAS_LAYOUT:     "USA2 TitleCase Retention DPV",   //the proweb configuration to use
    //" BBB PO Box Flag" "BBB PO Box Flag"
    QAS_LAYOUT:"BBB PO Box Flag", //the proweb configuration to use

    //set any onclick events and submit buttons to use pre and post validation
    PRE_ON_CLICK: null,
    POST_ON_CLICK: null,
    BUTTON_ID: "testValidateAddress",
    ...
    ADDRESS_FIELD_IDS: [
        ["atg_store_streetAddressInput", "atg_store_streetAddressOptionalInput", "atg_store_localityInput", "atg_store_stateSelect", "atg_store_postalCodeInput", "countryName", "poBoxFlag", "poBoxStatus"]
    ],
    ...
    COUNTRY_FIELD_IDS: ["atg_store_countryNameSelect"],
    ...
    PHONE_VALIDATE_COUNTRY: ["USA", //USA
    "US" //US
    ], //US Virgin Islands
    ...
    EMAIL_FIELD_IDS: ["atg_store_registerEmailAddress", "atg_store_profileMyInfoEditEmailAddress"],
    ...
    PHONE_FIELD_IDS: ["atg_store_telephoneInput"],
    ...
    TIMEOUT: 30000, //ajax timeout for address validation
    TIMEOUT_EMAILPHONE: 10000, //timeout for email and phone validation
    ...
    stopTriggerBtnClick: false, //Sapient: This will prevent the Submit Button Click Trigger.
    QAS_ZIP_INDEX: 4, //Sapient: Added a global variable to check for ZIP_Index and remove invalid ZIP codes returned in the resultset
    QAS_CITY_INDEX: 2 //Sapient: Added a global variable to check for CITY_Index and truncate city names returned in the resultset to 25 characters
};
...
var QAS_PROMPTS = {
    ...
    "PremisesPartial": {
        "header": "<b>Sorry, we think that your apartment/suite/unit number is missing or incorrect.</b><br/>To proceed, please enter your apartment/suite/unit or use your address as entered.",
    ...
    "StreetPartial": {
        "header": "<b>Sorry, we do not recognize your house or building number.</b><br />To proceed, please verify the number and choose from one of the options below.",
...
function setSubmitButonid(b_id) {
    ...
    var getAddresses = function () {
        ...
        for (i = 0; i < ids.length; i++) {
            ...
            for (j = 0; j < ids[i].length; j++) {
                ...
                var $field = $('#' + ids[i][j]),
                    fieldValue = encodeURIComponent($field.val());
                ...
                // don't why this was added, but commenting this for now.
                // it is prefixing "@" to firstnames and lastnames returned to the html form, which is not a required behavior
...
function Clean(searchString, country_3, ajaxErr) {
    ...
    var saveResult = function (xml) {
        /** SAPIENT:
         * Added call to waitClose method of Interface class because we added a call to waitOpen for opening the wait dialog after user selectes the Suggested Address from QAS to fill the respective form fields.
         */
        new Interface().waitClose();
        ...
        if (premClean && (me.verifylevel === "PremisesPartial")) {
            ...
            if ((me.verifylevel === "Verified") || (me.verifylevel === "VerifiedStreet") || (me.verifylevel === "VerifiedPlace") || (me.verifylevel === "InteractionRequired")) {
                /* Sapient: Added a check for invalid zip codes (non /^\d{5}(-[0-9]{4})?$/) and removed invalid part of the zip code returned
                also added a check for city names returned to truncate them to 25 characters
                $(xml).find("line").each(saveAddress); */

                $(xml).find("line").each(function (idx, elem) {
                    var txt = $(this).text();
                    if (QAS_Variables.QAS_LAYOUT === 'BBB PO Box Flag') {
                        if (idx === QAS_Variables.QAS_CITY_INDEX && !(txt.length <= 25)) {
                            //me.verifylevel = "None";
                            txt = txt.substr(0, 25);
                        }
                        if (idx === QAS_Variables.QAS_ZIP_INDEX && !/^\d{5}(-[0-9]{4})?$/.test(txt)) {
                            //me.verifylevel = "None";
                            txt = txt.replace(/-.{1,4}/, '');
                        }
                    }
    ...
    var doFormat = function (moniker) {
        /** SAPIENT:
         * Added call to waitOpen method of Interface class and we added a call to waitClose for closing the wait dialog after user selectes the Suggested Address from QAS and it gets filled in respective form fields.
         * This Helped to show the preloader in case of multiple Address fieldset validation. This was addded as there was a noticable time gap between second address call due to the format Ajax call of selected address.
         */
        new Interface().waitOpen();
        var ajaxParams = {
...
function Interface(editCall) {
    ...
    var buildPick = function () {
		...
		var aptMatch = QAS_Get_Apt_Regex(m_orig[0], m_orig[1]);
		
		
        for (i = 0; i < m_pickList.length; i++) {
		
			if( aptMatch !== undefined && aptMatch !== null && aptMatch !== "" ){
				var aptRegex = new RegExp(aptMatch, "i");
				m_pickList[i].addressText = decodeURIComponent(m_pickList[i].addressText);
				m_pickList[i].addressText = m_pickList[i].addressText.replace(aptRegex,"");
			}
		
            var alterNateClassName = (i === 0 || i % 2 === 0) ? "even" : "odd";
            if (m_pickList[i].fulladdress.toString().toLowerCase() === "true") {
                m_pickHtml += "<tr><td class=" + alterNateClassName + "><a href='#' class='QAS_StepIn' moniker='" + m_pickList[i].moniker + "'>" + decodeURIComponent(m_pickList[i].addressText) + "</a></td><td class=" + alterNateClassName + "><a href='#' class='QAS_StepIn' moniker='" + m_pickList[i].moniker + "'>" + decodeURIComponent(m_pickList[i].postCode) + "</a></td></tr>";
            } else {
                m_pickHtml += "<tr><td class=" + alterNateClassName + ">" + decodeURIComponent(m_pickList[i].addressText) + "</td><td class=" + alterNateClassName + ">" + decodeURIComponent(m_pickList[i].postCode) + "</td></tr>";
            }
        }
    };
    ...
    var buildMultPick = function () {
        ...
        for (i = 0; i < m_pickList.length; i++) {
            ...
            if (m_pickList[i].fulladdress.toString().toLowerCase() === "true") {
                ...
                m_pickHtml += "<tr><td class=" + alterNateClassName + "><a href='#' class='QAS_Refine' moniker='" + m_pickList[i].moniker + "'>" + decodeURIComponent(m_pickList[i].addressText) + "</a></td><td class=" + alterNateClassName + "><a href='#' class='QAS_Refine' moniker='" + m_pickList[i].moniker + "'>" + decodeURIComponent(m_pickList[i].postCode) + "</a></td></tr>";
            }
        }
    };
    ...
    var buildRightSide = function (callback) {
        ...
        function (evt) {
            /** SAPIENT:
             * Added preventDefault as Edit link was taking the control to top of page because of its HREF value as "#".
             */
            evt.preventDefault();
            $('#QAS_Dialog').dialog('close');
            m_editCall();
    ...
    var load = function () {
        ...
        //add div tag to page
        $(document.body).append("<div id='QAS_Dialog' title='" + QAS_PROMPTS.title + "'>" + "  <div class='QAS_Header ui-state-highlight'></div>" + "  <div class='QAS_Prompt'>" + "    <div class='QAS_PromptText'></div>" + "    <div class='QAS_Input'></div>" + "    <div class='QAS_PromptData'></div>" + "  </div>" + "  <div class='QAS_RightDetails'></div>" + "  <div class='QAS_Picklist'>" + "    <div class='QAS_MultPick'></div>" + "    <div class='QAS_ShowPick'></div>" + "    <div class='QAS_Pick'></div>" + "  </div>" + "</div>" + "<div id='QAS_Wait' title = '" + QAS_PROMPTS.waitMessage + "'></div>");
        ...
        $(".QAS_ShowPick").click(function (evt) {
            evt.preventDefault();
    ...
    this.waitOpen = function () {
        ...
        $('#QAS_Wait').parents('.ui-dialog:first').find('.ui-dialog-titlebar-close').css('display', 'none');
        $('#QAS_Wait').parents('.ui-dialog:first').find(".ui-dialog-content").hide();
    ...
    this.display = function () {
        /** SAPIENT:
         * Commented the window.scroll as it was not required for our modal window behavior.
         */
        //window.scroll(0, 0);
        ...
        //remove close button from top right of dialog
        //$('.ui-dialog-titlebar-close').css('display', 'none');
        $('#QAS_Dialog').parents('.ui-dialog:first').find('.ui-dialog-titlebar-close').css('display', 'none');
    ...
    this.setInterReq = function (cleaned, orig, message, acceptCallback, origCallback) {
        ...
        if(cleanedHtml.search("PO Box") > 0 ){
        	$('#QAS_AcceptOriginal').hide();
        }else{
    ...
    this.setPartial = function (pickList, orig, message, refineCallback, monikerCallback, origCallback) {
        ...
        function (evt) {
            /** SAPIENT:
             * Added preventDefault because the address link was taking control to top of page because of its HREF value as "#".
             */
            evt.preventDefault();
            $('#QAS_Dialog').dialog('close');
    ...
    this.setAptAppend = function (orig, message, refineCallback, noAptCallback, origCallback) {
        ...
        $(".QAS_PromptText").html(message.prompt);
        $(".QAS_PromptData").html("");
        $(".QAS_Input").html("<input type='text' id='QAS_RefineText' />" + "<input type='button' id='QAS_RefineBtn' value='" + message.button + "' />" + "<br />" + "<input type='button' id='QAS_NoApt' value='" + message.noApt + "' />");
        $(".QAS_MultPick").html("");
...
function EmailPhoneInterface(editCall) {
    ...
    this.displayResult = function (tdResult, callback) {
        ...
        if (thisResult.phone !== undefined && thisResult.phone !== null && thisResult.phone.ok === false) {
            ...
            $(".QAS_PhoneInput").html("<input type='text' id='QAS_PhoneRefineText' value='" + $("input#" + QAS_Variables.PHONE_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).val() + "' />");
        ...
        if (numDivs > 0) {
            ...
            function () {
                ...
                if (thisResult.phone !== undefined && thisResult.phone !== null && thisResult.phone.ok === false) {
                    ...
                    searchString = searchString + "&qasphone=" + $("#QAS_PhoneRefineText").val();
                }
                ...
                if (thisResult.email !== undefined && thisResult.email !== null && thisResult.email.ok === false) {
                    ...
                    searchString = searchString + "&qasemail=" + $("#QAS_EmailRefineText").val();
...
function EmailPhoneValidation(clickEvent, buttonID, nextProcess) {
    ...
    var checkForValidSearch = function () {
        ...
        for (iCounter = 0; iCounter < QAS_Variables.EMAIL_FIELD_IDS.length; iCounter++) {
            if ($("input#" + QAS_Variables.EMAIL_FIELD_IDS[iCounter]).length !== 0) {
        ...
        for (iCounter = 0; m_submit === false && iCounter < QAS_Variables.PHONE_FIELD_IDS.length; iCounter++) {
            if ($("input#" + QAS_Variables.PHONE_FIELD_IDS[iCounter]).length !== 0) {
    ...
    var prepareSearch = function () {
        ...
        if ($("input#" + QAS_Variables.EMAIL_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).length !== 0) {
            tempSearch = encodeURIComponent($("input#" + QAS_Variables.EMAIL_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).val());
            if (tempSearch !== "") {
                validSearch = true;
                searchString = searchString + "&qasemail=" + tempSearch;
            }
        }

        if (QAS_Variables.PHONE_VALIDATE_COUNTRY[0] === "ALL" || QAS_Variables.COUNTRY_FIELD_IDS.length < QAS_TEMP_VARS.EMAIL_PHONE_POS) {
            submitphone = true;
        } else {
        ...
        if (submitphone === true) {
            if ($("input#" + QAS_Variables.PHONE_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).length !== 0) {
                tempSearch = encodeURIComponent($("input#" + QAS_Variables.PHONE_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).val());
                if (tempSearch !== "") {
                    validSearch = true;
                    searchString = searchString + "&qasphone=" + tempSearch;
                }
            }
        }
    ...
    var processUIContinue = function (newSearchString) {
        ...
        if (QAS_TEMP_VARS.NUM_EMAIL_PHONE_SUBMITS < QAS_Variables.EMAIL_PHONE_NUM_SUBMITS || QAS_Variables.EMAIL_PHONE_NUM_SUBMITS === 0) {
    ...
    var emailPhoneUICallback = function (response) {
        ...
        if (response !== undefined && response !== null) {
            if (response.d !== undefined && response.d !== null) {
                ...
                if (response.d.additional_info !== null && response.d.additional_info !== undefined) {
                    // ecommerce platform error message
                    //if display errors is set, then display the error
                    if (QAS_Variables.DISPLAY_ERRORS) {
                        alert(response.d.additional_info + "\n Error from ecommerce platform.");
                    }
                } else {
                    if (response.d.email !== undefined && response.d.email !== null) {
                        if (response.d.email.ok === false) {
                            inter.displayResult(response.d, processUIContinue);
                            inter.display();
                            processedResult = true;
                        }
                    }

                    if (response.d.phone !== undefined && response.d.phone !== null && processedResult === false) {
                        if (response.d.phone.ok === false) {
                            inter.displayResult(response.d, processUIContinue);
                            inter.display();
                            processedResult = true;
                        }
    ...
    var emailPhoneStdCallback = function (response) {
        if (response !== null && response.d !== null && response !== undefined && response.d !== undefined) {
            if (response.d.email !== undefined && response.d.email !== null && response.d.email.ok === false) {
                $("label#" + QAS_Variables.EMAIL_ERR_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).show();

                if (QAS_Variables.DISPLAY_CUSTOM_EMAIL_ERR === true) {

                    message = EMAIL_ERR_MESSAGES[response.d.email.status_code];
            ...
            if (response.d.phone !== undefined && response.d.phone !== null && response.d.phone.ok === false) {
                $("label#" + QAS_Variables.PHONE_ERR_FIELD_IDS[QAS_TEMP_VARS.EMAIL_PHONE_POS]).show();

                if (QAS_Variables.DISPLAY_CUSTOM_PHONE_ERR === true) {

                    message = PHONE_ERR_MESSAGES[response.d.phone.status_code];
...
function Main(clickEvent, buttonID) {
    ...
    this.finish = function () {
        ...
        if (m_button !== "") {
            ...
            $('#' + m_button).parent('form').attr('onsubmit', '');
            /** SAPIENT:
             * unbind the event which we attached on DOM ready as QAS is trying to remove "onclick".
             * becasue we are not using "onclick" that is why we are unbinding our click event using jQuery.
             */
            if (QAS_Variables['unbindEventOnFinish']) {
                $('#' + m_button).unbind('click', QAS_Variables['unbindEventOnFinish']);
            }

            /** SAPIENT:
             * Added a check to trigger the click Submit Button Click event.
             * becasue in some cases we want our callback function to be executed which was defined in QAS call with Key as "fQASSuccessCallback".
             */
            if (BBB.fn.hasProperty(QAS_Variables, "stopTriggerBtnClick") === false || (BBB.fn.hasProperty(QAS_Variables, "stopTriggerBtnClick") && QAS_Variables['stopTriggerBtnClick'] === false)) {
                $('#' + m_button).click();
            }

            if (typeof QAS_Variables["fQASSuccessCallback"] === "function") {
                QAS_Variables["fQASSuccessCallback"]();
            }
        }
    };
    ...
    this.returnEarly = function () {
        ...
        if (typeof QAS_Variables["fQASEditAddressCallback"] === "function") {
            QAS_Variables["fQASEditAddressCallback"]();
        }
    };
    /*********
		Commented the Code for Ajax Error Handler and we have added our new method with same name
		//handle all ajax errors
	...
	**********/
    /** SAPIENT:
     * Changed the method ajaxError as per our requirement.
     * Also Added new method "openAlertDialog" to show the Alert Dialog in form of Modal dialog.
     */
    this.ajaxError = function (xml, text, msg) {
        if (text === "timeout") {
            //set match type to timeout
    ...
    this.openAlertDialog = function () {
        ...
        btnsObj[BBB.fn.getString('cancelBtn')] = function () {
            $(this).dialog("close");
        };
        BBB.modalDialog.openDialog({
            wrapperSetting: {
                type: 'html',
                modalContentHTML: BBB.fn.getString('QASAjaxFail')
            },
            uiDialiogSettings: {
                title: BBB.fn.getString('errTitle'),
                buttons: btnsObj
            }
        });
    };
...
function QAS_Get_Apt_Regex( inputLineOne, inputLineTwo ) {

		inputLineOne = decodeURIComponent(inputLineOne);
		inputLineTwo = decodeURIComponent(inputLineTwo);

		var aptOptions = ["Bsmt","Basement",
							"Bldg","Building",
							"Dept","Department",
							"Fl","Floor",
							"Frnt","Front",
		...
		var regexStr = "(";
		for( var i = 0; i < aptOptions.length; i++ ){
			if( i > 0 )
				regexStr += "|";
			regexStr += aptOptions[i];
		}

		regexStr += ") ?[a-z0-9]{1,}";
		var aptRegex = new RegExp(regexStr, "i");
		var match = aptRegex.exec(inputLineOne);
		if( match === null ){
			match = aptRegex.exec(inputLineTwo);
		}
