...
*
*********************************************************************************************************************************************************************/
console.log("Entering into TBSQAS Address verification service.");
var QAS_Variables = {
    //location of the proxy files - path needs to either be fixed or relative to the web page the js is loaded on
    PROXY_PATH:     "/tbsqas/qas_atg_proxy.jsp",    //Standard Pro Web proxy file
    ADD_PROXY_PATH: "/tbsqas/add_atg_proxy.jsp",    //Additional Countries proxy file
    TD_PROXY_PATH:  "/tbsqas/email_atg_proxy.jsp",     //Email and Phone validation proxy file

    //QAS_LAYOUT:     "USA2 TitleCase Retention DPV",   //the proweb configuration to use
    QAS_LAYOUT:     "BBB Layout",   //the proweb configuration to use
    ...
    ADDRESS_FIELD_IDS: [
        ["atg_store_streetAddressInput", "atg_store_streetAddressOptionalInput", "atg_store_localityInput", "atg_store_stateSelect", "atg_store_postalCodeInput"]
    ],
...
var QAS_PROMPTS = {
    ...
    "StreetPartial": {
        "header": "<b>Sorry, we don't recognize your house or building number.</b><br />To proceed, please verify the number and choose from one of the options below.",
    ...
    var getAddresses = function () {
        ...
        for (i = 0; i < ids.length; i++) {
            ...
            for (j = 0; j < ids[i].length; j++) {
                ...
                // Here we prepend the '@' symbol to the field specific to the appartment (in the example where the issue was reported.
                //  The index is 1, since this is the second field.  Please try this fix when possible.
                /*
                if( j == 1 || j == 0 ){
...
function Interface(editCall) {
    ...
    var buildRightSide = function (callback) {
        ...
        //For TBS QAS module button hiding.
...
function Main(clickEvent, buttonID) {
	...
	*/
	...
	this.openAlertDialog = function() {
		var btnsObj = {};
		btnsObj[BBB.glblStringMsgs.okBtn] = function () {
		...
		btnsObj[BBB.glblStringMsgs.cancelBtn] = function () {
			$(this).dialog("close");
		};
		BBB.modalDialog.openDialog({
			wrapperSetting: {
				type: 'html',
				modalContentHTML: BBB.glblStringMsgs.QASAjaxFail
			},
			uiDialiogSettings: {
				title: BBB.glblStringMsgs.errTitle,
				buttons: btnsObj
			}
		});
	};
