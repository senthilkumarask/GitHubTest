...
<body>
	...
	<c:set var="babySite" scope="request">false</c:set>
	<c:set var="usSite" scope="request">false</c:set>
	<c:set var="caSite" scope="request">false</c:set>
	<c:choose>
		<c:when test="${currentSiteId eq 'BuyBuyBaby'}">
			<c:set var="babySite" scope="request">true</c:set>
		</c:when>
		<c:when test="${currentSiteId eq 'BedBathUS'}">
			<c:set var="usSite" scope="request">true</c:set>
		</c:when>
		<c:when test="${currentSiteId eq 'BedBathCanada'}">
			<c:set var="caSite" scope="request">true</c:set>
		</c:when>
	</c:choose>
	...
	<dsp:form id="personalInfo" iclass="clearfix"
		...
		<table border="noborder" style="width: 500px;">
			...
			<tr>
				<td><strong>Site Name</strong><span class="required">*</span>
				</td>
				<td><dsp:input id="siteName" name="siteName" type="radio" bean="CSRFormHandler.siteName" value="BuyBuyBaby"/>Buy Buy Baby
					<br /> 
					<dsp:input id="siteName" name="siteName" type="radio" bean="CSRFormHandler.siteName" value="BedBathUS"/>Bed Bath & Beyond
					<br /> 
					<dsp:input id="siteName" name="siteName" type="radio" bean="CSRFormHandler.siteName" value="BedBathCanada"/>Bed Bath & Beyond Canada
				</td>
			</tr>
			<tr>
				<td></td>
