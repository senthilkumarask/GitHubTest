...
public class Picklist implements Serializable {
    ...
    public Picklist(QAPicklistType p) {
        ...
        */
        m_AutoStepinSafe = p.getAutoStepinSafe();
        m_AutoStepinPastClose = p.getAutoStepinPastClose();
        m_AutoFormatSafe = p.getAutoFormatSafe();
        m_AutoFormatPastClose = p.getAutoFormatPastClose();
        m_LargePotential = p.getLargePotential();
        m_MaxMatches = p.getMaxMatches();
        m_MoreOtherMatches = p.getMoreOtherMatches();
        m_OverThreshold = p.getOverThreshold();
        m_Timeout = p.getTimeout();

        // convert the lines in the picklist
        //axis2
		//List<PicklistEntryType> aItems = p.getPicklistEntry();
        List<PicklistEntryType> aItems = Arrays.asList(p.getPicklistEntryArray());
